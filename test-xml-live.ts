...
async function run() {
    ...
    while(true) {
        ...
        let status = await (await fetch(`${BRIDGE_URL}/api/sync/status/${opId}`, { headers: {"x-api-key": API_KEY, "bypass-tunnel-reminder": "true"} })).json();
        if (status?.operation?.status === "completed") {
            const match = status.operation.qbxmlResponse.match(/<EditSequence>([^<]+)<\/EditSequence>/);
...
<FullName>Wholesale</FullName>
    ...
    res = await fetch(`${BRIDGE_URL}/api/sync/direct-query`, {
        method: "POST",
        headers: { "x-api-key": API_KEY, "Content-Type": "application/json", "bypass-tunnel-reminder": "true" },
