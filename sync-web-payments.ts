...
export default async function syncWebPayments({
  ...
  for (const order of orders) {
    ...
    for (const pc of order.payment_collections) {
      ...
      for (const payment of pc.payments) {
        ...
        console.log(
          ` -> Found missing CAPTURED web payment ${medusaPaymentId} for Order ${order.id}. Syncing...`
        );
        ...
        await financeService.createCustomerPayments({
          ...
          method: payment.provider_id?.includes("authorize")
            ? "authorize_net"
  ...
  console.log(
    `Web Payment Sync Complete. Generated ${totalSynced} CustomerPayments/Applications.`
  );
}
