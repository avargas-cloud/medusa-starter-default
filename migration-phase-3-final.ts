...
async function migratePhase3() {
    ...
    try {
        ...
        const parentIds = new Set(allCategories.rows.map(c => c.parent_category_id).filter(id => id && allIds.has(id)))

        const leafCategories = allCategories.rows.filter(c => !parentIds.has(c.id))
        const parentCategories = allCategories.rows.filter(c => parentIds.has(c.id))
        ...
        try {
            ...
            for (const cat of leafCategories) {
                ...
                // Update category ID
                await client.query('UPDATE product_category SET id = $1 WHERE id = $2', [newId, oldId])
                ...
                if (categoriesMigrated % 10 === 0) {
                    console.log(`   ✅ ${categoriesMigrated}/${leafCategories.length} leaf categories migrated`)
                }
            }
            console.log(`   ✅ All ${leafCategories.length} leaf categories migrated`)
            ...
            while (remaining.size > 0) {
                ...
                for (const cat of Array.from(remaining)) {
                    ...
                    const parentAlreadyMigrated = cat.parent_category_id && cat.parent_category_id.match(/^pcat_01/)
                    const parentProcessed = cat.parent_category_id && processed.has(cat.parent_category_id)
            ...
            for (const cat of sortedParents) {
                ...
                if (parentsMigrated % 10 === 0) {
                    console.log(`   ✅ ${parentsMigrated}/${parentCategories.length} parent categories migrated`)
                }
            }
            console.log(`   ✅ All ${parentCategories.length} parent categories migrated`)
            ...
            if (verifyOldGone.rows.length > 0) {
                throw new Error(`Old IDs still exist: ${verifyOldGone.rows.map(r => r.id).join(', ')}`)
            }
            ...
            if (verifyNewExist.rows.length !== idMapping.size) {
                throw new Error(`Expected ${idMapping.size} new IDs, found ${verifyNewExist.rows.length}`)
            }
            ...
            if (orphanCheck.rows.length > 0) {
                throw new Error(`Orphaned categories found: ${orphanCheck.rows.map(r => r.name).join(', ')}`)
            }
            ...
            if (orphanedProducts > 0) {
                throw new Error(`${orphanedProducts} products linked to non-existent categories`)
            }
            ...
            console.log('='.repeat(80))
            console.log(`\n📊 Final Statistics:`)
            console.log(`   Total categories migrated: ${categoriesMigrated + parentsMigrated}`)
            console.log(`   Leaf categories: ${leafCategories.length}`)
            console.log(`   Parent categories: ${parentCategories.length}`)
