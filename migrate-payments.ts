...
export default async function migrateOldPayments({
  ...
  const existingApps = await financeService.listPaymentApplications(
    {},
    { take: 10000 }
  );
  ...
  for (const inv of invoices) {
    ...
    const invPayments = await invoiceService.listInvoicePayments(
      { invoice_id: inv.id },
      { take: 100 }
    );
    ...
    const appsForInvoice = existingApps.filter(
      (app: any) => app.invoice_id === inv.id
    );
    ...
    for (const payment of invPayments) {
      ...
      const matchIndex = poolOfApps.findIndex(
        (a) => Number(a.amount_applied) === Number(payment.amount)
      );
      if (matchIndex >= 0) {
      ...
      console.log(
        ` -> Migrating payment of ${payment.amount} for invoice ${inv.id} (Paid at: ${payment.paid_at})`
      );
  ...
  console.log(
    `Migration complete. Generated ${totalMigrated} CustomerPayments/Applications.`
  );
}
