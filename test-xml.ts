...
export default async function myScript({ container }: { container: any }) {
    ...
    const otherAddrs = addrs.filter(a => !(a.is_default_billing || a.metadata?.is_default_billing));
...
function buildCustomerMod(data: any): string {
  ...
  const billAddress = data.BillAddress || data.BillingAddress || data.billingAddress;
  ...
  const shipToAddresses: any[] = Array.isArray(data.ShipToAddress) ? data.ShipToAddress : [];
  ...
  const buildShipTo = (addr: any): string =>
    ...
    (addr.PostalCode ? `<PostalCode>${escapeXml(addr.PostalCode)}</PostalCode>` : '') +
    `</ShipToAddress>`;
  ...
  return `<CustomerModRq><CustomerMod>` +
    `<ListID>${escapeXml(data.ListID)}</ListID>` +
    `<EditSequence>${escapeXml(data.EditSequence)}</EditSequence>` +
    (data.IsActive !== undefined ? `<IsActive>${data.IsActive}</IsActive>` : '') +
    (name ? `<Name>${escapeXml(name)}</Name>` : '') +
    (companyName ? `<CompanyName>${escapeXml(companyName)}</CompanyName>` : '') +
    ...
    (customerType ? `<CustomerTypeRef><FullName>${escapeXml(customerType)}</FullName></CustomerTypeRef>` : '') +
    (priceLevel ? `<PriceLevelRef><FullName>${escapeXml(priceLevel)}</FullName></PriceLevelRef>` : '') +
