...
async function check() {
  try {
    ...
    const oRes = await pool.query('SELECT id, display_id, status FROM "order" WHERE id = $1', [orderId]);
    ...
    const fRes = await pool.query('SELECT f.id as fulfillment_id, f.provider_id FROM fulfillment f JOIN order_fulfillment of ON f.id = of.fulfillment_id WHERE of.order_id = $1', [order.id]);
    console.log("--- FULFILLMENTS ---");
    console.table(fRes.rows.length ? fRes.rows : [{ fulfillment_id: 'None' }]);

    const invRes = await pool.query('SELECT id, invoice_number, status, total, fulfillment_id FROM pos_invoice WHERE order_id = $1', [order.id]);
    ...
    const resvRes = await pool.query(`
      ...
      WHERE oi.order_id = $1 AND ri.deleted_at IS NULL`, [order.id]);
    console.log("--- RESERVATIONS (ALLOCATED) ---");
    console.table(resvRes.rows.length ? resvRes.rows : [{ reservation_id: 'None' }]);
