...
export default async function convertWebPaymentsToCents({
  container,
}: {
  ...
  console.log(
    `Found ${payments.length} Web Payments. Checking for decimal amounts...`
  );
  ...
  for (const payment of payments) {
    ...
    if (amountNum < 100000) {
      ...
      if (amountNum % 1 !== 0 || amountNum < 600) {
        ...
        console.log(
          ` -> Converted Web Payment ${payment.id} from ${amountNum} to ${newAmount} cents.`
        );
        totalFixed++;
      }
    }
  ...
  console.log(
    `Conversion Complete. Fixed ${totalFixed} CustomerPayments to Cents.`
  );
}
