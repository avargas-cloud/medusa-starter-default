...
async function compareFilters() {
    ...
    try {
        ...
        console.log(`   URL: /store/categories/${LED_STRIPS_ID}/products-with-filters`)
        ...
        const includeDescendants = categoryData.product_category?.metadata?.include_descendants_tree ?? true
        ...
        for (let i = 0; i < endpointFilters.length; i++) {
            ...
            const typeMatch = endpointFilter.filter_type === metadataFilter.filter_type
            ...
            if (endpointOptions.length !== metadataOptions.length) {
                ...
                console.log(`   Options count: Endpoint(${endpointOptions.length}) vs Metadata(${metadataOptions.length})`)
                mismatches++
                continue
            }
            ...
            if (allOptionsMatch) {
                console.log(`✅ Filter ${i + 1}: ${endpointFilter.display_name || endpointFilter.name} (${endpointOptions.length} options)`)
                perfectMatches++
            } else {
                console.log(`⚠️  Filter ${i + 1}: ${endpointFilter.name} - Option values/counts differ`)
                mismatches++
            }
        }
        ...
        console.log(`${"=".repeat(80)}`)
        console.log(`✅ Perfect Matches: ${perfectMatches}/${endpointFilters.length}`)
        ...
        if (mismatches === 0) {
            ...
            console.log(`   - Logic differences between endpoint and metadata generation`)
        }

        // 6. Show first filter as example
        console.log(`\n📄 Sample Comparison (First Filter):`)
        console.log(`\nFrom Endpoint:`)
        console.log(JSON.stringify(endpointFilters[0], null, 2).substring(0, 500) + "...")
        console.log(`\nFrom Metadata:`)
        console.log(JSON.stringify(metadataFilters[0], null, 2).substring(0, 500) + "...")
