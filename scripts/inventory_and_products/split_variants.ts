...
async function main() {
    ...
    for (const productId of TARGET_PRODUCT_IDS) {
        ...
        const pRes = await client.query<{
            ...
            handle: string; thumbnail: string; metadata: any; type_id: string | null
        }>('SELECT id,title,description,status,handle,thumbnail,metadata,type_id FROM product WHERE id=$1', [productId])

        if (!pRes.rows.length) { console.log(`⚠️  ${productId} not found\n`); continue }
        const product = pRes.rows[0]

        // Read all variants (including soft-deleted)
        const vRes = await client.query<{ id: string; title: string; sku: string | null }>(
            'SELECT id,title,sku FROM product_variant WHERE product_id=$1 ORDER BY created_at ASC', [productId]
        )
        const variants = vRes.rows

        // Read categories
        const catRes = await client.query<{ product_category_id: string }>(
            'SELECT product_category_id FROM product_category_product WHERE product_id=$1', [productId]
        )
        const categoryIds = catRes.rows.map(r => r.product_category_id)

        // Read current option
        const optRes = await client.query<{ id: string }>(
            'SELECT id FROM product_option WHERE product_id=$1 LIMIT 1', [productId]
        )
        const existingOptionId = optRes.rows[0]?.id

        console.log(`📦 "${product.title}" (${productId})`)
        for (const v of variants) console.log(`   • [${v.sku ?? 'NO SKU'}] "${v.title}"`)
        ...
        if (DRY_RUN) {
            ...
            for (const v of extras) console.log(`   → New product "${v.sku}" ← moved variant`)
            console.log()
            continue
        }
        ...
        try {
            ...
            await client.query('UPDATE product SET title=$1, deleted_at=NULL WHERE id=$2', [v1Title, productId])
            await client.query('UPDATE product_variant SET title=\'Default Title\', deleted_at=NULL WHERE id=$1', [v1.id])
            ...
            if (!origOptId) {
                ...
                await client.query('UPDATE product_option SET title=\'Title\' WHERE id=$1', [origOptId])
            }
            // Upsert option value "Default Title" for V1
            const origOptValRes = await client.query<{ id: string }>(
                'SELECT id FROM product_option_value WHERE option_id=$1 LIMIT 1', [origOptId]
            )
            ...
            if (!origOptValId) {
                ...
                await client.query('UPDATE product_option_value SET value=\'Default Title\' WHERE id=$1', [origOptValId])
            }
            // Link V1 to option value
            await client.query('DELETE FROM product_variant_option WHERE variant_id=$1', [v1.id])
            await client.query(
            ...
            for (const v of extras) {
                ...
                const newHandle = slugify(newTitle) + '-' + newProductId.slice(-6).toLowerCase()
                ...
                await client.query(
                    ...
                    [newProductId, newTitle, product.description, product.status, newHandle, product.thumbnail, product.metadata, product.type_id]
                )
                ...
                await client.query('DELETE FROM product_variant_option WHERE variant_id=$1', [v.id])
                await client.query(
                    'INSERT INTO product_variant_option (variant_id,option_value_id) VALUES ($1,$2)',
                    [v.id, newOptValId]
                )
