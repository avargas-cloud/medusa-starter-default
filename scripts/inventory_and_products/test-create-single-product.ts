...
export default async function testCreateSingleProduct({ container }: ExecArgs) {
    ...
    try {
        const [result] = await productModule.createProducts([{
            ...
            description: null,
            metadata: {
                sales_description: "TEST PRODUCT — This should be deleted automatically.",
                prerender: false,
                qb_imported: true,
    ...
    if (createdId) {
        ...
        try {
            ...
            logger.warn(`   ⚠️  Delete failed (manual cleanup needed): ${err.message}`)
    ...
    logger.info("   Run: npx medusa exec src/scripts/migrate/import-qb-single-products.ts")
    logger.info("=".repeat(60))
}
