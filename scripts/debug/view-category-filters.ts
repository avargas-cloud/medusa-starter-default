...
if (!categoryIdentifier) {
    console.error("❌ Usage: npx tsx scripts/debug/view-category-filters.ts [category-handle-or-id]")
    process.exit(1)
}
...
async function main() {
    // Fetch category
    const res = await fetch(`${basePath}/store/product-categories/${categoryIdentifier}`, {
        headers: {
    ...
    // Sort by handle
    const sorted = Object.entries(filters).sort((a, b) => a[0].localeCompare(b[0]))
