...
async function main() {
    ...
    if (!createRes.operationId) {
        throw new Error(`Failed to queue customer creation: ${JSON.stringify(createRes)}`);
    }

    const createOp = await waitForOp(createRes.operationId, 'Customer Creation');
    const custRet = (createOp.result?.QBXML?.QBXMLMsgsRs ?? createOp.result?.QBXMLMsgsRs)
    ...
    if (listId) {
        ...
        if (readRes.operationId) {
            ...
            const readRet = (readOp.result?.QBXML?.QBXMLMsgsRs ?? readOp.result?.QBXMLMsgsRs)
                ?.CustomerQueryRs?.CustomerRet;
            console.log(`   ✅ Verified! Name: ${readRet?.Name}, Balance: ${readRet?.Balance ?? '0.00'}`);
        }
    }
