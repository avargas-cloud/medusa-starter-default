...
// ─────────────────────────────────────────────────────────────
function resolveTargetGroup(qbPriceLevel: string | null | undefined): 'Wholesale' | 'Retail' {
...
async function main() {
    ...
    try {
        ...
        if (!wholesaleGroupId || !retailGroupId) {
            console.error('❌ Could not find Wholesale or Retail customer group in Medusa.')
            console.error('   Available groups:', groupRows.rows.map(r => r.name).join(', '))
            process.exit(1)
        }
        ...
        for (const customer of customerRows.rows) {
            ...
            const targetGroupId = targetGroup === 'Wholesale' ? wholesaleGroupId : retailGroupId

            const groups = currentGroups.get(customer.id) || new Set()
            const isInTargetGroup = groups.has(targetGroupId)
            const isInOtherGroup = targetGroup === 'Wholesale'
            ...
            if (isInTargetGroup && !isInOtherGroup) {
                ...
                if (isInOtherGroup) {
                    console.log(`   🔄 ${customer.email} — moving from ${targetGroup === 'Wholesale' ? 'Retail' : 'Wholesale'} → ${targetGroup}`)
                } else {
        ...
        if (toAddWholesale.length > 0) {
            ...
            console.log(`✅ Assigned ${toAddWholesale.length} customers to Wholesale`)
        }
        ...
        if (SYNC_MEILI && allAffected.length > 0) {
            ...
            const affectedRows = await db.query<{
                ...
                company_name: string; phone: string; has_account: boolean
            ...
            const newGroupRows = await db.query<{ customer_id: string; name: string }>(`
            ...
            for (const row of newGroupRows.rows) {
                if (!groupsByCustomer.has(row.customer_id)) groupsByCustomer.set(row.customer_id, [])
            ...
            const docs = affectedRows.rows.map(c => {
                ...
                const price_level = groups.includes('Wholesale') ? 'Wholesale' : 'Retail'
                return {
                    id: c.id,
                    email: c.email,
            ...
            console.log(`ℹ️  MeiliSearch sync skipped. Run with --sync-meili to sync.\n`)
        }
