...
async function main() {
    ...
    for (const r of dupCheck.rows) {
        ...
        console.log(`${flag} order_id=${r.order_id} rows=${r.cnt} totals_history=${r.totals_history}`)
    }
    ...
    for (const r of orderItems.rows) {
        console.log(`  v${r.version}: unit_price=${r.unit_price} raw_up=${JSON.stringify(r.raw_unit_price)} qty=${r.quantity} raw_qty=${JSON.stringify(r.raw_quantity)}`)
    }
    ...
    for (const r of summaryRows.rows) {
        console.log(`  id=${r.id}`)
        console.log(`  current=${r.totals?.current_order_total} original=${r.totals?.original_order_total} accounting=${r.totals?.accounting_total}`)
