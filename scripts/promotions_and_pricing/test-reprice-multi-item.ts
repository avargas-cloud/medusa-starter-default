...
const DB_URL = process.env.DATABASE_URL || "postgresql://postgres:hUMSVtteMnqSBZSuSGUBivBooMdRoKtj@interchange.proxy.rlwy.net:34919/railway"
...
async function run() {
    ...
    try {
        ...
        console.log(`   Response: success=${repriceData.success}, updatesApplied=${repriceData.updatesApplied}`)

        // Verify prices
        const cartAfter = await fetch(`${API_URL}/carts/${cartId}`, { headers: headers(token) })
        const { cart: updatedCart } = await cartAfter.json()

        console.log(`\n--- Prices after reprice ---`)
        for (const item of updatedCart.items || []) {
            const expected = variants.find((v: any) => v.variant_id === item.variant_id)
            const correct = Number(item.unit_price) === Number(expected?.wholesale_price)
            console.log(`   ${item.variant_id}: $${item.unit_price} (expected $${expected?.wholesale_price}) ${correct ? '✅' : '❌'} | is_custom_price=${item.is_custom_price}`)
        }
