...
async function testSaveConfiguration() {
    ...
    const testPayload = {
        active_filters: [
            { attribute_id: '01KFK5S9QVXKPTG4Y8ZED3XRVJ', order: 0, type: 'checkbox' },
            { attribute_id: '01KFK5SBN1R98VGVJ22R25YZQR', order: 1, type: 'checkbox' }
        ],
    ...
    try {
        const response = await fetch(`http://localhost:9000/admin/product-categories/${categoryId}/generate-filters`, {
            method: 'POST',
            headers: {
