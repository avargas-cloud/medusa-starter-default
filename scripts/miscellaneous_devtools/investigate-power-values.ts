...
export default async function ({ container }: ExecArgs) {
    ...
    for (const product of products) {
        ...
        const { data: links } = await query.graph({
            ...
            fields: ["attribute_value_id", "attribute_value.value", "attribute_value.attribute_key_id"],
            filters: { product_id: product.id }
        })

        // Filter for Power Consumption values
        const powerValues = links.filter((l: any) =>
            l.attribute_value?.attribute_key_id === attributeKeyId
        )
