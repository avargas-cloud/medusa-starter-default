...
async function checkCustomerDefaults() {
    ...
    try {
        ...
        if (schemaResult.rows.length === 0) {
            console.log('❌ CRITICAL: Columns default_billing_address_id and default_shipping_address_id DO NOT EXIST in customer table!');
        ...
        console.log(`   default_billing_address_id: ${customer.default_billing_address_id || 'NULL ❌'}`);
        console.log(`   default_shipping_address_id: ${customer.default_shipping_address_id || 'NULL ❌'}`);
        console.log('');
        ...
        console.log(`📍 Customer Addresses (${addressesResult.rows.length} total):`);
        addressesResult.rows.forEach((addr, index) => {
            const isBillingDefault = addr.id === customer.default_billing_address_id ? '✅ DEFAULT BILLING' : '';
            const isShippingDefault = addr.id === customer.default_shipping_address_id ? '✅ DEFAULT SHIPPING' : '';
        ...
        if (!customer.default_billing_address_id && !customer.default_shipping_address_id) {
            console.log('❌ NO default addresses are set on the customer record (both NULL)');
            console.log('   This means the endpoints did NOT update the customer table.');
