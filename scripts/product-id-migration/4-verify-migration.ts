...
async function main() {
    console.log("🚀 Product ID Migration - Phase 4: Comprehensive Verification\n")
    ...
    try {
        ...
        if (Number(orphanedCategoryLinks.rows[0].count) > 0) {
            errors.push(`${orphanedCategoryLinks.rows[0].count} orphaned category links`)
            console.log(`❌ Orphaned Category Links: ${orphanedCategoryLinks.rows[0].count}`)
        } else {
        ...
        for (const cat of categoriesWithSorting) {
            ...
            for (const id of productOrder) {
                if (id.includes("-") && id.startsWith("prod_")) {
                    sortingValid = false
                    errors.push(`Category "${cat.name}" has old format ID in sorting: ${id}`)
                }
            }
        }
        ...
        const reportPath = path.join(process.cwd(), "scripts/product-id-migration/verification-report.json")
        fs.writeFileSync(reportPath, JSON.stringify(report, null, 2), "utf-8")

        // 12. Summary
        console.log("\n─".repeat(80))
        console.log(`${errors.length === 0 ? "✅" : "❌"} PHASE 4 VERIFICATION ${report.status}\n`)
