...
async function main() {
    ...
    try {
        ...
        const variantCount = await db("product_variant").count("* as count").first()
        ...
        const categoryLinkCount = await db("product_category_product").count("* as count").first()
        ...
        const orphanedVariants = await db.raw(`
            ...
            WHERE p.id IS NULL
        `)
        console.log(`Orphaned Variants: ${orphanedVariants.rows[0].count} ${orphanedVariants.rows[0].count > 0 ? "⚠️" : "✅"}`)
        ...
        const orphanedImages = await db.raw(`
            ...
            WHERE p.id IS NULL
        `)
        console.log(`Orphaned Images: ${orphanedImages.rows[0].count} ${orphanedImages.rows[0].count > 0 ? "⚠️" : "✅"}`)
        ...
        const orphanedCategories = await db.raw(`
            ...
            WHERE p.id IS NULL
        `)
        console.log(`Orphaned Category Links: ${orphanedCategories.rows[0].count} ${orphanedCategories.rows[0].count > 0 ? "⚠️" : "✅"}`)
        ...
        if (allHaveCascade &&
            ...
            console.log("  2. Run Phase 1: npx tsx scripts/product-id-migration/1-generate-mappings.ts")
            console.log("  3. Run Phase 2: npx tsx scripts/product-id-migration/2-update-metadata.ts")
            console.log("  4. Run Phase 3: npx tsx scripts/product-id-migration/3-migrate-database.ts")
        } else {
