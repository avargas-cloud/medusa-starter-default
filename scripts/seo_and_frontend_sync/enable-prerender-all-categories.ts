...
async function enablePrerenderForAllCategories() {
    ...
    try {
        ...
        currentState.rows.forEach(row => {
            const status = row.prerender_value === 'true' ? '✅' :
                row.prerender_value === 'false' ? '❌' : '⚠️ ';
        ...
        if (total === with_prerender) {
            console.log(`✅ SUCCESS! All ${total} categories now have prerender=true\n`);
        } else {
            console.log(`⚠️  WARNING: ${total - with_prerender} categories still missing prerender=true\n`);
        }
