...
export function CategorySelector({
  ...
  const renderCategory = (category: Category, level: number = 0) => {
    ...
    return (
      <div key={category.id}>
        <div
          className={`flex items-center gap-2 px-2 py-1.5 rounded cursor-pointer hover:bg-ui-bg-base-hover ${
    ...
    <Container className="h-full overflow-y-auto">
      ...
      <div className="space-y-1">
        {rootCategories.length === 0 ? (
          <Text className="text-ui-fg-muted text-sm">No categories found</Text>
        ) : (
