...
export function AvailableFiltersSection({
  ...
  const inactiveAttributes = filteredAttributes.filter(
    (attr) => !activeFilters.has(attr.id) && !inheritedFilters.has(attr.id)
  );
    ...
    <div>
      ...
      {(() => {
        ...
        if (
          ...
          (availableAttrIds?.length === 0 || availableAttrIds === undefined)
        ) {
          return (
            <div className="border border-ui-border-base rounded-lg p-3 bg-ui-bg-subtle">
              ...
              <Text
                size="xsmall"
                className="text-ui-fg-subtle text-center mt-1.5"
              >
                Products exist in this category, but they don't have{" "}
                <code>metadata.attributes</code> defined
              </Text>
            </div>
          );
        }
