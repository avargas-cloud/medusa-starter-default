...
const InventoryShippingAttributesWidget = ({ data }: Props) => {
    const [weight, setWeight] = useState<string>(data.weight != null ? String(data.weight) : "")
    const [height, setHeight] = useState<string>(data.height != null ? String(data.height) : "")
    const [width, setWidth] = useState<string>(data.width != null ? String(data.width) : "")
    const [length, setLength] = useState<string>(data.length != null ? String(data.length) : "")
        ...
        <Container className="p-0">
            ...
            <div className="px-6 py-5 grid grid-cols-2 gap-x-6 gap-y-4">
                ...
                <div className="flex flex-col gap-1">
                    <Label size="small" weight="plus">Weight (lbs)</Label>
                    <Input
                        type="number"
                        step="any"
                        min="0"
                ...
                {/* Length */}
                <div className="flex flex-col gap-1">
                    <Label size="small" weight="plus">Length (in)</Label>
                    <Input
                        type="number"
                        step="any"
                        min="0"
                ...
                {/* Width */}
                <div className="flex flex-col gap-1">
                    <Label size="small" weight="plus">Width (in)</Label>
                    <Input
                        type="number"
                        step="any"
                        min="0"
                ...
                {/* Height */}
                <div className="flex flex-col gap-1">
                    <Label size="small" weight="plus">Height (in)</Label>
                    <Input
                        type="number"
                        step="any"
                        min="0"
            ...
            <div className="px-6 pb-4">
                <Text size="xsmall" className="text-ui-fg-subtle">
                    These values are used for UPS shipping rate calculations. Decimals are supported.
                </Text>
            </div>
        </Container>
    )
