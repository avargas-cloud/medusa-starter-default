...
// ── Widget ────────────────────────────────────────────────────────────────────
const CustomerTaxExemptWidget = ({ data }: DetailWidgetProps<HttpTypes.AdminCustomer>) => {
    ...
    const updateExempt = (val: string) => {
        setIsExempt(val)
        persist({ is_tax_exempt: val, tax_id: taxId, tax_exempt_doc_url: docUrl, tax_exempt_doc_name: docName })
    }

    const updateTaxId = () => {
        persist({ is_tax_exempt: isExempt, tax_id: taxId, tax_exempt_doc_url: docUrl, tax_exempt_doc_name: docName })
    }
    ...
    const removeDoc = async () => {
        setDocUrl("")
        setDocName("")
        await persist({ is_tax_exempt: isExempt, tax_id: taxId, tax_exempt_doc_url: "", tax_exempt_doc_name: "" })
    }
        ...
        <Container>
            <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                    ...
                    {isExempt === "Yes" && (
                        <Badge color="green" size="small">Exempt</Badge>
                    )}
                    {isExempt === "No" && (
                        <Badge color="grey" size="small">Taxable</Badge>
                    )}
                </div>
                {(saving || uploading) && (
                ...
                {docUrl ? (
                    ...
                    <div className="flex items-center gap-3 p-2 bg-ui-bg-subtle rounded border border-ui-border-base">
                        ...
                        <div className="flex-1 min-w-0">
                            <Text size="xsmall" className="font-medium truncate">{docName || "Certificate"}</Text>
                            <a
                                href={docUrl}
                                target="_blank"
                    ...
                    >
                        ...
                        <Text size="xsmall">
                            {uploading ? "Uploading…" : "Click to upload exemption certificate"}
                        </Text>
