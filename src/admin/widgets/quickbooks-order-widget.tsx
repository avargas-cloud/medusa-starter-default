...
const QBField = ({
  ...
  <div className="flex flex-col gap-1">
    ...
    <div
      className={`px-3 py-2 rounded-md bg-ui-bg-subtle border border-ui-border-base text-sm min-h-[36px] flex items-center ${
        mono ? "font-mono" : ""
      }`}
    >
      {value ? (
...
const StatusDot = ({ synced }: { synced: boolean }) => (
  <span
    className={`inline-block w-2 h-2 rounded-full ${
      synced ? "bg-ui-tag-green-icon" : "bg-ui-tag-orange-icon"
    }`}
  />
);
...
const QuickBooksOrderWidget = ({ data }: DetailWidgetProps<any>) => {
  ...
  const estimateTxnId: string | null =
    ...
    localMeta.qb_estimate_txn_id ??
    null;
  // Latest Invoice
  const invoicesArr: any[] = Array.isArray(localMeta.qb_invoices)
    ? localMeta.qb_invoices
    : [];
  const latestInvoice =
    invoicesArr.length > 0 ? invoicesArr[invoicesArr.length - 1] : null;
  const invoiceTxnId: string | null =
    latestInvoice?.txn_id ?? localMeta.qb_invoice_txn_id ?? null;
  const invoiceRef: string | null =
    latestInvoice?.ref_number ?? localMeta.qb_invoice_ref ?? null;
  // Latest Payment
  const paymentsArr: any[] = Array.isArray(localMeta.qb_payments)
    ? localMeta.qb_payments
    : [];
  const latestPayment =
    paymentsArr.length > 0 ? paymentsArr[paymentsArr.length - 1] : null;
  const paymentTxnId: string | null =
    latestPayment?.txn_id ?? localMeta.qb_payment_txn_id ?? null;
  ...
  const fetchMeta = useCallback(async () => {
    try {
      ...
      const hasTxnId =
        meta?.qb_sales_order?.txn_id || meta?.qb_sales_order_txn_id;
      if (hasTxnId) {
        setLocalMeta(meta);
  ...
  const handleSync = async (force = false) => {
    ...
    try {
      ...
      setLocalMeta((prev) => ({
        ...prev,
        qb_sales_order: {
          ...(prev.qb_sales_order && typeof prev.qb_sales_order === "object"
            ? prev.qb_sales_order
            : {}),
          txn_id:
            json.qbSoTxnId ??
            prev.qb_sales_order?.txn_id ??
            prev.qb_sales_order_txn_id,
          ref_number:
            json.qbSoRef ??
            prev.qb_sales_order?.ref_number ??
            prev.qb_sales_order_ref,
        ...
        {invoiceTxnId && (
          <div className="grid grid-cols-3 gap-4">
            ...
            {invoicesArr.length > 1 && (
              <div className="flex flex-col gap-1">
                <span className="text-[10px] text-ui-fg-muted uppercase tracking-widest font-medium">
                  Invoices
                </span>
                <span className="text-sm text-ui-fg-muted">
                  {invoicesArr.length} total
                </span>
              </div>
            )}
          </div>
