...
type CategoryWithMetadata = AdminProductCategory & {
  metadata?: {
    filter_config?: {
      available_filters?:
        | string[]
        | Array<{ attribute_id: string; order: number }>;
...
const CategoryFiltersWidget = ({
  ...
  if (loading) {
    return (
      <Container className="divide-y p-0">
        ...
        <div className="px-6 py-4 text-ui-fg-subtle">Loading...</div>
      </Container>
    );
  }
  ...
  if (filterConfig?.available_filters) {
    ...
    if (typeof first === "string") {
      ...
      availableAttrIds = (
        filterConfig.available_filters as Array<{ attribute_id: string }>
  ...
  if (filterConfig?.active_filters) {
    ...
    if (typeof first === "string") {
      ...
      activeFilterIds = (
        filterConfig.active_filters as Array<{ attribute_id: string }>
      ).map((f) => f.attribute_id);
    }
  }
        ...
        try {
          ...
          if (parentActives.length > 0) {
            ...
            if (typeof first === "string") {
              ...
              parentActiveIds = (
                parentActives as Array<{ attribute_id: string }>
              ).map((f) => f.attribute_id);
            }
          }
      ...
      <Container className="divide-y p-0">
        ...
        {inactiveAttrs.length > 0 && (
          <div className="px-6 py-4">
            ...
            <Text className="text-ui-fg-muted text-xs mt-3 pl-1">
              {" "}
              {/* ⭐ Added pl-1 */}
