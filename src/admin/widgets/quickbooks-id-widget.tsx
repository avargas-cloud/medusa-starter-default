...
const QuickBooksIdWidget = ({ data }: DetailWidgetProps<AdminProduct>) => {
  ...
  const variantsWithQb =
    data.variants?.filter((v) => v.metadata?.quickbooks_id) || [];
    ...
    <Container className="p-6">
        ...
        <div>
          ...
          <div className="space-y-2">
            {variantsWithQb.map((variant) => (
              <div
                key={variant.id}
                className="flex items-center justify-between p-3 bg-ui-bg-base border border-ui-border-base rounded-md"
              >
      ...
      <div className="mt-4 p-3 bg-ui-bg-subtle-hover rounded-md">
        <Text size="xsmall" className="text-ui-fg-subtle">
          💡 QuickBooks Desktop ListIDs are permanent identifiers used for
          synchronization
        </Text>
      </div>
    </Container>
