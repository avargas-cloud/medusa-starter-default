...
const ProductAttributesWidget = ({
  ...
  const {
    ...
    queryFn: async () => {
      return sdk.client.fetch<any>(
        `/admin/products/${initialProduct.id}/attributes`
      );
    },
  ...
  const variantKeys =
    (productData?.product?.metadata?.variant_attributes as string[]) || [];

  const [isManageModalOpen, setIsManageModalOpen] = useState(false);

  // Calculate Groups for Display & Sort (Variants first, then alphabetical)
  const groupedAttributes = groupAttributesByKey(attributes, variantKeys).sort(
    (a, b) => {
  ...
  const handleSave = async (
    ...
    console.log("🔥 [WIDGET] Modal is sending:", {
      ...
      attributes: selectedAttributes.map(
        (a) => `${a.attribute_key.handle}: ${a.value}`
      ),
    });
    ...
    const activeVariantKeys = Object.keys(variantFlags).filter(
      (keyId) => variantFlags[keyId]
    );
    ...
    try {
      ...
      console.log(
        "✅ UI fully refreshed with sorted attributes and variant badges"
      );
