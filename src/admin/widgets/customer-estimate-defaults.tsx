    ...
    <Container>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          {reps.length > 0 ? (
            <Select
              value={defaults.default_rep}
              onValueChange={(v) => update("default_rep", v)}
            >
            ...
            <Input
              value={defaults.default_rep}
              onChange={(e) => update("default_rep", e.target.value)}
              placeholder="e.g. AV"
        ...
        <div>
          ...
          <Select
            value={defaults.default_order_type}
            onValueChange={(v) => update("default_order_type", v)}
          >
            <Select.Trigger className="h-8 text-sm w-full">
        ...
        <div>
          ...
          <Select
            value={defaults.default_lead_time}
            onValueChange={(v) => update("default_lead_time", v)}
          >
            <Select.Trigger className="h-8 text-sm w-full">
              <Select.Value placeholder="Select lead time…" />
            </Select.Trigger>
            <Select.Content>
              {options.lead_times.map((t) => (
                <Select.Item key={t} value={t}>
                  {t}
                </Select.Item>
              ))}
        ...
        <div>
          ...
          <Select
            value={defaults.default_payment_terms}
            onValueChange={(v) => update("default_payment_terms", v)}
          >
            <Select.Trigger className="h-8 text-sm w-full">
