...
interface Category {
  ...
  metadata?: {
    ...
    filter_config?: {
      ...
      available_filters?:
        | string[]
        | Array<{ attribute_id: string; order: number; type: string }>;
      active_filters:
        | string[]
        | Array<{ attribute_id: string; order: number; type: string }>;
    };
...
export function useCategoryConfig(
  ...
  const [inheritedFilters, setInheritedFilters] = useState<Set<string>>(
    new Set()
  );
  const [inheritedFromParentName, setInheritedFromParentName] = useState<
    string | null
  >(null);
  ...
  const prevOverride = useRef<boolean>(false);
  ...
  useEffect(() => {
    ...
    if (config) {
      ...
      prevOverride.current = config.override_inheritance ?? false;
      ...
      if (config.active_filters && config.active_filters.length > 0) {
        ...
        if (typeof first === "string") {
          ...
          typeof first === "object" &&
          first &&
          "attribute_id" in first
        ) {
          activeIds = config.active_filters.map((f) =>
            typeof f === "string"
              ? f
              : (f as { attribute_id: string }).attribute_id
          );
        }
      }
      setActiveFilters(new Set(activeIds));

      // ⭐ Fetch inherited filters from parent if not overriding
      if (!config.override_inheritance && category.parent_category_id) {
        ...
        if (parent) {
          ...
          if (
            parentConfig?.active_filters &&
            parentConfig.active_filters.length > 0
          ) {
            ...
            if (typeof first === "string") {
              ...
              typeof first === "object" &&
              (first as any).attribute_id
            ) {
              parentActiveIds = (
                parentConfig.active_filters as Array<{ attribute_id: string }>
              ).map((f) => f.attribute_id);
            }
          }

          // ⭐ Parse child's available_filters: handles both string[] and object[]
          const childAvailableIds = (config.available_filters || []).map(
            (f: any) => (typeof f === "string" ? f : f.attribute_id)
          );
          ...
          const validInheritedIds = parentActiveIds.filter((id) =>
            childAvailableIds.includes(id)
          );
  ...
  // Handle override toggle separately
  useEffect(() => {
    // Skip if override hasn't actually changed
    if (prevOverride.current === overrideInheritance || !selectedCategoryId) {
      return;
    }

    prevOverride.current = overrideInheritance;
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      const filtersToSave =
        !overrideInheritance && inheritedFilters.size > 0
          ? Array.from(inheritedFilters)
          : Array.from(activeFilters);
      const activeFiltersArray = filtersToSave;
      const res = await fetch(
        `/admin/product-categories/${selectedCategoryId}/generate-filters`,
        {
          method: "POST",
          headers: {
    ...
    onSuccess: (data) => {
      toast.success(
        `Filters generated: ${data.filters_generated} filters for ${data.total_products} products`
      );
