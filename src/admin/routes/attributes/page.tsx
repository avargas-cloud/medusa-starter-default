...
export default function AttributesPage() {
  ...
  const [createAttributeModalOpen, setCreateAttributeModalOpen] =
    useState(false); // Attribute Modal (NEW)
  ...
  const renderAttributeRow = (attr: any) => (
    ...
    >
      ...
      <Badge size="small">{attr.options?.length || 0} options</Badge>
  ...
  return (
    <Container className="p-0 divide-y min-h-[600px]">
      ...
      <div className="px-6 py-4 flex flex-col gap-3">
          ...
          >
            <div className="flex items-center gap-2">
              {expandedSets.has("unassigned") ? (
                <ChevronDown className="text-ui-fg-subtle" />
              ) : (
                <ChevronRight className="text-ui-fg-subtle" />
              )}
              <Text weight="plus">Unassigned Attributes</Text>
              <Badge size="small" color="grey">
                {unassignedAttributes.length}
              </Badge>
            </div>
          </div>
            ...
            <div className="px-4 pb-4 pt-1 border-t border-ui-border-base/50">
              <div className="mb-2 flex items-center gap-2 mt-2">
                <Button
                  ...
                  size="small"
                  onClick={() =>
                    handleSelectAll(filterAttributes(unassignedAttributes))
                  }
                >
                  Select All
                </Button>
              ...
              <div className="flex flex-col divide-y divide-ui-border-base/50">
                {filterAttributes(unassignedAttributes).length > 0 ? (
                  ...
                  <Text className="text-ui-fg-muted italic py-2">
        ...
        {/* CUSTOM SETS */}
        {sets.map((set) => {
          const setAttributes = attributes.filter(
            (a) => a.attribute_set_id === set.id
          );
