    ...
    try {
      await onSave({
        ...
        metadata: {
          ...
          icon: icon.trim(),
          ledger_method: ledger,
          qb_method: qbMethod.trim() || null,
        },
      });
      onClose();
    } catch {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-ui-bg-base border border-ui-border-base rounded-xl shadow-2xl w-[460px] flex flex-col overflow-hidden">
        ...
        <div className="px-6 py-5 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Label className="mb-1 block text-sm">
                Display Name <span className="text-ui-fg-error">*</span>
              </Label>
              <Input
          ...
          <div>
            <Label className="mb-1 block text-sm">
              Internal Key <span className="text-ui-fg-error">*</span>
            </Label>
            <Input
              placeholder="e.g. visa"
              value={keyVal}
            ...
            <Text className="text-xs text-ui-fg-muted mt-1">
              Unique slug. Cannot change after creation.
            </Text>
          </div>
          <div>
            <Label className="mb-1 block text-sm">
              Ledger Method <span className="text-ui-fg-error">*</span>
            </Label>
            <select
              value={ledger}
            ...
            <Text className="text-xs text-ui-fg-muted mt-1">
              Maps to the Finance ledger payment category.
            </Text>
          </div>
          <div>
            <Label className="mb-1 block text-sm">QB Desktop Method</Label>
            <Input
              placeholder="e.g. Visa (leave blank if not synced to QB)"
              value={qbMethod}
              onChange={(e) => setQbMethod(e.target.value)}
            />
            <Text className="text-xs text-ui-fg-muted mt-1">
              Exact name in QuickBooks Desktop payment methods list.
            </Text>
          </div>
        </div>
...
function DefaultModal({
  ...
  const [context, setContext] = useState<string>(
    item?.context ?? KNOWN_CONTEXTS[0] ?? "Document Defaults"
  );
  const [customContext, setCustomContext] = useState("");

  const availableFields =
    (KNOWN_FIELDS_BY_CONTEXT as Record<string, string[]>)[context] || [];
  const initialField: string =
    item?.field_name ||
    (availableFields.length > 0 ? (availableFields[0] ?? "") : "");

  const [field, setField] = useState<string>(initialField);
  const [customField, setCustomField] = useState("");
  const [value, setValue] = useState<string>(item?.value ?? "");
  const [sortOrder, setSortOrder] = useState(
    item?.sort_order?.toString() ?? "0"
  );
  const [dataScope, setDataScope] = useState<string>(
    item?.data_scope ??
      (initialField ? FIELD_DEFAULT_SCOPE[initialField] : undefined) ??
      "customers,orders"
  );
  const [saving, setSaving] = useState(false);
  ...
  useEffect(() => {
    if (field && field !== "__custom__") {
      const contextForField = KNOWN_CONTEXTS.find((c) =>
        (KNOWN_FIELDS_BY_CONTEXT as Record<string, string[]>)[c]?.includes(
          field
        )
      );
      if (contextForField) setContext(contextForField);
      // Auto-set the default data_scope for the chosen known field
      const defaultScope = (FIELD_DEFAULT_SCOPE as Record<string, string>)[
        field
      ];
  ...
  const isCustomField =
    field === "__custom__" || !allKnownFields.includes(field);
  const isCustomContext =
    !KNOWN_CONTEXTS.some((c) => c === context) && context !== "__custom__";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-ui-bg-base border border-ui-border-base rounded-xl shadow-2xl w-[500px] max-h-[90vh] flex flex-col overflow-hidden">
        <div className="px-6 py-4 border-b border-ui-border-base flex items-center justify-between">
          <Heading level="h2">
            {item?.id ? "Edit System Default" : "New System Default"}
          </Heading>
          <button
            onClick={onClose}
            className="text-ui-fg-muted hover:text-ui-fg-base text-xl leading-none"
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <select
              value={
                field && allKnownFields.includes(field) ? field : "__custom__"
              }
              onChange={(e) => {
            ...
            >
              <optgroup
                ...
                className="bg-ui-bg-base text-ui-fg-base font-semibold"
              >
                {(KNOWN_FIELDS_BY_CONTEXT["Document Defaults"] ?? []).map(
                  (f) => (
                    <option
                      key={f}
                      value={f}
                      className="bg-ui-bg-base text-ui-fg-base font-normal"
                    >
                      {f}
                    </option>
                  )
              ...
              <optgroup
                label="Templates Footer"
                className="bg-ui-bg-base text-ui-fg-base font-semibold"
              >
                {(KNOWN_FIELDS_BY_CONTEXT["Templates Footer"] ?? []).map(
                  (f) => (
                    <option
                      key={f}
                      value={f}
              ...
              <optgroup
                ...
                className="bg-ui-bg-base text-ui-fg-base font-semibold"
              >
                {(KNOWN_FIELDS_BY_CONTEXT["Metadata Options"] ?? []).map(
                  (f) => (
                    <option
                      key={f}
                      value={f}
              ...
              <option
                value="__custom__"
                className="bg-ui-bg-base text-ui-fg-base"
              >
                Custom field…
              </option>
            </select>
          ...
          {!item?.field_name && (
            <div>
              ...
              <Text className="text-xs text-ui-fg-muted mt-1">
                At least one is required. Use ✏ on existing groups to change
                scope.
              </Text>
            </div>
          )}
        ...
        <div className="px-5 py-5 space-y-2">
          ...
          {ALL_DOC_SCOPES.map(({ key, label }) => {
            ...
            return (
              <label
                key={key}
                className="flex items-center gap-3 cursor-pointer select-none p-2 rounded-md hover:bg-ui-bg-subtle transition-colors"
              >
                <input
                  type="checkbox"
                  className="h-4 w-4 cursor-pointer"
                ...
                <div>
                  ...
                  <span className="block text-xs text-ui-fg-muted">
                    {key === "customers"
                      ? "Customer profile & customer-related docs"
          ...
          <Text className="text-xs text-ui-fg-muted pt-2">
            Updates all <strong>{fieldName}</strong> options at once.
          </Text>
        </div>
...
function ManualUserModal({
  ...
  const handleSave = async () => {
    ...
    try {
      await onSave(
        name.trim(),
        initials.trim().toUpperCase() || name[0]?.toUpperCase() || "?",
        isSalesRep
      );
      onClose();
    } catch {
      ...
      <div className="bg-ui-bg-base border border-ui-border-base rounded-xl shadow-2xl w-[400px] flex flex-col overflow-hidden">
        ...
        <div className="px-6 py-5 space-y-4">
          <Text className="text-xs text-ui-fg-muted">
            Adds a user that is not linked to a Medusa account (e.g. a channel
            or role like "Web").
          </Text>
          <div>
            <Label className="mb-1 block text-sm">
...
function DraggableOptionList({
  ...
  return (
    <div
      className={`divide-y divide-ui-border-base flex-1 relative ${saving ? "opacity-70 pointer-events-none" : ""}`}
    >
      {localItems.map((p: SystemDefault, idx) => (
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-ui-fg-muted w-4 inline-block text-right">
              {idx + 1}.
            </span>
          ...
          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            <Button
              ...
              size="small"
              onClick={() => onEdit(p)}
              className="h-6 w-6 p-0 text-ui-fg-muted hover:text-ui-fg-base"
            >
            ...
            {deleteConfirm === p.id ? (
              ...
              <Button
                ...
                className="h-6 w-6 p-0 text-ui-fg-muted hover:text-ui-error"
              >
...
const SystemDefaultsPage = () => {
  ...
  const [modal, setModal] = useState<Partial<SystemDefault> | null | false>(
    false
  );
  const [pmModal, setPmModal] = useState<Partial<SystemDefault> | null | false>(
    false
  );
  ...
  const handleSave = useCallback(
    async (data: Partial<SystemDefault>) => {
      if (modal && (modal as SystemDefault).id) {
        const r = await fetch(
          `/admin/system-defaults/${(modal as SystemDefault).id}`,
          {
            method: "PATCH",
            credentials: "include",
  ...
  const handleDelete = useCallback(
    async (id: string) => {
      try {
        await fetch(`/admin/system-defaults/${id}`, {
          method: "DELETE",
          credentials: "include",
        });
  ...
  const handleAddManualUser = useCallback(
    async (name: string, initials: string, isSalesRep: boolean) => {
      const existingUsers = contextGroups["Global"]?.["Sales Rep User"] ?? [];
      const maxSort = existingUsers.reduce(
        (max, u) => Math.max(max, u.sort_order),
        0
      );
      const newUserObj = {
  ...
  const handleUserUpdate = useCallback(
    async (item: SystemDefault, newParsedValue: any) => {
      try {
  ...
  const handleSaveScope = useCallback(
    async (items: SystemDefault[], newScope: string) => {
      await Promise.all(
        items.map((item) =>
  ...
  const handleReorder = useCallback(
    async (reordered: SystemDefault[]) => {
      const original = defaults.filter((d) =>
        reordered.some((r) => r.id === d.id)
      );
  ...
  const handleSavePm = useCallback(
    async (data: { value: string; metadata: PaymentMethodMeta }) => {
      if (pmModal && (pmModal as SystemDefault).id) {
        const r = await fetch(
          `/admin/system-defaults/${(pmModal as SystemDefault).id}`,
          {
            method: "PATCH",
            credentials: "include",
            headers: { "Content-Type": "application/json" },
        ...
        const nextOrder =
          defaults.filter((d) => d.context === "Payment Methods").length + 1;
        const r = await fetch("/admin/system-defaults", {
          method: "POST",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
  ...
  const contextOrder = [
    ...KNOWN_CONTEXTS,
    ...Object.keys(contextGroups).filter((c) => !KNOWN_CONTEXTS.includes(c)),
  ];
    ...
    <div className="p-6 space-y-6 max-w-5xl mx-auto pb-20">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <Text className="text-ui-fg-muted mt-1">
            Manage central dropdown options (Lead Times, Terms, Order Types,
            etc.) for the POS system.
          </Text>
        </div>
            ...
            <div className="flex items-center justify-between border-b border-ui-border-base pb-2">
              <div className="flex items-center gap-2">
                <Heading level="h2" className="text-lg">
                  POS Users / Sales Reps
                </Heading>
                <Badge color="blue">
                  {contextGroups["Global"]?.["Sales Rep User"]?.length ?? 0}{" "}
                  synced
                </Badge>
              </div>
                  ...
                  <tbody className="divide-y divide-ui-border-base">
                    {(contextGroups["Global"]?.["Sales Rep User"] || [])
                      ...
                      .map((item) => {
                        ...
                        return (
                          <tr
                            key={item.id}
                            className="hover:bg-ui-bg-base-hover group"
                          >
                    ...
                    {!contextGroups["Global"]?.["Sales Rep User"]?.length && (
                      <tr>
                        <td
                          colSpan={3}
                          className="px-4 py-8 text-center text-ui-fg-muted"
                        >
                          No users synced. Click 'Sync with Medusa Users' to
                          import active users.
                        </td>
                      </tr>
                    )}
          ...
          <div className="space-y-4 pt-6">
            <div className="flex items-center justify-between border-b border-ui-border-base pb-2">
              <div className="flex items-center gap-2">
                ...
                <Badge color="green">
                  {contextGroups["Payment Methods"]?.["Payment Method"]
                    ?.length ?? 0}{" "}
                  methods
                </Badge>
              </div>
                  ...
                  <tbody className="divide-y divide-ui-border-base">
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-4 py-3 text-xs text-ui-fg-muted">
                              {meta?.qb_method ?? (
                                <span className="italic text-ui-fg-subtle">
                                  None
                                </span>
                              )}
                            </td>
                              ...
                              <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                {deleteConfirm === item.id ? (
                                  ...
                                  <Button
                                    ...
                                    className="h-6 px-2 text-xs text-ui-fg-muted hover:text-ui-error"
                                  >
                                    Del
                                  </Button>
                                )}
                    ...
                    {!contextGroups["Payment Methods"]?.["Payment Method"]
                      ...
                      <tr>
                        <td
                          colSpan={6}
                          className="px-4 py-8 text-center text-ui-fg-muted italic text-sm"
                        >
                          No payment methods configured. Click "+ Add Method" or
                          reload to seed defaults.
                        </td>
                      </tr>
                    )}
                  </tbody>
            ...
            .map((contextName) => (
              <div key={contextName} className="space-y-4 pt-6">
                <div className="flex items-center gap-2 border-b border-ui-border-base pb-2">
                  ...
                  <Badge color={CONTEXT_COLORS[contextName] ?? "grey"}>
                    {
                      Object.values(contextGroups[contextName] ?? {}).flat()
                        .length
                    }{" "}
                    items
                  </Badge>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {Object.entries(contextGroups[contextName] ?? {}).map(
                    ([fieldName, items]) => {
                      const fieldScope: string | undefined = (items[0] as any)
                        ?.data_scope;
                      const scopeKeys = (fieldScope ?? "")
                        .split(",")
                        .map((s: string) => s.trim())
                        .filter(Boolean) as DocScopeKey[];
                              ...
                              {scopeKeys.length > 0 && (
                                <div className="flex gap-1 flex-wrap">
                                  {scopeKeys.map((sk) => (
                                    <Badge
                                      key={sk}
                                      color={SCOPE_BADGE_COLOR[sk] ?? "grey"}
                                      className="text-xs"
                                    >
                                      {ALL_DOC_SCOPES.find((s) => s.key === sk)
                                        ?.label ?? sk}
                                    </Badge>
                                  ))}
                                </div>
      ...
      {scopeModal && (
        <ScopeModal
          ...
          currentScope={
            (scopeModal.items[0] as any)?.data_scope ?? "customers,orders"
          }
