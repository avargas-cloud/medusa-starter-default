  ...
  const { data: categoriesData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        const { product_categories: roots } =
          await sdk.admin.productCategory.list({
            limit: 100,
        ...
        const byCategoriesRoot = roots.find(
          (c) => c.handle === "by-categories" || c.name === "BY CATEGORIES"
        );
        ...
        if (byCategoriesRoot) {
          ...
          const { product_categories: children } =
            await sdk.admin.productCategory.list({
              limit: 100,
        ...
        // 4. Custom Sorting
        const { CATEGORY_PRIORITY_LIST } =
          await import("../../products-advanced/category-sorting.js");
        ...
        return categoriesShim.sort((a, b) => {
          ...
          const priorityA = CATEGORY_PRIORITY_LIST.findIndex((p: string) =>
            nameA.includes(p.toLowerCase())
          );
          const priorityB = CATEGORY_PRIORITY_LIST.findIndex((p: string) =>
            nameB.includes(p.toLowerCase())
          );
