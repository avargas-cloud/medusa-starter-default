...
export const InventoryTable = ({
    ...
    const [selectedStockItem, setSelectedStockItem] = useState<{ id: string; title: string; sku: string } | null>(null)

    const formatPrice = (price: number | null | undefined, currencyCode: string) => {
        if (price === null || price === undefined || isNaN(price)) return `${currencyCode} $0.00`
    ...
    const HeaderCell = ({ column, children, className }: { column: string; children: React.ReactNode; className?: string }) => (
        <Table.HeaderCell
            className={clx("cursor-pointer hover:bg-ui-bg-subtle-hover transition-colors select-none whitespace-nowrap", className)}
    ...
    const StockCell = ({ value, colorZero = false, item }: { value: number; colorZero?: boolean; item: MeiliInventoryItem }) => (
        <Table.Cell className="w-16 text-center">
            ...
            >
                {value > 0 ? (
                    ...
                    <Badge color="red" size="small">0</Badge>
                ) : (
        ...
        <>
            ...
            <div className="flex-1 overflow-auto">
                <Table>
                    <Table.Header>
                        <Table.Row>
                            <Table.HeaderCell className="w-16 whitespace-nowrap">Image</Table.HeaderCell>
                            <HeaderCell column="title" className="max-w-[440px]">Title</HeaderCell>
                            <HeaderCell column="sku">SKU</HeaderCell>
                            {/* Numeric columns — compact width */}
                            <HeaderCell column="totalStock" className="w-20 text-center">In Stock</HeaderCell>
                            <HeaderCell column="totalReserved" className="w-20 text-center">Reserved</HeaderCell>
                            <Table.HeaderCell className="w-20 text-center whitespace-nowrap select-none">Available</Table.HeaderCell>
                            {/* Always: Retail Price */}
                            <HeaderCell column="price">Retail Price</HeaderCell>
                            {/* Dynamic price list columns */}
                            {enableDynamicPricing && priceLists.map((pl) => (
                                <Table.HeaderCell key={pl.id} className="whitespace-nowrap text-ui-fg-subtle">
                                    {pl.title}
                                </Table.HeaderCell>
                            ))}
                    ...
                    <Table.Body>
                        {items.length === 0 ? (
                            ...
                            items.map((item) => {
                                const available = Math.max(0, item.totalStock - item.totalReserved)
                                return (
                                        ...
                                        {enableDynamicPricing && priceLists.map((pl) => {
                                            ...
                                            const priceLabel = (
                                                <span className={clx(
                                                    ...
                                                    isFallback ? "text-ui-fg-disabled italic" : "text-ui-fg-subtle font-medium"
                                                )}>
                                        ...
                                        <Table.Cell>
                                            <DropdownMenu>
                                                ...
                                                <DropdownMenu.Content align="end">
                                                    <DropdownMenu.Item onClick={() => goToInventoryItem(item.id)}>
                                                        <PencilSquare className="mr-2" />
                                                        Edit Inventory
                                                    </DropdownMenu.Item>
                                                    {item.variantId && item.productId && (
                                                        <DropdownMenu.Item onClick={() => goToVariantEdit(item.productId, item.variantId)}>
                                                            <PencilSquare className="mr-2" />
                                                            Edit Variant
                                                        </DropdownMenu.Item>
                                                    )}
                                                    <DropdownMenu.Separator />
                                                    <DropdownMenu.Item className="text-ui-fg-error" onClick={() => { }}>
