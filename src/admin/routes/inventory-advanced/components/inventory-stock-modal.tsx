...
export const InventoryStockModal = ({
    ...
    const totalStocked = levels.reduce((s, l) => s + (l.stocked_quantity ?? 0), 0)
    const totalReserved = levels.reduce((s, l) => s + (l.reserved_quantity ?? 0), 0)
    const totalAvailable = levels.reduce((s, l) => s + (l.available_quantity ?? 0), 0)
        ...
        >
            ...
            >
                ...
                <div className="flex items-center justify-between px-5 py-4 border-b border-ui-border-base">
                    <div>
                        <p className="txt-compact-small text-ui-fg-muted font-mono">{sku}</p>
                            ...
                            {levels.length > 1 && (
                                <tfoot>
                                    <tr className="border-t border-ui-border-base bg-ui-bg-subtle">
                                        <td className="px-4 py-2 text-sm font-semibold text-ui-fg-subtle">Total</td>
                                        <td className="px-4 py-2 text-sm text-right text-ui-fg-muted">{totalReserved}</td>
                                        <td className="px-4 py-2 text-sm text-right font-semibold">{totalStocked}</td>
                                        <td className="px-4 py-2 text-sm text-right font-semibold text-ui-fg-interactive">{totalAvailable}</td>
                                    </tr>
                                </tfoot>
                            )}
                        </Table>
