...
import { Container, Heading, Text, Button, Table, Badge, Prompt } from "@medusajs/ui"
...
export function SequenceManagerCard() {
    ...
    const [editingSeq, setEditingSeq] = useState<keyof SequencesData | null>(null)
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            setSequences((prev: SequencesData | null) => prev ? { ...prev, [editingSeq]: numericValue } : null)
            setEditingSeq(null)
            setNewValue("")
            setConfirmText("")
        } catch (e: any) {
    ...
    return (
        <Container className="p-6">
            <div className="flex items-center justify-between mb-4">
                <div>
                    ...
                    <Text className="text-ui-fg-subtle text-sm">
                        Manage the internal numbering sequences used to generate document IDs matching QuickBooks Desktop.
                        Updates will take effect on the very next document created.
                    </Text>
                </div>
                <Button variant="secondary" size="small" onClick={fetchSequences} isLoading={loading}>
                    Refresh
                </Button>
            </div>
            ...
            <Table>
                ...
                <Table.Body>
                    {loading && !sequences ? (
                        ...
                        (Object.keys(SEQUENCE_LABELS) as (keyof SequencesData)[]).map((seqKey) => {
                            ...
                            return (
                                <Table.Row key={seqKey}>
                                    ...
                                    <Table.Cell>
                                        <span className="font-mono">{val !== null ? val : "Not Initialized"}</span>
                                    </Table.Cell>
            ...
            <Prompt open={!!editingSeq} onOpenChange={(open) => !open && setEditingSeq(null)}>
                <Prompt.Content>
                    <Prompt.Header>
                        <Prompt.Title>Update Document Sequence</Prompt.Title>
                        <Prompt.Description>
                            Warning: Changing the internal sequence number will affect all new documents generated of this type. 
                            Ensure the new number does not collide with existing QuickBooks records.
                        </Prompt.Description>
                    </Prompt.Header>
                    ...
                    <Prompt.Footer>
                        <Prompt.Cancel onClick={() => { setEditingSeq(null); setConfirmText(""); }}>Cancel</Prompt.Cancel>
                        <Button 
                            variant="primary"
                            onClick={handleSave} 
