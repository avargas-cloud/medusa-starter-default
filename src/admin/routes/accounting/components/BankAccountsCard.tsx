  ...
  const handleSetDefault = async (id: string, name: string) => {
    ...
    try {
      ...
      toast.success(`Default bank account set`, {
        description: `${name} is now the default for Write Checks`,
      });
      fetchAccounts();
    } catch (e) {
  ...
  return (
    <Container>
      <div className="p-4 border-b border-ui-border-base flex justify-between items-center">
        <div>
          <Heading level="h2" className="text-large font-medium mb-1">
            QuickBooks Bank Accounts
          </Heading>
          <Text className="text-ui-fg-subtle text-small">
            Map QuickBooks Bank Accounts using their ListID. Mark one as default
            for Write Checks (refunds).
          </Text>
        </div>
        <Button
          variant="secondary"
      ...
      <div className="p-4 flex gap-2 items-end bg-ui-bg-subtle border-b border-ui-border-base">
        <div className="flex-1">
          <Text className="text-small font-medium mb-1">
            Account Name (e.g. Cash Register)
          </Text>
          <Input
        ...
        <div className="flex-1">
          ...
          <Input
            placeholder="80000000-XXXXXXXXX"
            value={newListId}
        ...
        <Table.Body>
          {loading ? (
            <Table.Row>
              <Table.Cell
                {...{ colSpan: 5 }}
                className="text-center text-ui-fg-subtle p-4"
              >
                Loading...
              </Table.Cell>
            </Table.Row>
            ...
            accounts.map((acc) => (
              <Table.Row key={acc.id}>
                ...
                <Table.Cell>
                  <span
                    className={`px-2 py-1 rounded-full text-xs ${acc.is_active ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
                  >
