...
export const PendingRefundsCard = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (bankRes.ok) {
        ...
        // Only active banks
        setBankAccounts(
          (bData.qb_bank_accounts || []).filter((b: any) => b.is_active)
        );
      }
    } catch (e) {
  ...
  const handleApprove = async (refundId: string) => {
    ...
    try {
      ...
      toast.error("Failed to orchestrate sync", {
        description: (e as Error).message,
      });
    } finally {
      setSyncingId(null);
    ...
    <Container>
      <div className="p-4 border-b border-ui-border-base flex justify-between items-center">
        <div>
          ...
          <Text className="text-ui-fg-subtle text-small">
            Review and approve POS refunds to synchronize them with QuickBooks
            via Check + zero-dollar payment calculation.
          </Text>
        </div>
      ...
      <Table>
        ...
        <Table.Body>
          {loading ? (
            <Table.Row>
              <Table.Cell className="text-center text-ui-fg-subtle p-4">
                Loading...
              </Table.Cell>
              ...
              <Table.Row key={refund.id}>
                ...
                <Table.Cell>
                  <Select
                    ...
                    onValueChange={(val) =>
                      setSelectedBankId((p) => ({ ...p, [refund.id]: val }))
                    }
                  >
