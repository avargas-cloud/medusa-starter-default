...
import { Container, Heading, Button, Input, Label, Text, toast, Switch } from "@medusajs/ui"
...
const ShippingConfigPage = () => {
    ...
    useEffect(() => {
        const loadSettings = async () => {
            try {
                const res = await fetch('/admin/shipping-settings', { method: 'GET', credentials: 'include' })
                if (!res.ok) return
    ...
    const handleSave = async () => {
        ...
        if (overrideUpsGround) {
            if (isNaN(freeShipNum) || freeShipNum < 0) {
                toast.error("Validation Error", { description: "Free shipping minimum must be a valid non-negative number" })
                return
            }
            if (isNaN(regularNum) || regularNum < 0) {
                toast.error("Validation Error", { description: "Regular ground shipping price must be a valid non-negative number" })
                return
            }
            if (isNaN(longNum) || longNum < 0) {
                toast.error("Validation Error", { description: "Long item ground shipping price must be a valid non-negative number" })
                return
            }
        }
        ...
        try {
            ...
            if (!res.ok) {
                ...
                toast.error("Failed to save settings", { description: errorData.error || "Unknown error occurred" })
                return
            }
            ...
            toast.error("Failed to save settings", { description: error instanceof Error ? error.message : "Unknown error" })
        } finally {
            setLoading(false)
        }
    }
        ...
        <div className="flex flex-col gap-3 p-6 max-w-2xl">
            ...
            <Container>
                <div className="p-4">
                    <div className="flex items-center justify-between">
                        <div>
                            <Heading level="h3" className="text-sm font-medium">🚚 Override UPS Ground Shipping</Heading>
                            <Text className="text-xs text-ui-fg-subtle mt-1">
                                When <strong>enabled</strong>, use custom fixed pricing based on order total and item types.
                                When <strong>disabled</strong>, use UPS native Ground service with real-time rates.
                            </Text>
                        </div>
                        <div className="flex items-center gap-3 ml-4 shrink-0">
                            <Text className={`text-xs font-semibold ${overrideUpsGround ? 'text-green-600' : 'text-ui-fg-muted'}`}>
                    ...
                    {!overrideUpsGround && (
                        <div className="mt-3 p-2 rounded bg-ui-bg-subtle border border-ui-border-base">
                            <Text className="text-xs text-ui-fg-subtle">
                                ℹ️ Using UPS real-time rates. Enable override to set custom shipping prices below.
                            </Text>
                        </div>
                    )}
                </div>
            ...
            <Container>
                <div className="p-4 space-y-3">
                    ...
                    <div>
                        <Label htmlFor="free-shipping-min" className="mb-1 block text-xs">Minimum Order Amount ($)</Label>
                        <Input
                        ...
                        />
                        <Text size="small" className="text-ui-fg-subtle mt-1">Orders above this amount qualify for free shipping</Text>
                    </div>
                </div>
            </Container>
            ...
            <Container>
                <div className="p-4 space-y-3">
                    ...
                    <div>
                        <Label htmlFor="regular-ground-price" className="mb-1 block text-xs">Price for Regular Items ($)</Label>
                        <Input
                            ...
                            disabled={!overrideUpsGround}
                        />
                        <Text size="small" className="text-ui-fg-subtle mt-1">Flat shipping rate for regular items</Text>
                    </div>
                </div>
            </Container>
            ...
            <Container>
                <div className="p-4 space-y-3">
                    ...
                    <div>
                        <Label htmlFor="long-item-price" className="mb-1 block text-xs">Price for Long Items ($)</Label>
                        <Input
                            id="long-item-price"
                            type="number"
                            min="0"
                            ...
                            disabled={!overrideUpsGround}
                        />
                        <Text size="small" className="text-ui-fg-subtle mt-1">Flat rate when order contains items with long shipping profile</Text>
                    </div>
