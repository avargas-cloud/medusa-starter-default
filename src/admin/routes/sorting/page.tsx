...
export const SortingPage = () => {
  ...
  const { categories, subcategories, products, isLoading } =
    useSortingData(selectedCategoryId);
  ...
  useEffect(() => {
    if (selectedCategoryId && subcategories.length > 0) {
      ...
      console.log(
        "[Sorting Debug] Subcategories from API (sorted by rank):",
        subcategories.map((s) => `${s.name} (rank=${s.rank})`)
      );
  ...
  useEffect(() => {
    if (selectedCategoryId && products.length > 0) {
      ...
      const preservedOrder = normalizedConfigOrder.filter((id) =>
        existingIds.includes(id)
      );
      const newItems = existingIds.filter(
        (id) => !normalizedConfigOrder.includes(id)
      );

      // If no config exists, sort new items alphabetically as fallback
      const sortedNewItems =
        normalizedConfigOrder.length === 0
          ? newItems.sort((a, b) => {
      ...
      <div className="mb-6 flex items-start justify-between">
        <div>
          ...
          <p className="text-ui-fg-muted text-sm mt-2">
            Manage the display order of subcategories and products within each
            category. Drag items to reorder them.
          </p>
        </div>
        {selectedCategoryId && (
          <Button
