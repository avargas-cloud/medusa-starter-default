...
const LogRow = ({ log }: { log: LogEntry }) => {
  ...
  return (
    <div
      className={`border-b border-ui-border-base last:border-0 transition-colors ${
        log.status === "failed" ? "bg-red-50/5" : ""
      }`}
    >
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          {log.status === "failed" && log.error && (
            <Text
              className="text-xs text-red-500 font-mono truncate max-w-[36rem]"
              title={log.error}
            >
              ⚠{" "}
        ...
        <div className="flex items-center gap-3 shrink-0 text-right">
          {log.duration_ms !== null && (
            <Text className="text-xs text-ui-fg-muted hidden sm:block">
          ...
          {log.server_host && (
            <Text
              ...
              style={{
                color: log.server_host.startsWith("Railway")
                  ? "#f97316"
                  : "#94a3b8",
              }}
            >
              {log.server_host.startsWith("Railway")
                ? "⚡ Railway"
                : `💻 ${log.server_host.substring(0, 12)}`}
            </Text>
          )}
          <Text className="text-xs text-ui-fg-muted hidden md:block w-36">
            {formatDate(log.initiated_at)}
          </Text>
      ...
      {expanded && (
        <div className="px-10 pb-3 pt-1 space-y-1 bg-ui-bg-subtle/30 border-t border-ui-border-base/50">
          <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-xs">
            {log.event_type && (
              <>
                <Text className="text-ui-fg-muted">Event</Text>
                <Text className="font-mono">{log.event_type}</Text>
              </>
            )}
            {log.qb_txn_id && (
              <>
                <Text className="text-ui-fg-muted">QB TxnID</Text>
                <Text className="font-mono">{log.qb_txn_id}</Text>
              </>
            )}
            {log.qb_ref_number && (
              <>
                <Text className="text-ui-fg-muted">QB Ref#</Text>
                <Text className="font-mono">{log.qb_ref_number}</Text>
              </>
            )}
            {log.order_id && (
              <>
                <Text className="text-ui-fg-muted">Order ID</Text>
            ...
            {log.completed_at && (
              <>
                <Text className="text-ui-fg-muted">Completed</Text>
                <Text>{formatDate(log.completed_at)}</Text>
              </>
            )}
            {log.duration_ms !== null && (
              <>
                <Text className="text-ui-fg-muted">Duration</Text>
                <Text>{formatDuration(log.duration_ms)}</Text>
              </>
            )}
          </div>
          {/* Inventory sync changed items */}
          {log.operation === "inventory_sync" &&
            log.metadata?.changedItems &&
            log.metadata.changedItems.length > 0 && (
              <div className="mt-3">
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="bg-ui-bg-subtle border-b border-ui-border-base">
                        <th className="text-left px-3 py-1.5 text-ui-fg-muted font-medium">
                          SKU
                        </th>
                        <th className="text-right px-3 py-1.5 text-ui-fg-muted font-medium">
                          Before
                        </th>
                        <th className="text-right px-3 py-1.5 text-ui-fg-muted font-medium">
                          After
                    ...
                    <tbody>
                      {log.metadata.changedItems.map((item: ChangedItem) => (
                        <tr
                          key={item.sku}
                          className="border-b border-ui-border-base/50 last:border-0 hover:bg-ui-bg-subtle/50"
                        >
                          <td className="px-3 py-1.5">
                            <span className="font-mono">{item.sku}</span>
                            {item.anomaly && (
                              <span
                                className="ml-1.5 text-orange-400"
                          ...
                          <td className="px-3 py-1.5 text-right text-ui-fg-subtle">
                            {item.prev}
                          </td>
...
function ClearLogButton({ onDone }: { onDone: () => void }) {
  ...
  const handleClear = async () => {
    ...
    setLoading(true);
    try {
      await fetch("/admin/quickbooks/logs", {
        method: "DELETE",
        credentials: "include",
      });
      onDone();
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-ui-bg-base rounded-lg shadow-xl border border-ui-border-base p-5 w-80">
            ...
            <p className="text-xs text-ui-fg-subtle mb-4">
              This will permanently delete all entries from the QB sync log.
  ...
  const fetchLogs = useCallback(
    async (pg = page, st = status) => {
      ...
      try {
        ...
        const res = await fetch(`/admin/quickbooks/logs?${params}`, {
          credentials: "include",
        });
        if (!res.ok) return;
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <Text className="text-xs text-ui-fg-subtle mt-0.5">
              Background sync operations — Inventory, Price, Customer &amp; POS
              sync
            </Text>
          </div>
        ...
        <div className="rounded-lg border border-ui-border-base overflow-hidden">
          {loading && logs.length === 0 && (
            <div className="flex items-center justify-center h-24">
              <Text className="text-xs text-ui-fg-subtle animate-pulse">
                Loading...
              </Text>
            </div>
          )}
          {!loading && logs.length === 0 && (
            <div className="flex flex-col items-center justify-center h-24 gap-1">
              ...
              <Text className="text-xs text-ui-fg-muted">
                Order events and syncs will appear here
              </Text>
            </div>
          )}
          {logs.map((log) => (
