 ...
 */
export function SyncReportModal({ jobId, title, onClose }: SyncReportModalProps) {
    const [lines, setLines] = useState<LogLine[]>([])
    const [status, setStatus] = useState<"connecting" | "running" | "done" | "error" | "no-report">("connecting")
    ...
    useEffect(() => {
        ...
        src.onmessage = (e) => {
            ...
            if (msg.type === "log") {
                ...
                setLines(prev => [...prev, { line: msg.line, type: isWarn ? "warn" : "log" }])
                setStatus("running")
        ...
        >
            <div className="bg-ui-bg-base rounded-xl shadow-2xl w-full max-w-3xl mx-4 flex flex-col max-h-[80vh] border border-ui-border-base">
                ...
                <div className="flex-1 overflow-y-auto px-6 py-4 space-y-0.5">
                    {status === "no-report" && (
                        <div className="flex flex-col items-center justify-center h-40 gap-2">
                            <Text className="text-ui-fg-subtle text-sm">No report available yet.</Text>
                            <Text className="text-ui-fg-muted text-xs">Click "Sync Now" to start a new sync, then open this report.</Text>
                        </div>
                    )}
                    {status === "connecting" && lines.length === 0 && (
                        <div className="flex items-center justify-center h-40">
                            <Text className="text-ui-fg-subtle text-sm animate-pulse">Connecting to sync stream...</Text>
                        </div>
                    )}
                    {lines.map((l, i) => (
                ...
                <div className="px-6 py-3 border-t border-ui-border-base shrink-0 flex justify-between items-center">
                    <Text className="text-ui-fg-muted text-xs">{lines.length} log lines</Text>
                    <Button
                        variant={status === "running" || status === "connecting" ? "secondary" : "primary"}
                        size="small"
                        onClick={onClose}
                    >
                        {status === "running" || status === "connecting" ? "Running (close)" : "Close"}
                    </Button>
                </div>
            </div>
        </div>
