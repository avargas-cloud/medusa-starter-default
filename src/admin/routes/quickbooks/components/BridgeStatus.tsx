...
export function BridgeStatus() {
    ...
    const fetchStats = useCallback(async (silent = false) => {
        ...
        try {
            const res = await fetch("/admin/quickbooks/bridge", { credentials: "include" })
            const data = await res.json()
            setStats(data)
        } catch {
            setStats({ success: false, error: "Bridge unreachable", total: 0, pending: 0, processing: 0, completed: 0, failed: 0, qbBusy: false, qbBusyUntil: null, oldestPendingAgeMinutes: 0 })
        } finally {
    ...
    const busyUntilLabel = stats?.qbBusyUntil
        ? new Date(stats.qbBusyUntil).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit", second: "2-digit" })
        : null
                ...
                <div className="flex items-center justify-between mb-3">
                    <div>
                        <Heading level="h3" className="text-sm font-medium flex items-center gap-2">
                            🔌 QB Bridge Status
                            {stats && !stats.error && (
                                <span className="text-[10px] font-normal text-green-600">● Connected</span>
                            )}
                            {stats?.error && (
                                <span className="text-[10px] font-normal text-red-500">● Unreachable</span>
                            )}
                        </Heading>
                    ...
                    <div className="space-y-3">
                        ...
                        {stats.qbBusy && (
                            <div className="flex items-center justify-between rounded-md bg-orange-50 border border-orange-200 px-4 py-2.5">
                                <div>
                                    <p className="text-sm font-semibold text-orange-800">⚠️ QB Busy — queue paused</p>
                                    <p className="text-xs text-orange-600 mt-0.5">
                                        Web Connector returned "another update in progress".
                                        {busyUntilLabel && ` Auto-clears at ${busyUntilLabel}.`}
                                        {stats.pending > 0 && ` ${stats.pending} op${stats.pending !== 1 ? "s" : ""} waiting.`}
                                    </p>
                                </div>
                                <Button
                                    size="small"
                        ...
                        <div className="grid grid-cols-5 gap-2">
                            {([
                                { label: "Pending",    value: stats.pending,    color: stats.pending    > 0 ? "orange" : "grey" },
                                { label: "Processing", value: stats.processing, color: stats.processing > 0 ? "blue"   : "grey" },
                                { label: "Completed",  value: stats.completed,  color: "green" },
                                { label: "Failed",     value: stats.failed,     color: stats.failed     > 0 ? "red"    : "grey" },
                                { label: "Total",      value: stats.total,      color: "grey" },
                            ] as { label: string; value: number; color: "orange" | "blue" | "green" | "red" | "grey" }[]).map(({ label, value, color }) => (
                                <div key={label} className="flex flex-col items-center rounded-md border border-ui-border-base p-2 gap-1">
                                    <span className="text-[11px] text-ui-fg-muted">{label}</span>
                                    <Badge color={color} size="xsmall">{value}</Badge>
                                </div>
                            ))}
                        </div>
                        ...
                        {stats.pending > 0 && stats.oldestPendingAgeMinutes > 10 && (
                            <p className="text-xs text-orange-600">
                                ⏳ Oldest pending op: {stats.oldestPendingAgeMinutes}m ago — Web Connector may not be running.
                            </p>
                        )}
                    </div>
                ) : (
