...
const QuickBooksPage = () => {
  ...
  const [lastInventorySync, setLastInventorySync] = useState<string | null>(
    null
  );
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // Store hours
        if (config.store_hours_open_hour != null)
          setStoreOpenHour(String(config.store_hours_open_hour));
        if (config.store_hours_close_hour != null)
          setStoreCloseHour(String(config.store_hours_close_hour));
        if (config.store_sat_open != null)
          setStoreSatOpen(config.store_sat_open);
        if (config.store_sat_open_hour != null)
          setStoreSatOpenHour(String(config.store_sat_open_hour));
        if (config.store_sat_close_hour != null)
          setStoreSatCloseHour(String(config.store_sat_close_hour));
        if (config.store_sun_open != null)
          setStoreSunOpen(config.store_sun_open);
        if (config.store_sun_open_hour != null)
          setStoreSunOpenHour(String(config.store_sun_open_hour));
        if (config.store_sun_close_hour != null)
          setStoreSunCloseHour(String(config.store_sun_close_hour));
        if (config.store_hours_timezone)
          setStoreTimezone(config.store_hours_timezone);

        // Respect hours flags
        if (config.inventory_respect_hours != null)
        ...
        setInventoryInterval(
          config.inventory_interval_minutes != null
            ? String(config.inventory_interval_minutes)
            : "disabled"
        );
        setPriceInterval(
          config.price_interval_minutes != null
    ...
    const loadJobs = async () => {
      try {
        const [inv, prices, cust, recon] = await Promise.all([
          ...
          fetch("/admin/quickbooks/sync/last-job?type=reconcile", {
            credentials: "include",
          })
            .then((r) => r.json())
            .catch(() => ({ job_id: null })),
      ...
      toast.success(
          ...
          : "QuickBooks Integration DISABLED",
        {
          description: newValue
            ? "All syncs will run normally."
            : "All syncs are paused until re-enabled.",
        }
      );
    } catch (e) {
  ...
  const handleSaveStoreHours = async () => {
    try {
      ...
      toast.success("Store hours saved", {
        description: "Your store schedule has been updated.",
      });
    } catch (e) {
    ...
    try {
      ...
      await postConfig({
        price_sync_interval_minutes:
          priceInterval === "disabled" ? null : parseInt(priceInterval) * 60,
        price_respect_hours: priceRespectHours,
        price_sync_hour: priceSyncHour,
      });
      toast.success("Price sync saved", {
        description: `Scheduled daily at ${priceTimeOfDay}.`,
      });
    } catch (e) {
      toast.error("Failed to save", { description: (e as Error).message });
    }
    ...
    try {
      await postConfig({
        customer_sync_interval_minutes:
          customerInterval === "disabled"
            ? null
            : parseInt(customerInterval) * 60,
        customer_respect_hours: customerRespectHours,
      });
  ...
  const handleReconcile = async (isDryRun: boolean = false) => {
    ...
    try {
      ...
      setReportModal({
        jobId: data.job_id,
        title: `🔍 QB Reconcile ${isDryRun ? "(Dry Run)" : ""}`,
      });
      const recon = await fetch(
        "/admin/quickbooks/sync/last-job?type=reconcile",
        { credentials: "include" }
      )
        .then((r) => r.json())
        .catch(() => ({ job_id: null }));
      if (recon.job_id)
        setLastJobIds((p) => ({ ...p, reconcile: recon.job_id }));
    } catch (e) {
  ...
  return (
    <div className="flex flex-col gap-3 p-6 w-full">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <Text className="text-xs text-ui-fg-subtle mt-1">
                Master kill switch — when disabled, all QB syncs and order flows
                are paused immediately.
              </Text>
            </div>
            <div className="flex items-center gap-3">
              <Text
                className={`text-xs font-semibold ${qbEnabled === null ? "text-ui-fg-muted" : qbEnabled ? "text-green-600" : "text-red-500"}`}
              >
                {qbEnabled === null
                  ? "Loading..."
                  : qbEnabled
              ...
              <Switch
                ...
                onCheckedChange={() => {
                  if (qbEnabled !== null && !qbToggling) handleQbToggle();
                }}
          ...
          {qbEnabled === false && (
            <div className="mt-3 p-2 rounded bg-red-50 border border-red-200">
              <Text className="text-xs text-red-600">
                🔴 Integration is disabled. Re-enable to resume syncs and order
                flows.
              </Text>
            </div>
          )}
      ...
      <StoreHoursSection
        ...
        setStoreOpenHour={setStoreOpenHour}
        storeCloseHour={storeCloseHour}
        setStoreCloseHour={setStoreCloseHour}
        storeSatOpen={storeSatOpen}
        setStoreSatOpen={setStoreSatOpen}
        storeSatOpenHour={storeSatOpenHour}
        setStoreSatOpenHour={setStoreSatOpenHour}
        storeSatCloseHour={storeSatCloseHour}
        setStoreSatCloseHour={setStoreSatCloseHour}
        storeSunOpen={storeSunOpen}
        setStoreSunOpen={setStoreSunOpen}
        storeSunOpenHour={storeSunOpenHour}
        setStoreSunOpenHour={setStoreSunOpenHour}
        storeSunCloseHour={storeSunCloseHour}
        setStoreSunCloseHour={setStoreSunCloseHour}
      ...
      <SyncCard
        ...
        onViewReport={() =>
          setReportModal({
            jobId: lastJobIds.inventory ?? null,
      ...
      <SyncCard
        ...
        onViewReport={() =>
          setReportModal({
            jobId: lastJobIds.prices ?? null,
            title: "💵 Price Sync Report",
          })
        }
      ...
      <SyncCard
        ...
        onViewReport={() =>
          setReportModal({
            jobId: lastJobIds.customers ?? null,
            title: "👥 Customer Sync Report",
          })
        }
        ...
        footer={
          <div className="flex gap-2">
            ...
            <Button
              ...
              onClick={() =>
                setReportModal({
                  jobId: lastJobIds.reconcile ?? null,
                  title: "🔍 QB Reconcile Report",
                })
              }
              className="flex-1"
            >
