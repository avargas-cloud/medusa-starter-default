...
export const useOrdersList = (fulfillmentFilters: FulfillmentFilter[]) => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const params = new URLSearchParams({
          limit: "250",
          fields:
            "id,display_id,status,fulfillment_status,payment_status,total,currency_code,created_at,+metadata,+customer.first_name,+customer.last_name,+customer.email,+customer.phone,+customer.company_name,+shipping_address.first_name,+shipping_address.last_name,+shipping_address.phone,+shipping_address.company,+sales_channel.name",
        });
        const r = await fetch(`/admin/orders?${params}`, {
          credentials: "include",
        });
        if (r.ok) {
  ...
  const filtered = useMemo(() => {
    ...
    const q =
      search.toLowerCase().replace(/[^\d]/g, "") || search.toLowerCase();
    ...
    return fulfillmentFiltered.filter((o) => {
      ...
      const company =
        o.customer?.company_name ?? o.shipping_address?.company ?? "";
