...
import {
    Container, Heading, Button, Input, Label, Text, toast, Badge,
} from "@medusajs/ui"
...
// ── Group colour map ───────────────────────────────────────────────────────────
const GROUP_COLORS: Record<string, "blue" | "green" | "orange" | "purple" | "grey"> = {
    "Store Policy": "blue",
    "Scope of Work": "green",
    "Installation": "orange",
    "Projects": "purple",
...
const KNOWN_GROUPS = ["Store Policy", "Scope of Work", "Installation", "Projects"]
...
function PresetModal({
    ...
    const handleSave = async () => {
        ...
        setSaving(true)
        try {
            await onSave({ group_name: g, title: title.trim(), content: content.trim() })
            onClose()
        } catch {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="bg-ui-bg-base border border-ui-border-base rounded-xl shadow-2xl w-[600px] max-h-[90vh] flex flex-col overflow-hidden">
                <div className="px-6 py-4 border-b border-ui-border-base flex items-center justify-between">
                    <Heading level="h2">{preset?.id ? "Edit Preset" : "New Preset"}</Heading>
                ...
                <div className="px-6 py-4 border-t border-ui-border-base flex justify-end gap-2">
                    ...
                    <Button onClick={handleSave} isLoading={saving}>Save Preset</Button>
                </div>
            </div>
        </div>
    )
...
const NotePresetsPage = () => {
    ...
    const handleSave = useCallback(async (data: Partial<NotePreset>) => {
        if (modal && (modal as NotePreset).id) {
            const r = await fetch(`/admin/note-presets/${(modal as NotePreset).id}`, {
    ...
    const handleDelete = useCallback(async (id: string) => {
        try {
            await fetch(`/admin/note-presets/${id}`, { method: "DELETE", credentials: "include" })
                            ...
                            {grouped[groupName]!.map(p => (
                                <div key={p.id} className="px-4 py-3 flex gap-4">
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <Text className="text-xs text-ui-fg-muted mt-0.5 whitespace-pre-wrap line-clamp-2">{p.content}</Text>
                                    </div>
                                    <div className="flex items-start gap-2 flex-shrink-0">
                                        <Button variant="secondary" size="small" onClick={() => setModal(p)}>Edit</Button>
                                        {deleteConfirm === p.id ? (
                                            <>
                                                <Button variant="danger" size="small" onClick={() => handleDelete(p.id)}>Confirm</Button>
                                                <Button variant="secondary" size="small" onClick={() => setDeleteConfirm(null)}>Cancel</Button>
                                            </>
                                        ) : (
                                            <Button variant="danger" size="small" onClick={() => setDeleteConfirm(p.id)}>Delete</Button>
                                        )}
                                    </div>
                                </div>
                            ))}
