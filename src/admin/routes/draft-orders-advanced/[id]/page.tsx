...
const DraftOrderDetail = () => {
  ...
  const {
    ...
    s.order,
    s.handleAddShipping,
    s.handleReplaceShipping,
    s.handleUpdateShippingAmount
  );
  ...
  const p = usePageDerived({
    ...
    itemQtys: s.itemQtys,
    handleAddItem: s.handleAddItem,
    handleUpdateItem: s.handleUpdateItem,
    handleRemoveItem: s.handleRemoveItem,
    handleConvert: s.handleConvert,
  ...
  if (s.fetchError || !s.order)
    return (
      <div className="p-6">
        <Text className="text-ui-fg-error">
          Error: {s.fetchError ?? "Not found"}
        </Text>
      </div>
    );
  ...
  const customerName = order.customer
    ...
    : order.email || "—";
  const scName =
    order.sales_channel?.name === "Default Sales Channel"
      ? "Default"
      : (order.sales_channel?.name ?? "Default");
  const isWholesale = (() => {
    const cust = order.customer as any;
    return (
      (cust?.groups ?? []).some((g: any) =>
        (g.name ?? "").toLowerCase().includes("wholesale")
      ) ||
      ((cust?.metadata?.price_level as string) ?? "")
        .toLowerCase()
        .includes("wholesale")
    );
  })();
  const estimateInitialInfo: EstimateInfo = {
    rep: (order.metadata?.estimate_rep ??
      (order.customer as any)?.metadata?.default_rep ??
      "") as string,
    orderType: (order.metadata?.estimate_order_type ??
      (order.customer as any)?.metadata?.default_order_type ??
      "") as string,
    leadTime: (order.metadata?.estimate_lead_time ??
      (order.customer as any)?.metadata?.default_lead_time ??
      "") as string,
    paymentTerms: (order.metadata?.estimate_payment_terms ??
      (order.customer as any)?.metadata?.default_payment_terms ??
      "") as string,
  ...
  return (
    <>
      <div className="flex gap-4 p-6">
        ...
        <div className="flex-1 flex flex-col gap-4 min-w-0">
          <OrderHeader
            ...
            onConvert={p.handleConvertClick}
          ...
          <Container className="p-0 overflow-visible">
            ...
            <InlineItemsTable
              ...
              curr={curr}
              invQuery={s.invQuery}
              invResults={s.invResults}
              itemQtys={s.itemQtys}
              setItemQtys={s.setItemQtys}
              itemPrices={s.itemPrices}
              setItemPrices={s.setItemPrices}
              searchInvItems={s.searchInvItems}
              handleAddItem={p.handleAddItemWithTax}
              handleUpdateItem={p.handleUpdateItemWithTax}
              handleRemoveItem={p.handleRemoveItemWithTax}
              itemSaving={s.itemSaving}
          ...
          <Container className="p-0 overflow-hidden">
            ...
            <InlineNotes
              orderId={id!}
              initialNotes={
                (order.metadata?.estimate_notes as string | undefined) ?? ""
              }
            />
          </Container>
          <Container className="p-0 overflow-hidden">
            <InlineShipping
              ...
              shippingOptions={inlineShippingOptions}
              curr={curr}
              shippingSaving={s.shippingSaving}
              loadShippingOptions={loadShippingOptions}
              handleAddShipping={handleAddShippingInline}
          ...
          <Container className="p-0 overflow-hidden">
            ...
            <InlineTaxes
              orderId={id!}
              curr={curr}
              onTaxChange={p.setTaxAmount}
              onTaxRateChange={p.setTaxRate}
              triggerKey={p.taxTrigger}
            />
          </Container>
          <OrderTotals
            subtotal={p.computedSubtotal}
            shippingTotal={p.shippingDollars}
            discountTotal={p.discountDollars}
        ...
        <OrderSidebar
          ...
          estimateRef={
            s.localRef ??
            (order.metadata?.qb_estimate &&
            typeof order.metadata.qb_estimate === "object"
      ...
      <OrderDrawers
        ...
        selectedSc={s.selectedSc}
        setSelectedSc={s.setSelectedSc}
        handleSaveSalesChannel={s.handleSaveSalesChannel}
        emailForm={s.emailForm}
        setEmailForm={s.setEmailForm}
        handleSaveEmail={s.handleSaveEmail}
        shippingAddrForm={s.shippingAddrForm}
        setShippingAddrForm={s.setShippingAddrForm}
        handleSaveShippingAddr={s.handleSaveShippingAddr}
        billingAddrForm={s.billingAddrForm}
        setBillingAddrForm={s.setBillingAddrForm}
        handleSaveBillingAddr={s.handleSaveBillingAddr}
        ...
        handleTransfer={s.handleTransfer}
        ...
        metaNewKey={s.metaNewKey}
        setMetaNewKey={s.setMetaNewKey}
        metaNewVal={s.metaNewVal}
        setMetaNewVal={s.setMetaNewVal}
        handleSaveMetadata={s.handleSaveMetadata}
        handleAddMetaKey={s.handleAddMetaKey}
      />
            ...
            <div className="flex items-center gap-3 px-6 py-5 border-b border-ui-border-base">
              ...
              <div>
                <Heading level="h2" className="text-ui-fg-base">
                  Delete Draft Order #{order.display_id}?
                </Heading>
              ...
              <Text className="text-ui-fg-subtle text-sm">
                ...
                {!!(
                  ...
                  <span className="block mt-2 text-ui-fg-muted">
                    The linked QuickBooks Estimate will be marked as{" "}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-ui-bg-base border border-ui-border-base rounded-xl shadow-2xl w-full max-w-md mx-4 overflow-hidden">
            ...
            <div className="flex items-center gap-3 px-6 py-5 border-b border-ui-border-base">
              ...
              <div>
                <Heading level="h2" className="text-ui-fg-base">
                  Cancel Draft Order #{order.display_id}?
                </Heading>
                <Text size="small" className="text-ui-fg-muted mt-0.5">
                  The draft order will be kept for reference
                </Text>
              </div>
            </div>
            {/* Body */}
            <div className="px-6 py-5">
              <Text className="text-ui-fg-subtle text-sm">
                The draft order will remain in Medusa with status{" "}
