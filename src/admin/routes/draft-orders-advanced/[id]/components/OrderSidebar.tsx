        ...
        <div className="w-[340px] shrink-0 flex flex-col gap-4">
            ...
            <Container className="p-0 overflow-hidden">
                <div className="flex items-center justify-between px-6 py-4 border-b border-ui-border-base">
                    ...
                    {estimateStatus && <Badge color={badgeColor as "grey" | "blue" | "purple" | "orange" | "green" | "red"} size="small">{estimateStatus}</Badge>}
                </div>
                <div className="px-6 py-4">
                    <Select value={estimateStatus} onValueChange={onStatusChange} disabled={statusSaving}>
                        <Select.Trigger className="w-full"><Select.Value placeholder="Select a status..." /></Select.Trigger>
                        <Select.Content>{ESTIMATE_STATUSES.map(st => <Select.Item key={st} value={st}>{st}</Select.Item>)}</Select.Content>
                    </Select>
                    {statusSaving && <Text size="xsmall" className="text-ui-fg-muted mt-2">Saving...</Text>}
                </div>
            </Container>
            ...
            <Container className="p-0 overflow-hidden">
                ...
                <div className="px-6 py-4 space-y-4">
                    <div>
                        <Label className="mb-1 block text-sm text-ui-fg-subtle">Estimate Number</Label>
                        <div className="font-mono text-sm px-3 py-2 rounded-md bg-ui-bg-subtle border border-ui-border-base text-ui-fg-muted min-h-[36px]">
                            {estimateRef ?? <span className="text-ui-fg-muted italic">Not synced yet</span>}
                        </div>
                    </div>
                    <div>
            ...
            <Container className="p-0 overflow-hidden">
                ...
                <div className="px-6 py-4 space-y-2">
                    <button onClick={() => onOpenModal("metadata")}
                        className="flex items-center justify-between w-full text-sm text-ui-fg-base hover:text-ui-fg-interactive py-1">
                        <span>Metadata</span><PencilSquare className="text-ui-fg-muted" />
                    </button>
                    <button onClick={() => window.open(`/app/draft-orders/${id}`, "_blank")}
                        className="flex items-center justify-between w-full text-sm text-ui-fg-base hover:text-ui-fg-interactive py-1">
                        <span>Full native editor</span><ArrowUpRightOnBox className="text-ui-fg-muted" />
                    </button>
                </div>
            </Container>
        </div>
