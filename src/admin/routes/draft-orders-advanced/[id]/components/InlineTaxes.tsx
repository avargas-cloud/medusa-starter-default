...
export const InlineTaxes = ({ orderId, curr, triggerKey, onTaxChange }: Props) => {
    ...
    const fetchTax = useCallback(async () => {
        ...
        setLoading(true)
        try {
            const r = await fetch(`/admin/draft-orders/${orderId}/compute-tax`, { credentials: "include" })
            if (r.ok) {
    ...
    if (loading && !tax) {
        return (
            <div className="px-6 py-4">
                <Text size="small" className="text-ui-fg-muted">Computing taxes...</Text>
            </div>
        )
    }
            ...
            <div className="flex items-center gap-2">
                ...
                {tax && !saving && (
                    <Text size="xsmall" className="text-ui-fg-muted">
                        {tax.exempt ? <Badge color="green" size="small">Exempt</Badge> : tax.reason}
                    </Text>
                )}
            </div>
