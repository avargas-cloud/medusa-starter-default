...
export function getMissingEstimateFields(info: EstimateInfo): string[] {
  return REQUIRED_FIELDS.filter((f) => !info[f.key]?.trim()).map(
    (f) => f.label
  );
}
...
export const EstimateInfoBlock = ({
  orderId,
  customerId,
  initialInfo,
  onInfoChange,
  ...
  useEffect(() => {
    fetch("/admin/system-defaults", { credentials: "include" })
      ...
      .then((d) => {
        ...
        setPaymentTerms(
          defaults
            .filter((x: any) => x.field_name === "Payment Terms")
            .sort((a: any, b: any) => a.sort_order - b.sort_order)
            .map((x: any) => x.value)
        );
        setLeadTimes(
          defaults
            .filter((x: any) => x.field_name === "Lead Time")
            .sort((a: any, b: any) => a.sort_order - b.sort_order)
            .map((x: any) => x.value)
        );
        setOrderTypes(
          defaults
            .filter((x: any) => x.field_name === "Order Type")
            .sort((a: any, b: any) => a.sort_order - b.sort_order)
            .map((x: any) => x.value)
        );
  ...
  const saveFieldDefault = useCallback(
    async (fieldKey: keyof EstimateInfo) => {
    ...
    <Container className="p-0 overflow-hidden">
        ...
        <div className="flex items-center gap-2">
          ...
          {missing.length > 0 && (
            <Badge color="red" size="small" className="flex items-center gap-1">
              ...
              {missing.length === 1 ? "field" : "fields"} missing
            </Badge>
          )}
        </div>
        {saving && (
      ...
      <div className="px-6 py-4 grid grid-cols-2 gap-x-6 gap-y-4">
        ...
        <div>
          ...
          {reps.length > 0 ? (
            <Select value={info.rep} onValueChange={(v) => update("rep", v)}>
              <Select.Trigger className={triggerClass("rep")}>
                <Select.Value placeholder="Select rep…" />
              </Select.Trigger>
              <Select.Content>
                {reps.map((r) => (
        ...
        <div>
          ...
          >
            <Select.Trigger className={triggerClass("orderType")}>
        ...
        <div>
          ...
          >
            <Select.Trigger className={triggerClass("leadTime")}>
        ...
        <div>
          ...
          >
            <Select.Trigger className={triggerClass("paymentTerms")}>
