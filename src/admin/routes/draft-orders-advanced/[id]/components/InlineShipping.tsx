...
export const InlineShipping = forwardRef<InlineShippingHandle, Props>(
  function InlineShipping(
    ...
    const [replacingMethodId, setReplacingMethodId] = useState<string | null>(
      null
    );
    ...
    const applyLocalPickup = async (): Promise<boolean> => {
      ...
      const opt = freshOptions.find(
        (o) => o.name.toLowerCase().includes("miami") || isPickup(o.name)
      );
      if (!opt) return false;
    ...
    const handleLocalPickup = async () => {
      ...
      try {
        ...
        if (!found) {
          toast.warning(
            "Miami Store Pickup not found — please select manually."
          );
          await openPicker();
        }
      } catch (e: any) {
    ...
    const handleRemove = useCallback(
      async (methodId: string) => {
        ...
        try {
          const r = await fetch(
            `/admin/draft-orders/${orderId}/remove-shipping/${methodId}`,
            {
              method: "DELETE",
              credentials: "include",
            }
    ...
    const handlePriceBlur = (method: any) => {
      ...
      setPriceState((prev) => ({
        ...prev,
        [methodId]: { value: String(newAmount), editing: false, saving: true },
      }));
    };
      ...
      <div>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-ui-border-base">
          <div className="flex items-center gap-2">
            <span className="text-ui-fg-base text-base font-medium">
              {title}
            </span>
            {shippingSaving && (
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div
                    className={`flex items-center border border-ui-border-base rounded-md overflow-hidden bg-ui-bg-base focus-within:ring-1 focus-within:ring-ui-border-interactive h-7 ${ps.saving ? "opacity-60" : ""}`}
                  >
                    <span className="px-2 text-xs text-ui-fg-muted border-r border-ui-border-base bg-ui-bg-subtle select-none h-full flex items-center">
                      $
                    </span>
                    <input
                      type="text"
          ...
          <div className="px-6 py-4 border-t border-ui-border-base bg-ui-bg-subtle space-y-3">
            ...
            <div className="space-y-2">
              {shippingOptions.map((o) => (
                <button
                  key={o.id}
                  onClick={() => handleOptionClick(o.id, o.name)}
            ...
            <div>
              <Label className="text-xs text-ui-fg-muted block mb-1">
              ...
              <div className="flex items-center border border-ui-border-base rounded-md overflow-hidden bg-ui-bg-base focus-within:ring-1 focus-within:ring-ui-border-interactive">
                <span className="px-3 py-2 text-sm text-ui-fg-muted border-r border-ui-border-base bg-ui-bg-subtle select-none">
                  $
                </span>
                <input
                  type="text"
