...
export const InlinePromoInput = ({
  ...
  const filtered = promotions
    .filter(
      (p) =>
        !appliedCodes.includes(p.code) && !p.code?.startsWith("CUSTOM-DISC-")
    )
    .filter(
      (p) => !search || p.code?.toLowerCase().includes(search.toLowerCase())
    );
    ...
    return (
      <div className="py-3 text-center rounded-md border border-dashed border-ui-border-base">
        <Text size="small" className="text-ui-fg-muted">
          No promotions configured in this store.
        </Text>
      </div>
    );
  }
          ...
          {filtered.length > 0 ? (
            filtered.map((p) => {
              ...
              return (
                ...
                >
                  <span className="text-sm font-mono font-medium text-ui-fg-base">
                    {p.code}
                  </span>
                  {label && (
