...
export const SendEstimateModal = ({ open, onClose, onSuccess, orderId, displayId, customerEmail, total, curr }: Props) => {
    ...
    const fmtTotal = new Intl.NumberFormat("en-US", { style: "currency", currency: curr.toUpperCase() }).format(total)
    ...
    const loadPreview = async () => {
        ...
        try {
            const r = await fetch(`/admin/draft-orders/${orderId}/send-estimate`, { credentials: "include" })
            if (r.ok) {
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            if (j.preview_only) {
                toast.warning("SMTP not set up yet — configure SENDGRID_API_KEY in .env to enable sending.")
            } else if (j.success) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(0,0,0,0.6)" }}>
            <div className="bg-ui-bg-base rounded-xl shadow-2xl flex flex-col" style={{ width: "80vw", maxWidth: "1100px", height: "85vh" }}>

                {/* Header */}
                <div className="flex items-center justify-between px-6 py-4 border-b border-ui-border-base shrink-0">
                    <div>
                        <Text weight="plus" size="large">Send Estimate #{displayId}</Text>
                        <Text size="xsmall" className="text-ui-fg-muted mt-0.5">Preview and send to customer</Text>
                    </div>
                    <button onClick={onClose} className="text-ui-fg-muted hover:text-ui-fg-base text-xl font-bold w-8 h-8 flex items-center justify-center rounded-full hover:bg-ui-bg-subtle transition-colors">×</button>
                </div>
                    ...
                    <div className="w-72 shrink-0 flex flex-col border-r border-ui-border-base p-5 gap-4">
                        ...
                        <div className="mt-auto space-y-2">
                            <Text size="xsmall" className="text-ui-fg-muted">
                                Total: <span className="font-semibold text-ui-fg-base">{fmtTotal}</span>
                            </Text>
                            <button
                                onClick={handleSend}
                                disabled={sending || !to}
                            ...
                            <button onClick={onClose} className="w-full text-sm text-ui-fg-muted hover:text-ui-fg-base py-2">
                                Cancel
                            </button>
                        </div>
                    </div>
