...
export const AddrFormFields = ({
  ...
  useEffect(() => {
    ...
    fetch(`/admin/customers/${customerId}?fields=*addresses`, {
      credentials: "include",
    })
  ...
  return (
    <div className="space-y-3">
      ...
      {savedAddresses.length > 0 && (
        <div>
          ...
          <Text size="xsmall" className="text-ui-fg-muted mt-1">
            Selecting an address fills the fields below. You can still edit them
            manually.
          </Text>
      ...
      <div>
        <Label className="mb-1 block text-sm">Address 1</Label>
        <Input
      ...
      <div>
        <Label className="mb-1 block text-sm">Address 2 (optional)</Label>
        <Input
          value={form.address_2}
          onChange={(e) => onChange("address_2", e.target.value)}
        />
      </div>
      {(["city", "province", "postal_code", "country_code"] as const).map(
        (k) => (
          <div key={k}>
            <Label className="mb-1 block text-sm">
              {k === "city"
