...
export const useOrderFetch = (
    ...
    const fetchOrder = useCallback(async () => {
        ...
        try {
            ...
            const normalizedPreviewItems = preview?.items
                ? (preview.items as any[]).map((i: any) => ({ ...i, unit_price: normalizePrice(i.unit_price ?? 0) }))
                : null
            const merged = {
                ...rawOrder,
            ...
            try {
                ...
                const variantIds = [...new Set<string>(rawItems.map((i: any) => i.variant_id ?? i.variant?.id).filter(Boolean))]
                if (variantIds.length > 0) {
                    const vRes = await fetch(`/admin/product-variants?${variantIds.map(vid => `id[]=${vid}`).join("&")}&limit=50`, { credentials: "include" })
                    if (vRes.ok) {
                        ...
                        if (productIds.length > 0) {
                            const pRes = await fetch(`/admin/products?${productIds.map(pid => `id[]=${pid}`).join("&")}&limit=50`, { credentials: "include" })
                            if (pRes.ok) {
            ...
            try {
                ...
                const resolveUser = async (userId?: string): Promise<string | undefined> => {
                    ...
                    try {
                        ...
                        if (ur.ok) { const { user: u } = await ur.json(); const name = `${u?.first_name ?? ""} ${u?.last_name ?? ""}`.trim() || u?.email || userId; userCache[userId] = name; return name }
                    } catch { }
                    return undefined
                }
                const events: TimelineEvent[] = [{ id: merged.id + "-created", created_at: merged.created_at, title: "Created", description: "Draft order created" }]
                if (chRes.ok) {
                    ...
                    for (const ch of (order_changes ?? [])) {
                        ...
                        if (parts.length > 0 || ch.status === "confirmed") {
                            ...
                            events.push({ id: ch.id, created_at: ch.created_at, title, description: parts.join(" · ") || undefined, user: eventUser })
                        }
                    }
                }
                ...
                if (sentAt) events.push({ id: `email-sent-${sentAt}`, created_at: sentAt, title: "Email Sent", description: sentTo ? `Estimate emailed to ${sentTo}` : "Estimate emailed to customer", user: sentBy || undefined })
                setTimeline(events.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()))
    ...
    useEffect(() => {
        fetch("/admin/users/me", { credentials: "include" })
            ...
            .then(data => { const u = data?.user; if (u) setCurrentUser(`${u.first_name ?? ""} ${u.last_name ?? ""}`.trim() || u.email || "") })
            .catch(() => { })
    }, [])

    const addTimelineEvent = useCallback((title: string, description?: string, user?: string) => {
        const event: TimelineEvent = { id: `local-${Date.now()}`, title, description, created_at: new Date().toISOString(), user }
        localEventsRef.current = [event, ...localEventsRef.current]
        setLocalTick(t => t + 1)
    }, [])

    const mergedTimeline = (localTick >= 0 ? [...localEventsRef.current, ...timeline] : timeline)
        .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())

    return { order, setOrder, loading, fetchError, fetchOrder, timeline: mergedTimeline, currentUser, addTimelineEvent }
}
