...
export const useOrderPageState = (
    ...
    const [inlineShippingOptions, setInlineShippingOptions] = useState<{ id: string; name: string; amount: number | null }[]>([])

    useEffect(() => {
        if (!orderId) return
        const items: any[] = order?.items ?? []
        const variantIds = [...new Set(items.map((i: any) => i.variant?.id ?? i.variant_id).filter(Boolean))]
        if (variantIds.length === 0) return
            ...
            ; (async () => {
                try {
                    ...
                    const qs = variantIds.map(id => `variant_ids[]=${encodeURIComponent(id)}`).join("&")
                    const r = await fetch(`/admin/draft-orders/${orderId}/variant-prices?${qs}`, { credentials: "include" })
                    if (!r.ok) return
                    const { prices } = await r.json()

                    const priceMap: Record<string, PriceOption[]> = {}
                    for (const [variantId, entry] of Object.entries(prices as Record<string, any>)) {
