...
interface Deps {
    ...
    addTimelineEvent: (title: string, description?: string, user?: string) => void
    currentUser: string
}

export const usePageDerived = ({
    id, order, estimateStatus,
    itemPrices, itemQtys,
    handleAddItem, handleUpdateItem, handleRemoveItem,
    handleConvert, handleStatusChange,
    addTimelineEvent, currentUser,
}: Deps) => {
    // ── Tax state ─────────────────────────────────────────────────────────────
    ...
    useEffect(() => {
        if (!taxInitialized.current && order?.tax_total != null && order.tax_total > 0) {
    ...
    const handlePrintEstimate = useCallback(() => {
        const missing = currentEstimateInfo ? getMissingEstimateFields(currentEstimateInfo) : []
        if (missing.length > 0) {
            import("@medusajs/ui").then(({ toast }) =>
        ...
        iframe.onload = () => { setTimeout(() => { try { document.body.removeChild(iframe) } catch { } }, 120_000) }
        iframe.src = `/admin/draft-orders/${id ?? ""}/send-estimate?mode=print`
    }, [id, currentEstimateInfo])

    const handleEstimateSuccess = useCallback((sentTo: string) => {
        addTimelineEvent("Email Sent", `Estimate emailed to ${sentTo}`, currentUser || undefined)
    ...
    const computedSubtotal = (order?.items ?? []).reduce((sum: number, item: any) => {
        const price = itemPrices[item.id!] !== undefined ? parseFloat(itemPrices[item.id!]!) : (item.unit_price ?? 0)
    ...
    const computedTotal = computedSubtotal + shippingDollars - discountDollars + taxAmount
    ...
    return {
        ...
        showNoShippingModal, setShowNoShippingModal,
        shippingRef, shippingSectionRef,
        handleConvertClick, handleScrollToShipping, handleAutoLocalPickup,
        // Estimate
        showEstimateModal, setShowEstimateModal,
        currentEstimateInfo, setCurrentEstimateInfo,
        handleSendEstimate, handlePrintEstimate, handleEstimateSuccess,
        estimateTotal,
        // Computed totals
        computedSubtotal, shippingDollars, discountDollars, computedTotal,
    }
}
