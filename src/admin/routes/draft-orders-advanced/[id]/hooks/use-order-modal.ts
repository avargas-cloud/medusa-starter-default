...
export const useOrderModal = ({
    ...
    metadataForm, setMetadataForm, setMetaNewKey, setMetaNewVal,
    fetchOrder,
}: Deps) => {
    ...
    const openModal = async (type: ModalType) => {
        ...
        if (type === "billing-addr") setBillingAddrForm({
            ...
            company: (order?.billing_address as any)?.company ?? "", address_1: order?.billing_address?.address_1 ?? "",
            ...
            if (ar.ok) {
                ...
                (aj.order_changes ?? []).forEach((c: any) =>
                    (c.actions ?? []).forEach((a: any) => {
                        if (a.action === "ITEM_ADD" && a.details?.reference_id && a.id) map[a.details.reference_id] = a.id
                    })
                )
    ...
    const searchCustomers = (q: string) => {
        ...
        searchTimer.current = setTimeout(async () => {
            ...
            try {
                ...
                const responses = await Promise.allSettled([...params].map(p => fetch(`/admin/customers?${p}`, { credentials: "include" })))
                const seen = new Set<string>()
                const merged: typeof customers = []
                for (const res of responses) {
                    if (res.status !== "fulfilled" || !res.value.ok) continue
                    const j = await res.value.json()
                    for (const c of (j.customers ?? [])) { if (!seen.has(c.id)) { seen.add(c.id); merged.push(c) } }
                }
    ...
    // ── Save handlers ────────────────────────────────────────────────────────
    const handleSaveSalesChannel = async () => { setSaving(true); try { await patchOrder({ sales_channel_id: selectedSc }); toast.success("Sales channel updated"); setModal(null); fetchOrder() } catch (e: any) { toast.error(e.message) } finally { setSaving(false) } }
    const handleSaveEmail = async () => { setSaving(true); try { await patchOrder({ email: emailForm }); toast.success("Email updated"); setModal(null); fetchOrder() } catch (e: any) { toast.error(e.message) } finally { setSaving(false) } }
    const handleSaveShippingAddr = async () => { setSaving(true); try { await patchOrder({ shipping_address: shippingAddrForm }); toast.success("Shipping address updated"); setModal(null); fetchOrder() } catch (e: any) { toast.error(e.message) } finally { setSaving(false) } }
    const handleSaveBillingAddr = async () => { setSaving(true); try { await patchOrder({ billing_address: billingAddrForm }); toast.success("Billing address updated"); setModal(null); fetchOrder() } catch (e: any) { toast.error(e.message) } finally { setSaving(false) } }
    const handleTransfer = async (): Promise<void> => {
        if (!selectedCustomer) { toast.error("Select a customer first"); return }
        setSaving(true)
        try { await patchOrder({ customer_id: selectedCustomer }); toast.success("Ownership transferred"); setModal(null); fetchOrder() } catch (e: any) { toast.error(e.message) } finally { setSaving(false) }
    }
    ...
    return {
        modal, setModal, saving, itemActionMap,
        ...
        openModal, closeModal: () => setModal(null),
        searchCustomers,
