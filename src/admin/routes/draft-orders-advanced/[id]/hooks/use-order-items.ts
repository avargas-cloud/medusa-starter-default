...
export const useOrderItems = ({ id, order, setOrder }: Deps) => {
    ...
    searchTimer.current = setTimeout(async () => {
      ...
      try {
        const varRes = await fetch(
          `/admin/product-variants?q=${encodeURIComponent(q)}&limit=20&fields=id,title,sku,product_id,metadata`,
          { credentials: "include" }
        );
        if (!varRes.ok) {
          setInvResults([]);
        ...
        if (productIds.length > 0) {
          const pRes = await fetch(
            `/admin/products?${productIds.map((pid: string) => `id[]=${encodeURIComponent(pid)}`).join("&")}&limit=20&fields=id,title,thumbnail,metadata`,
            { credentials: "include" }
          );
          if (pRes.ok) {
        ...
        try {
          if (id) {
            const vidsParam = (variants as any[])
              .map((v: any) => `variant_ids[]=${v.id}`)
              .join("&");
            const prRes = await fetch(
              `/admin/draft-orders/${id}/variant-prices?${vidsParam}`,
              { credentials: "include" }
            );
            if (prRes.ok) {
        ...
        try {
          ...
          if (skus.length > 0) {
            ...
            const skuParams = skus
              .map((s: string) => `sku[]=${encodeURIComponent(s)}`)
              .join("&");
            const invRes = await fetch(
              `/admin/inventory-items?${skuParams}&limit=50`,
              { credentials: "include" }
            );
            if (invRes.ok) {
              const { inventory_items } = await invRes.json();
              const levelFetches = (inventory_items ?? []).map(
                async (inv: any) => {
                  ...
                  const matchedVariant = (variants as any[]).find(
                    (v: any) => v.sku === inv.sku
                  );
                  ...
                  for (const lev of inventory_levels ?? []) {
                    ...
                    const locName: string =
                      locationNameMap[locId] ??
                      lev.location?.name ??
                      lev.stock_location?.name ??
                      (locId || "Warehouse");
                    locationMap[vid].push({
        ...
        const results: VariantResult[] = (variants as any[]).map((v: any) => {
          ...
          if (varPrices?.default)
            priceOptions.push({
              label: "Default",
              amount: varPrices.default.amount,
            });
          ...
          const salesDescription: string | undefined =
            v.metadata?.sales_description ||
            prod?.metadata?.sales_description ||
            undefined;
          return {
            id: v.id,
            ...
            sku: v.sku ?? undefined,
            variantTitle:
              v.title && v.title !== prod?.title ? v.title : undefined,
  ...
  const handleAddItem = async (
    ...
    const optimisticItem = {
      ...
      title:
        matchedVariant?.salesDescription ?? matchedVariant?.title ?? variantId,
      subtitle: matchedVariant?.variantTitle ?? "",
      metadata: matchedVariant?.salesDescription
        ? { sales_description: matchedVariant.salesDescription }
        : undefined,
    ...
    setOrder((prev) =>
      prev
        ? { ...prev, items: [...(prev.items ?? []), optimisticItem as any] }
        : prev
    );
    setInvQuery("");
    setInvResults([]);
    try {
      ...
      if (unitPrice === undefined) {
        try {
          const varRes = await fetch(
            `/admin/product-variants/${variantId}?fields=*prices`,
            { credentials: "include" }
          );
          if (varRes.ok) {
            ...
            if (customerId && allPrices.length > 0) {
              try {
                const [plRes, custRes] = await Promise.all([
                  fetch(`/admin/price-lists?offset=0&limit=100`, {
                    credentials: "include",
                  }),
                  fetch(`/admin/customers/${customerId}?fields=*groups`, {
                    credentials: "include",
                  }),
                ]);
                if (plRes.ok && custRes.ok) {
                  ...
                  const groupIds = new Set(
                    (customer?.groups ?? []).map((g: any) => g.id)
                  );
                  const applicablePLIds = new Set(
                    (price_lists ?? [])
                      .filter((pl: any) =>
                        (pl.rules ?? []).some(
                          (r: any) =>
                            r.attribute === "customer_group_id" &&
                            groupIds.has(r.value)
                        )
                      )
                      .map((pl: any) => pl.id)
                  );
                  const plPrices = allPrices.filter((p: any) =>
                    applicablePLIds.has(p.price_list_id)
                  );
            ...
            if (unitPrice === undefined) {
              const defaultP =
                allPrices.find(
                  (p: any) => !p.price_list_id && p.currency_code === "usd"
      ...
      if (!noStockR.ok) {
        const j2 = await noStockR.json().catch(() => ({}));
        throw new Error(j2.message || "Could not add item");
      }
      toast.success("Item added");
      try {
        const freshR = await fetch(
          `/admin/orders/${id}?fields=+items.*,+items.metadata`,
          { credentials: "include" }
        );
        if (freshR.ok) {
          ...
          const matchedReal = freshItems
            .filter(
              (i: any) =>
                (i.variant_id ?? i.variant?.id) === variantId &&
                (i.quantity ?? 0) > 0
            )
            .sort(
              (a: any, b: any) =>
                    ...
                    items: (prev.items ?? []).map((i: any) =>
                      i.id === optimisticItem.id
                        ? {
                            ...
                            unit_price:
                              optimisticItem.unit_price !== 0
                                ? optimisticItem.unit_price
                                : matchedReal.unit_price,
                            quantity:
                              matchedReal.quantity ?? optimisticItem.quantity,
                          }
                        : i
                    ),
                  }
  ...
  const handleUpdateItem = async (itemId: string): Promise<void> => {
    ...
    const currentItem = orderRef.current?.items?.find(
      (i: any) => i.id === itemId
    );
    const price = parseFloat(
      itemPricesRef.current[itemId] ?? String(currentItem?.unit_price ?? 0)
    );
      ...
      prev
        ? {
            ...
            items: (prev.items ?? []).map((item: any) =>
              item.id === itemId
                ? { ...item, quantity: qty, unit_price: price }
                : item
            ),
          }
        : prev
  ...
  const handleRemoveItem = async (itemId: string): Promise<void> => {
    ...
    try {
      const r = await fetch(
        `/admin/draft-orders/${id}/delete-item-force?line_item_id=${encodeURIComponent(itemId)}`,
        {
          method: "DELETE",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
  ...
  return {
    ...
    invResults,
    itemQtys,
    setItemQtys: setItemQtysSafe,
    itemPrices,
    setItemPrices: setItemPricesSafe,
    itemSaving,
    searchInvItems,
    handleAddItem,
    handleUpdateItem,
