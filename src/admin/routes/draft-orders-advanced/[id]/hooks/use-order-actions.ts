...
export const useOrderActions = ({
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      if (!r.ok)
        throw new Error(j.message || `HTTP ${r.status}: ${r.statusText}`);
      ...
      toast.success(
        j.backorder_items_enabled
          ? "Converted to order! Some items are on backorder."
          : "Converted to order! Redirecting…"
      );
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      toast.dismiss(toastId);
      toast.success(
        "Draft order cancelled. QuickBooks Estimate marked inactive."
      );
    } catch (e: any) {
      toast.dismiss(toastId);
  ...
  const handleStatusChange = async (val: string) => {
    ...
    try {
      await patchOrder({ metadata: { order_status: val } });
      setEstimateStatus(val as EstimateStatus);
  ...
  const handleSync = async () => {
    ...
    const existingTxnId =
      (estMeta && typeof estMeta === "object"
        ? (estMeta as any).txn_id
        : null) ??
    ...
    try {
      ...
      const j = await r.json();
      if (j.success) {
        if (j.qbEstimateTxnId) {
          setLocalTxnId(j.qbEstimateTxnId);
          setLocalRef(j.qbEstimateRef ?? j.qbEstimateTxnId);
        }
