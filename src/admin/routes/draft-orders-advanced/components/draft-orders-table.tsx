...
const STATUS_BADGE_COLOR: Record<
  EstimateStatus,
  "grey" | "blue" | "purple" | "orange" | "green" | "red"
> = {
  Created: "grey",
  Sent: "blue",
  "Sent by Email": "blue",
...
const COLS =
  "grid-cols-[80px_110px_100px_minmax(110px,1fr)_minmax(110px,1fr)_minmax(170px,1.3fr)_120px_130px_80px_88px]";
const HEADERS = [
  "Ref Num",
  "QB Ref #",
  "Date",
    ...
    <div className="min-w-[900px]">
      ...
      >
        {HEADERS.map((h, i) => (
          <Text
            ...
            size="xsmall"
            weight="plus"
            className={`text-ui-fg-muted uppercase tracking-wider ${i === 9 ? "text-right" : ""}`}
          >
            {h}
          </Text>
        ))}
      ...
      {paginated.map((order) => {
        ...
        const qbRef =
          ...
          null;
        const s = (order.metadata?.order_status ??
          order.metadata?.estimate_status) as EstimateStatus | undefined;
        ...
        return (
          <div
            ...
            onAuxClick={(e) => {
              if (e.button === 1 && rowHref)
                window.open(rowHref(order.id), "_blank");
            }}
          >
            <Text size="small" weight="plus">
              {order.display_id}
            </Text>
            <Text size="small" className="font-mono text-ui-fg-subtle truncate">
              {qbRef ?? "—"}
            </Text>
            <Text size="small" className="text-ui-fg-subtle">
            ...
            <Text
              size="small"
              className="line-clamp-2 text-ui-fg-subtle leading-tight"
            >
            ...
            <div>
              <Badge
                color={s ? (STATUS_BADGE_COLOR[s] ?? "grey") : "grey"}
                size="small"
              >
                {s ?? "—"}
              </Badge>
...
export const DraftOrdersControls = ({
  ...
  <div className="flex items-center gap-3 flex-wrap px-4 py-3">
    ...
    >
      {showNotApproved
        ? "Hide Declined"
        : `Show Declined${notApprovedCount ? ` (${notApprovedCount})` : ""}`}
    </Button>
    ...
    >
      {showCancelled
        ? "Hide Cancelled"
        : `Show Cancelled${cancelledCount ? ` (${cancelledCount})` : ""}`}
    </Button>
...
export const DraftOrdersFooter = ({
  loading,
  sorted,
  page,
  totalPages,
  onPrev,
  onNext,
}: DraftOrdersFooterProps) => (
  <div className="flex items-center justify-between px-4 py-3">
    <Text size="xsmall" className="text-ui-fg-muted">
      {!loading &&
        `${sorted.length} draft order${sorted.length !== 1 ? "s" : ""}`}
    </Text>
    {totalPages > 1 && (
