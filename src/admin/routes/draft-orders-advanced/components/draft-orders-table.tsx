...
const COLS = "grid-cols-[56px_minmax(110px,1fr)_minmax(110px,1fr)_minmax(170px,1.3fr)_100px_120px_130px_80px_110px_88px]"
const HEADERS = ["#", "Company", "Customer", "Email", "Date", "Sales Channel", "Status", "QB", "QB Ref #", "Total"]
            ...
            {paginated.map(order => {
                ...
                return (
                    <div key={order.id}
                        ...
                        <Text size="small" className="text-right">{formatCurrency(order.total, order.currency_code)}</Text>
                    </div>
                )
            })}
        </div>
...
export const DraftOrdersControls = ({ search, onSearchChange, sort, onSortChange }: DraftOrdersHeaderProps) => (
