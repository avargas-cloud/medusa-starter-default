...
const DraftOrdersAdvancedList = () => {
  ...
  const handleSortChange = (v: Parameters<typeof setSort>[0]) => {
    setSort(v);
    setPage(0);
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <Text className="text-ui-fg-subtle text-sm mt-1">
            Manage draft orders and sync them to QuickBooks as Estimates.
          </Text>
        </div>
      ...
      <Container className="divide-y p-0 overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <DraftOrdersTable
            ...
            sorted={sorted}
            paginated={paginated}
            onRowClick={(id) => navigate(`/draft-orders-advanced/${id}`)}
            rowHref={(id) => `/app/draft-orders-advanced/${id}`}
          />
        </div>
        <DraftOrdersFooter
          loading={loading}
