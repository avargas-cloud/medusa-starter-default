...
const STATUS_COLORS: Record<
  string,
  "orange" | "green" | "red" | "grey" | "blue" | "purple"
> = {
  draft: "grey",
  issued: "blue",
  partial: "orange",
      ...
      <div className="bg-ui-bg-base border border-ui-border-base rounded-xl shadow-2xl w-full max-w-2xl mx-4 overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-ui-border-base flex-shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={onClose}
              className="text-ui-fg-muted hover:text-ui-fg-base transition-colors ml-1"
            >
              <svg
                className="w-4 h-4"
                fill="none"
          ...
          <div>
            ...
            <div className="border border-ui-border-base rounded-lg overflow-hidden">
              ...
              {(inv.items ?? []).map((item) => (
                <div
                  key={item.id}
                  className="grid grid-cols-[1fr_60px_90px_90px] items-start px-3 py-2.5 border-b border-ui-border-base last:border-0"
                >
                  <div>
                  ...
                  <Text size="small" className="text-right text-ui-fg-muted">
                    {item.quantity}
                  </Text>
                  <Text size="small" className="text-right">
                    {fmt(item.unit_price)}
                  </Text>
                  <Text size="small" className="text-right" weight="plus">
                    {fmt(item.total)}
                  </Text>
                </div>
              ))}
            ...
            <div className="w-64 space-y-1.5">
              ...
              <div className="flex justify-between">
                ...
                <Text
                  ...
                  weight="plus"
                  className={
                    inv.balance_due > 0 ? "text-orange-500" : "text-green-600"
                  }
                >
                  {fmt(inv.balance_due)}
                </Text>
...
const COLS =
  "grid-cols-[130px_90px_100px_100px_minmax(180px,1fr)_100px_130px_110px_100px_80px]";
const HEADERS = [
  "Invoice #",
  "QB Ref #",
  "Order #",
...
const InvoicesPage = () => {
  ...
  const [selectedInvoice, setSelectedInvoice] = useState<PosInvoice | null>(
    null
  );
    ...
    try {
      ...
      if (orderIds.length > 0) {
        ...
        const or = await fetch(
          `/admin/orders?${idsParam}&fields=id,display_id,metadata,*customer&limit=100`,
          { credentials: "include" }
        );
        if (or.ok) {
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <Text className="text-ui-fg-subtle text-sm mt-1">
              ...
              {!loading && (
                <span className="ml-1 text-ui-fg-muted">({sorted.length})</span>
              )}
            </Text>
          </div>
          <Button
          ...
          <div className="overflow-x-auto">
            {loading ? (
              ...
              <div className="p-6 flex flex-col items-center justify-center gap-2 py-10">
                <Text size="small" className="text-ui-fg-subtle">
                  {search
                    ...
                    : "No invoices yet. Create an invoice from an order in the POS."}
                </Text>
              </div>
            ) : (
              <div className="min-w-[900px]">
                {/* Header */}
                <div
                  className={`grid ${COLS} gap-x-3 px-4 py-2 bg-ui-bg-subtle border-b border-ui-border-base`}
                >
                  {HEADERS.map((h, i) => (
                    <Text
                      key={i}
                ...
                {/* Rows */}
                {paginated.map((inv) => {
                  const qb = orderMeta[inv.order_id] ?? {
                    ref: null,
                    synced: false,
                  };
                  return (
                    ...
                    >
                      ...
                      <Text
                        size="small"
                        className="font-mono text-ui-fg-subtle truncate"
                        title={qb.ref ?? undefined}
                      >
                        {qb.ref ?? "—"}
                      </Text>
                      {/* Order # */}
                      ...
                      <Text
                        ...
                        className={
                          qb.customerDisplay
                            ...
                            : "text-ui-fg-subtle truncate font-mono text-xs"
                        }
                        title={qb.customerDisplay ?? inv.customer_id}
                      >
                        {qb.customerDisplay ??
                      ...
                      <div>
                        <Badge
                          color={STATUS_COLORS[inv.status] ?? "grey"}
                          size="small"
                        >
                      ...
                      <Text
                        size="small"
                        className={`text-right font-medium ${inv.balance_due > 0 ? "text-orange-500" : "text-green-600"}`}
                      >
                        {fmt(inv.balance_due)}
                      </Text>
                      {/* QB Sync */}
                      <div className="flex justify-center">
                        {qb.synced ? (
                          <span
                            ...
                            className="text-green-500 font-bold text-base leading-none"
                          >
                            ✓
                          </span>
                        ) : (
                          <span
                            title="Not synced"
                            className="text-ui-fg-muted text-base leading-none"
                          >
                            —
                          </span>
                        )}
