...
cancelHooks.orderCanceled(
  async ({ order }: { order: any }, { container }: { container: any }) => {
    ...
    if (!order_id) {
      console.warn(
        `[emit-order-events] ⚠️ orderCanceled hook received no order.id — payload: ${JSON.stringify(order)}`
      );
      return;
    }
    try {
      ...
      console.log(
        `[emit-order-events] 📡 Emitted order.canceled for ${order_id}`
      );
    } catch (err: any) {
      console.warn(
        `[emit-order-events] ⚠️ Could not emit order.canceled: ${err.message}`
      );
    }
  }
);
