...
refreshCartItemsWorkflow.hooks.setPricingContext(
    async ({ cart_id, additional_data }, { container }) => {
        ...
        if ((additional_data as any)?.force_retail === true) {
            console.log(`[PRICING-HOOK-REFRESH] 🏷️ force_retail=true — applying retail prices`)
            return new StepResponse({})
        }
        ...
        if (!cart?.customer_id) {
            console.log("[PRICING-HOOK-REFRESH] 🛒 Guest cart — using default pricing")
            return new StepResponse({})
        }
        ...
        try {
            ...
            const groupIds = customer.groups.map((g: any) => g.id)
            console.log(`[PRICING-HOOK-REFRESH] 👑 Wholesale reprice — groups: ${groupIds.join(", ")}`)

            return new StepResponse({
                customer_group_id: groupIds,
            })
        } catch (error: any) {
            console.warn(`[PRICING-HOOK-REFRESH] ⚠️ Could not resolve customer groups:`, error.message)
            return new StepResponse({})
        }
    }
)
