...
const upsertAttributeValueStep = createStep(
  ...
  async (input: UpsertProductAttributeInput, { container }) => {
    ...
    const [key] = await service.listAttributeKeys({
      handle: input.attributeKeyHandle,
    });
    if (!key) {
      throw new Error(
        `Attribute Key with handle "${input.attributeKeyHandle}" not found`
      );
    }
...
export const upsertProductAttributeWorkflow = createWorkflow(
  ...
  (input: UpsertProductAttributeInput) => {
    ...
    const linkData = transform(
      { attributeValue, productId: input.productId },
      (data: { attributeValue: { id: string }; productId: string }) => [
        {
          [Modules.PRODUCT]: {
