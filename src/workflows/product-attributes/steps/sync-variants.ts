...
export const syncVariantsStep = createStep(
  ...
  async ({ productId, attributeKeyId }: SyncStepInput, { container }) => {
    ...
    logger.info(
      `[Sync Variants] Starting sync for Product: ${productId}, Key: ${attributeKeyId}`
    );
    ...
    let targetOption: any = product.options.find(
      (o: any) => o.title === attributeKey.label
    );
    ...
    for (const variant of product.variants) {
      ...
      const matchedValue = attributeKey.values.find(
        (v: any) => v.value === variant.title
      );
      ...
      if (matchedValue) {
        ...
        const existingOptionValue = variant.options.find(
          (opt: any) => opt.option_id === targetOption?.id
        );

        if (!existingOptionValue) {
          logger.info(
            `[Sync Variants] Healing Variant: ${variant.title} -> Linking to Option: ${attributeKey.label}`
          );
    ...
    if (updates.length > 0) {
      ...
      logger.info(
        `[Sync Variants] Successfully updated ${updates.length} variants.`
      );
    } else {
