...
const updateSingleProductStep = createStep(
    ...
    async ({ productId }: UpdateSingleProductInput, { container }) => {
        ...
        try {
            ...
            if (!product) {
                logger.warn(`[MEILI-INCREMENTAL] Product ${productId} not found, skipping sync`)
            ...
            const result = await index.addDocuments([meiliProduct], { primaryKey: "id" })

            // 5. Wait for indexing to complete
            await (client as any).tasks.waitForTask(result.taskUid)

            logger.info(`[MEILI-INCREMENTAL] ✅ Product ${productId} updated (${product.variants?.length || 0} variants)`)
            ...
            logger.error(`[MEILI-INCREMENTAL] ❌ Failed to update product ${productId}:`, error.message)
            return new StepResponse({
                success: false,
                productId: productId,
                title: '',
