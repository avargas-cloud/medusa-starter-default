...
export const _createVariantsStep = createStep(
  ...
  async (
    { productId, variantKeys, attributes, basePrice }: CreateVariantsInput,
    { container }
  ) => {
    ...
    const variantAttributes = variantKeys.map((keyId: any) => ({
      keyId,
      label:
        attributes.find((a: any) => a.attribute_key.id === keyId)?.attribute_key
          ?.label || "Unknown",
    ...
    if (combinations.length > 100) {
      throw new Error(
        `Too many combinations: ${combinations.length}. Max 100.`
      );
    }
    ...
    try {
      ...
      const newVariants =
        await productModuleService.createProductVariants(variantsData);
