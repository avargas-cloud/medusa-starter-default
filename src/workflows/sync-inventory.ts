...
export const syncInventoryToMeiliStep = createStep(
    ...
    async (_, { container }) => {
        ...
        const { data: allVariants } = await query.graph({
            ...
            fields: [
                "id", "sku", "metadata", "created_at", "updated_at",
                "price_set.id",
                "product.id", "product.title", "product.handle",
                "product.thumbnail", "product.status", "product.metadata",
                ...
                "prices.amount", "prices.currency_code", "prices.price_list_id",
                ...
                "options.value", "options.option.title",
            ],
            pagination: { skip: 0, take: 50000 }
        })
        console.log(`✅ [sync-inventory] Loaded ${allVariants.length} variants in ${Date.now() - t0}ms`)
        ...
        const meiliInventoryItems = allVariants.flatMap((variant: any) => {
            ...
            const pricesByList = priceSetId ? pricesByPriceSet.get(priceSetId) ?? {} : {}
            ...
            if (!variant.inventory_items || variant.inventory_items.length === 0) {
                ...
                return [{
                    ...
                    salesDescription: (variant?.metadata as any)?.sales_description
                        || (product?.metadata as any)?.sales_description || null,
        ...
        console.log(`🔄 [sync-inventory] Transformed ${validItems.length} valid items in RAM`)
        ...
        await index.updateSettings({
            ...
            sortableAttributes: ["title", "sku", "totalStock", "price", "totalReserved", "updated_at", "created_at"],
            searchableAttributes: ["title", "sku"]
        })

        // Atomic clear — wait for completion before uploading (prevents race condition)
        console.log("🗑️  [sync-inventory] Clearing index (waiting for completion)...")
        ...
        console.log(`🚀 [sync-inventory] Uploading ${chunks.length} chunks in parallel...`)
        const t1 = Date.now()
        ...
        console.log(`✅ [sync-inventory] Synced ${validItems.length} items in ${Date.now() - t1}ms`)
