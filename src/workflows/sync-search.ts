...
export const upsertToMeiliStep = createStep(
  ...
  async (
    ...
    { container }
  ): Promise<StepResponse<SyncSearchStepOutput>> => {
    ...
    if (input.type === "variant") {
      try {
        ...
        if (variant && variant.product_id) {
          ...
          logger.info(
            `🔗 [Sync Workflow] Resolved Product ID ${productId} from Variant ${input.id}`
          );
        } else {
          logger.warn(
            `⚠️ [Sync Workflow] Could not resolve Product ID for Variant ${input.id}. Skipping.`
          );
          return new StepResponse({
            success: false,
            reason: "Variant parent not found",
          });
        }
      } catch (err) {
        logger.error(
          `❌ [Sync Workflow] Error resolving variant parent: ${err}`
        );
    ...
    logger.info(
      `⚡ [Sync Workflow] Processing ${input.eventName} for Product ID: ${productId}`
    );
    ...
    try {
      ...
      if (!product) {
        ...
        if (input.eventName === "product.deleted") {
          ...
          logger.info(
            `🗑️ [Sync Workflow] Deleted product ${productId} from MeiliSearch`
          );
          return new StepResponse({ success: true, action: "deleted" });
        }
        logger.warn(
          `⚠️ [Sync Workflow] Product ${productId} not found in DB. Skipping index update.`
        );
        return new StepResponse({
          success: false,
      ...
      logger.info(
        `✅ [Sync Workflow] Successfully synced product ${product.title} (${product.id})`
      );
      return new StepResponse({
        success: true,
        action: "upserted",
      ...
      logger.error(
        `❌ [Sync Workflow] Failed to sync ${productId}: ${error.message}`
      );
