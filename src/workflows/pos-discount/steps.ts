  ...
  async (input: OverrideAdjustmentsInput, { container }) => {
    ...
    try {
      ...
      if (!orderChanges.length) {
        logger.warn(
          `[Override Step] No active order change found for order ${input.order_id}`
        );
      ...
      const adjustmentActions = actions.filter(
        (a: any) =>
          a.action === "ITEM_ADJUSTMENTS_REPLACE" ||
          a.action === "ITEM_ADJUSTMENT_ADD"
      );
      ...
      for (const action of adjustmentActions) {
        ...
        for (const adj of details.adjustments) {
          ...
          if (input.pct_discount != null && input.pct_discount > 0) {
            ...
            const correctAmount =
              Math.round(unitPrice * qty * input.pct_discount * 100) / 100;

            logger.info(
              `[Override Step] Overwriting action ${action.id} amt: ${adj.amount} -> ${correctAmount}`
            );
      ...
      if (actionsToUpdate.length > 0) {
        logger.info(
          `[Override Step] Patching ${actionsToUpdate.length} order_change_action payloads`
        );
