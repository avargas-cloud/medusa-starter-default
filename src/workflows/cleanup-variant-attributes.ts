  ...
  async ({ variantIds }: { variantIds: string[] }, { container }) => {
    ...
    if (lineItems.length > 0) {
      ...
      const affectedVariants = [
        ...new Set(lineItems.map((item) => item.variant_id)),
      ];
      throw new Error(
...
const cleanupVariantsStep = createStep(
  ...
  async (
    {
      ...
      variantIds,
      optionId,
    }: { product: any; variantIds: string[]; optionId?: string },
    { container }: any
  ) => {
    const productModuleService = container.resolve(Modules.PRODUCT);
  ...
  async (
    compensationData:
      | { productId: string; deletedVariantIds: string[] }
      | undefined
  ) => {
    // Note: In practice, variant restoration is complex.
    // Better to validate thoroughly BEFORE deletion.
    console.warn(
      `Cleanup failed after deleting ${compensationData?.deletedVariantIds?.length || 0} variants`
    );
  }
);
