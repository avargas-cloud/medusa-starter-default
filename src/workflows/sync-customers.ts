...
export const syncCustomersToMeiliStep = createStep(
    ...
    async (_, { container }) => {
        if (isSyncing) {
            ...
            return new StepResponse({ success: false, message: "Sync already in progress", synced: 0 })
        }
        ...
        try {
            ...
            const [customers] = await customerModule.listAndCountCustomers({}, {
                ...
                select: [
                    ...
                    "phone", "has_account", "metadata", "created_at", "updated_at"
                ],
                relations: ["groups"]
            })
            console.log(`✅ Loaded ${customers.length} customers from DB in ${Date.now() - t0}ms`)
            ...
            await Promise.all(uploadTasks.map(task => client.tasks.waitForTask(task.taskUid)))
            console.log(`✅ Synced ${docs.length} customers to MeiliSearch in ${Date.now() - t1}ms`)
