...
function needsMigration(meta: Record<string, any>): boolean {
  ...
  const hasNewSo =
    meta.qb_sales_order &&
    typeof meta.qb_sales_order === "object" &&
    meta.qb_sales_order.txn_id;
  const hasNewEst =
    meta.qb_estimate &&
    typeof meta.qb_estimate === "object" &&
...
function buildNewMetadata(meta: Record<string, any>): Record<string, any> {
  ...
  if (
    ...
    !(meta.qb_estimate && typeof meta.qb_estimate === "object")
  ) {
    newMeta.qb_estimate = {
  ...
  if (
    ...
    !(meta.qb_sales_order && typeof meta.qb_sales_order === "object")
  ) {
...
async function main() {
  ...
  try {
    ...
    const whereClause = OLD_FLAT_FIELDS.map((f) => `metadata ? '${f}'`).join(
      " OR "
    );
    ...
    console.log(
      `Found ${result.rows.length} orders with old flat QB metadata fields\n`
    );
    ...
    for (const row of result.rows) {
      ...
      if (!needsMigration(meta)) {
        console.log(
          `  ⏭️  Order #${row.display_id} (${row.id}) — already migrated, skipping`
        );
        skipped++;
        continue;
      }
      ...
      if (meta.qb_estimate_txn_id) {
        console.log(
          `     estimate:    ${meta.qb_estimate_txn_id} → qb_estimate.txn_id`
        );
      }
      if (meta.qb_sales_order_txn_id) {
        console.log(
          `     sales order: ${meta.qb_sales_order_txn_id} → qb_sales_order.txn_id`
        );
      }
      if (meta.qb_invoice_txn_id) {
        console.log(
          `     invoice:     ${meta.qb_invoice_txn_id} → qb_invoices[0].txn_id`
        );
      }
      if (meta.qb_payment_txn_id) {
        console.log(
          `     payment:     ${meta.qb_payment_txn_id} → qb_payments[0].txn_id`
        );
      }
    ...
    if (DRY_RUN) {
      console.log(
        "    ℹ️  Dry run: no changes written to DB. Remove DRY_RUN=true to apply."
      );
    }
  } finally {
    await client.end();
