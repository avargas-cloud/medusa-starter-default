...
export default async function fixMissingCompanyNames({ container }: ExecArgs) {
    ...
    log("=".repeat(65))
    log(isDryRun ? "📋  DRY RUN — no changes will be written" : "⚡  LIVE MODE — updates will be applied")
    log("=".repeat(65))
    ...
    log(`⏳  Waiting ${INITIAL_WAIT_MS / 60000} min before first poll (large dataset)...`)
    ...
    for (let attempt = 1; attempt <= MAX_POLL_ATTEMPTS; attempt++) {
        ...
        if (statusJson.operation?.status === "failed") {
            throw new Error(`QB sync failed: ${statusJson.operation.error || "unknown"}`)
        }
    ...
    for (const qb of qbCustomers) {
        const company = typeof qb.CompanyName === "string" ? qb.CompanyName.trim() : ""
    ...
    for (let i = 0; i < toUpdate.length; i += UPDATE_BATCH_SIZE) {
        ...
        await Promise.all(
            batch.map(async ({ id, qbListId }) => {
                try {
                    ...
                    await customerModule.updateCustomers(id, { company_name: companyName })
                    updated++
                } catch (err: unknown) {
        ...
        log(`   ✅  Progress: ${Math.min(i + UPDATE_BATCH_SIZE, toUpdate.length)} / ${toUpdate.length}`)
    }
    ...
    log(`   Errors:                ${errors}`)
    log(`   QB customers skipped (no CompanyName in QB): ${toUpdate.length - updated - errors}`)
    log("=".repeat(65))
}
