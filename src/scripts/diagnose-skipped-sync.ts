...
export default async function diagnoseSkipped({ container }: ExecArgs) {
    ...
    try {
        ...
        while (hasMore && page <= 5) { // Limit to 5 pages for diagnostics
        ...
        console.log(`   Found ${medusaProducts.length} Medusa products to check against.\n`);
        ...
        for (const wcP of wcProducts) {
            ...
            if (!exactMatch) {
                ...
                const potentialMatch = medusaProducts.find(mP =>
                    mP.handle === wcP.slug ||
                    mP.title === wcP.name
                );
                ...
                if (potentialMatch) {
                    foundButNoVariantMatch.push({
                        ...
                        medusa_variants: potentialMatch.variants.map((v: any) => v.sku).join(", ")
                    });
                } else {
        ...
        console.log("============================================================");
        console.log(`❌ SKIPPED REPORT (${missingInMedusa.length + foundButNoVariantMatch.length} total)`);
        console.log("============================================================\n");

        if (foundButNoVariantMatch.length > 0) {
            console.log(`⚠ FOUND IN MEDUSA BUT SKU MISMATCH (${foundButNoVariantMatch.length}):`);
            console.log(`(The product likely exists but the SKU in Medusa is different from WC)\n`);
            ...
            if (foundButNoVariantMatch.length > 20) console.log(`  ... and ${foundButNoVariantMatch.length - 20} more.`);
        }

        if (missingInMedusa.length > 0) {
            console.log(`\n🚫 COMPLETELY MISSING IN MEDUSA (${missingInMedusa.length}):`);
            console.log(`(These SKUs were not found in any variant and no product matched by handle/title)\n`);
            missingInMedusa.slice(0, 20).forEach(item => {
                console.log(`  - [${item.sku}] ${item.name}`);
            });
            if (missingInMedusa.length > 20) console.log(`  ... and ${missingInMedusa.length - 20} more.`);
        }
