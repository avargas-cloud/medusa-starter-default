...
async function run() {
    ...
    try {
        ...
        const token = jwt.sign({ actor_id: customerId, actor_type: "customer" }, JWT_SECRET, { expiresIn: "10m" })
        ...
        console.log(`✅ Test Variant: ${variant_id}, expected wholesale: $${wholesale_price}`)
        ...
        const syncRes = await fetch(`${API_URL}/carts/${cartId}/customer`, {
            method: "POST",
            headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}`, "x-publishable-api-key": PUBLISHABLE_KEY },
            body: JSON.stringify({})
        })
        const syncData = await syncRes.json()
        console.log(`✅ Customer linked to cart. customer_id = ${syncData.cart?.customer_id}`)
        ...
        if (item) {
            ...
            if (Number(item.unit_price) === Number(wholesale_price) && !item.is_custom_price) {
                console.log(`🎉 PASS: Wholesale price applied natively! is_custom_price=false!`)
                console.log(`   The Admin order list will now show correct totals.`)
            } else if (!item.is_custom_price) {
                console.log(`⚠️  is_custom_price=false (good!) but price $${item.unit_price} ≠ expected wholesale $${wholesale_price}`)
                console.log(`   Check if Price List rule targets the correct customer group.`)
            } else {
                console.log(`❌ FAIL: is_custom_price=${item.is_custom_price} — something is still forcing the price.`)
            }
        } else {
            console.log(`❌ No items found in cart. Raw response:`, JSON.stringify(fetchedData, null, 2).slice(0, 500))
        }
