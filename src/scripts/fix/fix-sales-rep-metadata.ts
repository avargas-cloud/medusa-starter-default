...
async function main() {
  ...
  try {
    ...
    for (const [initials, name] of repMap)
      nameToInitials.set(name.toLowerCase(), initials);
    ...
    try {
      ...
      if (e.code === "42703") {
        console.log(
          "⏭  pos_credit_memo.sales_rep column not yet created — run migration first, then re-run this script.\n"
        );
      } else throw e;
    }
...
async function fixTable(
  ...
  for (const row of rows) {
    ...
    if (!initials || !name) {
      console.log(
        `   ⚠️  ${row.id} — "${JSON.stringify(row.sales_rep)}" unresolvable, skipping`
      );
      skipped++;
      continue;
    }
    ...
    console.log(
      `   ${DRY_RUN ? "[dry]" : "[fix]"} ${row.id}  "${JSON.stringify(row.sales_rep)}" → ${canonical}`
    );
