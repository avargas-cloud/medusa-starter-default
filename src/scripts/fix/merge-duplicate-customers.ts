...
async function run() {
  ...
  try {
    ...
    console.log(
      `⚠️  Found ${duplicateEmails.length} email(s) with duplicates:\n`
    );
    ...
    for (const { normalized_email } of duplicateEmails) {
      ...
      console.log(
        `   MASTER → ${master.id} | QB=${isQbCustomer(master)} | name="${(master.first_name ?? "").trim()} ${(master.last_name ?? "").trim()}".trim() | email="${master.email}" | orders=${master.order_count}`
      );
      for (const d of duplicates) {
        console.log(
          `   DUPL   → ${d.id} | QB=${isQbCustomer(d)} | name="${(d.first_name ?? "").trim()} ${(d.last_name ?? "").trim()}".trim() | email="${d.email}" | orders=${d.order_count}`
        );
      }

      if (hasQbDuplicate) {
        console.log(
          `   → Skipped (QB customer involved — review QB_Customer_Email_Fix.xlsx)`
        );
        totalSkipped++;
        console.log();
        continue;
      ...
      if (!DRY_RUN) {
        for (const zombie of duplicates) {
          ...
          const ordersResult = await sql`
                    ...
                    `;
          console.log(
            `   ↳ Re-assigned ${ordersResult.count} order(s) from zombie → master`
          );
          ...
          try {
            const financeResult = await sql`
                        ...
                        `;
            console.log(
              `   ↳ Re-assigned ${financeResult.count} customer_payment record(s)`
            );
          } catch {
          ...
          const authResult = await sql`
                    ...
                    `;
          console.log(
            `   ↳ Re-linked ${authResult.length} auth_identity record(s)`
          );
        ...
        console.log(
          `   [DRY RUN] Would merge ${duplicates.length} zombie(s) into master ${master.id}`
        );
    ...
    if (DRY_RUN) {
      ...
      console.log(
        `   Skipped (QB): ${totalSkipped} group(s) — review QB_Customer_Email_Fix.xlsx`
      );
      ...
      console.log(
        `   Skipped : ${totalSkipped} QB group(s) — review QB_Customer_Email_Fix.xlsx\n`
      );
    }
  } finally {
    await sql.end();
