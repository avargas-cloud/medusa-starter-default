...
async function run() {
  ...
  try {
    ...
    console.log(
      `Found ${duplicateEmails.length} emails with duplicates. Building Excel...`
    );
    ...
    for (const { normalized_email } of duplicateEmails) {
      ...
      const isZombieGroup = customers.some(
        (c) =>
          !(c.first_name ?? "").trim() &&
          !(c.last_name ?? "").trim() &&
          c.order_count === "0"
      ...
      rows.push({
        ...
        "Legacy QB": "",
        "Suggested Role": "",
        "DECISION (merge/keep/change-email)": isZombieGroup
          ? "merge (auto-safe)"
          : "⚠️ REVIEW",
      });
    ...
    console.log(
      `   ${duplicateEmails.length} email groups — review the ⚠️ REVIEW rows before merging.\n`
    );
  } finally {
    await sql.end();
  }
