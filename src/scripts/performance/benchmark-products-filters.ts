...
export default async function (container: MedusaContainer) {
    ...
    for (let i = 1; i <= 5; i++) {
        ...
        try {
            const result = await fetchProductsWithFilters(container, LED_STRIPS_ID, 100, 0)
            ...
            console.log(`   📦 Products: ${result.products.length}/${result.pagination.total}`)
    ...
    if (avgTime < 300) {
        ...
        console.log("🔥 VERY SLOW: Response time > 3s - CRITICAL OPTIMIZATION NEEDED!")
    }
    console.log()
}
...
async function enrichProductsInline(products: any[], knex: any) {
    ...
    if (attributeLinks.length > 0) {
        ...
        const attributeValues = await knex("attribute_value")
            .select("attribute_value.id", "attribute_value.value", "attribute_key.handle", "attribute_key.label")
