...
async function testEstimateEndpoint() {
  ...
  try {
    ...
    if (healthData?.status === "healthy") {
      ...
      console.log(
        "⚠️  Bridge responded but status is not 'healthy':",
        healthData
      );
    }
  } catch (err: any) {
  ...
  if (process.env.QB_DRY_RUN === "true") {
    ...
    console.log(
      "✅ Dry run complete. Set QB_DRY_RUN=false to test against live Bridge."
    );
    process.exit(0);
  }
  ...
  try {
    ...
    if (status >= 200 && status < 300) {
      ...
      if (operationId) {
        ...
        console.log(
          `   → Poll for result: GET ${BRIDGE_URL}/api/sync/status/${operationId}`
        );
        ...
        try {
          ...
          if (pollData?.status === "completed") {
            ...
            console.log(
              `⏳ Status: ${pollData?.status || "unknown"} — try polling again in ~1 min`
            );
            console.log(
              `   curl -H "x-api-key: ${API_KEY}" ${BRIDGE_URL}/api/sync/status/${operationId}`
            );
          }
        } catch (pollErr: any) {
          console.log("⚠️  Poll failed:", pollErr.message);
          console.log(
            `   Manual poll: curl -H "x-api-key: ${API_KEY}" ${BRIDGE_URL}/api/sync/status/${operationId}`
          );
        }
      } else {
      ...
      console.log(
        "   → Need to add this endpoint to the Bridge before proceeding."
      );
