...
async function main() {
  ...
  if (!estRes.operationId)
    throw new Error(`Estimate queue failed: ${JSON.stringify(estRes)}`);
  ...
  if (!soRes.operationId)
    throw new Error(`Convert SO queue failed: ${JSON.stringify(soRes)}`);
