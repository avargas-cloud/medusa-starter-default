...
async function main() {
  ...
  if (soTxnId) {
    ...
    console.log(
      "[0/3] No QB_SO_TXN_ID provided — creating Sales Order automatically..."
    );
  ...
  if (queryRes.operationId) {
    ...
    const qRet = (
      queryOp.result?.QBXML?.QBXMLMsgsRs ?? queryOp.result?.QBXMLMsgsRs
    ...
    if (Number(finalBalance) === 0 || finalBalance === "0.00") {
      ...
      console.log(
        "   ⚠️  Balance not zero — check if payment applied correctly in QB"
      );
    }
  } else {
  ...
  console.log("\n=== SUMMARY ===");
  console.log(
    `   Sales Order: ${soTxnId}${soRefNumber ? ` (Ref: ${soRefNumber})` : ""}`
  );
