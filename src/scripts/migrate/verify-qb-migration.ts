...
export default async function verifyQbMigration({ container }: ExecArgs) {
    ...
    const qbInventoryItems = qbAll.filter(p => p.type === "ItemInventory" && p.sku)
    ...
    if (qbCount === medusaImportedCount) {
        ...
        logger.info(`   QuickBooks Items (${qbCount}) == Medusa Variants (${medusaImportedCount})`)
    } else {
        logger.warn(`⚠️ DISCREPANCY DETECTED!`)
        logger.warn(`   QuickBooks Items: ${qbCount}`)
        logger.warn(`   Medusa Variants:  ${medusaImportedCount}`)
        logger.warn(`   Difference:       ${Math.abs(qbCount - medusaImportedCount)}`)

        // Find missing SKUs
        const medusaSkus = new Set(variants.map((v: any) => v.sku?.trim()))
        const missing = qbInventoryItems.filter(qb => !medusaSkus.has(qb.sku.trim()))

        if (missing.length > 0) {
            logger.info(`\n🔍 Missing SKUs (${missing.length}):`)
            missing.slice(0, 20).forEach(m => logger.info(`   - ${m.sku}`))
            if (missing.length > 20) logger.info(`   ... and ${missing.length - 20} more`)
        }
    }
    logger.info("=".repeat(65))
}
