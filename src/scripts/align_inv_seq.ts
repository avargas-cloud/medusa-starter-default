...
async function run() {
  try {
    ...
    if (maxNum && !isNaN(maxNum) && maxNum >= 1000) {
      ...
      console.log(
        `Found existing invoices up to ${maxNum}. Setting sequence to start at ${nextNum}.`
      );
    } else {
      console.log(
        `No existing high invoices found. Sequence will safely start at ${nextNum}.`
      );
    }

    // 2. Set the sequence
    await pool.query(
      `SELECT setval('custom_invoice_seq', ${Math.max(nextNum - 1, 999)});`
    );
    console.log(
      `✅ custom_invoice_seq successfully aligned to dispense ${nextNum} next!`
    );
  } catch (e) {
