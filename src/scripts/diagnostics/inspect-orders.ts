...
async function inspectSchema() {
    ...
    `);
        console.log('Order-related tables:', tablesResult.rows.map(r => r.table_name));

        // Get columns for 'order' table
        if (tablesResult.rows.some(r => r.table_name === 'order' || r.table_name === 'orders')) {
            const orderTable = tablesResult.rows.find(r => r.table_name === 'order' || r.table_name === 'orders').table_name;
            const columnsResult = await client.query(`
        SELECT column_name, data_type 
        FROM information_schema.columns 
        WHERE table_schema='public' AND table_name=$1
      `, [orderTable]);
            console.log(`\nColumns in ${orderTable}:`, columnsResult.rows.map(r => r.column_name).join(', '));
      ...
      `);
            console.log(`\nColumns in order_summary:`, columnsResult.rows.map(r => r.column_name).join(', '));
