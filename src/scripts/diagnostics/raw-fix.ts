...
async function run() {
  ...
  for (const row of res.rows) {
    console.log(`- ${row.id} | ${row.title} | ${row.created_at} | fulfilled: ${row.fulfilled_quantity}`)
    
    // Only update items from today that have fulfilled_quantity > 0
    if (row.fulfilled_quantity > 0) {
      console.log(`  -> Resetting ${row.id}...`)
      await pool.query(`UPDATE order_item SET fulfilled_quantity = 0, delivered_quantity = 0 WHERE id = $1`, [row.id])
