...
async function diagnoseSkippedCategories() {
    ...
    try {
        ...
        const hasProductsNoAttrs = skippedCategories.filter(c => c.product_count > 0);
        ...
        hasProductsNoAttrs.forEach(c => {
            console.log(`   - ${c.name} (${c.product_count} products, scanning ${c.scanning_count} categories)`);
        });
