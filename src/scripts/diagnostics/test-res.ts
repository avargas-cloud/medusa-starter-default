...
export default async function testRes({ container }: { container: MedusaContainer }) {
  ...
  try {
    ...
    const res = await inv.listReservationItems({ line_item_id: ["ordli_01KMGWYHBMH3XQ0EZ4JXM0WXRB"] })
    console.log("Reservations from module (array):", JSON.stringify(res, null, 2))
  } catch(e: any) {
