...
async function diagnoseLEDStripsFilters() {
    ...
    try {
        ...
        console.log(`📂 Scanning ${categoryIdsToScan.length} categories (self + children)\n`);
        ...
        console.log(`📦 Found ${productsResult.rows.length} products (showing first 5):`);
        ...
        console.log(`✅ CORRECT: Found ${correctAttributesResult.rows.length} unique attributes:`);
        ...
        if (correctAttributesResult.rows.length > 10) {
            console.log(`   ... and ${correctAttributesResult.rows.length - 10} more`);
        }
        console.log();
        ...
        if (filterConfig?.active_filters) {
            console.log(`📝 filter_config has ${filterConfig.active_filters.length} active filters:`);

            // Get labels for these
            const activeIds = filterConfig.active_filters.map((f: any) => f.attribute_id);
            ...
            filterConfig.active_filters.slice(0, 10).forEach((f: any) => {
                console.log(`   - ${labelMap.get(f.attribute_id) || 'UNKNOWN'} (${f.attribute_id})`);
            });
            if (filterConfig.active_filters.length > 10) {
                console.log(`   ... and ${filterConfig.active_filters.length - 10} more`);
            }
        } else {
        ...
        if (filterConfig?.active_filters) {
            const activeIds = new Set(filterConfig.active_filters.map((f: any) => f.attribute_id));
            const correctIds = new Set(correctAttributesResult.rows.map(r => r.attribute_key_id));

            const inConfigNotInProducts = [...activeIds].filter(id => !correctIds.has(id));
            const inProductsNotInConfig = [...correctIds].filter(id => !activeIds.has(id));

            console.log(`\n🔍 MISMATCH ANALYSIS:`);
            console.log(`   ❌ In filter_config but NOT in products: ${inConfigNotInProducts.length}`);
            ...
            console.log(`   ℹ️  In products but NOT in filter_config: ${inProductsNotInConfig.length}`);
        }
