...
export default async function fix1236({ container }: { container: MedusaContainer }) {
  ...
  if (itemIds.length > 0) {
      await pool.query(`DELETE FROM order_line_item_tax_line WHERE item_id = ANY($1)`, [itemIds])
      const rawRate = JSON.stringify({ value: "7", precision: 20 })
      const genId = () => `taxline_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`
  ...
  if (summaryRes.rows[0]) {
      ...
      console.log(`Injected $${pos_tax_amount} tax to order_summary ${summaryId} and fixed accounting_total`)
  }
}
