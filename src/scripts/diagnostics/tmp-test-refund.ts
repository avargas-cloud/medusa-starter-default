    ...
    if (order?.payment_collections?.length) {
        ...
        for (const pc of order.payment_collections) {
            for (const payment of (pc.payments || [])) {
                ...
                const totalCaptured = (payment.captures || []).reduce((sum: number, c: any) => sum + Number(c.amount), 0)
                const totalRefunded = (payment.refunds || []).reduce((sum: number, r: any) => sum + Number(r.amount), 0)
                const availableToRefund = totalCaptured - totalRefunded
                
                console.log(`Payment ${payment.id} | Captured: ${totalCaptured} | Refunded: ${totalRefunded} | Available: ${availableToRefund}`)
            }
        }
    }
}
