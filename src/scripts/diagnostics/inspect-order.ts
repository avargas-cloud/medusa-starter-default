...
export default async function inspectOrder({
  ...
  const orderId = args.find(
    (a) => a.startsWith("order_") || a.startsWith("dord_") || /^\d+$/.test(a)
  );

  if (!orderId) {
    console.error("❌ Please provide an Order ID (order_... or 1533).");
    console.error(
      "Example: npx medusa exec ./src/scripts/diagnostics/inspect-order.ts 1533"
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (addr) {
      console.log(
        `Address: ${addr.address_1}, ${addr.city}, ${addr.province} ${addr.postal_code}`
      );
    } else {
    ...
    if (order.shipping_methods?.length) {
      order.shipping_methods.forEach((sm: any) => {
        console.log(
          `- ${sm.name || "Unknown"} (Amount: ${Number(sm.amount || 0).toFixed(2)})`
        );
      });
    } else {
    ...
    console.log(
      `Discount Total: ${Number(order.discount_total || 0).toFixed(2)}`
    );
    ...
    if (order.payment_collections?.length) {
      order.payment_collections.forEach((pc: any) => {
        console.log(
          `- [${pc.status.toUpperCase()}] Amount: ${Number(pc.amount || 0).toFixed(2)} (ID: ${pc.id})`
        );
      });
    } else {
    ...
    activeItems.forEach((item: any) => {
      ...
      console.log(
        `    Fulfilled Qty: ${item.detail?.fulfilled_quantity ?? (item.fulfilled_quantity || 0)}`
      );
      console.log(
        `    Allocated/Reserved Qty: ${allocatedQty} (Reservations: ${itemReservations.length})`
      );
      if (allocatedQty < item.quantity && !order.is_draft_order) {
        console.log(
          `    ⚠️ BACKORDERED/MISSING ALLOCATION: ${item.quantity - allocatedQty}`
        );
      }
      ...
      if (item.adjustments?.length) {
        item.adjustments.forEach((adj: any) => {
          console.log(
            `    🎁 Discount applied [${adj.code}]: -${Number(adj.amount || 0).toFixed(2)}`
          );
        });
      }
      console.log(
        `    (Note: For deep inventory/stock/allocated tracing, you can expand the query)`
      );
    });
    console.log("=========================================");
    ...
    const estimateTxnId =
      meta.qb_estimate?.txn_id ?? meta.qb_estimate_txn_id ?? null;
    const estimateRef =
      meta.qb_estimate?.ref_number ?? meta.qb_estimate_ref ?? null;
    const estimateEditSeq = meta.qb_estimate?.edit_sequence ?? null;
    const soTxnId =
      meta.qb_sales_order?.txn_id ?? meta.qb_sales_order_txn_id ?? null;
    const soRef =
      meta.qb_sales_order?.ref_number ?? meta.qb_sales_order_ref ?? null;
    const soEditSeq = meta.qb_sales_order?.edit_sequence ?? null;
    const invoices: any[] = Array.isArray(meta.qb_invoices)
      ? meta.qb_invoices
      : [];
    const payments: any[] = Array.isArray(meta.qb_payments)
      ? meta.qb_payments
      : [];
    ...
    console.log(
      `  QB List ID  : ${listId ?? "— not set (customer not in QB)"}`
    );
    console.log(`  Last Synced : ${syncedAt ?? "—"}`);
    console.log();
    console.log(
      `  Estimate    : ${estimateTxnId ? `TxnID=${estimateTxnId}  Ref=${estimateRef}  EditSeq=${estimateEditSeq ?? "—"}` : "— not synced"}`
    );
    console.log(
      `  Sales Order : ${soTxnId ? `TxnID=${soTxnId}        Ref=${soRef}        EditSeq=${soEditSeq ?? "—"}` : "— not synced"}`
    );
    if (invoices.length) {
      invoices.forEach((inv: any, i: number) => {
        console.log(
          `  Invoice[${i}]  : TxnID=${inv.txn_id}  Ref=${inv.ref_number}  EditSeq=${inv.edit_sequence ?? "—"}  FulfID=${inv.fulfillment_id ?? "—"}`
        );
      });
    } else {
    ...
    if (payments.length) {
      payments.forEach((pay: any, i: number) => {
        console.log(
          `  Payment[${i}]  : TxnID=${pay.txn_id}  Amount=$${(pay.amount / 100).toFixed(2)}  Method=${pay.method}`
        );
      });
    } else {
              ...
              : row.status === "failed"
                ...
                : row.status === "submitted"
                  ...
                  : row.status === "pending"
                    ...
                    : "⏭️";
          console.log(
            `  ${icon} [${row.step.padEnd(12)}] status=${row.status.padEnd(10)} retries=${row.retry_count}  TxnID=${row.qb_txn_id ?? "—"}  OpID=${row.bridge_op_id?.slice(0, 8) ?? "—"}  ts=${tsStr}`
          );
