...
export default async function inspectInvoice({ container }: { container: MedusaContainer }) {
  const query = container.resolve("query") as any
  const args = process.argv.slice(2)
  const invoiceId = args.find(a => a.startsWith("inv_") || a.startsWith("pinv_"))

  if (!invoiceId) {
    console.error("❌ Please provide an Invoice ID.")
    console.error("Example: npx medusa exec ./src/scripts/diagnostics/inspect-invoice.ts inv_123")
    process.exit(1)
  }
