...
async function run() {
  ...
  const res1 = await pool.query(`UPDATE order_line_item SET fulfilled_quantity = 0 WHERE order_id = $1`, [orderId]).catch(() => ({ rowCount: 0 }))
  ...
  `, [orderId]).catch(e => { console.error(e); return { rowCount: 0 } })
