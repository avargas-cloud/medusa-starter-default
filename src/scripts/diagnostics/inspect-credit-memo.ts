...
export default async function inspectCreditMemo({
  container,
}: {
  ...
  if (!memoId) {
    ...
    console.error(
      "Example: npx medusa exec ./src/scripts/diagnostics/inspect-credit-memo.ts cmemo_123"
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (!memo) {
      console.log(
        `❌ Credit Memo ${memoId} not found in 'credit_memo' entity.`
      );
      return;
    }
