...
export default async function run({
  ...
  if (updates.length > 0) {
    ...
    try {
      ...
      console.log(
        `✅ Bulk update complete! ${result.rowCount} rows updated in ${end - start}ms`
      );
    } catch (e) {
    ...
    console.log(
      `✅ Update complete! Updated: ${updates.length}, Unchanged: ${unchangedCount}`
    );
  } else {
    console.log(
      `✅ No updates needed. All matching variants are in sync. (Checked ${unchangedCount} variants)`
    );
  }
}
