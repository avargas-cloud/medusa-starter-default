...
export async function seedReceiptAccount(client: PoolClient) {
  await transaction(client, async () => {
    ...
    const caps = (
      await client.query(`SELECT (SELECT count(*) FROM bank_connection)::int c,
      (SELECT count(*) FROM bank_account)::int a`)
    ).rows[0];
    ...
    await client.query(
      ...
      VALUES($1,$2,$1,'EPT owned receipts v9 test','depository','USD',true,$3,'2000-01-01','0','1999-12-31',
        'Synthetic opening for isolated v9 fixture only',1)`,
      [account, connection, bank.qb_list_id]
    );
  });
}

export async function seedReceiptPayment(
  client: PoolClient,
  suffix: string,
  cents = 12001,
  date = day,
  ...
  await transaction(client, async () => {
    ...
    await client.query(
      `INSERT INTO customer_payment(id,customer_id,source,type,amount,raw_amount,currency,method,
...
export async function seedReceiptMovement(
  client: PoolClient,
  suffix: string,
  amount = "-120.01",
  date = day
  ...
  await transaction(client, async () => {
    ...
    await client.query(
      ...
      'EPT synthetic receipts v9 bank evidence','{}',now(),now())`,
...
export async function seedLegacyJournal(client: PoolClient) {
  ...
  const transactionId = await seedReceiptMovement(
    client,
    "legacy",
    "8.73",
    "2000-01-03"
  ...
  await transaction(client, async () => {
    ...
    const bank = (
      await client.query(
        `SELECT q.qb_list_id id,q.full_name name,q.account_type,'USD' currency
      FROM qb_account q JOIN bank_account a ON a.qb_list_id=q.qb_list_id WHERE a.id=$1`,
        [account]
      )
    ).rows[0];
    const expense = (
      await client.query(`SELECT qb_list_id id,full_name name,account_type,'USD' currency FROM qb_account
      WHERE is_active AND deleted_at IS NULL AND account_type='Expense' AND (currency IS NULL OR currency IN ('USD','US Dollar'))
      ORDER BY qb_list_id LIMIT 1`)
    ).rows[0];
    assert(bank && expense);
    const expenseId = rootPrefix + "legacy_expense";
    await client.query(
      `INSERT INTO bank_direct_expense(id,transaction_id,revision,nature,reference,description,attested,
    ...
    await client.query(
      `INSERT INTO bank_journal_entry(id,expense_id,transaction_id,kind,day,currency,amount_cents,
    ...
    for (const [role, source, debit, credit] of [
      ["bank", bank, 0, 873],
      ["expense", expense, 873, 0],
    ] as const) {
      await client.query(
...
export async function cleanReceiptFixtures(client: PoolClient) {
  await transaction(client, async () => {
    await withReviewLock(client);
    await client.query(
      "LOCK TABLE bank_journal_entry,bank_journal_line,bank_receipt_consumption,bank_receipt_accounting IN ACCESS EXCLUSIVE MODE"
    );
    const entries = (
      await client.query(
        `SELECT e.id FROM bank_journal_entry e WHERE starts_with(e.transaction_id,$1)
      OR e.receipt_id IN (SELECT id FROM bank_receipt_accounting WHERE starts_with(payment_id,$2))
      OR e.deposit_id IN (SELECT id FROM bank_deposit WHERE account_id=$3)`,
        [rootPrefix, paymentPrefix, account]
      )
    ...
    const entities = [
      ...
      ...(
        await client.query(
          "SELECT id FROM customer_payment WHERE starts_with(id,$1)",
          [paymentPrefix]
        )
      ).rows.map((row) => row.id as string),
    ];
    const foreign = async () =>
      ...
      FROM (SELECT e.id,to_jsonb(e) header,(SELECT jsonb_agg(l ORDER BY l.id) FROM bank_journal_line l WHERE l.entry_id=e.id) lines,
        ...
        FROM bank_journal_entry e WHERE NOT(e.id=ANY($1::text[]))) e`,
          [entries]
        )
      ).rows;
    ...
    await client.query(
      "DELETE FROM bank_receipt_accounting WHERE starts_with(payment_id,$1)",
      [paymentPrefix]
    );
    for (const [table, name] of guards)
      await client.query(`ALTER TABLE ${table} ENABLE TRIGGER ${name}`);
    assert.deepEqual(
      (
        await client.query(
          "SELECT tgname,tgenabled FROM pg_trigger WHERE tgname=ANY($1::text[]) ORDER BY tgname",
    ...
    assert.deepEqual(
      await foreign(),
      before,
      "Scoped immutable cleanup preserves all foreign journal/consumption data"
    );
    assert(
      !(await client.query("SELECT 1 FROM bank_receipt_accounting LIMIT 1"))
        .rowCount,
      "Never delete shared accounting setup if another operator created a receipt"
    );
    await client.query(
      "DELETE FROM bank_accounting_setup WHERE id='local-usd'"
    );
    await client.query(
    ...
    await client.query(
      `DELETE FROM bank_review_event WHERE starts_with(transaction_id,$1) OR starts_with(entity_id,$1)
      ...
      OR (entity_type='command' AND action='deposit_save' AND result->'deposit'->>'account_id'=$4
        AND starts_with(result->'deposit'->>'reference',$1))`,
      [rootPrefix, paymentPrefix, entities, account]
    );
    ...
    await client.query("DELETE FROM bank_deposit WHERE id=ANY($1::text[])", [
      deposits,
    ]);
    for (const table of [
      "bank_webhook_event",
    ...
    assert(
      !(
        await client.query(
          `SELECT 1 FROM customer_payment WHERE starts_with(id,$1) UNION ALL
      SELECT 1 FROM bank_connection WHERE id=$2 UNION ALL SELECT 1 FROM bank_review_event WHERE starts_with(entity_id,$1)
      OR starts_with(entity_id,$3) OR entity_id='local-usd'`,
          [paymentPrefix, connection, rootPrefix]
        )
      ).rowCount
    );
  });
...
export async function cleanLegacyProbe(client: PoolClient) {
  await transaction(client, async () => {
    ...
    const names = [
      "bank_journal_entry_immutable",
      "bank_journal_line_immutable",
    ];
    const guards = (
      await client.query(
        "SELECT tgname,tgenabled FROM pg_trigger WHERE tgname=ANY($1::text[]) ORDER BY tgname",
    ...
    await client.query(
      "DELETE FROM bank_journal_line WHERE entry_id IN (SELECT id FROM bank_journal_entry WHERE starts_with(transaction_id,$1))",
      [rootPrefix]
    );
    await client.query(
    ...
    await client.query("DELETE FROM bank_transaction WHERE connection_id=$1", [
      connection,
    ]);
    await client.query("DELETE FROM bank_account WHERE connection_id=$1", [
      connection,
    ]);
    await client.query("DELETE FROM bank_connection WHERE id=$1", [connection]);
  });
...
export async function receiptSnapshot(client: PoolClient) {
  const file = resolve(
    __dirname,
    "../../../../sandbox-artifacts/snapshots/pre-bank-receipts-v9.dump"
  );
  ...
  if (!existsSync(file)) {
    assert(
      !(
        await client.query(
          "SELECT to_regclass('public.bank_accounting_setup') AS name"
        )
      ).rows[0].name,
  ...
  const toc = await new Promise<string>((done, reject) => {
    ...
    child.on("close", (code) =>
      code === 0
        ...
        : reject(new Error("V9_SNAPSHOT_TOC_FAILED"))
    );
  ...
  assert.deepEqual(
    ...
    before,
    "Snapshot preserves protected documents"
  );
  console.log(
    `PASS v9 snapshot/TOC and ${Object.keys(before).length} protected table fingerprints`
  );
}
