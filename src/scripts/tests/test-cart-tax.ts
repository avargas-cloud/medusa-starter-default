...
const PUB_KEY = process.env.MEDUSA_PUBLISHABLE_KEY || 'pk_519e7f66680afc4ab0136ce701a7f6d1e8df2b8fc48a29b7a55616a05cb5b5f3';
...
async function runTaxTest() {
    ...
    // 1. Create a Cart
    console.log("\n[1] Creating Cart...");
    const cartRes = await storeFetch('/store/carts', { method: 'POST', body: JSON.stringify({}) });
    if (cartRes.status !== 200) return console.error("Failed to create cart:", cartRes);
    ...
    if (addRes.status !== 200) return console.error("Failed to add item:", addRes);
    ...
    if (addressRes.status !== 200) return console.error("Failed to set address:", addressRes);
    ...
    if (optsRes.status !== 200) return console.error("Failed to fetch options:", optsRes);
    const options = optsRes.data.shipping_options;

    if (!options || options.length === 0) return console.error("No shipping options available for this address.");
    const optionId = options[0].id;
    console.log(`Available Options: ${options.length}. Selecting ${options[0].name} (ID: ${optionId})`);

    // 5. Add Shipping Method
    console.log("\n[5] Adding Shipping Method to Cart...");
    const methodRes = await storeFetch(`/store/carts/${cartId}/shipping-methods`, {
        method: 'POST',
        body: JSON.stringify({ option_id: optionId })
    });
    if (methodRes.status !== 200) return console.error("Failed to add shipping method:", methodRes);
    ...
    if (cart.tax_total === 0 || cart.tax_total === '0') {
        console.log("❌ FAIL: Tax is 0 even with a Florida address. Pre-payment calculation failed.");
    } else {
        console.log("✅ SUCCESS: Tax was successfully calculated by Medusa pre-payment!");
    }
    ...
    if (collectionRes.status === 200 && collectionRes.data?.payment_collection?.id) {
        const collectionId = collectionRes.data.payment_collection.id;
        console.log(`✅ Collection created: ${collectionId}`);

        const sessionRes = await storeFetch(`/store/payment-collections/${collectionId}/payment-sessions`, {
            method: "POST",
            body: JSON.stringify({
        ...
        if (sessionRes.status === 200) {
            ...
            const completeRes = await storeFetch(`/store/carts/${cartId}/complete`, { method: 'POST' });
            ...
            if (completeRes.status === 200) {
                ...
                if (order.tax_total === 0 || order.tax_total === '0') {
                    console.log("❌ CRITICAL FAIL: Tax dropped to 0 DURING order completion!");
                } else {
                    console.log("✅ SUCCESS: Tax successfully carried over to final order!");
                }
            } else {
