...
async function testAuthMethodsEndpoint() {
    ...
    try {
        ...
        const loginResponse = await axios.post(`${BACKEND_URL}/auth/customer/emailpass`, {
            email: TEST_EMAIL,
        ...
        console.log(`  Show "Google Account" Badge: ${showGoogleBadge ? '✅ YES' : '❌ NO'}`)
        console.log(`  Reason: User ${has_google ? 'HAS' : 'does NOT have'} Google, ${has_password ? 'HAS' : 'does NOT have'} password`)
