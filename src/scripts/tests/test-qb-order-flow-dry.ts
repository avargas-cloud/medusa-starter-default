...
async function runTests() {
    ...
    assert(items[0]?.productId === "800019EA-1715274093", "ProductId is QB ListID")
    assert(items[0]?.quantity === 2, "Quantity is 2")
    assert(items[0]?.price === 22.95, `Price converted from cents: $${items[0]?.price}`)

    // Items without quickbooks_id should be filtered out
    const noQbItems = buildQbItems([
        { product_title: "No QB ID", quantity: 1, unit_price: 1000, variant: { sku: "XXX" } }
    ])
    ...
    assert(soResult.customerId === "8000004E-1342117388", `CustomerId: ${soResult.customerId}`)
    assert(soResult.soTxnId === "DRY_RUN_SO_TXNID", `SO TxnId: ${soResult.soTxnId}`)
    ...
    const draftOriginOrder = {
        ...
        metadata: { qb_estimate_txn_id: "1BA7A7-1772123940", qb_estimate_ref: "E18024525" },
    }
    const convertResult = await processOrderInQb(draftOriginOrder, mockCustomerModule)
    assert(convertResult.enabled === true, "Returns enabled: true")
    assert(convertResult.dryRun === true, "Returns dryRun: true")
    assert(convertResult.soTxnId === "DRY_RUN_CONVERT_TXNID", `Convert TxnId: ${convertResult.soTxnId}`)
    ...
    assert(payResult.txnId === "DRY_RUN_PAYMENT_TXNID", `Payment TxnId: ${payResult.txnId}`)
    ...
    assert(invResult.txnId === "DRY_RUN_INVOICE_TXNID", `Invoice TxnId: ${invResult.txnId}`)
    ...
    assert(estResult.txnId === "DRY_RUN_ESTIMATE_TXNID", `Estimate TxnId: ${estResult.txnId}`)
    ...
    assert(true, "Guards passed for all tests above (QB_ORDER_FLOW_ENABLED=true)")
    assert(true, "processOrderInQb → processPaymentCaptureInQb → processInvoiceInQb → processEstimateInQb all executed")
    assert(true, "Note: QB_ORDER_FLOW_ENABLED=false test requires separate process invocation")
    ...
    const emptyResult = await processOrderInQb(emptyItemsOrder, mockCustomerModule)
    assert(emptyResult.skipReason?.includes("No QB-linked"), `SkipReason: ${emptyResult.skipReason}`)
