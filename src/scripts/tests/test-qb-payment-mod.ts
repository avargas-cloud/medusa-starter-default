...
async function main() {
    ...
    if (!paymentTxnId || !invoiceTxnId) {
        ...
        console.log("Usage: npx -y tsx src/scripts/tests/test-qb-payment-mod.ts <PaymentTxnId> <InvoiceTxnId>");
        console.log("Example: npx -y tsx src/scripts/tests/test-qb-payment-mod.ts 6A5C-17112345 6A5B-17112344");
        process.exit(1);
    }

    console.log(`🔍 1. Fetching Payment ${paymentTxnId} from QB to get EditSequence...`);
    ...
    try {
        ...
        if (!editSequence) {
            console.error(`❌ Could not find EditSequence in QB response for Payment ${paymentTxnId}`);
            console.log("Raw Response Snippet:", JSON.stringify(rawResult).slice(0, 300));
            process.exit(1);
        }
