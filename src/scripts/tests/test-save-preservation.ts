...
async function testSavePreservation() {
    ...
    try {
        ...
        console.log('available_filters:', JSON.parse(before.rows[0].available_before || '[]').length);
        console.log('active_filters:', JSON.parse(before.rows[0].active_before || '[]').length);
