...
export const protectedTables = ["customer_payment", "payment_application", "pos_invoice", "pos_credit_memo",
  "vendor_bill", "qb_account", "treasury_distribution_log", "qb_order_pipeline"];

export async function fingerprints(client: PoolClient) {
  const extra = await client.query<{ tablename: string }>(`SELECT tablename FROM pg_tables WHERE schemaname='public'
  ...
  const tables = [...new Set([...protectedTables, ...extra.rows.map(row => row.tablename)])].sort();
  const result: Record<string, unknown> = {};
  for (const table of tables) {
    assert(/^[a-z_]+$/.test(table));
    result[table] = (await client.query(`SELECT count(*)::text AS count,
      md5(COALESCE(string_agg(md5(to_jsonb(t)::text),'' ORDER BY to_jsonb(t)::text),'')) AS hash FROM ${table} t`)).rows[0];
  }
  return result;
}
export async function bankingFingerprint(client: PoolClient) {
  const result: Record<string, unknown> = {};
  const tables = (await client.query<{ tablename: string }>("SELECT tablename FROM pg_tables WHERE schemaname='public' AND starts_with(tablename,'bank_') ORDER BY tablename")).rows;
  for (const { tablename } of tables) {
    assert(/^[a-z_]+$/.test(tablename));
    result[tablename] = (await client.query(`SELECT count(*)::text AS count,
      md5(COALESCE(string_agg(md5(to_jsonb(t)::text),'' ORDER BY to_jsonb(t)::text),'')) AS hash FROM ${tablename} t`)).rows[0];
  }
  return result;
}

export async function verifiedSnapshot(client: PoolClient) {
  const file = resolve(__dirname, "../../../../sandbox-artifacts/snapshots/pre-bank-accounting-v8.dump");
  const before = await fingerprints(client);
  if (!existsSync(file)) {
    assert(!(await client.query("SELECT to_regclass('public.bank_journal_entry') AS name")).rows[0].name,
  ...
  for (const table of Object.keys(before)) assert(toc.includes(` ${table} `), `Snapshot includes ${table}`);
  assert(toc.includes(" bank_transaction "));
  writeFileSync(file.replace(/dump$/, "toc"), toc, { mode: 0o600 });
  assert.deepEqual(await fingerprints(client), before, "Snapshot does not modify protected documents");
  console.log(`PASS pre-bank-accounting-v8 snapshot + TOC; ${Object.keys(before).length} protected fingerprints`);
}
...
export async function seedMovement(client: PoolClient, suffix: string, amount = "120.01", date = day) {
  ...
  await transaction(client, async () => {
    ...
    await client.query(`INSERT INTO bank_transaction(id,connection_id,account_id,provider_transaction_id,amount,currency,status,
      ...
      'EPT synthetic direct expense v8','{}',now(),now())`, [id, connection, account, amount, date]);
  });
  return id;
}
...
export async function cleanFixtures(client: PoolClient) {
  await transaction(client, async () => {
    ...
    const triggers = (await client.query(`SELECT tgname,tgenabled FROM pg_trigger
      WHERE tgname IN ('bank_journal_entry_immutable','bank_journal_line_immutable') ORDER BY tgname`)).rows;
    assert(triggers.length === 2 && triggers.every(row => row.tgenabled === 'O'), "Journal guards enabled before cleanup");
    const foreignBefore = (await client.query(`SELECT md5(COALESCE(string_agg(to_jsonb(e)::text,'' ORDER BY e.id),'')) h
      FROM (SELECT e.id,to_jsonb(e) AS entry,(SELECT jsonb_agg(l ORDER BY l.id) FROM bank_journal_line l WHERE l.entry_id=e.id) AS lines FROM bank_journal_entry e WHERE NOT starts_with(transaction_id,$1)) e`, [prefix])).rows;
    ...
    await client.query(`DELETE FROM bank_journal_line WHERE entry_id IN
      (SELECT id FROM bank_journal_entry WHERE starts_with(transaction_id,$1))`, [prefix]);
    await client.query("DELETE FROM bank_journal_entry WHERE starts_with(transaction_id,$1) AND kind='reversal'", [prefix]);
    await client.query("DELETE FROM bank_journal_entry WHERE starts_with(transaction_id,$1)", [prefix]);
    ...
    assert.deepEqual((await client.query(`SELECT tgname,tgenabled FROM pg_trigger
      WHERE tgname IN ('bank_journal_entry_immutable','bank_journal_line_immutable') ORDER BY tgname`)).rows, triggers);
    assert.deepEqual((await client.query(`SELECT md5(COALESCE(string_agg(to_jsonb(e)::text,'' ORDER BY e.id),'')) h
      FROM (SELECT e.id,to_jsonb(e) AS entry,(SELECT jsonb_agg(l ORDER BY l.id) FROM bank_journal_line l WHERE l.entry_id=e.id) AS lines FROM bank_journal_entry e WHERE NOT starts_with(transaction_id,$1)) e`, [prefix])).rows, foreignBefore);
    await client.query("DELETE FROM bank_direct_expense WHERE starts_with(transaction_id,$1)", [prefix]);
    ...
    await client.query("DELETE FROM bank_review_event WHERE starts_with(transaction_id,$1) OR starts_with(entity_id,$1) OR starts_with(actor_id,$2)", [prefix, actor]);
    const daily = (await client.query(`SELECT id,day FROM bank_day_close WHERE day=$1
      AND (snapshot::text LIKE $2 OR history::text LIKE $2)`, [day, `%${account}%`])).rows;
    for (const row of daily) {
      await client.query("DELETE FROM bank_review_event WHERE entity_type IN ('day','command') AND entity_id=$1", [row.day]);
      await client.query("DELETE FROM bank_day_close WHERE id=$1", [row.id]);
    }
    for (const table of ["bank_review_attachment", "bank_transaction_review"]) await client.query(`DELETE FROM ${table} WHERE starts_with(transaction_id,$1)`, [prefix]);
    await client.query("DELETE FROM bank_review_permission WHERE starts_with(id,'brp_e2e_accounting_v8')");
    for (const table of ["bank_webhook_event", "bank_sync_run", "bank_transaction", "bank_account"]) await client.query(`DELETE FROM ${table} WHERE connection_id=$1`, [connection]);
    await client.query("DELETE FROM bank_connection WHERE id=$1", [connection]);
    assert.equal((await client.query(`SELECT 1 FROM bank_journal_entry WHERE starts_with(transaction_id,$1) UNION ALL
      ...
      UNION ALL SELECT 1 FROM bank_review_event WHERE starts_with(transaction_id,$1) OR starts_with(entity_id,$1) OR starts_with(actor_id,$2)`, [prefix, actor])).rowCount, 0);
  });
}

export async function journalNegativeControls(client: PoolClient, entryId: string) {
  for (const table of ["bank_journal_entry", "bank_journal_line"]) {
    const where = table === "bank_journal_entry" ? "id=$1" : "entry_id=$1";
    await assert.rejects(client.query(`DELETE FROM ${table} WHERE ${where}`, [entryId]), /BANKING_JOURNAL_IMMUTABLE/);
  }
  ...
  await assert.rejects(transaction(client, async () => { await client.query(copy, [entryId, "expense"]); }), /BANKING_ALREADY_POSTED/);
  await assert.rejects(transaction(client, async () => {
    await client.query(copy.replace("expense_id,transaction_id,$2", "expense_id,$3,$2"), [entryId, "expense", prefix + "wrong_join"]);
  }), /BANKING_JOURNAL_SOURCE_INVALID/);
  for (const lines of [0, 1, 2]) {
    await assert.rejects(transaction(client, async () => {
      await client.query(copy, [entryId, "reversal"]);
      if (lines) await client.query(`INSERT INTO bank_journal_line(id,entry_id,role,account_list_id,account_snapshot,debit_cents,credit_cents)
        ...
        FROM bank_journal_line WHERE entry_id=$1 ORDER BY role LIMIT $2`, [entryId, lines]);
    }), /BANKING_JOURNAL_UNBALANCED/);
  }
  assert.equal((await client.query("SELECT 1 FROM bank_journal_entry WHERE id='bje_e2e_accounting_v8_negative'")).rowCount, 0);
  return 8;
}
