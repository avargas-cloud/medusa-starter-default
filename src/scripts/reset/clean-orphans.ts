...
async function main() {
    ...
    try {
        ...
        if (fIds.length > 0) {
            await client.query(`DELETE FROM order_fulfillment WHERE fulfillment_id = ANY($1::text[])`, [fIds])
            await client.query(`DELETE FROM fulfillment WHERE id = ANY($1::text[])`, [fIds])
