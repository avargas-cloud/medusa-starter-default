...
async function main() {
  ...
  if (INCLUDE_DRAFTS)
    console.log("   ⚠️  --include-drafts: Draft/POS orders WILL be deleted");
  else
    console.log(
      "   ℹ️  Draft/POS orders will be KEPT (pass --include-drafts to delete them)\n"
    );
  ...
  try {
    ...
    console.log(
      "1. Deleting Customer Finance Ledger (payment_application, invoice_payment, customer_payment)"
    );
    ...
    console.log(
      "2. Deleting Credit Memos (order_credit_line, credit_line, pos_credit_memo_item, pos_credit_memo)"
    );
    ...
    // ── Step 3: POS Invoices ─────────────────────────────────────────────
    console.log(
      "3. Deleting POS Invoices (invoice_tracking, pos_invoice_item, pos_invoice)"
    );
    ...
    // ── Step 4: Fulfillments ─────────────────────────────────────────────
    console.log(
      "4. Deleting Fulfillments (fulfillment_label, fulfillment_item, order_fulfillment, fulfillment)"
    );
    ...
    console.log(
      "4. Deleting Item Allocations / Reservations (reservation_item) and resetting levels"
    );
    await client.query("SAVEPOINT pre_reservations");
    try {
      await client.query(`DELETE FROM reservation_item`);
      await client.query(
        `UPDATE inventory_level SET reserved_quantity = 0, raw_reserved_quantity = '{"value": "0", "precision": 20}'::jsonb`
      );
      await client.query("RELEASE SAVEPOINT pre_reservations");
    } catch (e) {
      await client.query("ROLLBACK TO pre_reservations");
      console.warn(
        "   ⚠ reservation_item not found or could not update levels, skipping"
      );
    }
    ...
    console.log(
      `5. Deleting ${INCLUDE_DRAFTS ? "ALL" : "Confirmed"} Orders and Related Records...`
    );

    const ordersResult = await client.query(
      `SELECT id FROM "order" ${orderFilter}`
    );
    ...
    if (orderIds.length > 0) {
      ...
      const addressIds = addressResult.rows
        .map((r) => r.addr_id)
        .filter(Boolean);

      // Delete orders (cascades order_change, order_item, shipping_methods, etc.)
      await client.query(`DELETE FROM "order" WHERE id = ANY($1::text[])`, [
        orderIds,
      ]);
    ...
    if (INCLUDE_DRAFTS) {
      console.log(
        "7. Deep cleaning all remaining orphaned records (Nuclear Option)..."
      );
