...
async function main() {
  ...
  for (const table of tables) {
    try {
      ...
      console.log(
        `⚠️ ${table.padEnd(24)}: Error checking - ${(e as Error).message}`
      );
    }
  }
  ...
  if (hasRecords) {
    console.error(
      "❌ PURGE INCOMPLETE: Some records still exist in the database."
    );
    process.exit(1);
  } else {
    console.log(
      "✅ PURGE VERIFIED: All specified tables are completely empty."
    );
  }
