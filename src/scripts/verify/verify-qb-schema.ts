...
async function verifyQBTables() {
    ...
    try {
        ...
        const configData = await client.query(`SELECT * FROM quickbooks_config WHERE id = 'default'`);

        console.log('\n📝 Default config:');
        if (configData.rows.length > 0) {
            console.log('  ✅ Default row exists');
            console.log(`  • Inventory interval: ${configData.rows[0].inventory_interval_minutes} mins`);
            console.log(`  • Price interval: ${configData.rows[0].price_interval_minutes} mins`);
