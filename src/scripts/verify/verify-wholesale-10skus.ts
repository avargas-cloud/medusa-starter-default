...
export default async function verifyWholesale10Skus({ container }: ExecArgs) {
    ...
    if (!wholesalePriceListId) { logger.error("❌ Wholesale price list not found"); return }

    // 2. Get all wholesale prices (indexed by price_set_id)
    const allWholesale = await pricingService.listPrices({ price_list_id: [wholesalePriceListId] })
    ...
    const { data: variants } = await query.graph({
        ...
        fields: ["id", "sku", "prices.amount", "prices.currency_code", "prices.price_list_id", "price_set.id"],
    })

    // Filter variants that have both retail AND wholesale, pick 10 random
    const withPrices = variants.filter((v: any) => v.sku && v.price_set?.id && wholesaleByPriceSet.has(v.price_set.id))
    ...
    logger.info(`${"SKU".padEnd(28)} ${"Retail".padStart(8)} ${"Wholesale".padStart(10)} ${"Expected WS".padStart(12)} ${"plist_id OK".padStart(12)} ${"Status".padStart(8)}`)
    logger.info(`${"─".repeat(90)}`)

    let allOk = true

    for (const variant of shuffled) {
        const usdPrices = (variant.prices || []).filter((p: any) => p.currency_code === "usd" && !p.price_list_id)
        const retail = usdPrices.length > 0
