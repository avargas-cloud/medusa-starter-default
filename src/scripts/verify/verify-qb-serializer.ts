...
async function test3(): Promise<void> {
  ...
  try {
    ...
    const pool = getDbPool();
    await pool.query(
      `ALTER TABLE qb_order_pipeline DISABLE TRIGGER trg_qb_pipeline_updated_at`
    );
    await pool.query(
    ...
    await pool.query(
      `ALTER TABLE qb_order_pipeline ENABLE TRIGGER trg_qb_pipeline_updated_at`
    );
...
async function test6(): Promise<void> {
  ...
  try {
    ...
    if (fn2Start >= fn1End) {
      ...
      fail(
        name,
        `fn2 started ${overlap}ms BEFORE fn1 ended (concurrent — not sequential)`
      );
    }
  } catch (err: unknown) {
    fail(name, err instanceof Error ? err.message : String(err));
