...
async function verify() {
    ...
    try {
        ...
        const newAddress = customer.addresses.find((a: any) => a.metadata?.nickname === "Oficina TS");
        ...
        if (customer.default_billing_address_id === newAddress.id) {
            console.log("✅ Success: default_billing_address_id matched new address ID");
        } else {
            console.error(`❌ Failure: customer.default_billing_address_id (${customer.default_billing_address_id}) !== address.id (${newAddress.id})`);
        }

        // 3. Update Address
        console.log("\n3. Updating address with DEFAULT SHIPPING flag...");
        const updateRes = await client.post(`/store/customers/me/addresses/${newAddress.id}`, {
            metadata: {
        ...
        if (updatedCustomer.default_shipping_address_id === newAddress.id) {
            console.log("✅ Success: default_shipping_address_id matched updated address ID");
        } else {
            console.error(`❌ Failure: customer.default_shipping_address_id (${updatedCustomer.default_shipping_address_id}) !== address.id (${newAddress.id})`);
        }
