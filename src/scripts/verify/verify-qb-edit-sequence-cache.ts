...
async function runTests() {
  ...
  const miss = await getCachedEditSequence(
    "estimate",
    `NONEXISTENT-${TEST_ID}`
  );
  ...
  const mockLineIds = {
    "QB-ITEM-001": "TXN-LINE-A",
    "QB-ITEM-002": "TXN-LINE-B",
  };
  ...
  ok(
    "lineIds stored as JSON object",
    JSON.stringify(hit2?.lineIds) === JSON.stringify(mockLineIds)
  );
  ok(
    "lineIds[QB-ITEM-001] = TXN-LINE-A",
    hit2?.lineIds?.["QB-ITEM-001"] === "TXN-LINE-A"
  );
  ...
  ok(
    "lineIds preserved from previous cache (COALESCE)",
    hit3?.lineIds?.["QB-ITEM-001"] === "TXN-LINE-A"
  );
  ...
  ok(
    `pollUntilQbConfirmed detected 'confirmed' (got: ${outcome})`,
    outcome === "confirmed"
  );

  // Cleanup test rows
  await pool.query(`DELETE FROM qb_edit_sequence_cache WHERE qb_id = $1`, [
    TEST_ID,
  ]);
  ...
  for (const row of existing) {
    ...
    console.log(
      `    ${row.entity_type}/${row.qb_id} | editSeq=${row.edit_seq} | lineIds=${row.line_ids ? "✅ present" : "⬜ null (expected for pre-migration entries)"}`
    );
  }
