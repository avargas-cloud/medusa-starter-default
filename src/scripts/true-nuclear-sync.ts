    ...
    try {
        ...
        for (let i = 0; i < allCategories.length; i++) {
            ...
            const attributeValues = await knex('attribute_value as av')
                .join('product_product_productattributes_attribute_value as link', 'av.id', 'link.attribute_value_id')
                .whereIn('link.product_id', productIds)
                .select('av.attribute_key_id')
                .distinct()

            const uniqueAttrIds = attributeValues.map((row: any) => row.attribute_key_id)
            ...
            const existingActiveFilters = metadata?.filter_config?.active_filters || []
            ...
            console.log(`✅ ${progress} ${category.name} - ${uniqueAttrIds.length} filters configured`)
            phase1Success++
        }

        console.log('')
        console.log('═'.repeat(80))
        console.log(`✅ PHASE 1 Complete: ${phase1Success} configured, ${phase1Skipped} skipped`)
        console.log('═'.repeat(80))
        console.log('')
        ...
        console.log(`Generating counts for ${categoriesWithFilters.length} categories...\n`)
        ...
        for (let i = 0; i < categoriesWithFilters.length; i++) {
            ...
            try {
                ...
                console.log(`✅ ${progress} ${category.name} - ${result.filters.length} filters generated`)
                phase2Success++

            } catch (error: any) {
                console.error(`❌ ${progress} ${category.name} - Error: ${error.message}`)
                phase2Failed++
            }
        }
        ...
        console.log('')
        console.log('═'.repeat(80))
        console.log(`✅ PHASE 2 Complete: ${phase2Success} generated, ${phase2Failed} failed`)
        console.log('═'.repeat(80))
        console.log('')
        console.log('☢️ '.repeat(40))
