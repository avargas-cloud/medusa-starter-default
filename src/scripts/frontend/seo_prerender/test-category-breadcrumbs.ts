...
export default async function testCategoryBreadcrumbs({ container }: any) {
  ...
  const testCategories = [
    { handle: "by-categories", expectedDepth: 1, description: "Root category" },
    {
      handle: "led-drivers",
      expectedDepth: 2,
      description: "Parent category (has children)",
    },
    {
      handle: "dimmable-power-supplies",
      expectedDepth: 3,
  ...
  for (const test of testCategories) {
    ...
    try {
      ...
      if (breadcrumbs.length === test.expectedDepth) {
        ...
        logger.warn(
          `⚠️  Breadcrumb depth mismatch (expected ${test.expectedDepth}, got ${breadcrumbs.length})`
        );
      }
      ...
      if (children.length > 0) {
        ...
        if (children.length > 3)
          logger.info(`  ... and ${children.length - 3} more`);
