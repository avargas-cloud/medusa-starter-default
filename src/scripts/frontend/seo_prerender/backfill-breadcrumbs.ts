...
export default async function backfillBreadcrumbs({ container }) {
  ...
  try {
    ...
    logger.info(
      `[Phase 1] ${productsWithoutPrimary.length} products need primary category selection`
    );
    ...
    for (const product of productsWithoutPrimary) {
      try {
        ...
        if (
          !productWithCategories?.[0]?.categories ||
          productWithCategories[0].categories.length === 0
        ) {
          logger.warn(
            `  ⚠️  ${product.title} - No categories assigned, skipping`
          );
          continue;
        }
        ...
        const { result } = await buildCategoryBreadcrumbsWorkflow(
          container
        ).run({
    ...
    logger.info(
      `[Phase 2] ${productsNeedingBreadcrumbs.length} products need breadcrumb generation`
    );
    ...
    for (const product of productsNeedingBreadcrumbs) {
      try {
        ...
        phase2Count++;
        logger.info(
          `  ✅ [${phase2Count}] ${product.title} (${breadcrumbs.length} levels)`
        );
      } catch (error: any) {
        errorCount++;
        logger.error(`  ❌ Failed for ${product.title}:`, error.message);
    ...
    logger.info(`========================================`);
    logger.info(
      `  Phase 1 (Auto-select + breadcrumbs): ${phase1Count} products`
    );
    logger.info(
      `  Phase 2 (Generate breadcrumbs only): ${phase2Count} products`
    );
    logger.info(`  Total Updated: ${phase1Count + phase2Count}`);
    logger.info(`  Errors: ${errorCount}`);
    logger.info(
      `  Already Complete: ${allProducts.length - productsWithoutPrimary.length - productsNeedingBreadcrumbs.length}`
    );
  } catch (error: any) {
