...
export default async function enablePrerenderBulk({ container }: ExecArgs) {
  ...
  try {
    ...
    console.log(
      `✅ Total categories (including descendants): ${allCategoryIds.size}`
    );
    ...
    for (const categoryId of allCategoryIds) {
      ...
      if (categoriesUpdated % 10 === 0) {
        console.log(
          `   Updated ${categoriesUpdated}/${allCategoryIds.size} categories...`
        );
      }
    }
    ...
    for (const product of products) {
      ...
      if (productsUpdated % 50 === 0) {
        console.log(
          `   Updated ${productsUpdated}/${products.length} products...`
        );
      }
    }
