...
async function getCategoryBreadcrumbs(
  ...
  while (currentCategoryId) {
    try {
      ...
      console.log(
        `   ⚠️  Category ${currentCategoryId} not found, stopping breadcrumb trail`
      );
      break;
    }
  }
...
export default async function cleanProductMetadata({ container }: any) {
  ...
  for (const product of products) {
    ...
    if (metadata.primary_category_id) {
      try {
        await productModuleService.retrieveProductCategory(
          metadata.primary_category_id
        );
        // Category exists, keep it
      } catch (error) {
        console.log(`❌ ${product.title}`);
        console.log(
          `   Old primary_category_id: ${metadata.primary_category_id}`
        );

        // Get current categories
        const productWithCats = await productModuleService.retrieveProduct(
          product.id,
          {
        ...
        if (
          productWithCats.categories &&
          productWithCats.categories.length > 0
        ) {
    ...
    if (updates.primary_category_id) {
      try {
        const breadcrumbs = await getCategoryBreadcrumbs(
          updates.primary_category_id,
          productModuleService
        );
        const oldBreadcrumbs = JSON.stringify(
          metadata.main_category_breadcrumbs || []
        );
        ...
        if (oldBreadcrumbs !== newBreadcrumbs) {
          ...
          console.log(
            `   🔄 Updated breadcrumbs (${breadcrumbs.length} levels)`
          );
          needsUpdate = true;
        }
      } catch (error) {
        console.log(
          `   ⚠️  Could not calculate breadcrumbs: ${error instanceof Error ? error.message : error}`
        );
        errorCount++;
      }
    }
    ...
    if (needsUpdate) {
      try {
        ...
        console.log(
          `   💥 Error updating product: ${error instanceof Error ? error.message : error}`
        );
        errorCount++;
      }
    } else {
