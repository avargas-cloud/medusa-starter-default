...
async function fixOrphanedIdentities() {
  ...
  try {
    ...
    console.log(
      `🛠️ Buscando identidades OAuth (Google) huérfanas o sin vincular...\n`
    );
    ...
    if (orphans.rows.length === 0) {
      console.log(
        "✅ No se encontraron identidades OAuth huérfanas. ¡Todas tus cuentas anteriores están sanas!"
      );
      return;
    }

    console.log(
      `⚠️ Se encontraron ${orphans.rows.length} identidades huérfanas. Intentando repararlas...`
    );
    ...
    for (const orphan of orphans.rows) {
      ...
      if (customerResult.rows.length === 1) {
        ...
        if (!customerResult.rows[0].has_account) {
          ...
          console.log(
            `   🔸 Cuenta activada (has_account = true) retroactivamente.`
          );
        }

        console.log(
          `   ✅ Identidad ${orphan.auth_identity_id} enlazada exitosamente a ${customerId}. Reparada.`
        );
        fixedCount++;
      } else if (customerResult.rows.length > 1) {
        console.log(
          `   ❌ ERROR: Se encontraron ${customerResult.rows.length} customers con el email ${orphan.email}. Requiere unificación manual antes de enlazar el login de Google.`
        );
      } else {
        console.log(
          `   ❌ ERROR: No se encontró ningún customer con el email ${orphan.email}. La identidad de Google ${orphan.auth_identity_id} está completamente aislada en Medusa.`
        );
      }
    }
