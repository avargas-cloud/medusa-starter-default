export default async function debugGoogleAuth({ container }: { container: any }) {
  ...
  `
    ...
    if (authIdentities.length > 0) {
        ...
        for (const ai of authIdentities) {
            const pis = await sql`SELECT * FROM provider_identity WHERE auth_identity_id = ${ai.id}`
  ...
  `
    ...
    for (const row of allGoogleAuths) {
        ...
        if (authDataStr.includes("a.vargas") || userDataStr.includes("a.vargas")) {
            ...
            const ai = await sql`SELECT * FROM auth_identity WHERE id = ${row.auth_identity_id}`
