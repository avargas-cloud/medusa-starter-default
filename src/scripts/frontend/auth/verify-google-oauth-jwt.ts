...
export default async function verifyGoogleOAuthJWT({ container }: any) {
  ...
  try {
    ...
    console.log(
      "ℹ️  En producción, el Auth Module crearía el customer automáticamente"
    );
    console.log(
      "ℹ️  El callback debe buscar el customer creado y usar su ID en el JWT"
    );
    ...
    console.log(
      "  - customer_id (metadata):",
      decodedNew.app_metadata?.customer_id
    );
    ...
    console.log("");
    console.log(
      '🔧 Si /api/auth/me falla con "Request already authenticated as a customer"'
    );
    console.log(
      "   → El actor_id en el JWT es incorrecto (probablemente authIdentity.id)"
    );
    console.log("");
  } catch (error) {
