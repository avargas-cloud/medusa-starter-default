...
async function findAndCleanOrphans() {
  ...
  try {
    ...
    if (orphansRes.rows.length === 0) {
      console.log(
        "✅ Excellent! No orphaned auth identities found in the database. Everything is clean."
      );
      return;
    }

    console.log(
      `⚠️ Found ${orphansRes.rows.length} orphaned provider/auth identities!`
    );
    console.log("Here are the ghost records:");
    console.table(orphansRes.rows);

    console.log("\n🧹 Cleaning up orphaned provider identities...");
    const providerIds = orphansRes.rows
      .map((r) => r.provider_identity_id)
      .filter(Boolean);
    if (providerIds.length > 0) {
      ...
      console.log(
        `✅ Deleted ${deleteProviders.rowCount} provider_identity records.`
      );
    }

    console.log("\n🧹 Cleaning up orphaned auth identities...");
    const authIds = [
      ...new Set(orphansRes.rows.map((r) => r.auth_identity_id)),
    ];
    if (authIds.length > 0) {
