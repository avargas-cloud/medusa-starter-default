...
async function count(client: Client, table: string, where: string, params: any[] = []) {
    const res = await client.query(`SELECT COUNT(*) FROM ${table} WHERE ${where}`, params)
...
async function main() {
    ...
    for (const o of orders) {
        console.log(`  #${o.display_id.toString().padStart(4, " ")}  id=${o.id}  canceled_at=${o.canceled_at?.toISOString().slice(0, 10) ?? "?"}`)
    }
    hr()
    ...
    const [
        ...
        count(client, "payment_collection", `id IN (${PAY_COL_IDS_SQL})`, [orderIds]),
        count(client, "payment_session",
            `payment_collection_id IN (${PAY_COL_IDS_SQL})`,
            [orderIds]),
        count(client, "payment",
            `payment_collection_id IN (${PAY_COL_IDS_SQL})`,
            [orderIds]),
        count(client, "order_address",
            `id IN (
        ...
        count(client, "order_transaction", "order_id = ANY($1::text[])", [orderIds]),
        count(client, "order_shipping", "order_id = ANY($1::text[])", [orderIds]),
        count(client, "order_credit_line", "order_id = ANY($1::text[])", [orderIds]),
    ])
    ...
    try {
        ...
        `, [orderIds])
        const payColIds: string[] = payColResult.rows.map(r => r.payment_collection_id)
        ...
        let rPaySessions = 0, rPayments = 0, rPayCols = 0
        if (payColIds.length > 0) {
            const rs = await client.query(
                `DELETE FROM payment_session WHERE payment_collection_id = ANY($1::text[])`, [payColIds])
            rPaySessions = rs.rowCount ?? 0

            const rp = await client.query(
                `DELETE FROM payment WHERE payment_collection_id = ANY($1::text[])`, [payColIds])
            rPayments = rp.rowCount ?? 0

            // Delete pivot table entries first
            await client.query(
                `DELETE FROM order_payment_collection WHERE order_id = ANY($1::text[])`, [orderIds])

            const rpc = await client.query(
                `DELETE FROM payment_collection WHERE id = ANY($1::text[])`, [payColIds])
