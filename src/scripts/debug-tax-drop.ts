...
async function debugTaxDrop() {
    ...
    if (!variantId) {
        console.warn(`Could not find a variant via sku=${SKU}. Falling back to random...`);
    ...
    console.log(`   Province saved: ${addressRes.data.cart.shipping_address?.province}`);
    ...
    if (methodRes.status !== 200) return console.error("Failed to add shipping method");
    ...
    if (calcRes.status === 200) {
        ...
        console.log(`   /calculate returned ${calcRes.status} (Not supported or failed)`);
    }
    ...
    if (collectionId) {
        ...
        if (sessionRes.status === 200) {
            ...
            console.log(`   Shipping Method: ${preCompleteRes.data.cart.shipping_methods?.[0]?.name}`);
            console.log(`   Subtotal: ${preCompleteRes.data.cart.subtotal}`);
            console.log(`   Tax Total: ${preCompleteRes.data.cart.tax_total}`);

            if (preCompleteRes.data.cart.tax_total === 0 || preCompleteRes.data.cart.tax_total === '0') {
                console.log("❌ FAIL: Tax dropped BEFORE completion. Something wiped the tax or shipping address!");
                return;
            }
            ...
            if (completeRes.status === 200) {
                ...
                if (finalOrder.tax_total === 0 || finalOrder.tax_total === '0') {
                    console.log("❌ FAIL: Tax dropped DURING completion. The tax calculation module is failing to persist to the order.");
                } else {
