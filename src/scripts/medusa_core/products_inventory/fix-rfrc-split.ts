...
async function main() {
    ...
    const pRes = await client.query<{
        title: string; description: string; status: string; handle: string
        thumbnail: string; metadata: Record<string, unknown>; type_id: string | null
    }>(
        'SELECT title, description, status, handle, thumbnail, metadata, type_id FROM product WHERE id=$1',
        [PRODUCT_ID]
    )
    if (!pRes.rows.length) { console.error('❌ Source product not found'); process.exit(1) }
    const prod = pRes.rows[0]

    console.log(`📦 Source product: "${prod.title}" (${PRODUCT_ID})`)
    console.log(`   Option: "${OPTION_ID}" (Color Options)`)
    console.log(`   ├─ optval ${OPTVAL_5A_ID} "5A" → variant ${VARIANT_5A_ID} (ECTSK-RFRC1C5A)`)
    console.log(`   └─ optval ${OPTVAL_8A_ID} "8A" → variant ${VARIANT_8A_ID} (ECTSK-RFRC1C8A)\n`)
    ...
    console.log(`      - DELETE optval ${OPTVAL_8A_ID} from this product's option`)
    ...
    console.log(`      - MOVE variant ${VARIANT_8A_ID} → product_id = ${newProductId}`)
    console.log(`      - variant title → "Default Title"`)
    console.log(`      - prices on that variant are PRESERVED (no price changes)`)
    console.log(`      - metadata: remove qb_is_variant_parent`)
    console.log()
    ...
    try {
        ...
        await client.query(`DELETE FROM product_variant_option WHERE variant_id=$1`, [VARIANT_5A_ID])
        await client.query(
        ...
        await client.query(`DELETE FROM product_variant_option WHERE option_value_id=$1`, [OPTVAL_8A_ID])
        await client.query(`DELETE FROM product_option_value WHERE id=$1`, [OPTVAL_8A_ID])
        ...
        await client.query(
            ...
            [newProductId, prod.description, prod.status, newHandle, prod.thumbnail, JSON.stringify(newMetaB), prod.type_id]
        )
        ...
        await client.query(`DELETE FROM product_variant_option WHERE variant_id=$1`, [VARIANT_8A_ID])
        await client.query(
            `INSERT INTO product_variant_option (variant_id, option_value_id) VALUES ($1, $2)`,
            [VARIANT_8A_ID, newOptValId]
        )

        console.log(`   ✅ [B] New product created: ECTSK-RFRC1C8A → ${newProductId}`)
