...
function isLedStrip(skus: string[]): boolean {
  return skus.some((sku) => {
    ...
    return LED_STRIP_PREFIX_RULES.some((r) =>
      upper.startsWith(r.prefix.toUpperCase())
    );
  });
}
...
async function main() {
  ...
  const diagMap = Object.fromEntries(
    diagRes.rows.map((r) => [r.status, parseInt(r.count)])
  );
  console.log("📊 ESP variant products (all):");
  console.log(
    `   ✅ Already categorized : ${diagMap["already_categorized"] ?? 0}`
  );
  ...
  for (const product of products) {
    ...
    if (!isLedStrip(skus)) {
      skipped.push({
        ...
        reason: `no matching prefix — not a ${SCRIPT_TARGET} product`,
      });
      continue;
    }
    ...
    if (!matchedRule) {
      skipped.push({
        product: product.title,
        reason: `ESP SKUs [${espSkus.join(", ")}] — no suffix match`,
      });
      continue;
    }
    ...
    const categoryId = categoryMap.get(
      matchedRule.categoryName.toLowerCase().trim()
    );
    if (!categoryId) {
      skipped.push({
        product: product.title,
        reason: `Category "${matchedRule.categoryName}" NOT FOUND in Medusa`,
      });
      continue;
    }
  ...
  console.log(`  Skipped:  ${skipped.length}`);
  console.log(
    `  Mode:     ${DRY_RUN ? "DRY RUN (pass --apply to save)" : "APPLIED ✅"}`
  );
  console.log(`───────────────────────────────────────\n`);
