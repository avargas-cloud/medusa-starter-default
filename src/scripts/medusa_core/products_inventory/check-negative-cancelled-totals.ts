...
async function main() {
  ...
  console.log(
    "🔍 Checking for negative current_order_total in cancelled orders\n"
  );
  ...
  for (const r of negRows.rows) {
    console.log(
      `  #${r.display_id} → current=${r.current_total?.toFixed(2)}  original=${r.original_total?.toFixed(2)}  tx=${r.tx_total}  credit=${r.credit_total}`
    );
  }
  ...
  if (sampleOrder.rows.length > 0) {
    ...
    for (const item of items.rows) {
      console.log(
        `  - "${item.title}" qty=${item.quantity} unit_price=${item.unit_price} line=${item.line_total}`
      );
    ...
    for (const p of payments.rows) {
      ...
      if (p.session_id)
        console.log(
          `    session=${p.session_id} amount=${p.session_amount} status=${p.session_status}`
        );
    }
  }
