  ...
  try {
    ...
    const affectedProducts = products.filter((p: any) => {
      const activeVariants =
        p.variants?.filter((v: any) => !v.deleted_at) || [];
    ...
    const productsToProcess = limit
      ? affectedProducts.slice(0, limit)
      : affectedProducts;
    ...
    for (const product of productsToProcess) {
      ...
      try {
        ...
        const existingProduct = await productService.retrieveProduct(
          product.id,
          {
            relations: ["options"],
          }
        ...
        if (!dryRun) {
          ...
          logger.info(
            `   [DRY RUN] Would create option "Title" with value "${variantTitle}"`
          );
          logger.info(
            `   [DRY RUN] Would link variant ${variant.id} to option`
          );
        }
    ...
    logger.info(
      `Successfully ${dryRun ? "would be" : ""} updated: ${stats.success}`
    );
    ...
    if (dryRun) {
      ...
      logger.info(
        "   medusa exec ./src/scripts/fix-single-product-options.ts --live"
      );
      if (limit) {
        logger.info(`\n   Note: This test processed only ${limit} products.`);
        logger.info(
          `   Remove --limit flag to process all ${affectedProducts.length} products.`
        );
      }
    } else {
      logger.info("\n✅ Migration completed!");
      if (limit) {
        logger.info(
          `   Updated ${stats.success} of ${affectedProducts.length} total products`
        );
      } else {
