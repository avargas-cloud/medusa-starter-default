...
export default async function migrateProductAttributes({
  container,
}: ExecArgs) {
  try {
    ...
    const attributeService: ProductAttributesService =
      container.resolve("productAttributes");
    const remoteLink = container.resolve("remoteLink");

    // 1. Fetch products
    const products = await productModule.listProducts(
      {},
      { take: 9999, select: ["id", "title", "metadata"] }
    );
    ...
    for (const product of products) {
      ...
      const isTarget =
        product.id === "prod_100w-indoor-meanwell-power-supply-24vdc";
      ...
      if (
        !wcAttributes ||
        !Array.isArray(wcAttributes) ||
        wcAttributes.length === 0
      ) {
        if (isTarget)
          console.log(
            "   ❌ SKIPPING: No valid attributes found in metadata logic."
          );
        continue;
      }
      ...
      if (productsProcessed % 10 === 0)
        console.log(
          `🔄 Processing product ${productsProcessed + 1}/${products.length}...`
        );
      ...
      for (const attr of wcAttributes) {
        ...
        // Find/Create Key
        let [key] = await attributeService.listAttributeKeys(
          { handle },
          { take: 1 }
        );
        if (!key) {
      ...
      if (payloadQueue.length > 0) {
        console.log(
          `   🔗 Linking ${payloadQueue.length} attributes for product: ${product.title} (${product.id})`
        );
