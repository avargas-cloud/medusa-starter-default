...
export default async function activateInventoryForAllVariants({
  container,
}: {
  ...
  for (const variant of variants) {
    ...
    try {
      ...
      if (!DRY_RUN) {
        ...
        console.log(
          `✅ ${sku}: Created inventory item & enabled management (${inventoryItem.id})`
        );
      } else {
  ...
  if (result.inventoryItemsCreated > 0) {
    ...
    console.log(
      `📝 Note: All items start with 0 stock. Update quantities via admin or Excel upload.`
    );
  }
