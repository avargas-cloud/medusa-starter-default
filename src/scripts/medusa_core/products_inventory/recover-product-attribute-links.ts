    ...
    for (const product of products) {
        try {
            ...
            for (const option of productOptions) {
                ...
                if (!attributeKey) {
                    logger.warn(`   ⚠️  No attribute_key found for "${option.title}" - SKIPPING`)
                    skipped++
                    continue
                }

                logger.info(`   ✅ Matched to attribute: ${attributeKey.label} (${attributeKey.handle})`)
                ...
                for (const optionValue of optionValues) {
                    ...
                    if (!attributeValue) {
                        logger.warn(`      ⚠️  No attribute_value for "${optionValue.value}" - SKIPPING`)
                        continue
                    }

                    // Check if link already exists
                    const existingLink = await knex('product_product_productattributes_attribute_value')
                        .where({
