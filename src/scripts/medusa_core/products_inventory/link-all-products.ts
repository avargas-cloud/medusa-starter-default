...
export default async function linkAllProductsToChannels({ container }: ExecArgs) {
    ...
    while (true) {
        const products = await productModule.listProducts({}, { select: ["id"], skip, take: BATCH_SIZE })
    ...
    for (let i = 0; i < allProductIds.length; i += chunkSize) {
        ...
        try {
            ...
            logger.info(`   ✅ Linked ${linked} / ${allProductIds.length} products...`)
        } catch (err: any) {
            // remoteLink.create throws if a link already exists. We catch it and fallback to 1-by-1
            logger.info(`   ⚠️  Chunk ${i} had some already linked items, doing 1-by-1...`)
            ...
            logger.info(`   ✅ Recovered chunk ${i}, linked ${linked} / ${allProductIds.length} products...`)
        }
    }

    logger.info("\n" + "=".repeat(65))
    logger.info(`✅ SUCCESS: Ensured ${allProductIds.length} products are linked to ${channelIds.length} channels.`)
    logger.info("=".repeat(65))
}
