...
export default async function restructureProductTypes({ container }: any) {
  ...
  try {
    ...
    report.phase1_audit = {
      existingTypes: existingTypes.map((t: any) => ({
        id: t.id,
        value: t.value,
      })),
    ...
    const physicalProductType = createdTypes.find(
      (t: any) => t.value === "Physical Product"
    );
    ...
    const productsToMigrate = products.filter(
      (p: any) => !p.type_id || p.type_id !== physicalProductType.id
    );

    logger.info(
      `Migrating ${productsToMigrate.length} products to "Physical Product"...`
    );
    ...
    for (const product of productsToMigrate) {
      try {
        ...
        if (migratedCount % 50 === 0) {
          logger.info(
            `  Migrated ${migratedCount}/${productsToMigrate.length} products...`
          );
        }
      } catch (error: any) {
        logger.error(
          `  ✗ Failed to migrate product ${product.id}: ${error.message}`
        );
      }
    }
    ...
    const typesToDelete = existingTypes.filter(
      (t: any) => !standardTypes.includes(t.value)
    );
    ...
    for (const oldType of typesToDelete) {
      try {
        ...
        logger.error(
          `  ✗ Failed to delete "${oldType.value}": ${error.message}`
        );
      }
    }
