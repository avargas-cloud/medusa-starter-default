 ...
 */
export default async function migrateVariantSalesDescriptions({
  container,
}: ExecArgs) {
  ...
  const alreadyDone = allVariants.filter(
    (v: any) => v.metadata?.sales_description
  );
  const needsQBSync = allVariants.filter(
    (v: any) => !v.metadata?.sales_description && v.metadata?.quickbooks_id
  );
  const needsFallback = allVariants.filter(
    (v: any) => !v.metadata?.sales_description && !v.metadata?.quickbooks_id
  );
  ...
  if (needsQBSync.length > 0) {
    ...
    if (qbItems.length > 0) {
      ...
      for (const variant of needsQBSync) {
        ...
        if (!salesDesc) {
          warn(
            `   ⚠️  ${variant.sku ?? variant.id}: QB item ${qbId} not found / empty SalesDesc`
          );
          skippedEmpty++;
          continue;
        }
        ...
        try {
          ...
          updatedFromQB++;
          log(
            `   ✅ ${variant.sku ?? variant.id}: "${salesDesc.substring(0, 80)}${salesDesc.length > 80 ? "..." : ""}"`
          );
        } catch (e: any) {
      ...
      warn(
        "⚠️  No QB items — QB sync skipped, falling back for QB-linked variants too"
      );
      // Those variants will be picked up by the fallback below via product.metadata
      for (const variant of needsQBSync) {
        const productSalesDesc: string | undefined =
          variant.product?.metadata?.sales_description;
        if (!productSalesDesc) {
          skippedEmpty++;
          continue;
        }
        try {
          await productModule.updateProductVariants(variant.id, {
            metadata: {
              ...variant.metadata,
              sales_description: productSalesDesc,
            },
          });
          updatedFromFallback++;
        } catch (e: any) {
          warn(
            `   ❌ ${variant.sku ?? variant.id} (QB-fallback): ${e.message}`
          );
          errors++;
        }
      }
