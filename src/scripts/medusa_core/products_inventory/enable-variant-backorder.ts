...
export default async function enableVariantBackorder({ container }: ExecArgs) {
  ...
  try {
    ...
    console.log(
      `  Before: ${countBefore} variants already had allow_backorder = true`
    );
    ...
    console.log(
      `   After update  : ${countAfter} variants have allow_backorder = true`
    );
    console.log(
      `\nℹ️  Medusa's conversion workflow will now pass allow_backorder=true`
    );
    console.log(
      `   to inventory reservation creation, bypassing the stock quantity check.`
    );
  } catch (e: any) {
