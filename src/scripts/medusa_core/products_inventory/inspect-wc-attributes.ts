...
export default async function inspectWcAttributes({ container }: ExecArgs) {
  ...
  try {
    ...
    const attrsResponse = await fetch(
      `${WC_URL}/wp-json/wc/v3/products/attributes`,
      {
    ...
    if (!attrsResponse.ok) {
      throw new Error(
        `Failed to fetch attributes: ${attrsResponse.statusText}`
      );
    }
    ...
    for (const attr of attributes) {
      const termsResponse = await fetch(
        `${WC_URL}/wp-json/wc/v3/products/attributes/${attr.id}/terms`,
        {
          headers: { Authorization: authHeader },
        }
      );
      ...
      console.log(
        `📦 [${attr.name}] (slug: ${attr.slug}) - ${terms.length} options`
      );
    ...
    console.log("---------------------------------------------------");
    console.log(
      "⚠️  NOTE: This was a READ-ONLY inspection. No data was saved."
    );
  } catch (error) {
