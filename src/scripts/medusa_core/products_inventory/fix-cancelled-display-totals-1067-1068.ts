...
async function main() {
  ...
  console.log(
    `🔧 Fix Cancelled Display Totals for #${TARGET_DISPLAY_IDS.join(", #")} (DRY_RUN=${DRY_RUN})\n`
  );
  ...
  for (const row of current.rows) {
    ...
    console.log(
      `    current_order_total  = $${(row.current_total ?? 0).toFixed(4)}`
    );
    console.log(
      `    original_order_total = $${(row.original_total ?? 0).toFixed(4)}`
    );
    console.log(
      `    credit_line_total    = $${(row.credit_line_total ?? 0).toFixed(4)}`
    );
    console.log(
      `    accounting_total     = $${(row.accounting_total ?? 0).toFixed(4)}`
    );
    console.log(
      `    paid_total           = $${(row.paid_total ?? 0).toFixed(4)}\n`
    );
  }
  ...
  if (DRY_RUN) {
    ...
    console.log(
      "   Would zero out current_order_total and accounting_total for the above orders."
    );
  ...
  for (const row of current.rows) {
    ...
    if (result.rowCount && result.rowCount > 0) {
      console.log(
        `✅ #${row.display_id}: zeroed totals + raw_* fields (credit_line_total set to $${originalTotal.toFixed(4)})`
      );
    } else {
  ...
  for (const row of verify.rows) {
    console.log(
      `  #${row.display_id}: current_order_total=$${(row.current_total ?? 0).toFixed(4)}, credit_line_total=$${(row.credit_line_total ?? 0).toFixed(4)}`
    );
  }
