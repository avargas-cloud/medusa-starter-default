...
async function testEndpoint(categoryId: string, limit = 5) {
  ...
  try {
    ...
    console.log(
      `   Include descendants: ${data.category?.include_descendants_tree}`
    );
    ...
    if (data.products && data.products.length > 0) {
      ...
      if (firstProduct.price) {
        console.log(
          `   - Price: $${firstProduct.price.amount} ${firstProduct.price.currency_code}`
        );
      } else if (firstProduct.price_range) {
        console.log(
          `   - Price range: $${firstProduct.price_range.min.amount} - $${firstProduct.price_range.max.amount}`
        );
      }
    }
