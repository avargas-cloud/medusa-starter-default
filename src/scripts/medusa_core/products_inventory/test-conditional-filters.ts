...
async function testCategory(categoryId: string, testName: string) {
  ...
  try {
    ...
    console.log(
      `   include_descendants_tree: ${metadata.include_descendants_tree ?? "true (default)"}`
    );
    ...
    if (updatedCatResponse.ok) {
      ...
      const updatedMetadata: CategoryMetadata =
        updatedData.product_category?.metadata || {};

      console.log(`\n📊 Available Filters:`);
      const availableCount =
        updatedMetadata.filter_config?.available_filters?.length || 0;
...
async function main() {
  ...
  console.log(
    `   2. Edit category metadata to set include_descendants_tree: false`
  );
