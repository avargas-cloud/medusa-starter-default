...
export default async function deleteProductsBySku({ container }: ExecArgs) {
  ...
  console.log(
    isDryRun
      ? "📋 DRY RUN MODE - No se eliminará nada"
  ...
  try {
    ...
    const notFoundSkus = SKUS_TO_DELETE.filter(
      (sku) => !foundSkus.includes(sku)
    );
    ...
    console.log(
      `   • product_variant_inventory_item: ${stats.variantInventoryLinks}`
    );
 ...
 */
async function calculateDeletionStats(
  pool: Pool,
  productIds: string[]
