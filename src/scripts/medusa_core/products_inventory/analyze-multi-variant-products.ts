...
export default async function analyzeMultiVariantProducts({
  container,
}: {
  ...
  console.log(
    `Products with Orphaned Variants: ${analysis.productsWithOrphanedVariants.length}\n`
  );
  ...
  if (analysis.productsWithOrphanedVariants.length > 0) {
    ...
    console.log(
      "✅ No orphaned variants found! All multi-variant products have proper options.\n"
    );
  }
