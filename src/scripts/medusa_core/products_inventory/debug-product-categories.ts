...
export default async function checkProductCategories({ container }: any) {
    ...
    console.log('🏷️  Primary Category ID (metadata):', product.metadata?.primary_category_id || 'NOT SET')
    ...
    const categories = await knex('product_category_product as pcp')
        ...
        .select('pc.id', 'pc.name', 'pc.handle', 'pc.deleted_at', 'pc.parent_category_id')
    ...
    if (product.metadata?.primary_category_id) {
        ...
        if (primaryCat) {
            ...
            console.log(`   ⚠️  This is the problem! Category '${product.metadata.primary_category_id}' doesn't exist`)
        }
    }
