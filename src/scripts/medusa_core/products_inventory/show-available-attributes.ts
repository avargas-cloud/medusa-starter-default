...
async function main() {
    ...
    console.log("STATS:")
    console.log("=".repeat(80))
    console.log(`available_attributes: ${category.metadata?.available_attributes?.length || 0} items`)
    console.log(`filters: ${category.metadata?.filters?.length || 0} items`)
    console.log(`filters_metadata.total_products: ${category.metadata?.filters_metadata?.total_products || 'N/A'}`)
