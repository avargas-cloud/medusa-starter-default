...
export default async function linkProductsToAttributeValues({
  container,
}: {
  ...
  for (const product of multiVariantProducts) {
    ...
    try {
      ...
      for (const option of product.options) {
        ...
        console.log(
          `      📊 Variant Values: ${Array.from(variantValues).join(", ")}`
        );
      ...
      console.log(`\n   📝 Summary:`);
      console.log(
        `      - Attribute Values to Link: ${attributeValuesToLink.size}`
      );
      ...
      if (!DRY_RUN) {
        ...
        console.log(
          `      [DRY-RUN] Would link ${attributeValuesToLink.size} values and mark ${variantKeyIds.size} keys`
        );
      }
