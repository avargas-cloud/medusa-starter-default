...
async function assignCategoryThumbnails() {
  ...
  try {
    ...
    console.log(
      `📥 Listing files in MinIO bucket: ${BUCKET}/${CATEGORY_FOLDER}...`
    );
    ...
    console.log(
      `📊 Categories with woocommerce_image: ${result.rows.length}\n`
    );
    ...
    for (const cat of result.rows) {
      ...
      if (cat.thumbnail) {
        console.log(
          `⏭️  ${cat.name} - Already has thumbnail: ${cat.thumbnail}`
        );
        alreadySet++;
        continue;
      }
    ...
    if (notFound > 0) {
      console.log(
        `   4. Upload ${notFound} missing files to MinIO: ${BUCKET}/${CATEGORY_FOLDER}`
      );
    }
