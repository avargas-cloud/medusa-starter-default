export default async function ({ container }: any) {
    ...
    for (const cat of categoriesWithFilters) {
        ...
        const hasFilters = cat.metadata?.filters && Array.isArray(cat.metadata.filters) && cat.metadata.filters.length > 0
        ...
        const size = descendants > 50 ? "🔴 LARGE" : descendants > 10 ? "🟡 MEDIUM" : "🟢 SMALL"
    ...
    const needsSync = categoriesWithFilters.filter(c =>
        !c.metadata?.filters || c.metadata.filters.length === 0
    )
