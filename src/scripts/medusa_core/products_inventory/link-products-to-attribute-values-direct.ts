...
export default async function linkProductsToAttributeValuesDirect({
  container,
}: {
    ...
    try {
      ...
      for (const option of product.options) {
        ...
        for (const variant of product.variants) {
          ...
          if (
            variantOption?.value &&
            !variantValues.includes(variantOption.value.trim())
          ) {
        ...
        for (const variantValue of variantValues) {
          const matchingAttrValue = matchingAttrKey.values?.find(
            (val) =>
        ...
        if (matchedAttributeValues.length > 0) {
          console.log(
            `      ✅ Matched Attribute Values: ${matchedAttributeValues.join(", ")}`
          );
      ...
      console.log(`\n   📝 Summary:`);
      console.log(
        `      - Attribute Values to Link: ${attributeValuesToLink.length}`
      );
      ...
      if (!DRY_RUN) {
        ...
        console.log(
          `      ✅ Linked ${attributeValuesToLink.length} values and marked $ {variantKeyIds.length} keys`
        );
      } else {
        console.log(
          `      [DRY-RUN] Would link ${attributeValuesToLink.length} values and mark ${variantKeyIds.length} keys`
        );
      }
