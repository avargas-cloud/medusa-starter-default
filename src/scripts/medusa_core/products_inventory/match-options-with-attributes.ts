...
export default async function matchOptionsWithAttributes({
  container,
}: {
  container: MedusaContainer;
}) {
  console.log(
    "🔍 Deep Analysis: Matching Product Options with Attributes...\n"
  );
  ...
  const { data: attributes } = await query.graph({
    ...
    fields: ["id", "title", "label", "handle", "values.*"],
  });
  ...
  const { data: products } = await query.graph({
    entity: "product",
    fields: ["id", "title", "handle", "variants.*", "options.*"],
  });

  const multiVariantProducts = products.filter((p) => p.variants?.length > 1);
  console.log(
    `📦 Analyzing ${multiVariantProducts.length} Multi-Variant Products\n`
  );
  ...
  for (const result of results.filter((r) => r.canAutoLink)) {
    ...
    console.log(
      `   Option: "${result.productOption.title}" → Attribute: "${result.attributeMatch.attributeTitle}"`
    );
    console.log(`   Variants (${result.variants.length}):`);

    for (const variant of result.variants) {
      console.log(
        `      "${variant.title}" → "${variant.valueMatch.valueLabel}" ✓`
      );
  ...
  for (const result of results.filter((r) => r.needsManualReview)) {
    ...
    if (result.attributeMatch.found) {
      console.log(
        `   ✓ Attribute Found: "${result.attributeMatch.attributeTitle}"`
      );
      ...
      console.log(
        `   ❌ No matching attribute found for option "${result.productOption.title}"`
      );
    }
  }
