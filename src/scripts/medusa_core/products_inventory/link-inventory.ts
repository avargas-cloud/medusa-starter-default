...
async function run() {
  ...
  const linkModule: ILinkModule = container.resolve(
  ...
  console.log(
    `⚠️ Variantes SIN Inventory Item vinculado: ${unlinkedVariants.length}`
  );

  if (unlinkedVariants.length === 0) {
    console.log(
      "✅ Todas las variantes están vinculadas correctamente. Saliendo."
    );
    process.exit(0);
  }
  ...
  for (const v of unlinkedVariants) {
    try {
      ...
      console.error(
        `❌ Error arreglando variante ${v.sku || v.id}: ${err.message}`
      );
      errorCount++;
    }
  }
