...
export default async function healAllVariants({ container }: ExecArgs) {
  ...
  for (const product of products) {
    const productVariantAttributes = new Set(
    ...
    for (const option of product.options) {
      ...
      const matchingKey = allAttributeKeys.find(
        (k) =>
          k.label.toLowerCase() === option.title.toLowerCase() ||
          k.handle.toLowerCase() === option.title.toLowerCase()
      );

      if (matchingKey) {
        logger.info(
          `🔗 Match found: Option '${option.title}' -> Attribute '${matchingKey.label}' in Product ${product.id}`
        );
        ...
        for (const value of uniqueValues) {
          ...
          if (!attributeValue) {
            ...
            logger.info(
              `   ➕ Created new Attribute Value: '${value}' for Key '${matchingKey.handle}'`
            );
          }
