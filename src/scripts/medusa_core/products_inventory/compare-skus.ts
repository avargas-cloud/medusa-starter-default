...
export default async function compareSkus({ container }: ExecArgs) {
  ...
  const medusaLines = medusaContent
    .split("\n")
    .filter((l) => l.trim().length > 0);
  // Skip header if 'SKU' is first line
  const medusaData = medusaLines[0].includes("SKU")
    ? medusaLines.slice(1)
    : medusaLines;
  ...
  // Skip header
  const qbData = qbLines[0].toLowerCase().includes("sku")
    ? qbLines.slice(1)
    : qbLines;
  ...
  logger.info("\n📊 Comparison Result:");
  logger.info(
    `   ❌ SKUs in Medusa but MISSING in QuickBooks file: ${missingInQb.length}`
  );

  if (missingInQb.length > 0) {
    const outputPath = path.resolve(
      process.cwd(),
      "medusa_products_missing_in_qb.csv"
    );
    fs.writeFileSync(
      outputPath,
