...
export default async function propagateProductsToParents({ container }: ExecArgs) {
    ...
    try {
        ...
        console.log(`✅ Loaded ${productCategoryLinks.length} product-category links`)
        ...
        for (const product of products) {
            ...
            for (const categoryId of currentCategoryIdSet) {
                ...
                ancestors.forEach(ancestorId => allRequiredCategoryIds.add(ancestorId))
            }
            ...
            console.log(`   ✅ ${product.title}: +${newCategoriesToAdd.length} parent(s) (${addedNames})`)
        }
