...
async function diagnoseSingleProducts() {
  ...
  try {
    ...
    console.log("=".repeat(80));
    console.log(
      `Total products with 1 variant and NO options: ${affectedProducts.length}`
    );
    ...
    const byType = affectedProducts.reduce(
      (acc, p) => {
    ...
    const managedByAttributes = affectedProducts.filter(
      (p) => p.metadata?.managed_by === "attributes"
    );
    ...
    console.log(
      "      yarn ts-node scripts/fix-single-product-options.ts --dry-run"
    );
    console.log("   3. If looks good, run in LIVE mode:");
    console.log(
      "      yarn ts-node scripts/fix-single-product-options.ts --live"
    );
    console.log("=".repeat(80));
  } catch (error) {
