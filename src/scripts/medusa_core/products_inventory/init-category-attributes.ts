...
async function main() {
    ...
    const categoriesRes = await fetch(`${BACKEND_URL}/admin/product-categories?limit=9999&fields=%2Bmetadata`, {
        headers: {
    ...
    for (const category of product_categories) {
        ...
        try {
            const syncRes = await fetch(`${BACKEND_URL}/admin/product-categories/${category.id}/sync-attributes`, {
                method: "POST",
                headers: {
            ...
            if (syncRes.ok) {
                ...
                console.log(`   ✅ ${result.productCount} products, ${result.attributeCount} attributes`)
                synced++
            } else {
