...
async function main() {
  if (!orderId) {
    ...
    console.log(
      "Example: npx tsx diagnose-order-schema.ts order_01KJ899JZ04BXATV8XD3DA9K5K"
    );
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(
      `   order_item has: order_id=${hasOrderId}, item_id=${hasItemId}, quantity=${hasQuantity}`
    );
    ...
    if (hasOrderId && hasItemId) {
      ...
      if (items.rows.length) {
        ...
        for (const row of items.rows) {
          ...
          console.log(
            `   │    unit_price (order_item): $${priceOnItem.toFixed(2)}  ← ADMIN uses this`
          );
          console.log(
            `   │    unit_price (line_item):  $${priceOnLi.toFixed(2)}   ← Store API uses this`
          );
          console.log(`   │    line total: $${lineTotal.toFixed(2)}`);
          if (priceOnItem === 0)
            console.log(
              `   │    ❌ order_item.unit_price=0 STORED → ADMIN shows $0 items!`
            );
          if (priceOnLi === 0)
            console.log(
              `   │    ❌ order_line_item.unit_price=0 → Store API may be wrong`
            );
        }
        console.log(
          `   └──────────────────────────────────────────────────────`
    ...
    osCols.rows.forEach((r: any) =>
      console.log(`   ${r.column_name} (${r.data_type})`)
    );

    const osSummary = await client
      .query(`SELECT * FROM order_summary WHERE order_id = $1`, [orderId])
      .catch(() => ({ rows: [] }));
    if ((osSummary as any).rows.length) {
      ...
      console.log(
        `   totals (JSONB):\n${JSON.stringify(s.totals, null, 4)
          .split("\n")
          .map((l: string) => "   " + l)
          .join("\n")}`
      );
    ...
    if (!found) {
      ...
      console.log(
        `   Could not find via order_item join. Checking alternate tables...`
      );
      ...
      console.log(
        ...
        sumTables.rows.map((r: any) => r.table_name).join(", ") || "none"
      );
    }
    ...
    if (smColNames.includes("order_id")) {
      ...
      (shipping as any).rows.forEach((r: any) =>
