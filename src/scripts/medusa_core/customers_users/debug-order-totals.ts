...
async function main() {
  ...
  try {
    ...
    for (const order of ordersResult.rows) {
      ...
      console.log(
        `  Status: ${order.status} | Payment: ${order.payment_status} | Fulfillment: ${order.fulfillment_status}`
      );
      console.log(
        `  Currency: ${order.currency_code} | Region: ${order.region_id || "NULL ⚠️"}`
      );
      console.log(
        `  raw_original_order_total: ${total !== null ? total : "NULL"} → $${totalDollars}`
      );
      console.log(
        `  raw_subtotal:             ${order.raw_subtotal ?? "NULL"}`
      );
      console.log(
        `  raw_shipping_total:       ${order.raw_shipping_total ?? "NULL"}`
      );
      console.log(
        `  raw_tax_total:            ${order.raw_tax_total ?? "NULL"}`
      );
      console.log(
        `  raw_discount_total:       ${order.raw_discount_total ?? "NULL"}`
      );
    }
    ...
    console.log("═══════════════════════════════════════════════════════════");
    console.log(
      "💳 PAYMENTS — authorized_amount vs captured_amount vs refunded_amount"
    );
    console.log("═══════════════════════════════════════════════════════════");
    ...
    try {
      ...
      for (const pay of paymentsResult.rows) {
        ...
        console.log(
          `  Order Status: ${pay.order_status} | Payment Status: ${pay.payment_status}`
        );
        console.log(`  amount:           $${(pay.amount / 100).toFixed(2)}`);
        console.log(
          `  authorized_amount:$${pay.authorized_amount !== null ? (pay.authorized_amount / 100).toFixed(2) : "NULL"}`
        );
        console.log(
          `  captured_amount:  $${(pay.captured_amount / 100).toFixed(2)}`
        );
        console.log(
          `  refunded_amount:  $${(pay.refunded_amount / 100).toFixed(2)}`
        );
        ...
        console.log(
          `  → Admin "Order Total" = captured - refunded = $${adminTotal.toFixed(2)}`
        );
      }
    } catch (e: any) {
      console.log(
        "  ⚠️  Tabla payment no accesible directamente, intentando payment_collection..."
      );
      ...
      `);
      console.log(
        "  Tablas de payment disponibles:",
        pcResult.rows.map((r) => r.table_name).join(", ")
      );
    }
    ...
    try {
      ...
      if (refundsResult.rows.length === 0) {
        ...
        for (const ref of refundsResult.rows) {
          ...
          console.log(
            `  Refund amount: $${(ref.amount / 100).toFixed(2)} | Reason: ${ref.reason || "NULL"} | Note: ${ref.note || "NULL"}`
          );
        }
      }
    } catch (e: any) {
    ...
    for (const row of authorizedOrders.rows) {
      ...
      console.log(
        `  raw_original_order_total = ${rawTotal ?? "NULL"} ($${rawTotal ? (rawTotal / 100).toFixed(2) : "?"})`
      );
      console.log(`  Payments encontrados: ${row.payment_count}`);
      console.log(
        `  authorized_amount total: ${row.total_authorized ?? "NULL"}`
      );
      console.log(
        `  captured_amount total:   ${row.total_captured ?? "NULL"} → sin captura = 0`
      );
      console.log(`  refunded_amount total:   ${row.total_refunded ?? "NULL"}`);
      console.log(`  Admin fórmula (captured - refunded): ${adminFormula}`);
      console.log(
        `  ← Por eso el admin muestra "—" (captured=0, refunded=0 → $0 en Authorized)`
      );
    }
