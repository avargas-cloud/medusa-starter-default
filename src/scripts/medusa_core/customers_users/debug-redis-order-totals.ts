...
async function main() {
    ...
    try {
        ...
        const [cursor, keys] = await redis.scan('0', 'MATCH', '*order*', 'COUNT', 100);
        ...
        const [, medusaKeys] = await redis.scan('0', 'MATCH', 'medusa:*', 'COUNT', 100);
        console.log(`   Keys medusa: ${medusaKeys.length}`);
        medusaKeys.slice(0, 10).forEach(k => console.log(`   - ${k}`));

        const [, cacheKeys] = await redis.scan('0', 'MATCH', 'cache:*', 'COUNT', 100);
