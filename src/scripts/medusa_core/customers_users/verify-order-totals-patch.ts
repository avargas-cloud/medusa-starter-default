...
const SERVICE_FILE = path.resolve(
  process.cwd(),
  "node_modules/@medusajs/order/dist/services/order-module-service.js"
);
...
function checkCodeFix(): boolean {
  ...
  const hasBug = content.includes(
    "requiredRelationsForTotals.some((val) => val.startsWith(field))"
  );

  if (hasFix && !hasBug) {
    console.log(
      "✅ Patch is in place: `requires...includes(field)` found, bug line is gone"
    );
    return true;
  } else if (!hasFix && hasBug) {
    console.log(
      "❌ Patch NOT applied: still has buggy `val.startsWith(field)` logic"
    );
    console.log(
      "   Run the patch manually or check patches/@medusajs+order+2.13.0.patch"
    );
    return false;
  } else if (hasFix && hasBug) {
    console.log(
      "⚠️  Both lines found — manual edit may have left old code. Verify the file manually."
    );
    return false;
  } else {
    console.log(
      "⚠️  Neither pattern found — Medusa may have changed their implementation."
    );
    return false;
  }
}
...
async function checkAdminAPI(): Promise<boolean> {
  ...
  if (!ADMIN_TOKEN) {
    ...
    console.log(
      "   2. Open DevTools at http://localhost:9000/app → Network tab"
    );
    console.log(
      "   3. Find any request → copy the 'Authorization: Bearer <TOKEN>' value"
    );
    console.log(
      "   4. Re-run: MEDUSA_ADMIN_TOKEN=<TOKEN> npx dotenv-cli -e .env -- npx tsx src/scripts/verify/verify-order-totals-patch.ts"
    );
  ...
  try {
    ...
    if (!res.ok) {
      console.error(
        `❌ API call failed (${res.status}) — check if backend is running at ${MEDUSA_URL}`
      );
      return false;
    }
    ...
    for (const order of orders) {
      ...
      const onlyShipping =
        Math.abs(itemTotal) < 0.01 &&
        shipping > 0 &&
        order.status !== "canceled";

      const icon = onlyShipping ? "❌" : "✅";
      console.log(
        `${icon} #${order.display_id} [${order.status}]  total=${total?.toFixed?.(2)}  item_total=${itemTotal?.toFixed?.(2)}  shipping=${shipping?.toFixed?.(2)}`
      );
      ...
      items.forEach((i: any) => {
        ...
        console.log(
          `   ${qtyIcon} "${i.title}" qty=${qty} × $${i.unit_price?.toFixed?.(2)} = $${line?.toFixed?.(2)}`
        );
      });
    ...
    console.log(
      "   Is the local backend running? Check: curl http://localhost:9000/health"
    );
    return false;
  }
}
...
async function main() {
  ...
  if (codeFix && apiFix) {
    ...
    if (ADMIN_TOKEN) {
      console.log(
        "   Both code check and API check passed — ready for deployment!"
      );
    } else {
      console.log(
        "   Code check passed. Please also verify manually in the Admin UI:"
      );
      console.log(
        "   → Restart local backend: tmux send-keys -t medusa-dev C-c && sleep 2 && yarn dev"
      );
      console.log(
        "   → Open http://localhost:9000/app/orders and confirm totals show correctly"
      );
    }
  } else {
