...
if (!email) {
    console.error('❌ Usage: npx -y tsx src/scripts/debug-case3-customer.ts email@example.com')
    process.exit(1)
}
    ...
    try {
        ...
        if (Array.isArray(metadata)) {
            ...
            metadata.forEach((item, i) => {
                console.log(`   [${i}]:`, typeof item === 'string' ? 'STRING' : 'OBJECT')
        ...
        console.log('   Has Temp Password:', hasTemporaryPassword ? '✅ YES' : '❌ NO')
        console.log('   Has Expiration:', hasActivationExpires ? '✅ YES' : '❌ NO')
        ...
        if (authResult.rows.length === 0) {
            ...
            console.log('   App Metadata:', JSON.stringify(auth.app_metadata, null, 2))
        }
        ...
        if (!customer.has_account && isLegacy && hasActivationToken && hasTemporaryPassword) {
            ...
            console.log('   Status: Registration initiated but activation data not saved')
            console.log('   Possible Issue: Email sending failed or metadata not saved')
            ...
            console.log('🚨 CRITICAL ISSUE: has_account=true but NO provider_identity')
            console.log('   This means password was never created')
            console.log('   Fix: Account needs to be reset and re-activated')
        } else if (customer.has_account && authResult.rows.length > 0 && providerResult.rows.length > 0) {
