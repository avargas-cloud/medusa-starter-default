...
export default async function verifyCustomerIntegrity({ container }: ExecArgs) {
  ...
  for (const c of customers) {
    // 1. Email Checks
    const isPlaceholder =
      c.email.endsWith("@ecopowertech.com") &&
      c.metadata?.email_is_placeholder === true;
    ...
    if (groups.length > 0) {
      ...
      if (groupNames.includes("Wholesale Customers"))
        stats.assignedToWholesale++;
    ...
    if (qbPriceLevel) {
      ...
      const isWholesaleLevel = ["Wholesale", "Distributor"].includes(
        qbPriceLevel
      );
  ...
  console.log(
    `   Real Emails: ${stats.validEmails} (${((stats.validEmails / count) * 100).toFixed(1)}%)`
  );
  console.log(
    `   Placeholders: ${stats.placeholderEmails} (${((stats.placeholderEmails / count) * 100).toFixed(1)}%)`
  );
  ...
  if (stats.priceLevelMismatches > 0) {
    console.log(
      `   ⚠️ Logic Mismatches (Level vs Group): ${stats.priceLevelMismatches}`
    );
  }

  console.log(`\n📍 Addresses:`);
  console.log(
    `   With Address: ${stats.withAddress} (${((stats.withAddress / count) * 100).toFixed(1)}%)`
  );
