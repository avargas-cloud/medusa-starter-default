...
async function main() {
  ...
  try {
    ...
    for (const row of r.rows) {
      ...
      console.log(
        `   │  order.version (used by Admin to filter items): ${row.order_version}`
      );
      ...
      console.log(
        `   │  order_line_item.raw_unit_price: ${row.oli_raw_unit_price}`
      );

      console.log(`   │`);
      if (row.oi_version !== row.order_version) {
        console.log(
          `   │  ⚠️  order_item version (${row.oi_version}) ≠ order.version (${row.order_version})`
        );
        console.log(
          `   │     Admin filters items WHERE version=${row.order_version} — this version may not match!`
        );
      } else {
        console.log(
          `   │  ✅ order_item.version matches order.version (${row.order_version}) — Admin will read this row`
        );
      }
      const raw = row.oi_raw_unit_price
        ? JSON.parse(row.oi_raw_unit_price)
        : null;
      if (raw?.value === "0" || raw?.value === 0) {
        console.log(
          `   │  ❌ raw_unit_price.value=0 → decorateCartTotals computes $0 for this item → Admin shows shipping-only total`
        );
      } else if (raw) {
        console.log(
          `   │  ✅ raw_unit_price.value=${raw.value} → decorateCartTotals should compute correct item price`
        );
      } else {
        console.log(
          `   │  ℹ️  raw_unit_price is NULL → formatOrder falls back to order_line_item.raw_unit_price`
        );
      }
      console.log(`   └──`);
    }
