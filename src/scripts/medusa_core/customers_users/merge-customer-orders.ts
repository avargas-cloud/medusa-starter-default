...
async function main() {
  ...
  for (const c of customers) {
    const { rows: orders } = await db.query(
      `SELECT count(*) as n FROM "order" WHERE customer_id = $1`,
      [c.id]
    );
    const { rows: authIds } = await db.query(
      `SELECT id, app_metadata FROM auth_identity WHERE app_metadata->>'customer_id' = $1`,
      [c.id]
    );
    console.log(
      `  • ${c.id} | has_account=${c.has_account} | deleted=${c.deleted_at ? "✅ deleted" : "❌ active"} | orders=${orders[0].n} | auth_identities=${authIds.length}`
    );
  ...
  const activeCustomer =
    customers.find((c) => !c.deleted_at && c.has_account) ||
  ...
  for (const o of allLegacyOrders) {
    console.log(
      `   Order #${o.display_id} [${o.status}] — from ${o.customer_id}`
    );
  }
  ...
  for (const legacyId of legacyIds) {
    ...
    if (aiUpdated && aiUpdated > 0) {
      console.log(
        `✅ Re-linked ${aiUpdated} auth_identity record(s) from ${legacyId} → ${activeCustomer.id}`
      );
    }
  }
