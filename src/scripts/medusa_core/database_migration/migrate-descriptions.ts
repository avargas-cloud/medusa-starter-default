...
async function migrateDescriptions(dryRun = true) {
  ...
  try {
    ...
    console.log(
      "🔄 Starting description migration to metadata.long_description..."
    );
    console.log(
    ...
    for (const product of products) {
      ...
      if (product.metadata?.long_description) {
        console.log(
          `⏭️  Skipping "${product.title}" - already has long_description`
        );
    ...
    console.log(
      `Products ${dryRun ? "to migrate" : "migrated"}: ${totalMigrated}`
    );
