...
async function resetSequence() {
  ...
  try {
    ...
    if (!exists) {
      console.error(
        `❌ Sequence '${targetSequence}' not found. Please verify the name from the list above.`
      );
      ...
      if (guess) {
        ...
        await client.query(
          `ALTER SEQUENCE "${guess.relname}" RESTART WITH 1001;`
        );
