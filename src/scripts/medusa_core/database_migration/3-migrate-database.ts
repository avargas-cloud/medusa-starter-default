...
async function main() {
  ...
  try {
    ...
    console.log("\n📋 Verifying mappings...");
    const mappingCount = await db("product_id_migration")
      .count("* as count")
      .first();
    ...
    if (oldFormatCount && Number(oldFormatCount.count) > 0) {
      console.log(
        `❌ ERROR: ${oldFormatCount.count} products still have old format IDs!`
      );
    ...
    if (Number(orphanedVariants.rows[0].count) > 0) {
      console.log(
        `❌ ERROR: ${orphanedVariants.rows[0].count} orphaned variants found!`
      );
    ...
    if (Number(orphanedCategories.rows[0].count) > 0) {
      console.log(
        `❌ ERROR: ${orphanedCategories.rows[0].count} orphaned category links found!`
      );
      process.exit(1);
    }
    ...
    console.log(
      "  3. Run: npx tsx scripts/product-id-migration/4-verify-migration.ts"
    );
    console.log("  4. Re-index MeiliSearch if needed");
    console.log();
    console.log(
      "⚠️  Keep 'product_id_migration' table for 30 days as rollback reference"
    );
    console.log();
  } catch (error) {
