...
async function migratePhase2() {
    ...
    try {
        ...
        try {
            ...
            console.log(`   ✅ ${childrenUpdated.rows.length} children re-linked to new parent ID`)
            ...
            if (childCount !== childrenResult.rows.length) {
                throw new Error(`Expected ${childrenResult.rows.length} children, found ${childCount}`)
            }
