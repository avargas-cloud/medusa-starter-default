...
export default async function ({ container }: { container: any }) {
  ...
  if (!wholesalePriceList) {
    ...
    const [created] = await knex("price_list")
      .insert({
  ...
  const priceSets = await knex("price_set").select("id").limit(500);
  ...
  for (const ps of priceSets) {
    ...
    // Update base price
    await knex("price").where("id", basePrice.id).update({
      price_list_id: null,
      rules_count: 0,
    });
    ...
    for (let i = 1; i < prices.length; i++) {
      await knex("price").where("id", prices[i].id).update({
        price_list_id: wholesalePriceList.id,
        rules_count: 0,
      });
    }
