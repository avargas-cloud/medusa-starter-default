...
async function importCategoryImages() {
  ...
  try {
    ...
    const categoriesWithImages = allWooCategories.filter(
      (cat) => cat.image?.src
    );
    ...
    for (const wooCategory of categoriesWithImages) {
      ...
      if (medusaId) {
        ...
        console.log(
          `⚠️  ${wooCategory.name} (slug: ${wooCategory.slug}) - Not found in Medusa`
        );
      }
    }
    ...
    try {
      ...
      console.log(`\n📊 Summary:`);
      console.log(
        `   WooCommerce categories processed: ${allWooCategories.length}`
      );
      console.log(`   Categories with images: ${categoriesWithImages.length}`);
      ...
      console.log(`\n📝 Next Step:`);
      console.log(
        `   Run Phase 5B to find images in MinIO and assign thumbnails`
      );
      console.log(
        `   Command: npx tsx migration-phase-5b-assign-thumbnails.ts\n`
      );
    } catch (error) {
