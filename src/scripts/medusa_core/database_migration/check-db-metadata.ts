...
async function checkMetadata() {
  const client = new Client({
    connectionString:
      ...
      "postgresql://postgres:postgres@localhost:5432/medusa-docker",
  });
    ...
    for (const row of result.rows) {
      ...
      if (row.metadata) {
        ...
        if (metadata.sorting_config) {
          ...
          console.log(`\n   ✅ SORTING CONFIG:`);
          console.log(
            `      Subcategory Order: ${sc.subcategory_order?.length || 0} items`
          );
          if (sc.subcategory_order && sc.subcategory_order.length > 0) {
            console.log(
              `      IDs: ${JSON.stringify(sc.subcategory_order.slice(0, 3))}...`
            );
          }
          console.log(
            `      Product Order: ${sc.product_order?.length || 0} items`
          );
          if (sc.product_order && sc.product_order.length > 0) {
            console.log(
              `      IDs: ${JSON.stringify(sc.product_order.slice(0, 3))}...`
            );
          }
        } else {
        ...
        if (metadata.filter_config) {
          console.log(
            `   ✅ filter_config: ${metadata.filter_config.active_filters?.length || 0} filters`
          );
        }
