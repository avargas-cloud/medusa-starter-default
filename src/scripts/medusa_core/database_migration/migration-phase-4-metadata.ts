    ...
    try {
      ...
      for (const cat of categoriesWithSorting.rows) {
        ...
        if (sortingConfig.subcategory_order) {
          newSubcategoryOrder = sortingConfig.subcategory_order.map(
            (item: string) => {
              ...
              const newId =
                catHandleToId.get(item) || catHandleToId.get(`pcat_${item}`);
              if (newId) {
        ...
        if (sortingConfig.product_order) {
          newProductOrder = sortingConfig.product_order.map((item: string) => {
            ...
            // Try to find by handle
            const newId =
              prodHandleToId.get(item) || prodHandleToId.get(`prod_${item}`);
            if (newId) {
              productsUpdated++;
              needsUpdate = true;
              return newId;
        ...
        if (needsUpdate) {
          ...
          categoriesUpdated++;
          console.log(
            `   ✅ ${cat.name}: ${newSubcategoryOrder.length} subcats, ${newProductOrder.length} products`
          );
        }
      }
      ...
      console.log(`\n📊 Migration Summary:`);
      console.log(
        `   Categories with updated sorting_config: ${categoriesUpdated}`
      );
