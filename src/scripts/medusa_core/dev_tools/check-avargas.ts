...
async function checkAccount() {
  ...
  try {
    ...
    if (customers.rows.length === 0) {
      console.log(
        `❌ ALERTA: No existe ningún usuario (Customer) con el correo ${email} en la base de datos.`
      );
      return;
    }
    ...
    if (!customer.has_account) {
      console.log(
        `⚠️ ALERTA: La cuenta existe pero NO está activada (has_account=false). Si este usuario inicia sesión con Google, activará su cuenta normalmente usando nuestro nuevo código.`
      );
    }
    ...
    if (auths.rows.length === 0) {
      console.log(
        `⚪ INFO: El usuario ${email} NUNCA ha iniciado sesión con Google (o si lo hizo en el pasado, se borró su sesión).`
      );
      console.log(
        `   Cuando lo haga por primera vez, el sistema creará su identidad y la atará perfecta y automáticamente a su customer_id: ${customer.id}`
      );
    } else {
