...
async function createBackup() {
  ...
  try {
    ...
    const catProducts = await client.query(
      "SELECT * FROM product_category_product"
    );
    backup.tables.product_category_product = catProducts.rows;
    console.log(
      `  ✅ Backed up ${catProducts.rows.length} category-product links`
    );
    ...
    console.log(
      `  ✅ Backed up ${products.rows.length} products with metadata`
    );
    ...
    console.log(
      `📊 Backup size: ${(Buffer.byteLength(JSON.stringify(backup)) / 1024 / 1024).toFixed(2)} MB\n`
    );
    ...
    console.log(
      `   Category-Product Links: ${backup.tables.product_category_product.length}`
    );
    console.log(
      `   Products with Metadata: ${backup.tables.product_metadata.length}`
    );

    // Count categories by type
    const leafCategories = backup.tables.product_category.filter(
      (c: any) =>
        !backup.tables.product_category.some(
          (child: any) => child.parent_category_id === c.id
        )
    );
