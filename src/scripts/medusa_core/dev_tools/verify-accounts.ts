export default async function queryAccounts({ container }: { container: any }) {
  ...
  for (const email of emails) {
    ...
    for (const ai of authIdentities) {
      console.log(
        `  - AI ID: ${ai.id} | Provider: ${ai.provider_identities ? ai.provider_identities.map((p: any) => p.provider).join(",") : "none"} | app_metadata:`,
        JSON.stringify(ai.app_metadata)
      );
      ...
      const providerIdentities = await sql`
          ...
          `;
      console.log(
        `    🔗 Attached Provider Identities (Count: ${providerIdentities.length}):`
      );
      for (const pi of providerIdentities) {
        console.log(
          `      - PI ID: ${pi.id} | Provider: ${pi.provider} | Email in metadata: ${pi.user_metadata?.email}`
        );
      }
    }
    ...
    for (const pi of floatingPIs) {
      ...
      if (
        ud.includes(email) &&
        !authIdentities.find((a) => a.id === pi.auth_identity_id)
      ) {
        if (floatingCount === 0)
          console.log(`\n⚠️ ORPHANED/FLOATING Google Identities for ${email}:`);
        console.log(
          `  - PI ID: ${pi.id} | AuthIdentity ID: ${pi.auth_identity_id}`
        );
        floatingCount++;
      }
    }
