...
async function testDefaultAddresses() {
  ...
  try {
    ...
    addresses.forEach((addr, i) => {
      ...
      console.log(
        `      Default Billing: ${addr.is_default_billing ? "✅" : "❌"}`
      );
      console.log(
        `      Default Shipping: ${addr.is_default_shipping ? "✅" : "❌"}`
      );
    });
    ...
    console.log(
      `   Address 1 is_default_billing: ${addr1After?.is_default_billing ? "✅" : "❌"}`
    );
    console.log(
      `   Address 2 is_default_billing: ${addr2After?.is_default_billing ? "✅" : "❌"}`
    );
    ...
    console.log(
      `     - Billing: ${addr1Final?.is_default_billing ? "✅" : "❌"}`
    );
    console.log(
      `     - Shipping: ${addr1Final?.is_default_shipping ? "✅" : "❌"}`
    );
    console.log("   Address 2:");
    console.log(
      `     - Billing: ${addr2Final?.is_default_billing ? "✅" : "❌"}`
    );
    console.log(
      `     - Shipping: ${addr2Final?.is_default_shipping ? "✅" : "❌"}`
    );

    if (
      !addr1Final?.is_default_billing &&
      addr1Final?.is_default_shipping &&
      addr2Final?.is_default_billing &&
      !addr2Final?.is_default_shipping
    ) {
      console.log(
        "   ✅ PASS: Billing and shipping can be different addresses"
      );
    } else {
