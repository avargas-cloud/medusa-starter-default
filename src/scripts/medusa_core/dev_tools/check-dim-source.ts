 ...
 */
export default async function checkDimSource({
  container,
}: {
  container: any;
}) {
  ...
  console.log(
    `   weight=${variant?.weight}  height=${variant?.height}  width=${variant?.width}  length=${variant?.length}`
  );

  // 2. product metadata (if any shipping dims stored there)
  const productMeta = await knex("product")
    .where("id", variant?.product_id)
    .select("metadata")
    .first();
  console.log("\n📋 product.metadata (shipping-related keys):");
  const meta = productMeta?.metadata || {};
  const shippingKeys = Object.keys(meta).filter((k) =>
    /weight|height|width|length|dim|ship/i.test(k)
  );
  ...
  if (attrs.length > 0) {
    attrs.forEach((a: any) =>
      console.log(`   ${JSON.stringify(a.attribute_key)}: ${a.attribute_value}`)
    );
  } else {
    console.log("   (none found)");
  }
  ...
  console.log(
    `   weight=${inv?.weight}  height=${inv?.height}  width=${inv?.width}  length=${inv?.length}`
  );
