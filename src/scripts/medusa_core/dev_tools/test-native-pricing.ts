...
export default async function testNativePricing({
  container,
}: {
  ...
  try {
    ...
    if (!priceLists.length)
      throw new Error("Could not find Wholesale price list");
    const priceList = priceLists[0];
    console.log(
      `✅ Wholesale Price List ID: ${priceList.id} (${priceList.title})`
    );

    console.log("\n2. Finding a Variant with a Wholesale Price...");
    const priceRes = await pool.query(
      `SELECT price_set_id, amount FROM price WHERE price_list_id = $1 LIMIT 1`,
      [priceList.id]
    );
    if (priceRes.rows.length === 0)
      throw new Error("No prices found in wholesale list");
    ...
    console.log(
      `✅ Expected Wholesale Price: ${expectedWholesaleAmount} (Raw DB)`
    );
    ...
    console.log(
      "Rules attached to this Price List:",
      JSON.stringify(priceList.rules, null, 2)
    );

    let targetGroupIds: string[] = [];
    if (priceList.rules) {
      const groupRule = priceList.rules.find(
        (r: any) => r.attribute === "customer_group_id"
      );
      if (groupRule?.value) {
        targetGroupIds = Array.isArray(groupRule.value)
          ? groupRule.value
          : [groupRule.value];
      }
    }
    ...
    if (targetGroupIds.length === 0) {
      console.log(
        "⚠️ No customer_group_id rule found on the price list! Using a fallback group."
      );
      const groupRes = await pool.query(
        `SELECT id FROM customer_group LIMIT 1`
      );
    ...
    if (Number(calculatedWholesale) === Number(expectedWholesaleAmount)) {
      ...
      console.log(
        "Medusa dynamically calculated the wholesale price based on the Rules Engine context."
      );
    } else {
