 ...
 * - Fast checkout (AuthNet token + POST /store/carts/${cartId}/complete)
...
const AUTHNET_CLIENT_KEY =
  "5A5NMkXMUACqG6z9jkhp2AvTw6E72a7P6q9f53BWbvg779LNWtp8y4eJ4BdpqtwC";
...
function section(n: number, t: string) {
  console.log(`\n── STEP ${n}: ${t} ${"─".repeat(Math.max(0, 50 - t.length))}`);
}
function dollars(v: any): string {
...
async function getOpaqueData() {
  ...
  if (data?.messages?.resultCode !== "Ok")
    throw new Error(`AuthNet: ${data?.messages?.message?.[0]?.text}`);
  ...
  try {
    ...
    for (const product of products ?? []) {
      ...
      for (const variant of product.variants ?? []) {
        const addRes = await fetch(
          `${MEDUSA_URL}/store/carts/${cart.id}/line-items`,
          {
            method: "POST",
            headers: HEADERS,
        ...
        if (addRes.ok) {
          ...
          pass(`Added: "${product.title}" × 2 (${variant.id})`);
          break;
        }
      }
    }
    ...
    for (const item of items) {
      const price = item.unit_price ?? 0,
        qty = item.quantity ?? 0;
      if (price === 0) {
        fail(`unit_price=0 for "${item.title}"`);
        failed = true;
      } else
        pass(
          `"${item.title}" x${qty} @ ${dollars(price)} = ${dollars(price * qty)}`
        );
    }
    info(
      `cart.item_subtotal=${dollars(cartCheck?.item_subtotal)} | cart.subtotal=${dollars(cartCheck?.subtotal)} | cart.total=${dollars(cartCheck?.total)}`
    );
    ...
    pass(
      `Token: ${opaqueData.dataDescriptor} | ${opaqueData.dataValue.slice(0, 30)}...`
    );

    // ── STEP 6: Fire fast-checkout ────────────────────────────────────────
    section(6, "POST /store/carts/${cartId}/complete");
    ...
    if (!response.ok) {
      fail(`Checkout failed: ${data?.error}`);
      failed = true;
      return;
    }
    ...
    if (!storeOrder) {
      ...
      console.log(
        `   │  order.total             = ${dollars(storeOrder.total)}`
      );
      console.log(
        `   │  order.subtotal          = ${dollars(storeOrder.subtotal)}`
      );
      console.log(
        `   │  order.item_subtotal     = ${dollars(storeOrder.item_subtotal)}`
      );
      console.log(
        `   │  order.shipping_subtotal = ${dollars(storeOrder.shipping_subtotal)}`
      );
      console.log(
        `   │  order.tax_total         = ${dollars(storeOrder.tax_total)}`
      );
      console.log(
        `   │  order.discount_total    = ${dollars(storeOrder.discount_total)}`
      );
      console.log(`   └──────────────────────────────────────────────────────`);
      ...
      for (const item of storeItems) {
        ...
        console.log(
          `      qty=${qty} | unit_price=${dollars(price)} | line=${dollars(price * qty)}`
        );
        if (price === 0 || qty === 0) {
          fail(`unit_price or qty is 0!`);
          failed = true;
        }
      }
    }
    ...
    try {
      ...
      if (!orderRow.rows.length) {
        ...
        console.log(
          `   │  total          = ${dollars(o.total)}   ← admin list shows this`
        );
        ...
        const storedShipping = Number(
          o.shipping_subtotal ?? o.shipping_total ?? 0
        );
        if (storedTotal > 0 && storedTotal > storedShipping + 5) {
          pass(
            `DB total ${dollars(storedTotal)} > shipping ${dollars(storedShipping)} — ITEMS PRICED IN DB ✅`
          );
        } else {
          fail(
            `DB total ${dollars(storedTotal)} ≈ shipping only ${dollars(storedShipping)} — BUG: items not priced in stored total ❌`
          );
          failed = true;
        }
      }
      ...
      if (!lineItems.rows.length) {
        ...
        for (const li of lineItems.rows) {
          ...
          if (!li.unit_price || Number(li.unit_price) === 0) {
            fail(`unit_price=0 in DB for this item!`);
            failed = true;
          }
        }
      }
      ...
      if (!orderItemRows.rows.length) {
        ...
        for (const oi of orderItemRows.rows) {
          ...
          console.log(
            `   │    quantity:        ${oi.quantity}  raw_quantity.value="${rawQtyVal}"  ${qtyOk ? "✅" : "❌ ZERO — Admin will show $0 subtotal!"}`
          );
          console.log(
            `   │    unit_price:      ${dollars(oi.unit_price)}  raw_unit_price.value="${rawPriceVal}"  ${priceOk ? "✅" : "❌ ZERO"}`
          );
          if (!qtyOk) {
            fail(
              `raw_quantity.value="0" — decorateCartTotals will compute qty=0!`
            );
            failed = true;
          }
          if (!priceOk) {
            fail(`raw_unit_price.value="0" — items not priced!`);
            failed = true;
          }
        }
        console.log(
