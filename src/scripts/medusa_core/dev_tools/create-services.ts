...
export default async function createServices({ container }: any) {
  ...
  try {
    ...
    if (!serviceType) {
      throw new Error(
        "Product Type 'Service' not found. Please run restructure-product-types.ts first."
      );
    }
    ...
    for (const service of services) {
      try {
        ...
        if (existingProducts.length > 0) {
          logger.info(
            `  ⚠️  Service "${service.title}" already exists (Handle: ${service.handle})`
          );
          continue;
        }
        ...
        logger.error(
          `  ✗ Failed to create "${service.title}": ${error.message}`
        );
      }
    }
