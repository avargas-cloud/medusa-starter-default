  ...
  try {
    ...
    if (tableExists) {
      ...
      const { confirm } = await import("readline/promises")
        ...
        .then((rl) =>
          rl.question("Drop and recreate? (yes/no): ").then((answer) => {
            rl.close();
    ...
    if (products.length === 0) {
      console.log(
        "ℹ️  No products need migration (all IDs are already ULID format)"
      );
      await db.destroy();
      return;
    }
    ...
    const outputPath = path.join(
      process.cwd(),
      "scripts/product-id-migration/mappings.json"
    );
    ...
    console.log(
      "  2. Run: node scripts/product-id-migration/2-update-metadata.ts"
    );
    console.log();
  } catch (error) {
