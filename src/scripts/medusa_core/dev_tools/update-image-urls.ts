...
async function updateImageURLs(dryRun = true) {
  ...
  try {
    ...
    console.log(
      `Mode: ${dryRun ? "DRY RUN (preview only)" : "LIVE (will update)"}`
    );
    ...
    console.log(
      `  ${dryRun ? "Would update" : "Updated"} ${thumbnailResult} thumbnails`
    );
    ...
    console.log(
      `  ${dryRun ? "Would update" : "Updated"} ${imageResult} images`
    );
    ...
    console.log(
      `  ${dryRun ? "Would update" : "Updated"} ${categoryResult} category images`
    );
    ...
    console.log("=".repeat(60));
    console.log(
      `Total URLs ${dryRun ? "to update" : "updated"}: ${totalUpdated}`
    );
    console.log("=".repeat(60));
...
async function updateProductThumbnails(
  client: Client,
  dryRun: boolean
): Promise<number> {
  const findQuery = `
