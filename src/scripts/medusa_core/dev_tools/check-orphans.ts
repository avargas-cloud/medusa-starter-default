...
async function checkOrphans() {
    ...
    `, [email]);
        ...
        if (customers.rows.length === 1) {
            console.log(`✅ Todo perfecto. Solo hay 1 cliente con este correo en la DB:\n`, customers.rows[0]);
        } else {
            console.log(`⚠️ ALERTA: Hay ${customers.rows.length} clientes con este correo:\n`, customers.rows);
        }
        console.log('--------------------------------------------------\n');
    ...
    `, [`%${email}%`]);
        ...
        if (auths.rows.length === 0) {
          ...
          WHERE app_metadata::text LIKE $1;
        `, [`%${customers.rows[0].id}%`]);
                console.log(`Se encontró 1 identidad vinculada al customer_id:\n`, authsById.rows);
            } else {
                console.log('Ninguna identidad de Auth encontrada.');
            }
        } else if (auths.rows.length === 1) {
            console.log(`✅ Todo en orden. Solo hay 1 identidad OAuth:\n`, auths.rows[0]);
        } else {
            console.log(`⚠️ ALERTA: Hay ${auths.rows.length} identidades Auth (posibles duplicados/huérfanos):\n`, auths.rows);
        }
