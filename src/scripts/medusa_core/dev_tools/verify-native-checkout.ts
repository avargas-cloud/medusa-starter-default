...
async function verifyNativeCheckout() {
    ...
    try {
        ...
        const optsRes = await fetch(`${MEDUSA_URL}/store/shipping-options?cart_id=${cartId}`, { headers })
        ...
        console.log('Sending: { cartId, email, shippingAddress, shippingMethodId, opaqueData }')
        ...
        const checkoutV2Res = await fetch(`${MEDUSA_URL}/store/checkout-v2`, {
            ...
            body: JSON.stringify({
                ...
                opaqueData: { dataDescriptor: "COMMON.ACCEPT.INAPP.PAYMENT", dataValue: "dummy-token-for-test" }
            })
        })

        const resText = await checkoutV2Res.text()
        let completionData;
        try { completionData = JSON.parse(resText); } catch { }

        if (!checkoutV2Res.ok) {
            console.error(`❌ Checkout V2 Failed (${checkoutV2Res.status}):`, completionData?.error || resText)
        ...
        const summaryRes = await client.query(`SELECT jsonb_pretty(totals) AS totals FROM order_summary WHERE order_id = $1`, [completionData.orderId])
        if (summaryRes.rows.length > 0) {
            console.log(`\n📦 Order Summary Totals:\n`, summaryRes.rows[0].totals)
        }

        const itemsRes = await client.query(`SELECT * FROM order_item WHERE order_id = $1`, [completionData.orderId])
        ...
        if (itemIds.length > 0) {
            const lineItemsRes = await client.query(`SELECT * FROM order_line_item WHERE id = ANY($1)`, [itemIds])
