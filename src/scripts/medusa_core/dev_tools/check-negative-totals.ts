...
async function checkNegativeOrders() {
  ...
  try {
    ...
    console.log(
      "Checking for any negative totals assigned to canceled orders...\n"
    );
    ...
    for (const order of orders) {
      ...
      if (Number(currentTotal) < 0) {
        console.log(
          `❌ Order #${order.display_id} | ID: ${order.id} | Negative Total: $${currentTotal}`
        );
        hasNegatives = true;
      } else {
    ...
    if (hasNegatives) {
      ...
      console.log(
        "💡 Note: If you see negative totals in the Admin UI, it is a UI calculation bug, not a database error."
      );
    }
  } catch (error) {
