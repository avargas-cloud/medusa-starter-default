...
export default async function verifyLinks({ container }: ExecArgs) {
  ...
  const targetProduct =
    products.find((p) => p.metadata?.wc_attributes) || products[0];
  ...
  console.log(
    `📋 Metadata 'wc_attributes': ${wcAttributes?.length || 0} items found.`
  );
  ...
  if (attributeLinks.length > 0) {
    console.log(
      "   ✅ Links exist! Sample:",
      JSON.stringify(attributeLinks[0], null, 2)
    );
  } else {
  ...
  if (wcAttributes?.length > 0 && attributeLinks.length === 0) {
    ...
    console.log(
      "   👉 RUN MIGRATION: npx medusa exec ./src/scripts/migrate-product-attributes.ts"
    );
