...
async function investigateBillingAddress() {
  ...
  try {
    ...
    if (billingColumns.length === 0) {
      console.log(
        '   ❌ No se encontraron columnas con "billing" o "default_address"'
      );
    } else {
    ...
    addressColumns.forEach((col, i) => {
      console.log(
        `${i + 1}. ${col.column_name} (${col.data_type}) ${col.is_nullable === "YES" ? "NULL" : "NOT NULL"}`
      );
    });
    ...
    if (testCustomer) {
      ...
      console.log(
        `Direct columns:`,
        Object.keys(testCustomer).filter((k) => k !== "addresses_json")
      );
