...
async function verifyLintingSystem(): Promise<void> {
    ...
    runTest(
        ".prettierignore existe",
        () => fs.existsSync(path.join(process.cwd(), ".prettierignore"))
    );

    runTest(
        "tsconfig.json tiene strict mode",
        () => {
    ...
    if (failedTests === 0) {
        console.log("🎉 ¡Todos los tests pasaron! El sistema de linting está configurado correctamente.");
        process.exit(0);
    } else {
