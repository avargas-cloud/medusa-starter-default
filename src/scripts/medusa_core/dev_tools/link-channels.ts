...
export default async function linkQbProductsToChannels({
  container,
}: ExecArgs) {
  ...
  if (!fs.existsSync(resultsFile)) {
    logger.error(
      "❌ Results file not found from import step. Cannot determine which products to link."
    );
    return;
  }
  ...
  logger.info("\n" + "=".repeat(65));
  logger.info(
    `✅ SUCCESS: Linked ${productIds.length} products to ${channelIds.length} channels.`
  );
  logger.info("=".repeat(65));
}
