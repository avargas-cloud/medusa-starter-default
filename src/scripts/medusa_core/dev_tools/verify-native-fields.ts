...
async function verifyNativeFields() {
  ...
  try {
    ...
    const hasBillingAddressId = columns.some(
      (c) => c.column_name === "billing_address_id"
    );
    ...
    if (!customer) {
      ...
      console.log(
        `   metadata.default_shipping_address_id: ${customer.metadata?.default_shipping_address_id || "NULL"}`
      );
    }
