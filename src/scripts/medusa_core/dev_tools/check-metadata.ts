...
async function checkMetadata() {
  try {
    ...
    const ledCategories = allCategories.filter(
      (c: any) =>
    ...
    for (const cat of ledCategories) {
      ...
      if (cat.metadata && Object.keys(cat.metadata).length > 0) {
        ...
        if (cat.metadata.sorting_config) {
          ...
          console.log(
            `      - subcategory_order: ${sc.subcategory_order?.length || 0} items`
          );
          console.log(
            `      - product_order: ${sc.product_order?.length || 0} items`
          );
        } else {
        ...
        if (cat.metadata.filter_config) {
          console.log(
            `   ✅ filter_config: ${cat.metadata.filter_config.active_filters?.length || 0} filters`
          );
        }
