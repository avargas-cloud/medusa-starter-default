...
async function main() {
  ...
  try {
    ...
    const mappings = await db("product_id_migration").select(
      "old_id",
      "new_id"
    );
    ...
    console.log(
      `📦 Found ${categories.length} categories with sorting config\n`
    );
    ...
    for (const category of categories) {
      ...
      const newProductOrder = productOrder.map((oldId: string) => {
        ...
        if (!newId) {
          console.log(
            `⚠️  Warning: No mapping found for ${oldId} in category "${category.name}"`
          );
      ...
      const hasChanges = productOrder.some(
        (id: string, idx: number) => id !== newProductOrder[idx]
      );
      ...
      console.log(
        `   Old IDs: ${productOrder.slice(0, 2).join(", ")}${productOrder.length > 2 ? "..." : ""}`
      );
      console.log(
        `   New IDs: ${newProductOrder.slice(0, 2).join(", ")}${newProductOrder.length > 2 ? "..." : ""}\n`
      );
    }
    ...
    const logPath = path.join(
      process.cwd(),
      "scripts/product-id-migration/metadata-updates.json"
    );
    fs.writeFileSync(logPath, JSON.stringify(updates, null, 2), "utf-8");
    ...
    console.log(
      "  2. Verify categories in Admin UI still show correct sorting"
    );
    console.log(
      "  3. Run: npx tsx scripts/product-id-migration/3-migrate-database.ts"
    );
    console.log();
  } catch (error) {
