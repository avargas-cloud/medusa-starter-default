...
async function verifyNativeDefaults() {
    ...
    try {
        ...
        addresses.forEach((addr, i) => {
            ...
            console.log(`      is_default_billing: ${addr.is_default_billing ? '✅ TRUE' : '❌ false'}`)
            console.log(`      is_default_shipping: ${addr.is_default_shipping ? '✅ TRUE' : '❌ false'}`)
            console.log()
        })

        // 3. Verificar solo hay 1 default de cada tipo
        const billingDefaults = addresses.filter(a => a.is_default_billing === true)
        const shippingDefaults = addresses.filter(a => a.is_default_shipping === true)

        console.log('3️⃣  Validación de unicidad:\n')
        console.log(`   Default billing addresses: ${billingDefaults.length} ${billingDefaults.length === 1 ? '✅' : billingDefaults.length === 0 ? '⚠️  (ninguno)' : '❌ (más de 1!)'}`)
        console.log(`   Default shipping addresses: ${shippingDefaults.length} ${shippingDefaults.length === 1 ? '✅' : shippingDefaults.length === 0 ? '⚠️  (ninguno)' : '❌ (más de 1!)'}`)
