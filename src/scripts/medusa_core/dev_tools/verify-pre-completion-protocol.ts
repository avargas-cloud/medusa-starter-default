...
async function verifyPreCompletionProtocol(): Promise<void> {
  ...
  runTest("Agent-Policy.md contiene sección 10", () => {
    const content = fs.readFileSync(".agent/Agent-Policy.md", "utf-8");
    return content.includes("## 10. Protocolo de Verificación Pre-Completitud");
  });

  runTest("Protocolo menciona 'OBLIGATORIO'", () => {
    const content = fs.readFileSync(".agent/Agent-Policy.md", "utf-8");
    return content.includes("OBLIGATORIO");
  });

  runTest("Protocolo incluye checklist de verificación", () => {
    const content = fs.readFileSync(".agent/Agent-Policy.md", "utf-8");
    return (
  ...
  runTest("verify-before-completion.md existe", () =>
    fs.existsSync(".agent/workflows/verify-before-completion.md")
  );

  runTest("Workflow tiene frontmatter YAML", () => {
  ...
  runTest("Workflow incluye steps del protocolo", () => {
    const content = fs.readFileSync(
      ".agent/workflows/verify-before-completion.md",
      "utf-8"
    );
  ...
  runTest("scripts/ directorio existe", () => fs.existsSync("scripts"));

  runTest("pre-completion-check.mjs existe", () =>
    fs.existsSync("scripts/pre-completion-check.mjs")
  );

  runTest("Script tiene shebang correcto", () => {
    const content = fs.readFileSync(
  ...
  runTest("Script ejecuta checks de TypeScript", () => {
    const content = fs.readFileSync(
      "scripts/pre-completion-check.mjs",
      "utf-8"
    );
  ...
  runTest("Script apunta a pre-completion-check.mjs", () => {
