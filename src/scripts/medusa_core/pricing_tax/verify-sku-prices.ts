...
export default async function verifySkuPrices({ container }: any) {
    ...
    try {
        ...
        variant.prices.forEach((price: any, i: number) => {
            ...
            logger.info(`\n   [${i + 1}] ${label}`)
            logger.info(`       Amount: $${price.amount.toFixed(2)} ${price.currency_code}`)
            logger.info(`       Price List ID: ${price.price_list_id || "null (default)"}`)
        ...
        if (defaultPrice && wholesalePrice) {
            const discount = ((defaultPrice.amount - wholesalePrice.amount) / defaultPrice.amount * 100).toFixed(2)
