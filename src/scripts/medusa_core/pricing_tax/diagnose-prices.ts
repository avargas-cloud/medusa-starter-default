...
export default async function diagnosePrices({ container }: any) {
  ...
  try {
    ...
    const variantPriceSetCount = await knex("product_variant_price_set").count(
      "* as count"
    );

    logger.info(
      `📊 Total variant-price_set links: ${variantPriceSetCount[0].count}`
    );
    ...
    if (prices.length === 0) {
      ...
      for (const variantId of variantIds.slice(0, 2)) {
        ...
        logger.info(
          `   ✅ Variant ${variantId}: price_set_id = ${variantPriceSet.price_set_id}`
        );
        ...
        pricesForSet.forEach((p: any) => {
          logger.info(
            `         - $${p.amount} ${p.currency_code} ${p.price_list_id ? "(price list)" : "(default)"}`
          );
        });
      }
    } else {
      ...
      prices.forEach((p: any) => {
        logger.info(
          `   - Variant ${p.variant_id}: $${p.amount} ${p.currency_code}`
        );
      });
    }
