...
export default async function ({ container }: { container: any }) {
  ...
  console.log(
    "🧹 Cleaning up any existing wholesale prices for legacy (LEG) items..."
  );
  const deletedCount = await knex("price")
    .where("price_list_id", priceList.id)
    .whereIn("price_set_id", (builder: any) => {
      builder
        .select("product_variant_price_set.price_set_id")
  ...
  for (let i = 0; i < basePrices.length; i += BATCH_SIZE) {
    ...
    await Promise.all(
      batch.map(async (basePrice: any) => {
        const wholesaleAmount = (parseFloat(basePrice.amount) * 0.925).toFixed(
          2
        );
