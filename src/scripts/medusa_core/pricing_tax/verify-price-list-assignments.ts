...
export default async function verifyPriceLists({ container }: any) {
  ...
  try {
    ...
    const wholesaleMetadata = allCustomers.filter(
      (c: any) => c.metadata?.qb_price_level === "Wholesale"
    );
    const retailMetadata = allCustomers.filter(
      (c: any) =>
    ...
    const wholesaleNotInGroup = wholesaleMetadata.filter(
      (c: any) => !c.groups?.some((g: any) => g.name === "Wholesale")
    );
    ...
    logger.info(
      `  With qb_price_level='Wholesale': ${wholesaleMetadata.length}`
    );
    logger.info(
      `  With qb_price_level='Retail' or null: ${retailMetadata.length}`
    );
    logger.info("─".repeat(70));
    logger.info(
    ...
    logger.info(
      `❌ Wholesale metadata + NOT in any group: ${wholesaleNotInGroup.length}`
    );
    logger.info("─".repeat(70));
