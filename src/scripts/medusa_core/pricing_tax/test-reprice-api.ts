...
async function run() {
    ...
    try {
        ...
        console.log(`✅ Variant: ${variant_id} | Expected wholesale: $${wholesale_price}`)
        ...
        if (!nativeAddRes.ok) throw new Error("Guest add failed: " + JSON.stringify(await nativeAddRes.json()))
        ...
        if (!repriceRes.ok) throw new Error("Reprice failed: " + JSON.stringify(repriceData))
        ...
        console.log(`\n--- VERDICT ---`)
        if (Number(finalItem?.unit_price) === Number(wholesale_price) && !finalItem?.is_custom_price) {
            console.log(`🎉 PASS: reprice works natively! Wholesale price applied, is_custom_price=false!`)
        } else if (finalItem?.unit_price === guestItem?.unit_price) {
            console.log(`⚠️  Price unchanged after reprice. updateCartWorkflow may not trigger repricing.`)
            console.log(`   This means we may need a different approach.`)
        } else {
            console.log(`❌ FAIL: unexpected result. is_custom_price=${finalItem?.is_custom_price}, price=${finalItem?.unit_price}`)
        }
