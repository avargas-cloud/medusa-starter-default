...
export default async function testWholesale3Skus({ container }: ExecArgs) {
  ...
  const targetVariants = variants.filter((v: any) =>
    TARGET_SKUS.includes(v.sku)
  );
  ...
  if (existingInPriceList.length > 0) {
    await pricingService.deletePrices(
      existingInPriceList.map((p: any) => p.id)
    );
  ...
  for (const p of verified) {
    logger.info(
      `   ✅ price_id: ${p.id} | $${p.amount} USD | price_list_id: ${p.price_list_id}`
    );
  }
