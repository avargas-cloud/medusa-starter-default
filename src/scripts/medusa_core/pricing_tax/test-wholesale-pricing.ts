...
async function testWholesalePricing() {
    ...
    try {
        ...
        pricesQuery.rows.forEach(p => {
            ...
            console.log(`   - $${parseFloat(p.amount).toFixed(2)} ${p.currency_code.toUpperCase()} (${type})`);
        });
        ...
        if (customerGroupIds.length > 0) {
            ...
            if (matchingRules.rows.length > 0) {
                console.log(`   ✅ Found ${matchingRules.rows.length} matching price list(s):`);
                ...
                if (applicablePrices.length > 0) {
                    ...
                    console.log(`   WILL SEE: Base price $${basePrices.length > 0 ? parseFloat(basePrices[0].amount).toFixed(2) : 'N/A'}`);
                }
            } else {
        ...
        console.log(`✅ Groups: ${customerGroupIds.length > 0 ? groupsQuery.rows.map(g => g.group_name).join(', ') : 'None'}`);
        console.log(`✅ Product: ${productId}`);
        console.log(`✅ Variants: ${variantsQuery.rows.length}`);
        console.log(`✅ Base Prices: ${basePrices.length}`);
        console.log(`✅ Price List Prices: ${listPrices.length}`);
        ...
        if (customerGroupIds.length === 0) {
            ...
            console.log('❌ ISSUE: Price lists have no rules linking them to customer groups!');
            ...
            if (matchingRules.rows[0].count === '0') {
                console.log('❌ ISSUE: Customer groups don\'t match any price list rules!');
                console.log(`   Customer groups: ${customerGroupIds.join(', ')}`);
