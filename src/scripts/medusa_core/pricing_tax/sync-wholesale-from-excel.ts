...
export default async function syncWholesaleFromExcel({
  ...
  try {
    log(
      `📊 Starting Wholesale Price Sync from Excel${dryRun ? " [DRY RUN — no changes will be written]" : ""}`
    );
    ...
    log(
      `Updated:                 ${stats.updated}${dryRun ? " (would update)" : ""}`
    );
    log(
      `Created:                 ${stats.created}${dryRun ? " (would create)" : ""}`
    );
    log(
      `Deleted ($0 → retail):   ${stats.deleted}${dryRun ? " (would delete)" : ""}`
    );
