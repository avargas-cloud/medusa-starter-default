...
export default async function ({ container }: ExecArgs) {
  ...
  console.log(
    "🔍 Checking for variants with duplicate default retail prices...\n"
  );
  ...
  for (const v of variants) {
    ...
    const retailPrices = (v.price_set.prices as any[]).filter(
      (p: any) => !p.price_list_id
    );
  ...
  if (withDuplicates.length === 0) {
    console.log(
      "✅ All clean — no variants with duplicate default retail prices!"
    );
  } else {
    console.log(
      `⚠️  Found ${withDuplicates.length} variants with duplicate retail prices:\n`
    );
    withDuplicates.forEach((v) => {
      console.log(
        `  • ${v.sku}: ${v.count} prices — $${v.amounts.map((a) => a.toFixed(2)).join(", $")}`
      );
    });
  }
