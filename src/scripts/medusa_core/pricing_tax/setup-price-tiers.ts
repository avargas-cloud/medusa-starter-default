  ...
  try {
    ...
    const groupsToCreate = [
      ...
      },
      {
        ...
        metadata: {
          ...
          description: "Trade/contractor customers (10% discount)",
        },
      },
    ];
    ...
    for (const groupData of groupsToCreate) {
      ...
      if (existing.length > 0) {
        logger.info(
          `  ✓ Customer Group "${groupData.name}" already exists (ID: ${existing[0].id})`
        );
        ...
        logger.info(
          `  ✓ Created Customer Group "${groupData.name}" (ID: ${created.id})`
        );
        report.customerGroups.push(created);
      }
    }

    const wholesaleGroup = report.customerGroups.find(
      (g) => g.name === "Wholesale"
    );
    ...
    if (existingPriceLists.length > 0) {
      ...
      logger.info(
        `  ✓ Wholesale Price List already exists (ID: ${wholesalePriceList.id})`
      );
    } else {
    ...
    for (let i = 0; i < variants.length; i += batchSize) {
      ...
      try {
        ...
        logger.info(
          `  Created prices for ${report.variantsProcessed}/${variants.length} variants...`
        );
      } catch (error: any) {
