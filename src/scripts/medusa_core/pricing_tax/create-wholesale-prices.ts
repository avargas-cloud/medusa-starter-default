...
export default async function createWholesalePrices({ container }: any) {
  ...
  try {
    ...
    for (const priceSet of priceSets) {
      ...
      const defaultPrice = priceSet.prices?.find(
      ...
      const existingWholesale = priceSet.prices?.find(
        (p: any) => p.price_list_id === wholesalePriceList.id
      );
    ...
    if (created > 0) {
      ...
      if (sampleSet) {
        ...
        const wholesaleP = sampleSet.prices?.find(
          (p: any) => p.price_list_id === wholesalePriceList.id
        );

        if (defaultP && wholesaleP) {
          const discount = (
            ((defaultP.amount - wholesaleP.amount) / defaultP.amount) *
            100
          ).toFixed(1);
