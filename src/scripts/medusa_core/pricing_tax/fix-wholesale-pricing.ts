...
export default async function fixWholesalePricing({ container }: ExecArgs) {
  ...
  for (const priceSet of priceSets) {
    for (const price of priceSet.prices ?? []) {
      ...
      if (hasRules) {
        logger.info(
          `  ⚠️  Price ${price.id} has ${rules.length} rules - marking for cleanup`
        );
  ...
  if (pricesToDelete.length > 0) {
    logger.info(
      `  🗑️  Deleting ${pricesToDelete.length} prices with broken rules...`
    );
    ...
    logger.info(
      `  ✅ Reset ${pricesToDelete.length} prices to base prices (no rules)`
    );
  } else {
  ...
  const wholesaleListMatch = existingPriceLists.filter(
  ...
  for (const priceSet of cleanPriceSets) {
    ...
    if (basePrice) {
      ...
      logger.info(
        `  💰 ${priceSet.id}: $${baseAmount} → $${discountedAmount} (wholesale)`
      );
    }
  }
  ...
  logger.info(
    "   - Wholesale prices: 7.5% discount for Wholesale customer group"
  );
