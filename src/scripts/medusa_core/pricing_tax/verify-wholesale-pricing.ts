...
export default async function verifyWholesalePricing({ container }: any) {
  ...
  try {
    ...
    if (priceList.status !== "active") {
      log(
        `⚠️  WARNING: Price list status is "${priceList.status}" (should be "active")`
      );
    }
    ...
    if (variants.length > 0) {
      ...
      const wholesalePrice = variant.prices?.find(
        (p: any) => p.price_list_id === priceList.id
      );
      ...
      if (wholesalePrice) {
        ...
        if (defaultPrice) {
          const discount = (
            ((defaultPrice.amount - wholesalePrice.amount) /
              defaultPrice.amount) *
            100
          ).toFixed(1);
    ...
    log("Summary:");
    log(
      `  - Price List: ${priceList.status === "active" ? "✅ Active" : "⚠️  " + priceList.status}`
    );
