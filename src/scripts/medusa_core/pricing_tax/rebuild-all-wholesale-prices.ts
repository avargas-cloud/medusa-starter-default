...
export default async function rebuildAllWholesalePrices({
  container,
}: ExecArgs) {
  ...
  logger.info(
    `✅ Price List: ${wholesalePriceList.title} (${wholesalePriceList.id})`
  );
  ...
  for (const priceSet of priceSets) {
    ...
    const usdPrices = (priceSet.prices || []).filter(
      (p: any) => p.currency_code === "usd" && !p.price_list_id // base retail (no price list FK)
    );
    ...
    const retailPrice = usdPrices.reduce((max: any, p: any) =>
  ...
  logger.info(
    `   Built ${newWholesalePrices.length} new wholesale prices (${skipped} price sets skipped — no retail USD price)`
  );
  ...
  logger.info(
    `   ${newWholesalePrices.length} products now have correct wholesale pricing (10% off retail)`
  );
  logger.info(
    `   price_list_id set as FK — price_list: ${wholesalePriceList.id}`
  );
