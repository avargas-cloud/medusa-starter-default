...
export default async function createPriceSets({ container }: ExecArgs) {
  ...
  try {
    ...
    const variantsNeedingPriceSets = variants.filter(
    ...
    for (let i = 0; i < variantsNeedingPriceSets.length; i += batchSize) {
      ...
      try {
        ...
        const priceSets =
          await pricingModule.createPriceSets(priceSetDefinitions);
        ...
        logger.info(
          `  ✓ Created ${created}/${variantsNeedingPriceSets.length} Price Sets...`
        );
      } catch (error: any) {
    ...
    logger.info("\n📝 Next Step:");
    logger.info(
      "Run: yarn medusa exec ./src/scripts/assign-placeholder-prices.ts"
    );
