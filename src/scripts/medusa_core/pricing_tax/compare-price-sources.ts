...
export default async function comparePriceSources({ container }: any) {
    ...
    try {
        ...
        prices.forEach((price: any, i: number) => {
            const label = price.price_list_id ? `PRICE LIST ${price.price_list_id}` : "DEFAULT"
            log(`   [${i + 1}] ${label}`)
        ...
        log(`Price list prices: ${prices.filter((p: any) => p.price_list_id).length}`)

        if (prices.length > 1) {
            log(`\n⚠️  MULTIPLE PRICES DETECTED!`)
            log(`   Admin UI variant detail might show wrong price if multiple exist`)
        }
        log("=".repeat(70))
