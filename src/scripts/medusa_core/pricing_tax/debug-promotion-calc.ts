...
async function main() {
  ...
  for (const row of itemsRes.rows) {
    ...
    console.log(
      `  - ${row.title?.substring(0, 40)}: unit=$${row.unit_price} qty=${row.quantity} total=$${lineTotal.toFixed(2)} tax_inclusive=${row.is_tax_inclusive}`
    );
  }
  ...
  for (const row of adjRes.rows) {
    ...
    console.log(
      `  ${tag} code=${row.code} amount=$${row.amount} item=${row.item_id.substring(0, 20)}`
    );
  }
  ...
  for (const p of promoRes.rows) {
    console.log(
      `  code=${p.code} status=${p.status} is_tax_inclusive=${p.is_tax_inclusive} type=${p.type} value=${p.value}`
    );
    if (p.type === "percentage" && p.code === "GOOGLE-REVIEW") {
      const expectedOnFullSubtotal = Number(
        ((itemSubtotal * p.value) / 100).toFixed(2)
      );
      const taxRate = 0.07;
      const taxInclusiveSubtotal = Number(
        (itemSubtotal * (1 + taxRate)).toFixed(2)
      );
      const expectedOnTaxInclusive = Number(
        ((taxInclusiveSubtotal * p.value) / 100).toFixed(2)
      );
      console.log(`\n  📐 Expected Discount Calculation:`);
      console.log(
        `     Pre-tax subtotal ($${itemSubtotal.toFixed(2)}) × ${p.value}% = $${expectedOnFullSubtotal.toFixed(2)} ← CORRECT`
      );
      console.log(
        `     Tax-inclusive ($${taxInclusiveSubtotal.toFixed(2)}) × ${p.value}% = $${expectedOnTaxInclusive.toFixed(2)} ← WRONG`
      );
      console.log(
        `     Medusa stored adjustment: $${activeAdjTotal.toFixed(2)}`
      );
