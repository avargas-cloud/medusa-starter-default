...
async function run() {
  ...
  try {
    ...
    const res = await fetch(`${MEDUSA_URL}/store/carts/${cartId}?fields=*items,*shipping_methods,shipping_methods.tax_lines,items.tax_lines`, {
      headers: { "x-publishable-api-key": pubKey }
    });
    ...
    cart.shipping_methods?.forEach((sm: any) => console.log(sm.name, "Tax:", sm.tax_total, sm.tax_lines))
