...
const BRIDGE_URL = process.env.QUICKBOOKS_BRIDGE_URL || "https://qb.eptbridge.com"
const API_KEY = process.env.QUICKBOOKS_BRIDGE_API_KEY || "mQb-7k9Pzx4RwN2vL8jT3bY6hF5nC1aD"
...
async function testSiteQuery() {
    ...
    try {
        ...
        if (!initRes.ok) {
            console.error(`❌ Error querying bridge: ${initRes.status} ${initRes.statusText}`)
        ...
        console.log(`✅ Success! Operation Queued with ID: ${initData.operationId}`)
        ...
        while (attempts < MAX_POLLS) {
            ...
            const statusRes = await fetch(`${BRIDGE_URL}/api/sync/status/${initData.operationId}`, {
                headers: {
                    "x-api-key": API_KEY,
                    "bypass-tunnel-reminder": "true"
                }
            ...
            if (op.status === "completed") {
                ...
                // 3. Inspect Data
                let items = []
                const qbMsgsRs = op.result?.QBXML?.QBXMLMsgsRs || op.result?.QBXMLMsgsRs
                if (qbMsgsRs) {
                ...
                if (items.length > 0) {
                    ...
                    const eap = items.find((i: any) => i.ItemInventoryRef?.FullName === "EAP-AS1-8S")
                    if (eap) {
                        console.log("\n🔍 Found 'EAP-AS1-8S' (SKU-001):")
                        console.log(`   - QuantityOnHand: ${eap.QuantityOnHand}`)
                        console.log(`   - Is this the expected Principal Warehouse quantity?`)
                    } else {
                ...
                const fs = require('fs')
                fs.writeFileSync('test_qb_site_query_debug.json', JSON.stringify(op, null, 2))
