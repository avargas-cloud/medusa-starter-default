  ...
  try {
    ...
    for (let i = 0; i < allCategories.length; i++) {
      ...
      if (metadata?.filter_config?.active_filters?.length > 0) {
        alreadyConfigured++;
        console.log(
          `✅ [${i + 1}/${allCategories.length}] ${category.name} - Already has ${metadata.filter_config.active_filters.length} filters`
        );
        continue;
      }
      ...
      if (!products || products.length === 0) {
        wouldSkip++;
        console.log(
          `⊘  [${i + 1}/${allCategories.length}] ${category.name} - No products, WOULD SKIP`
        );
        continue;
      }
      ...
      const uniqueAttrIds = attributeRows.map(
      ...
      if (uniqueAttrIds.length === 0) {
        wouldSkip++;
        console.log(
          `⊘  [${i + 1}/${allCategories.length}] ${category.name} - ${products.length} products but NO attributes, WOULD SKIP`
        );
        continue;
      }
      ...
      wouldConfigure++;
      console.log(
        `🆕 [${i + 1}/${allCategories.length}] ${category.name} - WOULD CONFIGURE ${uniqueAttrIds.length} filters from ${products.length} products`
      );
    }
    ...
    console.log(
      `   Phase 2: Would generate filters for ${alreadyConfigured + wouldConfigure} categories`
    );
    console.log("");
