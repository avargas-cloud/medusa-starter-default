 ...
 */
async function importShortDescriptionsEnhanced(
  dryRun = true,
  targetFailed = false
) {
  ...
  try {
    ...
    console.log(
      "🔄 Starting ENHANCED WooCommerce short_description re-sync..."
    );
    console.log(
    ...
    if (targetFailed) {
      console.log(
        `Target: ONLY products where description = long_description (failed syncs)`
      );
    }
      ...
      try {
        ...
        for (const wcProduct of products) {
          ...
          if (!match.medusa_id) {
            ...
            console.log(
              `❌ Not found: WC ID ${wcProduct.id} - ${wcProduct.name}`
            );
            continue;
          }
          ...
          // Log match
          console.log(
            `✅ Match via ${match.strategy.toUpperCase()}: WC:${wcProduct.id} → ${match.medusa_title}`
          );
          console.log(
            `   Short desc: ${wcProduct.short_description.substring(0, 80)}...`
          );
...
function printSummary(stats: any, totalProcessed: number, dryRun: boolean) {
  ...
  console.log(`\n🎯 MATCHING STRATEGIES:`);
  console.log(
    `  ✅ Matched by WC ID (metadata.wc_id):  ${stats.matched_by_wc_id}`
  );
  console.log(
    `  ✅ Matched by Handle/Slug (fallback):  ${stats.matched_by_handle}`
  );
  console.log(
    `  ✅ Matched by SKU (last resort):       ${stats.matched_by_sku}`
  );
  console.log(
    `  ❌ Not found in Medusa:                ${stats.not_found.length}`
  );
  console.log(
    `  ⏭️  No short_description in WC:        ${stats.no_short_desc.length}`
  );

  console.log(`\n📝 RESULTS:`);
  console.log(
    `  Products ${dryRun ? "to update" : "updated"}: ${stats.updated}`
  );
  ...
  if (dryRun) {
    ...
    console.log(
      "   Add --failed-only to target only products that failed previously"
    );
  } else {
    console.log("\n✅ Re-sync completed!");
