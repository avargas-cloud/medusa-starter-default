...
function formatValue(suffix: string, title: string): string {
  if (title === "Color Options") {
    ...
    if (/^\d{2}K$/i.test(suffix))
      return suffix.toUpperCase().replace("K", "00K");
...
export default async function importQbVariantProducts({ container }: ExecArgs) {
  ...
  const groups: { baseSku: string; suffixLen: number; variants: any[] }[] =
    analysis.groups || [];
  ...
  const existingHandles = new Set(
    existingProducts.filter((p: any) => p.handle).map((p: any) => p.handle)
  );

  // Filter out groups where the parent product already exists
  const toCreate = groups.filter(
    (g) => !existingHandles.has(slugify(g.baseSku))
  );
  const skipped = groups.length - toCreate.length;

  logger.info(`   Already in Medusa: ${existingProducts.length} products`);
  logger.info(
    `   To create via Groups: ${toCreate.length} (skipping ${skipped} parent handles already exist)`
  );
  ...
  for (let batchIdx = 0; batchIdx < batches; batchIdx++) {
    ...
    try {
      ...
      if ((batchIdx + 1) % 5 === 0 || batchIdx === batches - 1) {
        logger.info(
          `   ✅ Progress: ${created}/${toCreate.length} products created (batch ${batchIdx + 1}/${batches})`
        );
      }
    } catch (err: any) {
      failed += batch.length;
      ...
      errors.push({
        baseSku: `batch_${batchIdx + 1}: ${skuList}`,
        error: err.message,
      });
      ...
      for (const input of productInputs) {
        try {
          ...
          logger.error(
            `      ❌ Failed: ${input.title} — ${singleErr.message}`
          );
        }
      }
    }
