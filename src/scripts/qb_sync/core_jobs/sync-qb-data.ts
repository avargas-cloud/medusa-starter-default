...
export default async function syncQbData({ container, args }: ExecArgs) {
  ...
  const limit = process.env.SYNC_LIMIT
    ? parseInt(process.env.SYNC_LIMIT)
    : limitArg
  ...
  if (missingSkus.length > 0) {
    const missingCsv = ["SKU,ListID,MPN"]
      .concat(missingSkus.map((i) => `${i.sku},${i.listId},${i.mpn}`))
      .join("\n");
  ...
  for (const item of foundItems) {
    ...
    if (needsUpdate) {
      if (mode === "run") {
        ...
        logger.info(
          `   ✅ Updated ${item.sku}: QB=${item.listId}, MPN=${item.mpn}`
        );
      } else {
        logger.info(
          `   [DRY RUN] Would update ${item.sku}: QB=${item.listId}, MPN=${item.mpn}`
        );
      }
      updated++;
    } else {
