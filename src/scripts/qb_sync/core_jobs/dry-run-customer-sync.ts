...
export default async function dryRunCustomerSync({ container }: ExecArgs) {
  ...
  logger.info(
    `⏳ Waiting 2 minutes before first poll (7200 customers take time)...`
  );
  ...
  while (attempts < MAX_POLL_ATTEMPTS) {
    attempts++;
    logger.info(
      `⏳ Polling (${attempts}/${MAX_POLL_ATTEMPTS}) — waiting for QB to complete...`
    );
    ...
    try {
      ...
      if (rawStatus.success && rawStatus.operation?.status === "completed") {
        ...
        const raw =
          rawStatus.operation?.result?.QBXML?.QBXMLMsgsRs?.CustomerQueryRs
            ?.CustomerRet;
  ...
  if (qbCustomers.length === 0) {
    logger.error(
      "❌ No customer data received from QB after all polling attempts"
    );
    logger.info(
      `Last raw status (non-data fields): ${JSON.stringify(
        {
          success: rawStatus?.success,
          operation: rawStatus?.operation,
          dataType: typeof rawStatus?.data,
          dataLength: Array.isArray(rawStatus?.data)
            ? rawStatus.data.length
            : "not array",
  ...
  logger.info(
    `  - With real email:    ${wouldImport.length - dummyEmail.length}`
  );
  ...
  if (wouldImport.length > 0) {
    ...
    for (const qb of wouldImport.slice(0, 20)) {
      ...
      logger.info(
        `   - ${qb.Name} | ${email} | ${qb.PriceLevelRef?.FullName || "no price level"}`
      );
    }
    if (wouldImport.length > 20)
      logger.info(`   ... and ${wouldImport.length - 20} more`);
  } else {
    logger.info(
      "✅ All QB customers are already in Medusa — nothing to import!"
    );
  }
  ...
  for (const [level, count] of Object.entries(priceLevelCounts).sort(
    (a, b) => b[1] - a[1]
  )) {
