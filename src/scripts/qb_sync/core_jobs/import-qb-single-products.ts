...
export default async function importQbSingleProducts({ container }: ExecArgs) {
  ...
  if (!fs.existsSync(ANALYSIS_FILE)) {
    ...
    logger.error(
      `   Run first: npx medusa exec src/scripts/migrate/analyze-qb-products-migration.ts`
    );
    return;
  }
  ...
  const existingSkus = new Set(
    existingVariants.filter((v: any) => v.sku).map((v: any) => v.sku.trim())
  );
  ...
  logger.info(
    `   To create: ${toCreate.length} (skipping ${skipped} already exist)`
  );
  ...
  for (let batchIdx = 0; batchIdx < batches; batchIdx++) {
    ...
    try {
      ...
      const skuList = batch
        ...
        .slice(0, 100);
      logger.error(
        `   ❌ Batch ${batchIdx + 1} failed (${batch.length} items): ${err.message}`
      );
      errors.push({
        sku: `batch_${batchIdx + 1}: ${skuList}`,
        error: err.message,
      });
