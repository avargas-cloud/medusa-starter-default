...
async function queryEditSequence(endpoint: QbEndpoint, txnId: string): Promise<string | null> {
    try {
...
async function main() {
    ...
    for (const row of ests) {
        ...
        if (!seq) { failed++; await sleep(DELAY_MS); continue }
