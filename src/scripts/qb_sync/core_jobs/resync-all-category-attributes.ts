...
async function main() {
    console.log("🔄 Starting full category attributes re-sync with recursive aggregation...\n")

    const basePath = "http://localhost:9000"

    // Fetch all categories using the admin endpoint
    const categoriesResponse = await fetch(`${basePath}/admin/product-categories?limit=9999`, {
        credentials: "include"
    })
    ...
    console.log(`\n${"=".repeat(60)}`)
    console.log(`✅ Re-sync complete: ${successCount} success, ${errorCount} errors`)
