 ...
 */
export default async function verifyInventoryBeforeSync({ container }: ExecArgs) {
    ...
    const { data: variants } = await query.graph({
        entity: "variant",
        fields: ["id", "sku", "title", "metadata", "inventory_items.inventory_item_id"]
    })

    const qbVariants = variants.filter((v: any) => v.metadata?.quickbooks_id)
    const withInventory = qbVariants.filter((v: any) => v.inventory_items?.[0]?.inventory_item_id)
    const noInventory = qbVariants.filter((v: any) => !v.inventory_items?.[0]?.inventory_item_id)
    ...
    top20.forEach(({ sku, stock }, i) => {
        ...
        logger.info(`   ${String(i + 1).padStart(2)}. ${sku.padEnd(20)} ${String(stock).padStart(5)} ${bar}`)
    })
    logger.info("")
    ...
    if (zeroSkus.length > 0) {
        ...
        if (zeroSkus.length > 15) logger.info(`   ... and ${zeroSkus.length - 15} more`)
        logger.info("")
    }

    // SKUs missing inventory item
    if (noInventory.length > 0) {
        logger.info(`⚠️  SKUs missing inventory item (${noInventory.length}) — these will be SKIPPED by the sync:`)
        noInventory.slice(0, 10).forEach((v: any) => logger.info(`   • ${v.sku}`))
        if (noInventory.length > 10) logger.info(`   ... and ${noInventory.length - 10} more`)
        logger.info("")
        logger.info("   ➡️  Run 'enable-inventory-management' to fix these before syncing.")
    }
