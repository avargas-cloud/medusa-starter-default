    ...
    if (matchedVariants.length > 0) {
      ...
      matchedVariants.slice(0, 5).forEach((v: any) => {
        ...
        if (v.dimensions.length)
          console.log(`    Length: ${v.dimensions.length}"`);
        if (v.dimensions.width)
          console.log(`    Width: ${v.dimensions.width}"`);
        if (v.dimensions.height)
          console.log(`    Height: ${v.dimensions.height}"`);
        if (v.dimensions.weight)
          console.log(`    Weight: ${v.dimensions.weight} lb`);
      });
    }
