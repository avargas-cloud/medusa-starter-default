...
export default async function debugSyncComparison({ container }: ExecArgs) {
  ...
  try {
    ...
    if (latestMeili.hits.length > 0) {
      ...
      console.log(
        `MEILI HEAD: ID=${hit.id}, Title=${hit.title}, updated_at=${hit.updated_at}`
      );
  ...
  if (latestProduct) {
    ...
    console.log(
      `DB HEAD: ID=${latestProduct.id}, Title=${latestProduct.title}, updated_at=${dbLastUpdate.toISOString()} (Ts: ${dbLastUpdate.getTime()})`
    );
  } else {
  ...
  console.log(
    `Count Sync: ${dbCount} === ${meiliCount} ? ${dbCount === meiliCount}`
  );
