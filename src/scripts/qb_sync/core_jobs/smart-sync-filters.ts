...
export default async function ({ container }: ExecArgs) {
  ...
  const toProcess = categoriesWithFilters.filter((cat: any) => {
    ...
    if (count > 50) {
      console.log(
        `   ⏭️  Skipping "${cat.name}" - ${count} descendants (too slow)`
      );
      return false;
    }
    return true;
  ...
  for (const category of toProcess) {
    ...
    try {
      ...
      if (
        Array.isArray(filterConfig.active_filters) &&
        filterConfig.active_filters.length > 0
      ) {
        ...
        if (typeof first === "string") {
          ...
          activeFilterIds = filterConfig.active_filters.map(
  ...
  console.log(
    `   - Skipped (too large): ${categoriesWithFilters.length - toProcess.length}`
  );
  console.log(
