...
const STOP_WORDS = new Set([
    "the", "and", "for", "with", "this", "that", "from", "are", "has", "not",
    "led", "light", "lighting", "finish", "color", "size", "base", "each"
])

function hasDescriptionOverlap(items: QBItem[]): boolean {
    const withDesc = items.filter(i => i.description && i.description.length > 5)
    ...
    for (const item of withDesc) {
        ...
        common = common === null ? words : new Set([...common].filter(w => words.has(w)))
    }
...
// ── Variant detection ─────────────────────────────────────────────────────────
const EXCLUDED_PREFIXES = ["E01", "ECNA", "ET", "EG", "FI", "GL", "EU", "KUZ", "LUX-L", "MAX", "SAT", "SID", "SUN", "VAX", "SUP-AP-IP-RM1-", "SUP-AP-IP-SM1-", "TS-Mirror-", "WE-LOV-", "WE-LPSRFC2"]
...
function detectVariantGroups(items: QBItem[]): {
    ...
    const buildPrefixMap = (items: QBItem[], trimLen: number): Map<string, QBItem[]> => {
    ...
    const candidates2 = new Map([...buildPrefixMap(items, 2)].filter(([, v]) => v.length >= 2))
    const candidates3 = new Map([...buildPrefixMap(items, 3)].filter(([, v]) => v.length >= 2))

    const assignedSkus = new Set<string>()
    const finalGroups: VariantGroup[] = []
    const rejected: Rejected[] = []

    const tryAddGroup = (base: string, groupItems: QBItem[], suffixLen: number): boolean => {
    ...
    for (const [base, groupItems] of candidates3) {
        const unassigned = groupItems.filter(i => !assignedSkus.has(i.sku))
        const alreadyCovered = finalGroups.some(g =>
            g.baseSku.startsWith(base) || base.startsWith(g.baseSku)
        )
    ...
    const singles = items.filter(i => !assignedSkus.has(i.sku)).sort((a, b) => a.sku.localeCompare(b.sku))
    return { singles, groups: finalGroups, rejected }
}

// ── Main ──────────────────────────────────────────────────────────────────────
export default async function analyzeQbProductsMigration({ container }: ExecArgs) {
    ...
    logger.info(`📦 QB Inventory items: ${qbInventory.length}  (${qbAll.length} total)`)
    ...
    const existingSkus = new Set(variants.filter((v: any) => v.sku).map((v: any) => v.sku.trim()))
    logger.info(`   Medusa variants with SKU: ${existingSkus.size}`)

    const unmatched: QBItem[] = qbInventory.filter(item => !existingSkus.has(item.sku))
    ...
    const totalVariantsInGroups = groups.reduce((sum, g) => sum + g.items.length, 0)
    ...
    lines.push(`━━━ SINGLE PRODUCTS (${singles.length}) ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`)
    lines.push("  (No variant siblings detected — 1 product + 1 default variant each)")
    lines.push("")
    singles.forEach((item, i) => {
        lines.push(`  - Product ${i + 1}: ${item.sku}  (Default, SKU: ${item.sku})`)
        if (item.description) lines.push(`    Description: ${item.description.slice(0, 80)}${item.description.length > 80 ? "..." : ""}`)
        if (item.price) lines.push(`    Price: $${item.price}`)
    })
    lines.push("")

    lines.push(`━━━ VARIANT PRODUCTS (${groups.length} groups, ${totalVariantsInGroups} items) ━━━━━━━━━━━━━━━━`)
    lines.push("  (Shared SKU prefix + consistent suffix type + description overlap)")
    lines.push("")
    groups.forEach((group, i) => {
        lines.push(`  - Product ${i + 1}: ${group.baseSku}  [${group.items.length} variants, suffix -${group.suffixLen}]`)
        group.items.forEach((item, j) => {
            lines.push(`      --- Variant ${j + 1}: ${item.sku}   ($${item.price})`)
            if (item.description) lines.push(`               ${item.description.slice(0, 70)}${item.description.length > 70 ? "..." : ""}`)
        })
    })
    lines.push("")

    lines.push(`━━━ REJECTED CANDIDATE GROUPS (${rejected.length}) ━━━━━━━━━━━━━━━━━━━━━━`)
    lines.push("  (These looked like variant families but failed validation)")
    lines.push("")
    rejected.forEach((r, i) => {
        lines.push(`  - Rejected ${i + 1}: prefix="${r.prefix}"  [${r.count} items]`)
    ...
    lines.push("RESUMEN")
    lines.push("━".repeat(70))
    lines.push(`  Productos totales en QuickBooks (inventory):    ${qbInventory.length}`)
    lines.push(`  Ya en Medusa (skip):                           ${alreadyInMedusa}`)
    lines.push(`  Items faltantes por procesar:                  ${unmatched.length}`)
    lines.push("")
    lines.push(`  Productos por crear en Medusa:                 ${totalProductsToCreate}`)
    lines.push(`    → Productos individuales (single):           ${singles.length}`)
    lines.push(`    → Grupos variantes:                         ${groups.length}`)
    lines.push(`    → Variantes totales a crear:                ${singles.length + totalVariantsInGroups}`)
    lines.push(`    → Grupos rechazados (bad grouping):         ${rejected.length}`)
    lines.push("━".repeat(70))
    ...
    const outputJson = {
        ...
        singles: singles.map(s => ({ sku: s.sku, description: s.description, price: s.price })),
        groups: groups.map(g => ({
            baseSku: g.baseSku,
            suffixLen: g.suffixLen,
            variants: g.items.map(v => ({ sku: v.sku, description: v.description, price: v.price })),
        })),
        rejected,
    }
    fs.writeFileSync(OUTPUT_JSON_FILE, JSON.stringify(outputJson, null, 2), "utf8")
    ...
    logger.info(`    → Variantes totales:    ${singles.length + totalVariantsInGroups}`)
