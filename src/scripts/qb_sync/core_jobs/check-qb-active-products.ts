...
async function fetchWithRetry(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.warn(
        `[retry] Bridge returned ${res.status} (attempt ${attempt}/${maxRetries})`
      );
      if (attempt < maxRetries) await new Promise((r) => setTimeout(r, 8_000));
    } catch (err: any) {
      console.warn(
        `[retry] Fetch error (attempt ${attempt}/${maxRetries}): ${err.message}`
      );
...
function parseXmlItems(rawXml: string): any[] {
  ...
  const sampleBlocks =
    invBlocks.length > 0 ? invBlocks.slice(0, 5) : blocks.slice(0, 5);
  if (sampleBlocks.length > 0) {
    fs.writeFileSync(RAW_SAMPLE_FILE, sampleBlocks.join("\n\n---\n\n"), "utf8");
    console.log(
      `📝 Raw XML sample (ItemInventoryRet) saved to: ${RAW_SAMPLE_FILE}`
    );
  }
...
function parseJsonItems(queryRs: any): any[] {
  const inventoryItems: any[] = Array.isArray(queryRs.ItemInventoryRet)
    ? queryRs.ItemInventoryRet
    : queryRs.ItemInventoryRet
      ? [queryRs.ItemInventoryRet]
      : [];
  const nonInventoryItems: any[] = Array.isArray(queryRs.ItemNonInventoryRet)
    ? queryRs.ItemNonInventoryRet
    : queryRs.ItemNonInventoryRet
      ? [queryRs.ItemNonInventoryRet]
      : [];
  const serviceItems: any[] = Array.isArray(queryRs.ItemServiceRet)
    ? queryRs.ItemServiceRet
    : queryRs.ItemServiceRet
      ? [queryRs.ItemServiceRet]
      : [];

  return [...inventoryItems, ...nonInventoryItems, ...serviceItems]
    .map((item) => ({
      listId: item.ListID ?? "",
...
async function main() {
  ...
  while (attempts < MAX_POLL_ATTEMPTS) {
    ...
    if (statusJson.success && statusJson.operation) {
      ...
      if (op.status === "completed") {
        ...
        // Try structured JSON
        const queryRs =
          op.result?.QBXML?.QBXMLMsgsRs?.ItemQueryRs ?? op.result?.ItemQueryRs;
        if (queryRs) {
        ...
        console.log(
          "   Available keys:",
          JSON.stringify(Object.keys(op.result ?? {}))
        );
        break;
      }
      ...
      if (op.status === "failed") {
        console.error(
          `❌ QB operation failed: ${op.error || op.message || "Unknown"}`
        );
        process.exit(1);
      }
  ...
  const activeItems = rawItems.filter(
    (i) => i.isActive !== false && i.isActive !== "false"
  );
  ...
  console.log(`\n💡 Quick stats:`);
  console.log(
    `   cat ${OUTPUT_FILE} | python3 -c "import sys,json; d=json.load(sys.stdin); print(f'Active: {d[\\\"totalActive\\\"]}, Inactive: {d[\\\"totalInactive\\\"]}')"\n`
  );
}
