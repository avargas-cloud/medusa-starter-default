...
async function main() {
  ...
  const { data: productCategories } = await query.graph({
    ...
    filters: {
      $or: [{ id: category.id }, { parent_category_id: category.id }],
    },
  });
  ...
  const attributeIds = [
    ...new Set(links.map((l: any) => l.attribute_value_id)),
  ];
  ...
  for (const [handle, values] of groupedByKey) {
    const key = (attributes as any[]).find(
      (a) => a.attribute_key.handle === handle
    )?.attribute_key;
    if (!key) continue;
