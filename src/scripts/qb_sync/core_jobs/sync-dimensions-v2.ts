...
async function main() {
  ...
  console.log(
    `[WooCommerce] Found ${wcProducts.length} products with dimensions/weight\n`
  );
  ...
  for (const wcProduct of wcProducts) {
    ...
    if (!medusaProduct) {
      ...
      if (!medusaProductBySKU) {
        console.log(
          `⚠️  Not found: WC ID ${wcProduct.id}, SKU ${wcProduct.sku}`
        );
        skippedCount++;
        continue;
      }
      ...
      console.log(
        `✓ Updated (by WC ID): ${wcProduct.sku} - ${wcProduct.title}`
      );
      updatedCount++;
    }

    console.log(
      `  Dimensions: ${wcProduct.length}" x ${wcProduct.width}" x ${wcProduct.height}", Weight: ${wcProduct.weight} lb\n`
    );
  }
...
async function updateProductDimensions(
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to update product ${productId}: ${response.statusText}`
    );
  }
}
