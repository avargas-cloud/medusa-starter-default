...
export default async function dryRunInventorySync({ container }: ExecArgs) {
  ...
  if (anomalies.length > 0) {
    logger.info(`\n${"⚠️  ".repeat(10)}`);
    logger.info(
      `⚠️  ${anomalies.length} ANOMALIES DETECTED — review carefully before running live sync:`
    );
    anomalies.forEach((a) => {
    ...
    logger.info(
      "   curl -s -X POST http://localhost:9000/admin/quickbooks/sync/inventory \\"
    );
  ...
  if (onlyInQb.length > 0) {
    ...
    if (onlyInQb.length > 10)
      logger.info(`   ... and ${onlyInQb.length - 10} more`);
  }
  ...
  if (onlyInMedusa.length > 0) {
    ...
    if (onlyInMedusa.length > 10)
      logger.info(`   ... and ${onlyInMedusa.length - 10} more`);
  }
  ...
  logger.info(
    `   No inventory item:    ${result.stats.skippedNoInventoryItem}`
  );
