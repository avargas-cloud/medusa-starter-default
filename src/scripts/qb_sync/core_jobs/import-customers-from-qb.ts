    ...
    for (const qbCustomer of customers) {
      try {
        ...
        if (!customerEmail) {
          ...
          if (validEmails.length > 0) {
            ...
            if (validEmails.length > 1) {
              ...
              logger.info(
                `  📧 Multiple emails: ${validEmails[0]} + ${altEmails.length} more`
              );
            }
          } else {
            ...
            logger.warn(
              `  ⚠️  Invalid email: ${qbCustomer.Email.substring(0, 50)} → Generated dummy`
            );
          }
        }

        // Parse name
        const { first_name, last_name, fromFields } =
          parseCustomerName(qbCustomer);
        if (fromFields) {
        ...
        const contactName = extractContactName(
          qbCustomer.BillingAddress.Address2 || ""
        );
        ...
        if (!isDryRun) {
          ...
          if (existing.length > 0) {
            logger.info(
              `  ⏭️  Skipping ${customerEmail} — already exists in Medusa (id: ${existing[0].id})`
            );
            continue;
          }
          ...
          const customer = await customerModule.createCustomers({
            ...
            metadata: {
              ...
              qb_price_level:
                qbCustomer.PriceLevel === "Standard"
                  ? "Retail"
                  : qbCustomer.PriceLevel === "Distributor"
                    ? "Wholesale"
          ...
          if (
            billingStreet &&
            qbCustomer.BillingAddress.City &&
            qbCustomer.BillingAddress.PostalCode
          ) {
          ...
          if (stats.imported <= 5) {
            ...
            if (billingStreet) {
              logger.info(
                `     Billing: ${billingStreet}, ${qbCustomer.BillingAddress.City}`
              );
            }
...
function findRealStreetAddress(
  ...
  const streetKeywords =
    /\b(st|street|ave|avenue|rd|road|blvd|boulevard|dr|drive|ln|lane|ct|court|pl|place|way|nw|ne|sw|se)\b/i;
  ...
  const checkOrder: Array<keyof QBAddress> = [
    "Address2",
    "Address3",
    "Address1",
    "Address4",
...
function parseCustomerName(
  ...
  const fullName = customer.Name.replace(
    /\b(LLC|Inc|Corp|Ltd|Co)\b\.?/gi,
    ""
  ).trim();
...
async function ensureCustomerGroups(
  ...
  const groupsToCreate = [
    {
      ...
      handle: "wholesale",
      metadata: { qb_price_level: "Wholesale" },
    },
    {
      name: "Retail Customers",
      handle: "retail",
      metadata: { qb_price_level: "Retail" },
    },
  ];
  ...
  if (!isDryRun) {
    ...
    logger.info(
      "🔧 Customer Groups (dry-run, will be created on real import)\n"
    );
