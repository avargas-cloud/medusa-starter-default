...
async function verifySubscribers() {
  ...
  try {
    ...
    assert(
      typeof orderSub.default === "function",
    ...
    assert(
      events.includes("order.payment_captured"),
      "Handles order.payment_captured"
    );
    assert(
      events.includes("order.fulfillment_created"),
      "Handles order.fulfillment_created"
    );
    assert(
      orderSub.config?.context?.subscriberId === "qb-order-subscriber",
      `SubscriberId: ${orderSub.config?.context?.subscriberId}`
    );
  } catch (err: any) {
  ...
  try {
    const draftSub =
      await import("../../subscribers/qb-draft-order-subscriber");

    assert(
      typeof draftSub.default === "function",
      "Default export is a function"
    );
    ...
    assert(
      events.includes("draft_order.created"),
      "Handles draft_order.created"
    );
    assert(
      draftSub.config?.context?.subscriberId === "qb-draft-order-subscriber",
      `SubscriberId: ${draftSub.config?.context?.subscriberId}`
    );
  } catch (err: any) {
    console.log(`  ❌ Import failed: ${err.message}`);
    failed++;
