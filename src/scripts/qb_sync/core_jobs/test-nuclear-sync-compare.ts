...
async function runNuclearSync() {
  ...
  try {
    const response = await fetch(
      `${BACKEND_URL}/admin/product-categories/nuclear-sync`,
      {
        method: "POST",
        headers: {
    ...
    console.log(
      `   Phase 1: ${result.phase1.processed} categories processed, ${result.phase1.skipped} skipped`
    );
    console.log(
      `   Phase 2: ${result.phase2.generated} filters generated, ${result.phase2.failed} failed`
    );
...
async function compareFilters() {
  ...
  try {
    ...
    for (
      let i = 0;
      i < Math.max(endpointFilters.length, metadataFilters.length);
      i++
    ) {
      ...
      for (let j = 0; j < endpointOptionsCount; j++) {
        ...
        if (
          endpointOpt.value !== metadataOpt.value ||
      ...
      if (optionMismatch) {
        ...
        console.log(
          `✅ Filter ${i + 1}: ${endpointFilter.name} (${endpointOptionsCount} options) - MATCH`
        );
      }
    }
