...
export default async function syncInventoryDimensions({
  container,
}: {
  container: any;
}) {
  ...
  for (const row of links) {
    const hasInventoryDims =
      row.inventory_weight ||
      row.inventory_height ||
      row.inventory_width ||
      row.inventory_length;

    // Already has dimensions → skip
    if (hasInventoryDims) {
      console.log(
        `⏭  SKIP  [${row.inventory_sku || row.inventory_item_id}] "${row.inventory_title}" — already has dimensions`
      );
      skipped++;
      continue;
    }

    const hasProductDims =
      row.product_weight ||
      row.product_height ||
      row.product_width ||
      row.product_length;
    ...
    if (!hasProductDims) {
      noSourceList.push({
        sku: row.variant_sku || row.inventory_sku,
    ...
    console.log(
      `        Values  : weight=${row.product_weight ?? "❌"}  height=${row.product_height ?? "❌"}  width=${row.product_width ?? "❌"}  length=${row.product_length ?? "❌"}`
    );
    if (missing.length > 0) {
  ...
  if (DRY_RUN) {
    ...
    console.log(
      `   To apply: DRY_RUN=false npx medusa exec src/scripts/sync-inventory-dimensions.ts`
    );
  } else {
