...
export default async function linkOrphanedVariantsToAttributesV3({
  container,
}: {
  ...
  for (const product of multiVariantProducts) {
    ...
    try {
      for (const option of product.options) {
        ...
        console.log(
          `      ✅ Matched to Attribute: "${matchingAttrKey.label}"`
        );
        ...
        for (const variant of product.variants) {
          ...
          const alreadyLinked = variant.options?.some(
            (vo) =>
          ...
          const matchingValue = matchingAttrKey.values?.find(
            (val) =>
          ...
          if (matchingValue) {
            variantsToLink.push({
              ...
              variantTitle,
              matchingValue,
              needsOptionValue: !existingOptionValues.has(
