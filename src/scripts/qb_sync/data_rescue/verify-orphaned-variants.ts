...
export default async function verifyOrphanedVariants({
  container,
}: {
  ...
  console.log(
    `📊 Total variants to audit: ${multiVariantProducts.reduce((sum, p) => sum + p.variants.length, 0)}\n`
  );
  ...
  console.log(
    `  - Variants with NO options: ${report.summary.variantsWithNoOptions}`
  );
  console.log(
    `  - Variants with MISSING options: ${report.summary.variantsWithMismatchedOptions}`
  );
  console.log(
    `  - Variants with EMPTY option values: ${report.summary.variantsWithEmptyOptionValues}`
  );
    ...
    for (const [productId, orphans] of byProduct) {
      ...
      for (const orphan of orphans) {
        ...
        if (orphan.issue === "NO_OPTIONS") {
          ...
          console.log(
            `      Issue: ❌ Empty value for option "${orphan.optionTitle}"`
          );
          console.log(`      Expected: "${orphan.expectedValue}"`);
