...
export default async function ({ container }: ExecArgs) {
  ...
  for (const value of allValues) {
    ...
    const publishedLinks = links.filter((l: any) =>
      publishedIds.includes(l.product_id)
    );

    if (publishedLinks.length > 0) {
      console.log(
        `✅ ${value.value}: ${publishedLinks.length} PUBLISHED product(s)`
      );
    } else {
      console.log(
        `⚠️  ${value.value}: ${links.length} link(s) but ALL are DRAFT products`
      );
    }
  }
