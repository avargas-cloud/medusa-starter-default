...
async function bridgeFetch(method: string, path: string): Promise<any> {
  ...
  if (!res.ok)
    throw new Error(
      `Bridge ${method} ${path} → ${res.status}: ${await res.text()}`
    );
  return res.json();
}
...
async function pollForInventory(operationId: string): Promise<any[]> {
  for (let attempt = 1; attempt <= POLL_MAX_ATTEMPTS; attempt++) {
    console.log(
      `  ⏳ Polling (${attempt}/${POLL_MAX_ATTEMPTS}) — waiting for QB Web Connector...`
    );
    ...
    if (op.status === "completed") {
      ...
      throw new Error(
        "QB returned completed but no ItemSitesQueryRs in result"
      );
    }
...
async function main() {
  ...
  if (!operationId)
    throw new Error("Bridge did not return operationId for inventory fetch");
  ...
  console.log(
    `  🗺️  Mapped ${Object.keys(skuToListId).length} unique QB items by name\n`
  );
  ...
  if (matched.length === 0) {
    console.log(
      "No matches found — check that QB item names match Medusa SKUs exactly.\n"
    );
    await db.end();
    return;
  }
  ...
  console.log(
    `${DRY_RUN ? "🔵 [DRY RUN]" : "💾"} Saving quickbooks_id to ${matched.length} variants...`
  );
  let saved = 0,
    failed = 0;
  ...
  if (unmatched.length > 0) {
    console.log(
      `⚠️  ${unmatched.length} SKUs had no QB match — set them manually or add the products to QB first.`
    );
