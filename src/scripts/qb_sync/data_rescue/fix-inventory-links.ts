...
async function run() {
  ...
  try {
    ...
    const [variants, count] = await productService.listAndCountVariants(
      {},
      {
        select: ["id", "sku", "title", "manage_inventory", "inventory_items"],
        take: 20000,
      }
    );
    ...
    const unlinkedVariants = variants.filter(
      (v) => !v.inventory_items || v.inventory_items.length === 0
    );
    console.log(
      `⚠️ Found ${unlinkedVariants.length} variants WITHOUT an Inventory Item.`
    );
    ...
    console.log(`\n📊 Breakdown:`);
    console.log(
      `  - ${noManage.length} have manage_inventory: false. (These technically shouldn't sync anyway)`
    );
    console.log(
      `  - ${toFix.length} have manage_inventory: true and NEED fixing.`
    );
    ...
    if (process.argv.includes("--dry-run")) {
      ...
      console.log(
        toFix
          ...
          .slice(0, 10)
          .join(", ") +
          (toFix.length > 10 ? ` ...and ${toFix.length - 10} more` : "")
      );
      process.exit(0);
    }
    ...
    for (const variant of toFix) {
      try {
        ...
        console.log(
          `Would create inv item ${invItem.id} for variant ${variant.id}`
        );
