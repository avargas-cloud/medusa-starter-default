...
const results: Record<
  string,
  { success: boolean; txnId?: string; refNumber?: string; error?: string }
> = {};
...
async function runFullCycle() {
  ...
  results.estimate = {
    success: !!estPoll.txnId,
    txnId: estPoll.txnId,
    refNumber: estPoll.refNumber,
  };
  console.log(
    `✅ Estimate → TxnID: ${estPoll.txnId}, Ref: ${estPoll.refNumber}\n`
  );
  ...
  results.salesOrder = {
    success: !!soPoll.txnId,
    txnId: soPoll.txnId,
    refNumber: soPoll.refNumber,
  };
  console.log(
    `✅ Sales Order → TxnID: ${soPoll.txnId}, Ref: ${soPoll.refNumber}\n`
  );
  ...
  results.payment = {
    success: !!payPoll.txnId,
    txnId: payPoll.txnId,
    refNumber: payPoll.refNumber,
  };
  console.log(
    `✅ Payment → TxnID: ${payPoll.txnId}, Ref: ${payPoll.refNumber}\n`
  );
  ...
  results.invoice = {
    success: !!invPoll.txnId,
    txnId: invPoll.txnId,
    refNumber: invPoll.refNumber,
  };
  console.log(
    `✅ Invoice → TxnID: ${invPoll.txnId}, Ref: ${invPoll.refNumber}\n`
  );
  ...
  if (!applyResult.success) {
    ...
    results.applyPayment = {
      success: true,
      txnId: applyPoll.txnId,
      refNumber: applyPoll.refNumber,
    };
...
function printSummary() {
  ...
  for (const [name, result] of steps) {
    if (!result) {
      ...
      console.log(
        `  ✅ ${name}: TxnID=${result.txnId || "?"}, Ref=${result.refNumber || "?"}`
      );
    } else {
