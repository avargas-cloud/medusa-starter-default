...
async function main() {
    ...
    try {
        ...
        const response = await fetch('http://localhost:9000/store/sync-dimensions-temp', {
            method: 'POST',
            headers: {
        ...
        if (result.variants && result.variants.length > 0) {
            ...
            result.variants.slice(0, 5).forEach((v: any) => {
                ...
                console.log(`    ${v.dimensions.length}" x ${v.dimensions.width}" x ${v.dimensions.height}", ${v.dimensions.weight} lb`);
            });
        }
