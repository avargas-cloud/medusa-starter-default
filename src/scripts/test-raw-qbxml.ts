  ...
  <QBXMLMsgsRq onError="continueOnError">
    <ItemInventoryQueryRq>
      ...
      <IncludeRetElement>Cost</IncludeRetElement>
        ...
        const operationId = initRes.operationId || initRes.operation?.id || initRes.id
        console.log(`✅ Operation Queued! ID: ${operationId}`)

        const result = await pollRawOperationResult(operationId)
        
        fs.writeFileSync("/home/alejo/webapps/ecopowertech-workspace/backend/qb_raw_sync_output.json", JSON.stringify(result, null, 2))
