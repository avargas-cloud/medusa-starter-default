...
async function run() {
    ...
    const orders = await orderModule.listOrders({}, { relations: ["shipping_methods"], take: 1 })
    if (!orders.length) return console.log("No orders")
    const order = orders[0]
    
    console.log("Order:", order.id, "Shipping Methods:", order.shipping_methods?.map((sm: any) => ({ id: sm.id, amount: sm.amount })))
    ...
    if (order.shipping_methods?.length) {
        ...
        try {
            if (typeof orderModule.updateOrderShippingMethods === 'function') {
                ...
                console.log("Available methods:", Object.keys(orderModule).filter(k => k.toLowerCase().includes('ship')))
            }
        } catch (e) {
            console.error(e)
        }
