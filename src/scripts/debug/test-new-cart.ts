  ...
  try {
    ...
    const cartRes = await fetch(`${MEDUSA_URL}/store/carts`, {
      ...
      body: JSON.stringify({
        ...
        shipping_address: { first_name:"Test", last_name:"User", address_1:"123 Main St", city:"Miami", country_code:"us", province:"FL", postal_code:"33101" }
      })
    });
    const { cart } = await cartRes.json();
    if (!cart) { console.error("Cart creation failed"); return; }
    ...
    console.log("\nExpected tax_total = item_subtotal * 0.07 =", (finalCart.item_subtotal * 0.07).toFixed(4));
