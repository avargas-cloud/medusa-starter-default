        ...
        if (prices.rows.length > 0) {
            ...
            if (rules.rows.length > 0) {
                ...
                rules.rows.forEach(rule => {
                    console.log(`  - Price $${rule.price_amount} requires: ${rule.rule_attribute} = "${rule.rule_value}"`)
                })

                console.log('\n❌ PROBLEM: calculatePrices() expects these context values but we may not be providing them!')
                console.log('   Solution: Either provide the context OR remove the rules for base prices.')
            } else {
