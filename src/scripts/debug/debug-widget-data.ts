export default async function ({ container }: any) {
    ...
    if (category.metadata?.available_attributes) {
        ...
        console.log("  - First 3:", category.metadata.available_attributes.slice(0, 3))
    }
    ...
    console.log("  - Sample IDs:", attrKeys.slice(0, 3).map((a: any) => ({ id: a.id, handle: a.handle })))
    ...
    if (category.metadata?.filter_config?.active_filters) {
        ...
        const firstActiveId = typeof activeIds[0] === "string" ? activeIds[0] : activeIds[0]?.attribute_id
