...
// Peek sheets 1, 4, 7 (Jan, Apr, Jul) — first 5 rows each
for (const name of ["1", "4", "7"]) {
  try {
    const xml = getSheetXml(zip, name);
    ...
    rows.slice(0, 5).forEach((row: any, i: number) => {
      ...
      const vals = cells.slice(0, 8).map((c: any) => cellVal(c, shared).slice(0, 20).padEnd(22)).join(" | ");
      console.log(`  Row ${i + 1}: ${vals}`);
    });
  } catch (e: any) {
    console.log(`Sheet "${name}": ERROR — ${e.message}`);
  }
}
