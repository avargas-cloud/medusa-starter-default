...
async function run() {
  ...
  try {
    ...
    const result = await pollOperationResult(
      enqueueRes.operation_id,
      console.log
    );

    if ((result as any).status === "failed") {
      console.error(
        "❌ Failed to query accounts from QB:",
        (result as any).error
      );
      return;
    }
    ...
    // Parse result
    let accounts =
      (result as any).result?.AccountRet ||
      (result as any).result?.QBXML?.QBXMLMsgsRs?.AccountQueryRs?.AccountRet;
    if (!accounts) {
    ...
    console.log(
      "Copia el ListID de la cuenta que desees y pégalo en el POS para mapearlo al refund."
    );
  } catch (e: any) {
