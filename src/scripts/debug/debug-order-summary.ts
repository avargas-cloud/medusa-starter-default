...
async function main() {
    ...
    for (const row of summaryResult.rows) {
        console.log(`#${row.display_id} (${row.status}) → current_order_total=${row.current_order_total} | original=${row.original_order_total} | accounting=${row.accounting_total}`)
    ...
    for (const row of itemsResult.rows) {
        ...
        console.log(`#${row.display_id} v${row.version} "${row.title}" → unit_price=${uprice}, qty=${qty}`)
    }
