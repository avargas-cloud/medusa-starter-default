...
export default async function myScript({ container }: ExecArgs) {
  ...
  const authHeaders = {
    Authorization:
      "Basic " +
      Buffer.from("admin@ecopowertech.com:vQY23WvG").toString("base64"),
  };
  ...
  try {
    ...
    const draftRes = await fetch(`${base}/admin/draft-orders`, {
      ...
      body: JSON.stringify({
        ...
        items: [
          {
            variant_id: "variant_01KFRNPMS8K91A2HQ49W5JRB88",
            quantity: 1,
            unit_price: 50,
          },
        ],
    ...
    const cvRes = await fetch(
      `${base}/admin/draft-orders/${draft_order.id}/convert-force`,
      {
        method: "POST",
        headers,
      }
