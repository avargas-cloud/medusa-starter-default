...
export default async function cleanupAndRenameSKUs({ container }: ExecArgs) {
  ...
  try {
    ...
    if (deleteProducts.length > 0) {
      console.log(
        `   ✓ Encontrados ${deleteProducts.length} productos para eliminar:\n`
      );
      ...
      console.log(
        `      • TOTAL registros: ${Object.values(deleteStats).reduce((a, b) => a + b, 0)}`
      );
    ...
    console.log(
      `   • ${Object.keys(SKU_OPERATIONS.rename).length} renombrados a procesar\n`
    );

    const renameResults = await processRenames(
      pool,
      SKU_OPERATIONS.rename,
      isDryRun
    );
...
async function getProductsBySku(
  pool: Pool,
  skus: string[]
): Promise<Array<{ id: string; sku: string; title: string }>> {
...
async function calculateDeletionStats(pool: Pool, productIds: string[]) {
  ...
  if (variantIds.length > 0) {
    const variantPlaceholders = variantIds
      .map((_, i) => `$${i + 1}`)
      .join(", ");
...
async function processRenames(
  ...
  for (const [oldSku, newSku] of Object.entries(renameMap)) {
    ...
    if (!isDryRun) {
      ...
      console.log(
        `      ✓ Renombrado: ${result.variantsRenamed} variantes, ${result.inventoryRenamed} inventory items`
      );
      results.push(result);
    } else {
