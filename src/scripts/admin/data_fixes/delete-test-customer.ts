...
export default async function deleteTestCustomer(container: MedusaContainer) {
  ...
  try {
    ...
    if (knex) {
      ...
      if (
        deletedIdentities &&
        deletedIdentities.rows &&
        deletedIdentities.rows.length > 0
      ) {
        authDeleted = true;
        authId = deletedIdentities.rows[0].id;
        console.log(
          `✅ Provider identities deleted: ${deletedIdentities.rows.length}`
        );
      }
    }
    ...
    if (customerId) {
      await knex("customer").where("id", customerId).del();
