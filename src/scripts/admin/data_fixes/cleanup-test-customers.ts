...
async function main() {
  ...
  console.log(
    `🔧 Mode: ${DRY_RUN ? "🔍 DRY RUN (no changes)" : "💾 LIVE (deleting)"}\n`
  );
  ...
  console.log(
    `📋 CANDIDATOS A ELIMINAR (${candidates.rowCount} cuentas sin QB ID):\n`
  );
  for (const c of candidates.rows) {
    const tag = c.is_legacy === "true" ? "[LEGACY]" : "[TEST]";
    console.log(
      `  ${tag} ${c.first_name} ${c.last_name} | ${c.email} | ${c.id}`
    );
  }
  ...
  if (DRY_RUN) {
    ...
    console.log(
      "   Para ejecutar: DRY_RUN=false npx tsx src/scripts/delete/cleanup-test-customers.ts"
    );
    await client.end();
    return;
  }
  ...
  await client.query(
    `DELETE FROM customer_address WHERE customer_id = ANY($1::text[])`,
    [ids]
  );
