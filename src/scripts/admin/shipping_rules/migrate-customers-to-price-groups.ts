...
export default async function migrateCustomersToPriceGroups({
  container,
}: any) {
  ...
  try {
    ...
    if (!retailGroup || !wholesaleGroup) {
      throw new Error(
        "Required customer groups not found. Run setup-price-tiers.ts first."
      );
    }
    ...
    const wholesaleCustomers = allCustomers.filter(
      (c: any) => c.metadata?.qb_price_level === "Wholesale"
    );
    const retailCustomers = allCustomers.filter(
      (c: any) =>
    ...
    for (const customer of wholesaleCustomers) {
      const inWholesaleGroup = customer.groups?.some(
        (g: any) => g.id === wholesaleGroup.id
      );
      ...
      if (inWholesaleGroup) {
        ...
        if (wholesaleMoved % 100 === 0) {
          log(
            `  ✓ Moved ${wholesaleMoved}/${wholesaleCustomers.length} to Wholesale group...`
          );
        }
      }
    }
    ...
    log(
      `  ✅ Wholesale: ${wholesaleMoved} moved, ${wholesaleAlready} already in group`
    );
    ...
    for (const customer of retailCustomers) {
      const inRetailGroup = customer.groups?.some(
        (g: any) => g.id === retailGroup.id
      );
      ...
      if (inRetailGroup) {
        ...
        if (retailMoved % 100 === 0) {
          log(
            `  ✓ Moved ${retailMoved}/${retailCustomers.length} to Retail group...`
          );
        }
      }
    }
    ...
    return {
      ...
      stats: {
        wholesale: {
          ...
          moved: wholesaleMoved,
          already: wholesaleAlready,
        },
        retail: {
          total: retailCustomers.length,
          moved: retailMoved,
          already: retailAlready,
        },
      },
    };
  } catch (error: any) {
