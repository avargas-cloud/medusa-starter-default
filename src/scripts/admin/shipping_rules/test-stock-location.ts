...
async function testStockLocation() {
  ...
  try {
    ...
    for (const row of result.rows) {
      ...
      if (allPresent) {
        console.log(
          `✅ All required UPS shipper fields present for "${row.name}"`
        );
        console.log(
          `   → UPS Rate API will use: ${shipperName}, ${shipperAddress}, ${shipperCity} ${shipperState} ${shipperZip} ${shipperCountry}`
        );
      } else {
