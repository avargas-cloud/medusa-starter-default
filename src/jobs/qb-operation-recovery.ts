...
function extractEditSequence(op: any): string | undefined {
    ...
    return (
        op.editSequence || r.EditSequence ||
...
async function applyRecoveryResult(
    ...
    switch (log.operation) {
        case "sales_order": {
            ...
            const patch: Record<string, any> = { qb_so_txn_id: txnId, qb_sync_status: "synced" }
            ...
            if (editSequence) await upsertEditSequenceCache(client, "sales_order", txnId, editSequence)
            logger.info(`${LOG_PREFIX} ✅ Recovered sales_order for order ${log.order_id} → TxnID: ${txnId}`)
            break
        }

        case "estimate": {
            if (!log.draft_order_id) break
            const patch: Record<string, any> = { qb_estimate_txn_id: txnId, qb_sync_status: "synced" }
            ...
            if (editSequence) await upsertEditSequenceCache(client, "estimate", txnId, editSequence)
            logger.info(`${LOG_PREFIX} ✅ Recovered estimate for draft ${log.draft_order_id} → TxnID: ${txnId}`)
            break
        }

        case "payment": {
            if (!log.order_id) break
            const patch: Record<string, any> = { qb_txn_id: txnId, qb_sync_status: "synced" }
            ...
            `, [log.order_id, JSON.stringify(patch)])
            logger.info(`${LOG_PREFIX} ✅ Recovered payment for order ${log.order_id} → TxnID: ${txnId}`)
            break
        }

        case "invoice": {
            if (!log.order_id) break
            const patch: Record<string, any> = { qb_invoice_txn_id: txnId, qb_sync_status: "synced" }
            if (refNumber) patch.qb_ref_number = refNumber
            await client.query(`
                UPDATE pos_invoice
                SET metadata = COALESCE(metadata,'{}') || $2::jsonb
            ...
            `, [log.order_id, JSON.stringify(patch)])
            if (editSequence) await upsertEditSequenceCache(client, "invoice", txnId, editSequence)
            logger.info(`${LOG_PREFIX} ✅ Recovered invoice for order ${log.order_id} → TxnID: ${txnId}`)
            break
        }

        case "sales_receipt": {
            if (!log.order_id) break
            const patch: Record<string, any> = { qb_sales_receipt_txn_id: txnId, qb_sync_status: "synced" }
            if (refNumber) patch.qb_sales_receipt_ref_number = refNumber
            await client.query(
                `UPDATE "order" SET metadata = COALESCE(metadata,'{}') || $2::jsonb WHERE id = $1`,
                [log.order_id, JSON.stringify(patch)]
            )
            logger.info(`${LOG_PREFIX} ✅ Recovered sales_receipt for order ${log.order_id} → TxnID: ${txnId}`)
            break
        }

        default:
            logger.info(`${LOG_PREFIX} ℹ️  No entity update handler for operation '${log.operation}' (TxnID: ${txnId})`)
    }
}
...
export default async function qbOperationRecovery(container: MedusaContainer) {
    ...
    try {
        ...
        logger.info(`${LOG_PREFIX} Found ${stuckLogs.length} stuck operation(s) — resuming...`)

        for (const log of stuckLogs) {
            try {
                const statusRes = await bridgeFetch("GET", `/api/sync/status/${log.qb_operation_id}`)
                const op = statusRes?.operation

                if (!op) continue

                if (op.status === "completed") {
                    const txnId: string | undefined = op.txnId || op.result?.TxnID || op.listId || op.result?.ListID
                    const refNumber: string | undefined = op.refNumber || op.result?.RefNumber
                    ...
                    // Update entity
                    await applyRecoveryResult(client, log, txnId, refNumber, editSequence, logger)
                    ...
                    logger.warn(`${LOG_PREFIX} ⚠️  Operation ${log.qb_operation_id} failed in bridge (${log.operation})`)

                } else {
                    // still pending/processing in bridge — leave it, check next run
                    logger.info(`${LOG_PREFIX} ⏳ Operation ${log.qb_operation_id} still ${op.status} — will retry`)
                }

            } catch (err: any) {
                logger.warn(`${LOG_PREFIX} ⚠️  Could not recover ${log.qb_operation_id}: ${err.message}`)
            }
        }
