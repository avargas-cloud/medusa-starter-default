...
function buildEmailHtml(
  ...
  const rows = results
    .map((r) => {
      ...
      const orderId = r.entry.order_display_id
        ? `#${r.entry.order_display_id}`
        : "—";
      ...
      const errorText = r.errorMsg
        ? `<br><small style="color:#ef4444">${r.errorMsg}</small>`
        : "";
...
<div style="max-width:760px;margin:0 auto">
    ...
    <div style="background:#fff;border:1px solid #e2e8f0;border-top:none;border-radius:0 0 12px 12px;overflow:hidden">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:13px">
            ...
            : `<p style="text-align:center;color:#94a3b8;padding:32px">No QB operations recorded in the last 24 hours.</p>`
        }
    </div>
...
export default async function qbNightlyVerifyHandler(
  ...
  try {
    ...
    console.log(
      `${TAG} 🏁 Done. Confirmed: ${confirmed} | Failed: ${failed} | Pending: ${pending} | Expired: ${expired}`
    );
  } catch (err: any) {
