/**
 * src/api/pos/document-templates/route.ts
 * GET  /pos/document-templates   — list templates (pos_user auth via storeCors)
 * POST /pos/document-templates   — create template
 *
 * Mirror of /admin/document-templates but under the /pos/ prefix so that
 * storeCors applies (ecopowertech-store-pos.vercel.app is in storeCors).
 * POS users authenticate with pos_user tokens which satisfy storeCors auth.
 */
...
export const GET = async (req: MedusaRequest, res: MedusaResponse) => {
    ...
    try {
        ...
        console.error("[POS/DocumentTemplates] GET list error:", err)
    ...
    try {
        ...
        const result = await db.query(
            `INSERT INTO pos_document_template
             ...
             RETURNING *`,
            [
                id, body.name, body.doc_type, body.is_default ?? false,
                body.thumbnail ?? null,
        ...
        console.error("[POS/DocumentTemplates] POST create error:", err)
