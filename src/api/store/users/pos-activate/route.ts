...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
    ...
    // ── 1. Verify invite JWT ───────────────────────────────────────────────────
    let decoded: { email: string; pos_user_id: string; type: string; first_name?: string; last_name?: string }
    try {
        decoded = jwt.verify(token, JWT_SECRET) as typeof decoded
    } catch (err: any) {
        if (err.name === 'TokenExpiredError') {
            return res.status(400).json({ error: 'Invite link has expired. Ask an admin to resend the invite.' })
        }
    ...
    try {
        ...
        if (!authResult.success || !authResult.authIdentity) {
            ...
            return res.status(400).json({ error: authResult.error ?? 'Failed to create credentials.' })
        }

        logger?.info?.(`[POS-ACTIVATE] Auth identity created: ${authResult.authIdentity.id}`)
        ...
        if (!userId) {
            const existingUsers = await userModule.listUsers({ email: decoded.email }).catch(() => [])
