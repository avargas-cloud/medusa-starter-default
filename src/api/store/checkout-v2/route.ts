...
async function resolveShippingOptionId(
    ...
    try {
        ...
        if (!options || options.length === 0) {
            console.warn("[checkout-v2] resolveShippingOptionId: No shipping options found")
            return null
        }
...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
    ...
    try {
        ...
        try {
            ...
            if (cartRes.ok) {
                ...
                if (cartData?.total && cartData.total > 0) {
                    ...
                    console.log(`[checkout-v2] 💰 Authoritative total (Store API): ${amountCents} cents = $${(amountCents / 100).toFixed(2)}`)
                }
            } else {
                console.warn(`[checkout-v2] Store API cart fetch returned ${cartRes.status}`)
            }
        } catch (err: any) {
            console.warn("[checkout-v2] Store API cart total fetch failed:", err.message)
        }
        ...
        console.log(`[checkout-v2] 🔍 orderResult properties:`, Object.keys(orderResult || {}))
