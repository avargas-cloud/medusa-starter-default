...
export const GET = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  try {
    ...
    const unorderedSubcats =
      allSubcategories?.filter((s: any) => !subcategoryOrder.includes(s.id)) ||
      [];
    ...
    const unorderedProducts =
      allProducts?.filter((p: any) => !productOrder.includes(p.id)) || [];
    ...
    console.error(
      `[STORE-SORTING] Error fetching sorting for category ${categoryId}:`,
