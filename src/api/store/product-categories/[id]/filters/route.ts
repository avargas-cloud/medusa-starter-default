...
export const GET = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  try {
    ...
    const getFiltersWithInheritance = async (
      categoryId: string,
    ...
    if (activeFilters.length > 0) {
      ...
      const attributeModuleService: any =
        req.scope.resolve("productAttributes");
      ...
      filters = activeFilters
        .map((filterConfig) => {
          const attributeKey = attributeKeys.find(
            (k: any) => k.id === filterConfig.attribute_id
          );
          ...
          return {
            ...
            filter_type:
              attributeKey.filter_type || filterConfig.type || "checkbox", // Use attribute config or fallback
      ...
      const inherited =
        categoryResult.data[0].metadata?.filters_inherited || false;
      ...
      filters = filters.map((filter) => {
        ...
        products.forEach((product: any) => {
          ...
          const productAttributeValueIds = productLinks.map(
            (l: any) => l.attribute_value_id
          );

          // Find attribute values that match THIS filter's handle
          const matchingValues = allAttributeValues.filter(
            (av: any) =>
    ...
    return res.json({
      ...
      filters,
      inherited:
        category.metadata?.filter_config?.override_inheritance !== true,
    });
  } catch (error: any) {
