  ...
  try {
    ...
    try {
      ...
      if (
        stores &&
        stores.length > 0 &&
        Array.isArray(stores[0]!.metadata?.non_wholesale_prefixes)
      ) {
        activeStoreConfig = stores[0]!.metadata!
          .non_wholesale_prefixes as string[];
      }
    } catch (err: any) {
      console.error(
        "[Batch Prices] ⚠️ Error fetching store metadata:",
        err.message
      );
    }
    ...
    const dynamicPricingEnabled =
      process.env.ENABLE_DYNAMIC_PRICING !== "false";
    ...
    const { data: products } = await query.graph({
      entity: "product",
      fields: ["id", "title", "variants.*", "variants.price_set.id"],
      filters: {
        id: product_ids,
      },
    });
    ...
    console.log(
      `[Batch Prices] ✅ First calculatedPrice:`,
      JSON.stringify(calculatedPrices[0], null, 2)
    );
    ...
    for (const data of priceSetData) {
      const calculatedPrice = calculatedPrices.find(
        (p: any) => p.id === data.priceSetId
      );

      if (!calculatedPrice) {
        console.warn(
          `[Batch Prices] No price found for price_set ${data.priceSetId}`
        );
        continue;
      }
