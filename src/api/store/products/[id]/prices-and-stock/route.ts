  ...
  try {
    ...
    try {
      ...
      if (
        stores &&
        stores.length > 0 &&
        (stores[0] as any).metadata?.non_wholesale_prefixes
      ) {
        activeStoreConfig = (stores[0] as any).metadata
          .non_wholesale_prefixes as string[];
      }
    } catch (err: any) {
      console.error(
        "[PRICES-STOCK] ⚠️ Error fetching store metadata:",
        err.message
      );
    }
    ...
    if (priceSetIds.length > 0) {
      try {
        ...
        console.log(
          `[PRICES-STOCK] ✅ Calculated ${calculatedPrices.length} prices`
        );
        ...
        console.error(
          `[PRICES-STOCK] ❌ Price calculation error:`,
          error.message
        );
      }
    }
