 ...
 *
 * Extends default Medusa single product endpoint to include product attributes.
 * Uses batch queries to avoid N+1 problem (even for single product).
 */
export const GET = async (req: MedusaRequest, res: MedusaResponse) => {
  try {
    ...
    console.log(
      `[PRODUCT-ATTRS] 📦 Fetching attributes for product: ${product.title}`
    );
    ...
    console.log(
      `[PRODUCT-ATTRS] 🔗 Found ${allLinks.length} links, ${allAttributeValues.length} unique values`
    );
    ...
    allLinks.forEach((link: any) => {
      const attributeValue = allAttributeValues.find(
