...
export const GET = async (req: MedusaRequest, res: MedusaResponse) => {
  try {
    ...
    const breadcrumbs =
      originalProduct?.metadata?.main_category_breadcrumbs || null;
    ...
    if (priceSetIds.length > 0) {
      ...
      console.log(
        `[WITH-PRICES] 💵 Calculated ${calculatedPrices.length} prices (customer-aware)`
      );
    }

    // Create variants with calculated prices
    const variantsWithPrices = variants.map((variant: any) => {
      const priceData = calculatedPrices.find(
        (p) => p.id === variant.price_set?.id
      );
