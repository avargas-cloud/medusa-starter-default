...
async function resolveShippingOptionId(
  ...
  try {
    ...
    if (!options || options.length === 0) {
      console.warn(
        "[fast-checkout] resolveShippingOptionId: No shipping options found"
      );
      return null;
    }
  ...
  try {
      ...
      await updateCartWorkflow(req.scope).run({
        input: {
          ...
          ...(medusaBillingAddress
            ? { billing_address: medusaBillingAddress }
            : {}),
        },
      });
      console.log(
        `[fast-checkout] ✅ Cart updated (email=${email}, billing=${medusaBillingAddress?.city ?? "n/a"})`
      );
    }
    ...
    try {
      ...
      if (cartRes.ok) {
        ...
        if (cartData?.total && cartData.total > 0) {
          ...
          console.log(
            `[fast-checkout] 💰 Authoritative total (Store API): ${amountCents} cents = $${(amountCents / 100).toFixed(2)}`
          );
        }
      } else {
        console.warn(
          `[fast-checkout] Store API cart fetch returned ${cartRes.status}`
        );
      }
    } catch (err: any) {
      console.warn(
        "[fast-checkout] Store API cart total fetch failed:",
        err.message
      );
    }
    ...
    if (cartItemsForValidation.length > 0) {
      try {
        ...
        for (const item of cartItemsForValidation) {
          const rawUpValue = parseFloat(
            (item.raw_unit_price as any)?.value ?? "0"
          );
          const rawQtyValue = parseFloat(
            (item.raw_quantity as any)?.value ?? "0"
          );
          ...
          if (needsPriceFix || needsQtyFix) {
            ...
            rawFixes.push(patch);
            console.log(
              `[fast-checkout] 🔧 Cart raw fix needed: "${item.title || item.variant_id}" price=${item.unit_price} qty=${item.quantity}`
            );
          }
        }
        if (rawFixes.length > 0) {
          ...
          console.log(
            `[fast-checkout] ✅ Fixed ${rawFixes.length} cart item(s) raw BigNumber fields → completeCartWorkflow will compute correct totals`
          );
        } else {
          console.log(
            `[fast-checkout] ✅ Cart items raw_unit_price already correct — no fix needed`
          );
        }
      } catch (rawFixErr: any) {
        // Non-fatal: log and continue. The updateOrderItem in step 7b will compensate.
        console.warn(
          `[fast-checkout] ⚠️ Cart raw BigNumber fix failed (non-fatal): ${rawFixErr.message}`
        );
      }
    }
    ...
    try {
      ...
      const stockMatch = msg.match(
        /Not enough stock available for item (\S+)/i
      );
      if (stockMatch) {
        ...
        try {
          ...
          const cartDetail = await cartModule
            .retrieveCart(cartId, {
              relations: [
                "items",
                "items.variant",
          ...
          const matchedItem = cartDetail?.items?.find((i: any) =>
            i.variant?.inventory_items?.some(
              (ii: any) => ii.inventory_item_id === itemId || ii.id === itemId
            )
          );
          if (matchedItem) {
            productName =
              matchedItem.title ?? matchedItem.variant?.title ?? productName;
        ...
        console.warn(
          `[fast-checkout] ⚠️ Stock error for "${productName}"${skuPart}: ${msg}`
        );
        return res.status(400).json({
    ...
    console.log(`\n================================================`);
    console.log(
      `Evento 0: fast-checkout FINALIZADO MÓDULO CART (Antes de retorno al cliente)`
    );
    console.log(`================================================`);
    ...
    if (orderId) {
      try {
        ...
        if (seq) {
          ...
          console.log(
            `[fast-checkout] ✅ Document number assigned: ${documentNumber}`
          );
        }
        ...
        console.log(
          `[fast-checkout] ✅ Order metadata tagged (Placed Online / Online Order / Web)`
        );
      } catch (metaErr: any) {
        console.warn(
          `[fast-checkout] ⚠️ Could not tag order metadata (non-fatal): ${metaErr.message}`
        );
      }
    }
