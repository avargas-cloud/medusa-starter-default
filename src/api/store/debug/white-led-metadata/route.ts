...
export const GET = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  try {
    ...
    return res.json({
      ...
      stats: {
        available_attributes_count:
          category.metadata?.available_attributes?.length || 0,
        filters_count: category.metadata?.filters?.length || 0,
        filters_metadata_total_products:
          category.metadata?.filters_metadata?.total_products || null,
      },
    });
  } catch (error: any) {
