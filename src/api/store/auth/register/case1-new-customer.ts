...
export async function handleNewCustomerRegistration(
    ...
    try {
        ...
        console.log(`✅ Auth identity created with hashed password: ${authIdentity.id}`)

        // Step 2: Use native Medusa workflow to create customer account
        const { createCustomerAccountWorkflow } = await import('@medusajs/core-flows')
        ...
        console.log('🔍 DEBUG - Token payload:', JSON.stringify({
        ...
        return res.status(500).json({
            error: "Registration failed",
            details: registrationError instanceof Error ? registrationError.message : 'Unknown error'
        })
    }
}
