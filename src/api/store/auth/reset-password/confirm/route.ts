...
export const POST = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  try {
    ...
    const resetExpiresRaw = customer.metadata?.reset_expires as
      | string
      | undefined;
    const resetExpiresMs = resetExpiresRaw
    ...
    if (!emailpassProvider) {
      ...
      console.log(
        `ℹ️  No emailpass provider found for ${customer.email} — creating one (dual auth)`
      );
    ...
    if (emailpassProvider.auth_identity_id !== matchingIdentity.id) {
      const [currentAuthIdentity] = await sql`
                ...
                LIMIT 1
            `;
      const isAdminAuthIdentity = !!(currentAuthIdentity?.app_metadata as any)
        ?.user_id;
      ...
      if (isAdminAuthIdentity) {
        ...
        const existingCustomerId = (currentAuthIdentity?.app_metadata as any)
          ?.customer_id;
        if (existingCustomerId !== customer.id) {
          console.log(
            `🔧 Admin+customer same email: injecting customer_id into admin auth_identity`
          );
          await sql`
        ...
        console.log(
          `🔧 Re-linking emailpass provider to correct auth_identity: ${matchingIdentity.id}`
        );
        await sql`
