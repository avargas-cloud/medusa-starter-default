...
export const GET = async (req: MedusaRequest, res: MedusaResponse) => {
    ...
    try {
        await db.connect()
        const result = await db.query('SELECT * FROM pos_document_template WHERE id = $1', [req.params.id])
...
export const DELETE = async (req: MedusaRequest, res: MedusaResponse) => {
    ...
    try {
        await db.connect()
        const check = await db.query('SELECT id FROM pos_document_template WHERE id = $1', [req.params.id])
        if (!check.rows[0]) return res.status(404).json({ error: "Template not found" })
        await db.query('DELETE FROM pos_document_template WHERE id = $1', [req.params.id])
