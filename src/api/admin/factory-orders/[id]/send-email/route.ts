/**
 * POST /admin/factory-orders/:id/send-email
 *
 * Generates the selected Factory Order template as a PDF and sends it to the
 * vendor (the trading partner / agent that places the order at the factory).
 * Mirrors purchase-orders/send-email; the POS frontend supplies the print-page
 * localStorage payload because the FO print route is frontend-template based.
 */
...
interface FactoryOrderEmailRecord {
  id: string;
  number?: string | null;
...
function foDocumentNumber(value: string | number): string {
  return String(value).replace(/^FO-/i, "");
}
...
function buildPreviewHtml(
  fo: FactoryOrderEmailRecord,
  vendor: VendorEmailRecord | null
) {
  const foNumber = foDocumentNumber(fo.number ?? fo.draft_number ?? fo.id);
  const vendorName =
    vendor?.company_name ??
    vendor?.full_name ??
    vendor?.name ??
    fo.vendor_name_snapshot ??
    "Vendor";
  const total = new Intl.NumberFormat("en-US", {
    style: "currency",
    currency: (fo.currency_code ?? "USD").toUpperCase(),
  }).format((fo.total_cents ?? 0) / 100);
...
<div class="box">
<h2>Factory Order ${escapeHtml(foNumber)}</h2>
<p>Vendor: ${escapeHtml(vendorName)}</p>
<p>Date: ${escapeHtml(formatDate(fo.ordered_at))}</p>
...
export async function GET(req: AuthenticatedMedusaRequest, res: MedusaResponse) {
  ...
  const fo = (await service
    ...
    .catch(() => null)) as unknown as FactoryOrderEmailRecord | null;
  if (!fo) return void res.status(404).send("Factory order not found");
  ...
  try {
    ...
    vendor = await catalog.retrieveQbVendor(fo.vendor_id);
  } catch {
    vendor = null;
  }

  res.setHeader("Content-Type", "text/html");
  res.send(buildPreviewHtml(fo, vendor));
}

export async function POST(req: AuthenticatedMedusaRequest, res: MedusaResponse) {
  const { id } = req.params as { id: string };
  const body = (req.body ?? {}) as SendFoEmailBody;
  const service = getFactoryOrdersService(req);
  ...
  if (!fo) {
    return void res
      .status(404)
      .json({ success: false, message: "Factory order not found" });
  }
  ...
  if (!vendorEmail) {
    return void res
      .status(400)
      .json({ success: false, message: "No vendor email found" });
  }
  ...
  const displayId = body.displayId ?? fo.number ?? fo.draft_number ?? fo.id;
  const foNumber = foDocumentNumber(displayId);
  const vendorName =
    vendor?.company_name ??
    vendor?.full_name ??
    vendor?.name ??
  ...
  const resolvedTemplateId =
    body.templateId ?? (await fetchDefaultTemplateId("factory_order"));
  ...
  try {
    if (resolvedTemplateId && body.localStoragePayloads) {
      ...
      if (displayId) params.set("displayId", foNumber);
      const printUrl = `${posUrl}/print/fo/${resolvedTemplateId}?${params}`;
    ...
    console.error("[fo-send-email] PDF generation failed:", err);
  }
  ...
  const bodyHtml = body.emailBody
    ...
    : `<p>Dear ${escapeHtml(vendorName)},</p><p>Please find attached Factory Order #${escapeHtml(foNumber)}. Kindly review the details and confirm receipt at your earliest convenience.</p>`;
...
${bodyHtml}
<div class="box">
<div><b>FO #:</b> ${escapeHtml(foNumber)}</div>
<div><b>Date:</b> ${escapeHtml(formatDate(fo.ordered_at))}</div>
...
<p style="margin-top:40px;font-size:12px;color:#94a3b8;text-align:center">Ecopowertech Inc. · 2760 W 84th St, Unit 4, Hialeah, FL 33016</p>
  ...
  try {
    await sendMail({
      ...
      subject: body.subject ?? `Factory Order ${foNumber} from EcoPowerTech`,
      html,
    ...
    console.error("[fo-send-email] sendMail failed:", message);
