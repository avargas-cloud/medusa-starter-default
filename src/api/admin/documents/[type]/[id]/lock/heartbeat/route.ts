...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  const token =
    (req.headers['x-lock-token'] as string) ||
    (req.body as any)?.token

  if (!token) {
    return res.status(400).json({ error: 'x-lock-token header or body.token required' })
  }

  const existing = await redis.get(key)
  if (!existing) {
    return res.status(410).json({ error: 'Lock expired — document is now unlocked' })
  }
