...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  if (result === "OK") {
    return res
      .status(200)
      .json({ locked: true, token: lockToken, message: "Lock acquired" });
  }
...
export async function DELETE(req: MedusaRequest, res: MedusaResponse) {
  ...
  if (isForceRequest) {
    ...
    const clientCode =
      (req.headers["x-unlock-code"] as string) || (req.body as any)?.unlockCode;

    if (!serverCode) {
      return res
        .status(503)
        .json({ error: "Force unlock not configured on server" });
    }
