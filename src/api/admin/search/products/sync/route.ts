...
export const POST = async (req: MedusaRequest, res: MedusaResponse) => {
  try {
    ...
    const dbLastUpdate = latestProduct
      ? new Date(latestProduct.updated_at)
      : new Date();

    console.log(
      `🔍 [Sync Check] DB Count: ${dbCount} | Meili Count: ${meiliCount}`
    );
    console.log(
      `🔍 [Sync Check] DB Last Upd: ${dbLastUpdate.toISOString()} | Meili: ${meiliLastUpdate.toISOString()}`
    );
    ...
    console.log(
      `🔍[Sync Status] Count Match: ${isCountSync}, Time Diff: ${timeDiff}ms, Time Sync: ${isTimeSync}, Force: ${force}`
    );
