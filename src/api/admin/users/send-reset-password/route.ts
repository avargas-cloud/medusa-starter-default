  ...
  try {
    ...
    try {
      ...
      req.scope
        .resolve("logger")
        ?.warn?.(
          `[ResetPW] generateResetPasswordToken failed: ${authErr.message}`
        );
      return res
        .status(404)
        .json({ error: "User not found or provider not available" });
    }
    ...
    req.scope
      .resolve("logger")
      ?.info?.(`[ResetPW] ✅ Reset email sent to ${email}`);
    return res
      .status(200)
      .json({ success: true, message: `Reset email sent to ${email}` });
  } catch (err: any) {
