...
export async function POST(
  ...
  if (!JWT_SECRET) {
    logger.error(
      "[POS invite] JWT_SECRET no esta configurado - operacion rechazada"
    );
  ...
  try {
    ...
    if (existing) {
      ...
      logger?.info?.(
        `[POS-INVITE] Created pos_user ${posUser.id} for ${email}`
      );
    }
    ...
    try {
      ...
      logger?.warn?.(
        `[POS-INVITE] ⚠️ mailer failed (${emailErr?.message}). Returning activate_url.`
      );
    }
