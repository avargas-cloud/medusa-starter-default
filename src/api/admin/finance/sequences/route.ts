    ...
    try {
        for (const seq of VALID_SEQUENCES) {
            try {
                ...
                results[seq] = result.rows[0].last_value || result.rows[0].LAST_VALUE || null
            } catch (e: any) {
    ...
    try {
        ...
        for (const [seq, newValue] of Object.entries(body)) {
            if (!VALID_SEQUENCES.includes(seq)) {
                return res.status(400).json({ error: `Invalid sequence identifier: ${seq}` })
            }
            if (typeof newValue !== "number" || newValue < 1) {
                return res.status(400).json({ error: `Invalid value for sequence ${seq}. Must be a positive integer.` })
            }
            ...
            console.log(`[Sequences] Manually updated ${seq} to next value ${newValue}`)
        }
