...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
    ...
    const { void_reason, voided_by } = req.body as { void_reason?: string; voided_by?: string }
    ...
    try {
        ...
        if (["refunded", "partial_refunded"].includes(payment.status)) {
            return res.status(400).json({ error: "Cannot void a payment that has already been refunded" })
        }

        if ((payment as any).source === "web") {
            return res.status(403).json({ error: "Cannot void web checkout payments via the POS" })
        }
        ...
        for (const app of activeApplications) {
            ...
            if (app.invoice_id) {
                try {
                    ...
                    const invoice = await invoiceService.retrievePosInvoice(app.invoice_id)
                    const balanceDue = Math.max(0, Number(invoice.total) - totalInvoicePaid)
                    const newStatus =
                        totalInvoicePaid <= 0 ? "issued" : balanceDue <= 0 ? "paid" : "partial"

                    await invoiceService.updatePosInvoices(
                        { id: app.invoice_id },
                        { amount_paid: totalInvoicePaid, balance_due: balanceDue, status: newStatus }
                    )
            ...
            setTimeout(async () => {
                try {
                    await handlePosPaymentVoided({
                        event: { name: "pos.payment.voided", data: { payment_id: paymentId } },
                        container: req.scope as any,
                        pluginOptions: {},
                    })
                } catch (execErr: any) {
                    logger.error(`[void-payment] QB void handler failed: ${execErr.message}`)
                }
            }, 100)
        }
