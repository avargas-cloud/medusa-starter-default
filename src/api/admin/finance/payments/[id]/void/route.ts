...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  try {
    ...
    // 3.5. referential_deposit is NOT decremented here any more — the
    // `recompute_order_money()` (migration 1781600000000) derives it from
    // the order_money_projection authority when this payment's
    // applications are voided, in the same transaction.
    //
    // This path subtracted only when `locked_order_id` was set. A payment
    // applied to an order WITHOUT that pointer left the field standing —
    // one of the ways the mirror got ahead of the money.
