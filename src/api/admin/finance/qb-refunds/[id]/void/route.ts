...
export const POST = async (req: MedusaRequest, res: MedusaResponse) => {
    ...
    const isRefundable = payment.type === "refund" ||
        (payment.type !== "refund" && ["refunded", "partial_refunded"].includes(payment.status))
    if (!isRefundable) {
    ...
    return res.json({ success: true, voided: true, check_txn_id: checkTxnId ?? null })
}
