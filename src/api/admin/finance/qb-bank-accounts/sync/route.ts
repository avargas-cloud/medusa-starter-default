...
async function pollOperation(operationId: string, maxAttempts = 20, intervalMs = 3000): Promise<any> {
    for (let i = 0; i < maxAttempts; i++) {
        await new Promise(r => setTimeout(r, intervalMs))
        const res = await fetch(`${BRIDGE_URL}/api/sync/status/${operationId}`, { headers: HEADERS })
        const data = await res.json()
        const status = data.operation?.status
        if (status === "completed") return data.operation.result
        if (status === "failed") throw new Error(data.operation.error || "QB operation failed")
    }
...
export const POST = async (req: MedusaRequest, res: MedusaResponse) => {
    ...
    try {
        ...
        const initRes = await fetch(`${BRIDGE_URL}/api/accounts`, { headers: HEADERS })
        const initData = await initRes.json()
        if (!initData.operationId) throw new Error("Bridge did not return operationId")

        // 2. Poll for result
        const result = await pollOperation(initData.operationId)

        // 3. Extract bank accounts from QBXML response
        const accountRets = result?.QBXML?.QBXMLMsgsRs?.AccountQueryRs?.AccountRet ?? []
        const bankAccounts = accountRets.filter((a: any) => a.AccountType === "Bank")

        if (bankAccounts.length === 0) {
            return res.json({ success: true, synced: 0, message: "No bank accounts found in QuickBooks" })
        }
        ...
        for (const acct of bankAccounts) {
            ...
            if (!existing) {
                await financeService.createQbBankAccounts({ name, list_id: listId, type: "Bank" })
                created++
        ...
        logger.info(`[QB Bank Accounts Sync] created=${created} updated=${updated} total=${bankAccounts.length}`)
