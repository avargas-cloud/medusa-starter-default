...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
    ...
    try {
        ...
        const dbApplications = await financeService.listPaymentApplications({ id: applicationId }, {
            relations: ['payment']
        })
        ...
        if (application.voided_at) {
            return res.status(400).json({ error: 'This payment application is already voided' })
        }
        ...
        if (application.payment.source === 'web') {
            return res.status(403).json({ error: 'Cannot void web checkout applications via the finance module.' })
        }
        ...
        const currentPaymentDesc = await financeService.retrieveCustomerPayment(application.payment.id, {
            relations: ['applications']
        })
        
        const activeApplications = currentPaymentDesc.applications.filter((app: any) => !app.voided_at)
        const totalStillApplied = activeApplications.reduce((sum: number, app: any) => sum + Number(app.amount_applied), 0)
        
        const newPaymentStatus = totalStillApplied === 0 ? 'available' : 'partially_applied'
        ...
        if (application.invoice_id) {
            const invoice = await invoiceService.retrievePosInvoice(application.invoice_id)
            if (invoice) {
                ...
                const allInvoicePayments = await invoiceService.listInvoicePayments({ invoice_id: application.invoice_id })
                ...
                await invoiceService.updatePosInvoices(
                    ...
                    { 
                        ...
                        status: newInvoiceStatus === 'partial' && totalInvoicePaid === 0 ? 'issued' : newInvoiceStatus 
                    }
                )
            }
        }
