...
export async function GET(_req: MedusaRequest, res: MedusaResponse): Promise<void> {
    ...
    try {
        ...
        const getOptions = (name: string) =>
            Array.from(new Set(rows.filter(r => r.field_name === name || r.field_name === (name === 'Payment Terms' ? 'Terms' : name)).map(r => r.value)))

        const getSalesReps = () => {
            return rows.filter(r => r.context === 'Global' && r.field_name === 'Sales Rep User')
                .map(r => {
