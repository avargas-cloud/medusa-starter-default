...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
    ...
    if (!amount || amount <= 0) return res.status(400).json({ error: 'amount must be positive (in cents)' })
    ...
    try {
        ...
        if (payment.status === 'voided') return res.status(400).json({ error: 'Cannot apply a voided payment' })
        if (payment.status === 'applied') return res.status(400).json({ error: 'Payment has already been fully applied' })
        ...
        if (amount > available) {
            return res.status(400).json({ error: `Amount (${amount}) exceeds available balance (${available})` })
        }

        // 2. Load invoice
        const invoice = await invoiceService.retrievePosInvoice(invoice_id)
        if (!invoice) return res.status(404).json({ error: 'Invoice not found' })
        if (invoice.status === 'voided') return res.status(400).json({ error: 'Cannot apply to a voided invoice' })
        ...
        const allInvPayments = await invoiceService.listInvoicePayments({ invoice_id })
        const totalPaid = allInvPayments.reduce((s: number, p: any) => s + Number(p.amount), 0) + amount
        const balanceDue = Math.max(0, Number(invoice.total) - totalPaid)
        await invoiceService.updatePosInvoices(
            { id: invoice_id },
            { amount_paid: totalPaid, balance_due: balanceDue, status: balanceDue <= 0 ? 'paid' : 'partial' }
        )
