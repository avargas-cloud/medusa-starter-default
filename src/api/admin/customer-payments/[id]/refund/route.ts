...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
    ...
    try {
        ...
        const activeApps = ((payment as any).applications ?? []).filter((a: any) => !a.voided_at)
        const alreadyApplied = activeApps.reduce((s: number, a: any) => s + Number(a.amount_applied ?? 0), 0)
        const alreadyRefunded = Number((payment as any).metadata?.refund_amount ?? 0)
        const available = Math.max(0, Number(payment.amount) - alreadyApplied - alreadyRefunded)
        ...
        return res.json({ payment: updated, refunded: refundAmount, full_refund: isFullRefund })
    } catch (err: any) {
