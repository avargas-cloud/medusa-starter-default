...
const VALID_METHODS = ['cash', 'check', 'card', 'ach', 'zelle', 'credit_memo', 'stripe', 'authorize_net', 'other']

// QB FullName for each pos_payment_method value (used in PaymentMethodRef)
const QB_PAYMENT_METHOD_NAMES: Record<string, string> = {
    cash: 'Cash', visa: 'Visa', mastercard: 'Mastercard', discover: 'Discover',
    amex: 'American Express', capital_one: 'Capital One', debit_card: 'Debit Card',
    check: 'Check', money_order: 'Check', checking_account: 'ACH / Bank Transfer',
    e_check: 'ACH / Bank Transfer', transfer: 'ACH / Bank Transfer',
    wire_transfer: 'Wire Transfer', zelle: 'Zelle', paypal: 'Other',
}
...
export async function PATCH(req: MedusaRequest, res: MedusaResponse) {
    ...
    try {
        ...
        if (pos_payment_method !== undefined && pos_payment_method !== meta.pos_payment_method) {
            const qbMethodName = QB_PAYMENT_METHOD_NAMES[pos_payment_method] ?? pos_payment_method
            ...
            if (isSalesReceiptPayment && orderId) {
                ...
                ;(async () => {
                    try {
                        ...
                        if (srTxnId && editSeq) {
                            const { bridgeFetch } = require('../../../../lib/quickbooks/client/core')
                            const modResp = await bridgeFetch('PUT', `/api/sales-receipts/${srTxnId}`, {
                ...
                if (qbTxnId && qbTxnId !== 'SYNCED_VIA_RECEIPT') {
                    ;(async () => {
                        try {
                            ...
                            let editSeq: string | undefined
                            const qResp = await bridgeFetch('GET', `/api/payments/${qbTxnId}`)
                            if (qResp?.operationId) {
                                const raw = await pollRawOperationResult(qResp.operationId)
                                const ret = raw?.QBXML?.QBXMLMsgsRs?.ReceivePaymentQueryRs?.ReceivePaymentRet
                            ...
                            if (editSeq) {
                                const modResp = await bridgeFetch('PUT', `/api/payments/${qbTxnId}`, {
                                    EditSequence: editSeq,
                                    paymentMethod: qbMethodName,
                                })
                                req.scope.resolve('logger')?.info?.(
                                ...
                                if (modResp?.operationId) {
                                    try {
                                        const modRaw = await pollRawOperationResult(modResp.operationId)
                                        const modRet = modRaw?.QBXML?.QBXMLMsgsRs?.ReceivePaymentModRs?.ReceivePaymentRet
                                            ?? modRaw?.QBXMLMsgsRs?.ReceivePaymentModRs?.ReceivePaymentRet
