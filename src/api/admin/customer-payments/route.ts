...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
    ...
    try {
        const payments = await financeService.listCustomerPayments(
            {},
            { order: { received_at: 'DESC' }, relations: ['applications'] }
        )
        ...
        const result = enriched.map((p: any) => {
            ...
            const amountApplied = activeApplications.reduce((sum: number, a: any) => sum + Number(a.amount_applied ?? 0), 0)
            const availableBalance = Math.max(0, Number(p.amount) - amountApplied)
            return { ...p, amount_applied: amountApplied, available_balance: availableBalance }
        })
...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
    const financeService = req.scope.resolve(FINANCE_MODULE)
    const {
        customer_id, amount, method, reference, notes, received_at, created_by
    } = req.body as any
