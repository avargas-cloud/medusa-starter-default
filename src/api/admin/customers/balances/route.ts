...
export async function GET(req: MedusaRequest, res: MedusaResponse): Promise<void> {
    ...
    try {
        ...
        const paymentCreditResult = await client.query<{ customer_id: string; payment_credit: string }>(
            `SELECT
                cp.customer_id,
                COALESCE(SUM(
                    cp.amount - COALESCE(
        ...
        const ledgerCreditResult = await client.query<{ customer_id: string; ledger_balance: string }>(
            `SELECT
                customer_id,
