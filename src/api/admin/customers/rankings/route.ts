...
export async function GET(req: MedusaRequest, res: MedusaResponse): Promise<void> {
    ...
    try {
        ...
        const creditResult = await client.query<{ customer_id: string; credit: string }>(
            `SELECT
                cp.customer_id,
                COALESCE(SUM(
                    cp.amount - COALESCE(
        ...
        const ledgerResult = await client.query<{ customer_id: string; ledger: string }>(
            `SELECT
                customer_id,
        ...
        for (const row of arResult.rows) arMap[row.customer_id] = parseFloat(row.ar)

        const creditMap: Record<string, number> = {}
        for (const row of creditResult.rows) creditMap[row.customer_id] = parseFloat(row.credit)

        const ledgerMap: Record<string, number> = {}
        for (const row of ledgerResult.rows) ledgerMap[row.customer_id] = parseFloat(row.ledger)
