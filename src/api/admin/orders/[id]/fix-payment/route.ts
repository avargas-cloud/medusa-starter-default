...
export async function POST(req: MedusaRequest, res: MedusaResponse): Promise<void> {
    ...
    try {
        ...
        if (!orderRes.ok) {
            return void res.status(400).json({ message: "Could not fetch order total" })
        }
        const { order } = await orderRes.json()
        const correctTotal: number | null = order?.total > 0 ? order.total : null

        if (correctTotal === null) {
            return void res.status(200).json({ success: true, skipped: true, reason: "zero total" })
        }
        ...
        if (!pcRes.ok) {
            return void res.status(200).json({ success: true, skipped: true, reason: "no payment collections" })
        }
        ...
        for (const col of collections) {
            if (col.amount === correctCents) {
                console.log(`[fix-payment] collection ${col.id} already correct (${correctCents} cents)`)
                continue
            }
            console.log(`[fix-payment] patching collection ${col.id}: ${col.amount} → ${correctCents} cents`)
            const patchRes = await fetch(`${base}/admin/payment-collections/${col.id}`, {
                method: "POST",
                headers: authHeaders,
        ...
        res.status(200).json({ success: true, correct_total: correctTotal, patched })
    } catch (e: any) {
