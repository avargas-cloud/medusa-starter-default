...
import { writePipelineRow, findLastInFlightSoToggleRow } from "../../../../../lib/quickbooks/qb-pipeline"
...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
    ...
    try {
        ...
        const orders = await orderService.listOrders({ id: orderId }, { relations: ["items"] })
        ...
        const newMeta = action === "close"
            ? { ...meta, pos_closed: true, pos_closed_at: new Date().toISOString() }
            : { ...meta, pos_closed: false, pos_reopened_at: new Date().toISOString() }

        await orderService.updateOrders([{ id: orderId, metadata: newMeta }])
        console.log(`${LOG_PREFIX} ✅ Metadata updated — pos_closed=${newMeta.pos_closed}`)
        ...
        if (!locationId) {
            console.warn(`${LOG_PREFIX} No stock location found — skipping reservation management`)
        }

        // 4. Release existing reservations
        const negativeStockItems: { sku: string; variant_id: string; available: number }[] = []

        if (locationId) {
            try {
                for (const item of order.items || []) {
                    const existing = await inventoryModule.listReservationItems({ line_item_id: item.id })
                    if (existing?.length) {
                        await inventoryModule.deleteReservationItems(existing.map((r: any) => r.id))
                        console.log(`${LOG_PREFIX} 🗑️ Released ${existing.length} reservation(s) for item ${item.id}`)
                    }
                }
            } catch (err: any) {
                console.warn(`${LOG_PREFIX} Failed to release reservations: ${err.message}`)
            }
                ...
                try {
                    ...
                    try {
                        ...
                        if (!levels?.length) {
                            ...
                            const available = Number(level.stocked_quantity) - Number(level.reserved_quantity) - quantity
                            if (available < 0) {
                    ...
                    console.log(`${LOG_PREFIX} ✅ Reserved ${quantity}× for variant ${variantId}`)
                } catch (err: any) {
                    console.warn(`${LOG_PREFIX} Reservation failed for ${variantId}: ${err.message?.slice(0, 100)}`)
                }
            }
        }
        ...
        const medusaRefNumber = meta.document_number ? String(meta.document_number) : null
        ...
        let pipelineStatus: "pending" | "waiting" | "skipped" = soTxnId ? "pending" : "skipped"
        ...
        if (soTxnId) {
            try {
                ...
                if (dependsOnRowId) {
                    ...
                    console.log(`${LOG_PREFIX} In-flight row detected (${dependsOnRowId}) — chaining as waiting`)
                }
            } catch (depErr: any) {
                console.warn(`${LOG_PREFIX} Could not check in-flight rows: ${depErr.message}`)
            }
        }
        ...
        try {
            ...
            console.log(`${LOG_PREFIX} ✅ Pipeline row written (step=${pipelineStep}, status=${pipelineStatus})`)
        } catch (pErr: any) {
            console.warn(`${LOG_PREFIX} Could not write pipeline row: ${pErr.message}`)
        }
        ...
        if (!soTxnId) {
            ...
            qbSkipped = true
            console.log(`${LOG_PREFIX} Chained as waiting — QB sync deferred until dependency resolves`)
        } else {
            try {
                const qbResult = action === "close"
                    ? await closeSalesOrderInQb(soTxnId, (msg) => console.log(msg))
                    : await reopenSalesOrderInQb(soTxnId, (msg) => console.log(msg))

                if (qbResult.success) {
                    console.log(`${LOG_PREFIX} ✅ QB SO ${action} queued (op: ${qbResult.data?.operationId})`)
                    try {
                        await writePipelineRow({
                            orderId,
                            referenceId: orderId,
                    ...
                    console.warn(`${LOG_PREFIX} QB SO ${action} failed: ${qbResult.error}`)
                    try {
                        await writePipelineRow({
                            orderId,
                            referenceId: orderId,
