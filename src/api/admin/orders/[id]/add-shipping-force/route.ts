  ...
  try {
    ...
    if (dbUrl) {
      try {
        ...
        const vRes = await pool.query<{ version: number }>(
          `SELECT version FROM "order" WHERE id = $1 LIMIT 1`,
          [id]
        );
        ...
        console.log(
          `[orders/add-shipping-force] Cleared existing shipping methods for order ${id} (version ${currentVersion})`
        );
      } catch (e: any) {
        console.warn(
          `[orders/add-shipping-force] Failed to initialize version/cleanup:`,
          e?.message
        );
      }
    }
    ...
    if (shipping_option_id) {
      ...
      console.log(
        `[orders/add-shipping-force] Applied ${shippingOptionName} ($${amountDollars}) to order ${id}`
      );
        ...
        try {
          ...
          if (shippingMethodId) {
            ...
            if (currentVersion > 1) {
              ...
              console.log(
                `[orders/add-shipping-force] Upgraded shipping link ${shippingMethodId} to match order version ${currentVersion}`
              );
            }
            ...
            console.log(
              `[orders/add-shipping-force] ✅ Inserted 0% tax_line for shipping method ${shippingMethodId}`
            );
          }
        } catch (te: any) {
          console.warn(
            `[orders/add-shipping-force] Tax line insert failed (non-fatal):`,
            te?.message
          );
        }
      }
      ...
      console.log(
        `[orders/add-shipping-force] Shipping option clearing completed for order ${id}`
      );
    }
