...
export async function POST(
    ...
    try {
        ...
        const order = await orderModule.retrieveOrder(orderId, { relations: ["shipping_methods"] })
        
        if (!order.shipping_methods || order.shipping_methods.length === 0) {
            res.status(404).json({ message: "No shipping methods found on this order" })
            return
        }
        ...
        if (typeof orderModule.updateOrderShippingMethods === "function") {
            ...
            console.log(`[update-shipping-force] updateOrderShippingMethods OK: ${smId} → amount=${amount}`)
            res.status(200).json({ success: true, method: "updateOrderShippingMethods" })
            return
        }
        ...
        if (typeof orderModule.updateShippingMethods === "function") {
            ...
            console.log(`[update-shipping-force] updateShippingMethods OK: ${smId} → amount=${amount}`)
