...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  if (!items?.length || !location_id) {
    return res
      .status(400)
      .json({ message: "items and location_id are required" });
  }
  ...
  async function bindFulfillmentToInvoice(
    ...
    try {
      ...
      console.log(
        `[create-fulfillment-force] ✅ Bound fulfillment ${fulfillmentId} to Invoice ${invId}`
      );

      // Feature Upgrade: inherit QB ref from invoice metadata onto the new fulfillment metadata
      try {
        const invRes = await pool.query(
          `SELECT metadata FROM pos_invoice WHERE id = $1`,
          [invId]
        );
        const invMeta = invRes.rows[0]?.metadata || {};

        if (invMeta.qb_txn_id) {
          const fulfillmentModule = req.scope.resolve(
            Modules.FULFILLMENT
          ) as any;
          const fRes =
          ...
          console.log(
            `[create-fulfillment-force] ✅ Copied QB Ref ${invMeta.qb_ref_number} onto Medusa Fulfillment ${fulfillmentId}`
          );
        }
      } catch (metaErr: any) {
        console.warn(
          `[create-fulfillment-force] Failed to inherit QB meta to fulfillment: ${metaErr?.message}`
        );
      }
    } catch (err: any) {
      console.warn(
        `[create-fulfillment-force] Failed to bind to pos_invoice: ${err?.message}`
      );
    }
  }
  ...
  try {
    ...
    console.log(
      `[create-fulfillment-force] 🔄 Strategy 1: native workflow for order=${orderId}`
    );
      ...
      try {
        ...
        for (const reqItem of items) {
          try {
            ...
            if (!variantId) {
              console.warn(
                `[create-fulfillment-force] Pre-B: no variant_id for item ${reqItem.id}, skipping`
              );
              continue;
            }
            ...
            if (!inventoryItemId) {
              console.warn(
                `[create-fulfillment-force] Pre-B: no inventory_item for variant ${variantId}, skipping`
              );
              continue;
            }
            ...
            console.log(
              `[create-fulfillment-force] ✅ Pre-B: created reservation for item ${reqItem.id} (inv=${inventoryItemId})`
            );
          } catch (reservErr: any) {
            // Non-fatal: fall through to Strategy 2 if this fails
            console.warn(
              `[create-fulfillment-force] Pre-B: reservation failed for ${reqItem.id}: ${reservErr?.message?.slice(0, 80)}`
            );
          }
        }
      } finally {
    ...
    if (dbUrl && items.length > 0) {
      ...
      try {
        ...
        console.log(
          `[create-fulfillment-force] ✅ Patched fulfilled_quantity for ${items.length} order_item rows (Strategy 1)`
        );
      } catch (sqlErr: any) {
        console.warn(
    ...
    console.warn(
      `[create-fulfillment-force] ⚠️ Strategy 1 failed (${workflowErr?.message?.slice(0, 100)}), falling back to Strategy 2...`
    );
  }
  ...
  try {
    ...
    console.log(
      `[create-fulfillment-force] 🔄 Strategy 2: module services, items=${fulfillmentItems.length}`
    );
    ...
    try {
      ...
      dbProviders.push(
        ...(rows as any[]).map((p: any) => p.id).filter(Boolean)
      );
    } catch {
      /* non-fatal */
    }
    ...
    console.log(
      `[create-fulfillment-force] Provider candidates (${candidates.length}):`,
    ...
    const deliveryAddress = orderData?.shipping_address?.country_code
      ? (() => {
          const { id, created_at, updated_at, deleted_at, ...clean } =
            orderData.shipping_address as any;
          return clean;
        })()
      : {
    ...
    for (const providerId of candidates) {
      try {
        ...
        console.log(
          `[create-fulfillment-force] ✅ Created with provider=${providerId}: ${fulfillment.id}`
        );
        break;
      } catch (err: any) {
        console.warn(
          `[create-fulfillment-force] provider=${providerId} failed: ${err?.message?.slice(0, 80)}`
        );
      }
    }
    ...
    if (!fulfillment) {
      try {
        ...
        console.log(
          `[create-fulfillment-force] ✅ Created (no provider): ${fulfillment.id}`
        );
      } catch (err: any) {
        throw new Error(
          `All provider attempts failed. Last error: ${err?.message}`
        );
      }
    }
    ...
    if (dbUrl && items.length > 0) {
      ...
      try {
        ...
        console.log(
          `[create-fulfillment-force] ✅ Patched fulfilled_quantity for ${items.length} order_item rows`
        );
      } catch (sqlErr: any) {
        console.warn(
          `[create-fulfillment-force] fulfilled_quantity patch failed (non-fatal): ${sqlErr?.message}`
    ...
    console.error(
      `[create-fulfillment-force] ❌ Strategy 2 also failed: ${moduleErr?.message}`
    );
    return res
      .status(500)
      .json({ message: moduleErr?.message ?? "Fulfillment creation failed" });
  }
}
