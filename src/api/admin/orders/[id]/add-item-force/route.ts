...
export async function POST(
  ...
  try {
    ...
    try {
      ...
      if (!variant) {
        console.warn(
          `[add-item-force] productModule returned no variant for ${variant_id}`
        );
      } else {
        ...
        if (resolvedProductTitle) {
          ...
          variantTitle =
            vAny?.title !== resolvedProductTitle ? vAny?.title : undefined;
        } else if (fallbackTitle) {
          console.warn(
            `[add-item-force] product relation missing for ${variant_id}, using variant title "${fallbackTitle}"`
          );
          title = fallbackTitle;
        } else {
          console.error(
            `[add-item-force] Both product.title and variant.title are empty for ${variant_id}`
          );
        }
      ...
      console.error(
        `[add-item-force] productModule.listProductVariants threw for ${variant_id}:`,
        e?.message
      );
    }
    ...
    if (!title) {
      console.warn(
        `[add-item-force] Falling back to admin API for variant ${variant_id}`
      );
      try {
        ...
        if (varRes.ok) {
          ...
          if (product) {
            ...
            salesDescription = product.metadata?.sales_description as
              | string
              | undefined;
            console.log(
              `[add-item-force] Fallback API resolved title="${title}" sku="${variantSku}" sdesc="${salesDescription ?? "none"}" for ${variant_id}`
            );
          }
        }
      } catch (e2: any) {
        console.error(
          `[add-item-force] Admin API fallback also failed for ${variant_id}:`,
          e2?.message
        );
      }
    }
    ...
    if (!title) {
      console.error(
        `[add-item-force] Could not resolve product title for ${variant_id}. Aborting to prevent corrupted line item.`
      );
      res.status(422).json({
    ...
    console.log(
      `[orders/add-item-force] title="${title}" sdesc="${salesDescription ?? "none"}" sku="${variantSku}"`
    );
    ...
    try {
      ...
      try {
        ...
        inventoryItemId =
          variantData?.[0]?.inventory_items?.[0]?.inventory_item_id ?? null;
      } catch (e: any) {
      ...
      console.warn(
        `[orders/add-item-force] Reservation creation failed (non-fatal):`,
        resErr?.message
      );
    }
