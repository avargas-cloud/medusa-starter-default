...
export async function PATCH(
  ...
  const { items } = req.body as {
    items: { id: string; damaged_qty: number }[];
  };
  ...
  try {
    ...
    if (!creditMemo) {
      res
        .status(404)
        .json({ success: false, message: "Credit Memo not found" });
      return;
    }

    // Credit memo de una orden WEB → PIN de supervisor: marcar damaged mueve
    // inventario del contrato que el cliente armó solo. Antes de toda mutación.
    ...
    for (const update of items) {
      const dbItem = (creditMemo.items as any[]).find(
        (i: any) => i.id === update.id
      );
      if (!dbItem) {
        logger.warn(
          `[damaged update] Item ${update.id} not found on credit memo ${id} — skipping`
        );
        continue;
      }

      const oldDamaged = dbItem.damaged_qty || 0;
      const newDamaged = Math.max(
        0,
        Math.min(update.damaged_qty, dbItem.quantity)
      );
      ...
      if (locationId && dbItem.variant_id) {
        try {
          ...
          logger.warn(
            `[damaged update] Inventory adjustment failed for variant ${dbItem.variant_id}: ${invErr.message}`
          );
        }
      }
    ...
    await pgConnection("pos_damaged_item")
      .where({ credit_memo_id: id })
      .delete();

    const updatedItems = await creditMemoService.listPosCreditMemoItems({
      credit_memo_id: id,
    });
    ...
    // Este es el camino que EDITA el ajuste: el credit memo ya está completado
    // y su documento existe en QuickBooks, así que un cambio de defectuosos es
    // un InventoryAdjustmentMod sobre el mismo documento, nunca uno nuevo.
