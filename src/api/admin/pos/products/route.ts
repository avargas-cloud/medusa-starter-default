...
export const POST = async (req: MedusaRequest<CreatePosProductInput>, res: MedusaResponse) => {
    try {
        ...
        if (qbOperationId && variantId) {
            ;(async () => {
                try {
                    logger.info(`[pos-sync] Polling operation ${qbOperationId} for new Product/Variant ${variantId}...`)
                    const pollResult = await pollOperationResult(qbOperationId, (msg) => logger.info(msg))
                    
                    if (pollResult.txnId) { // txnId acts as listId for Items and Customers in pollOperationResult

                        // Update it directly or via native fetch to avoid medusa update overhead
                        logger.info(`[pos-sync] Got ListID ${pollResult.txnId}. Updating variant ${variantId} metadata.`)
                    ...
                    logger.error(`[pos-sync] Failed to poll/update new item ListID: ${err.message}`)
                }
            })()
        }
