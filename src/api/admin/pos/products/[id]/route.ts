...
import { updatePosProductWorkflow, UpdatePosProductInput } from "../../../../../workflows/pos/update-pos-product"
...
export const POST = async (req: MedusaRequest, res: MedusaResponse) => {
    ...
    try {
        ...
        if (!qbId) {
            return res.status(400).json({ error: "This product is not linked to Quickbooks (Missing qb_id). Please create it properly first." })
        }
        ...
        if (!qbEditSequence) {
             logger.warn(`Missing qb_edit_sequence for variant ${variant.id}. QB might reject the modification.`)
        }
        ...
        if (errors && errors.length > 0) {
            ...
            return res.status(400).json({ error: errors[0]?.error?.message || "Failed to update product" })
        }
        ...
        if (qbOperationId && variantId) {
            ;(async () => {
                try {
                    logger.info(`[pos-sync] Polling operation ${qbOperationId} for modified Product/Variant ${variantId}...`)
                    const pollResult = await pollOperationResult(qbOperationId, (msg) => logger.info(msg))
                    
                    if (pollResult.editSequence) {
                        logger.info(`[pos-sync] Update successful! Got NEW EditSequence ${pollResult.editSequence}. Saving to variant...`)
                    ...
                    logger.error(`[pos-sync] Failed to poll/update modified item EditSequence: ${err.message}`)
                }
            })()
        }
