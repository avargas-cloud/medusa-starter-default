...
export async function POST(
  ...
  const { backlighting_project_id, backlighting_seq_id, linked_by } =
    req.body as {
  ...
  console.log(
    `[set-bl-link] REQUEST order=${id} bl_project=${backlighting_project_id ?? "(missing)"} seq=${backlighting_seq_id ?? "null"}`
  );

  if (!backlighting_project_id) {
    console.warn(
      `[set-bl-link] 400 — backlighting_project_id missing for order=${id}`
    );
  ...
  try {
    ...
    console.log(
      `[set-bl-link] current metadata keys=${Object.keys(currentMetadata).join(",") || "(none)"}`
    );
    ...
    console.log(
      `[set-bl-link] OK order=${id} bl_project=${backlighting_project_id} seq=${backlighting_seq_id ?? "null"}`
    );
