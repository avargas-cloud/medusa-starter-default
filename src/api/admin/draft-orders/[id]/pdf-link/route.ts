...
export async function POST(req: MedusaRequest, res: MedusaResponse): Promise<void> {
  ...
  const resolvedTemplateId = templateId ?? (await fetchDefaultTemplateId(docType))

  if (!resolvedTemplateId || !docId) {
    res.status(400).json({ success: false, error: "templateId and docId are required" })
    return
  }
  ...
  try {
    ...
    if (!templateData) {
      console.warn(`[pdf-link] Template ${resolvedTemplateId} not found — print page may hang`)
    }
    ...
    const pdfBuffer = await generatePdfFromUrl(printUrl, posState, templateData)
