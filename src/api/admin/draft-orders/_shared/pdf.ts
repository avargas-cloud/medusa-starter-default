...
export async function fetchDefaultTemplateId(docType: string): Promise<string | null> {
  const db = await dbConnect()
  try {
    const result = await db.query(
...
export async function generatePdfFromUrl(
  ...
  try {
    ...
    const pdfBuffer = await page.pdf({ format: "Letter", printBackground: true, scale: 1 })
