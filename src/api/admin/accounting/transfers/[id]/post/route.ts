...
/**
 * POST /admin/accounting/transfers/:id/post
 *   → 201 { status: "posted", entry_id, transfer } · 200 { status: "already_posted", … }
...
export async function POST(
  ...
  try {
    ...
    const result = await postBankTransfer(client, id, actorId);
    return res
      .status(result.status === "posted" ? 201 : 200)
      .json({ ...result, transfer: await getBankTransfer(client, id) });
  } catch (error) {
