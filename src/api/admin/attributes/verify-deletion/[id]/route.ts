...
export async function GET(
    ...
    try {
        ...
        try {
            const [attribute] = await productAttributesService.listAttributeKeys({ id })
        ...
        if (categoriesResponse.ok) {
            ...
            results.checks.categories_with_reference = categoriesWithRef.map((cat: any) => ({
                id: cat.id,
                name: cat.name,
        ...
        // Summary
        results.summary = {
            fully_deleted: !results.checks.attribute_key_exists &&
                results.checks.attribute_values_count === 0 &&
                results.checks.product_links_count === 0 &&
