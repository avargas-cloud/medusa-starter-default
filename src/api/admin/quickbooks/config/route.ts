  ...
  try {
    ...
    const validateMinutes = (
      ...
      if (val !== undefined && val !== null && (val < 1 || val > 10080)) {
        res
          .status(400)
          .json({ error: `${field} must be between 1 and 10080 (7 days)` });
        return false;
      }
      return true;
    };
    ...
    if (integration_enabled !== undefined)
      set("integration_enabled", integration_enabled);

    // Store hours
    if (store_hours_open_hour !== undefined)
      set("store_hours_open_hour", store_hours_open_hour);
    if (store_hours_close_hour !== undefined)
      set("store_hours_close_hour", store_hours_close_hour);
    if (store_sat_open !== undefined) set("store_sat_open", store_sat_open);
    if (store_sat_open_hour !== undefined)
      set("store_sat_open_hour", store_sat_open_hour);
    if (store_sat_close_hour !== undefined)
      set("store_sat_close_hour", store_sat_close_hour);
    if (store_sun_open !== undefined) set("store_sun_open", store_sun_open);
    if (store_sun_open_hour !== undefined)
      set("store_sun_open_hour", store_sun_open_hour);
    if (store_sun_close_hour !== undefined)
      set("store_sun_close_hour", store_sun_close_hour);
    if (store_hours_timezone !== undefined)
      set("store_hours_timezone", store_hours_timezone);

    // Respect hours flags
    if (inventory_respect_hours !== undefined)
    ...
    if (body.price_sync_hour !== undefined)
    ...
    if (shipping_item_id !== undefined)
      set("shipping_item_id", shipping_item_id);
    if (default_sales_tax_code !== undefined)
      set("default_sales_tax_code", default_sales_tax_code);
