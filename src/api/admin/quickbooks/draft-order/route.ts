 ...
 */
async function getQbConfig(): Promise<{
...
export async function POST(
  ...
  try {
    ...
    if (qbItems.length === 0) {
      res.status(400).json({
        error:
          "No active items in this draft order have a QuickBooks ID (variant.metadata.quickbooks_id). Add QB-linked products first.",
      });
      return;
    }
    ...
    const isCancelled =
      (order.metadata?.order_status ?? order.metadata?.estimate_status) ===
        "Cancelled" ||
      (order.metadata?.order_status ?? order.metadata?.estimate_status) ===
        "cancelled";
    const memo =
      `Draft Order #${(order as any).display_id || orderId} - ${customer.first_name || ""} ${customer.last_name || ""}`.trim();
    ...
    const salesTaxCode = taxExempt ? undefined : qbConfig.defaultSalesTaxCode;
...
export async function DELETE(
  ...
  try {
    ...
    if (!orderResp.ok) {
      ...
      console.warn(
        `[QB] Cannot fetch order ${orderId} for QB deactivation: ${orderResp.status}`
      );
    ...
    if (!estimateTxnId) {
      ...
      res.json({
        success: true,
        skipped: true,
        skipReason: "No QB estimate linked to this draft order",
      });
      return;
    }
    ...
    if (!result.enabled) {
      res.json({
        success: true,
        skipped: true,
        skipReason: "QB integration disabled",
      });
      return;
    }
