...
import { ensureCustomerInQb, syncCustomerToQb } from "../../../../lib/quickbooks/order-flow-core"
...
export async function POST(
    ...
    if (process.env.QB_ORDER_FLOW_ENABLED !== "true") {
        res.json({ success: true, skipped: true, skipReason: "QB_ORDER_FLOW_ENABLED is false" })
        return
    }
    ...
    try {
        ...
        res.status(404).json({ success: false, error: `Customer not found: ${err.message}` })
        return
    }
    ...
    const customerForQb = {
        ...
        addresses: (customer.addresses ?? []).map((a: any) => ({
            ...
            is_default_billing: a.is_default_billing ?? a.metadata?.is_default_billing ?? false,
            is_default_shipping: a.is_default_shipping ?? a.metadata?.is_default_shipping ?? false,
            metadata: a.metadata ?? {},
        })),
    }
