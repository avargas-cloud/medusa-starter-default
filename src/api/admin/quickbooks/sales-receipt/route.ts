...
import { createSalesReceiptInQb, voidSalesReceiptInQb } from "../../../../lib/quickbooks/qb-bridge-client"
...
export async function POST(req: MedusaRequest, res: MedusaResponse): Promise<void> {
    ...
    if (!ENABLED) {
        res.json({ success: true, skipped: true, message: "QB integration disabled" })
        return
    }
    ...
    try {
        ...
        const { data: [order] } = await query.graph({
            ...
            fields: [
                ...
                "total", "currency_code", "customer_id",
                "sales_channel_id",
                "items.*", "items.item.unit_price",
                "items.variant.*", "items.variant.metadata",
        ...
        if (!customer && order.customer_id) {
            customer = await customerModule.retrieveCustomer(order.customer_id, { relations: ["addresses"] })
        }

        const custResult = await ensureCustomerInQb(customer, customerModule)
        if (!custResult.success || !custResult.qbCustomerId) {
            res.status(500).json({ error: custResult.error || "Could not ensure customer in QB" })
            return
        }
        ...
        if (!result.success) {
            res.status(500).json({ error: result.error || "Failed to create Sales Receipt in QB" })
            return
        }
...
export async function DELETE(req: MedusaRequest, res: MedusaResponse): Promise<void> {
    ...
    try {
        ...
        if (!txnId) {
            res.json({ success: true, skipped: true, message: "No Sales Receipt TxnID found — nothing to void" })
            return
        }
