...
const VALID_TYPES: PosDocType[] = ['estimate', 'order', 'invoice', 'return', 'payment']
...
async function resolveId(client: InstanceType<typeof Client>, type: PosDocType, raw: string): Promise<string | null> {
    const id = raw.trim()
...
export async function GET(req: MedusaRequest, res: MedusaResponse): Promise<void> {
    ...
    try {
        ...
        if (type === 'estimate' || type === 'order') {
            ...
            const order = await orderModule.retrieveOrder(internalId).catch(() => null)
            if (!order) {
        ...
        if (type === 'invoice') {
            ...
            const [invoice] = await invoiceService.listPosInvoices({ id: [internalId] }).catch(() => [])
            if (!invoice) {
        ...
        if (type === 'payment') {
            ...
            const payment = await financeService.retrieveCustomerPayment(internalId).catch(() => null)
            if (!payment) {
        ...
        const msg = error instanceof Error ? error.message : 'Failed to load QB metadata'
...
export async function PUT(req: MedusaRequest, res: MedusaResponse): Promise<void> {
    ...
    try {
        ...
        if (type === 'estimate' || type === 'order') {
            ...
            const existingMeta: Record<string, unknown> = { ...((order as any).metadata ?? {}) }

            if (type === 'estimate') {
                const existingEst = (typeof existingMeta.qb_estimate === 'object' && existingMeta.qb_estimate !== null && !Array.isArray(existingMeta.qb_estimate))
                ...
                const existingSo = (typeof existingMeta.qb_sales_order === 'object' && existingMeta.qb_sales_order !== null && !Array.isArray(existingMeta.qb_sales_order))
        ...
        const msg = error instanceof Error ? error.message : 'Failed to update QB metadata'
