...
function parseMethod(methodName: string): string {
    ...
    if (n.includes("visa") || n.includes("master") || n.includes("card")) return "card"
...
async function syncFromQB(client: Client, year: number): Promise<void> {
    ...
    const qbMsgs = (raw?.QBXML as any)?.QBXMLMsgsRs || (raw?.QBXMLMsgsRs as any) || raw
    const payRetRaw =
    ...
    const payList: any[] = payRetRaw ? (Array.isArray(payRetRaw) ? payRetRaw : [payRetRaw]) : []
    const unapplied = payList.filter(p => parseFloat(p.UnusedPayment || "0") > 0)

    // Build Medusa customer lookup
    const qbListIds = [...new Set(unapplied.map((p: any) => p.CustomerRef?.ListID).filter(Boolean))] as string[]
...
export async function POST(
    ...
    const body = (req.body ?? {}) as { action?: string; year?: number; txn_id?: string }
    ...
    try {
        ...
        if (action === "apply") {
            ...
            if (!staged) {
                res.status(404).json({ error: `Payment ${txn_id} not found in staging table` })
                return
            }
            if (staged.status === "applied") {
                res.status(409).json({ error: "Payment already applied", applied_payment_id: staged.applied_payment_id })
                return
            }
            if (!staged.medusa_customer_id) {
                res.status(422).json({ error: "No Medusa customer match for this payment" })
                return
            }
            ...
            const payment = await financeService.createCustomerPayments({
                ...
                method: staged.method as "cash" | "check" | "card" | "ach" | "zelle" | "other",
