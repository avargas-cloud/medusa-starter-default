...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  ...
  try {
    const result = await pg.raw(
      `WITH ${CM_REFUNDS_BY_CUSTOMER_CTE},
       ...
       returned_at_grain AS (
         SELECT COALESCE(NULLIF(TRIM(c.metadata->>'acquisition_channel'), ''), 'Unknown') AS channel,
       ...
       gross AS (
       SELECT
         COALESCE(NULLIF(TRIM(c.metadata->>'acquisition_channel'), ''), 'Unknown') AS channel,
       ...
       SELECT g.*, COALESCE(rg.cm_refunded, 0)::bigint AS cm_refunded,
              COALESCE(sg.shipping_cents, 0)::bigint AS shipping_cents,
              COALESCE(rg2.returned_cost, 0) AS returned_cost
       FROM gross g
       LEFT JOIN refunds_at_grain rg ON rg.channel = g.channel
       LEFT JOIN ship_at_grain sg ON sg.channel = g.channel
       LEFT JOIN returned_at_grain rg2 ON rg2.channel = g.channel
    ...
    return res.status(500).json({ error: "Failed to fetch acquisition report" })
  }
}
