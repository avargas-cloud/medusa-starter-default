    ...
    const result = await pg.raw(
      `WITH ${CM_REFUNDS_BY_CUSTOMER_CTE},
       ...
       returned_at_grain AS (
         SELECT COALESCE(NULLIF(TRIM(c.metadata->>'qb_price_level'), ''), 'Unknown') AS customer_type,
       ...
       gross AS (
       SELECT
         COALESCE(NULLIF(TRIM(c.metadata->>'qb_price_level'), ''), 'Unknown') AS customer_type,
       ...
       FROM gross g
       LEFT JOIN refunds_at_grain rg ON rg.customer_type = g.customer_type
