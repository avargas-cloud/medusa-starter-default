...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  ...
  try {
    const result = await pg.raw(
      `WITH vb_costs AS (
       ...
       SELECT
         ...
         COUNT(DISTINCT CASE WHEN vbc.has_bill IS NOT NULL THEN pol.product_variant_id END)::int AS received_products,
    ...
    const rows = (result.rows as any[]).map(r => ({
      ...
      received_products: Number(r.received_products),
    }))

    return res.json({ rows })
  } catch (err) {
    return res.status(500).json({ error: "Failed to fetch China by factory" })
  }
}
