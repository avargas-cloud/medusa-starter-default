...
function bankExpenseDocument(
  row: Record<string, unknown>
): Pick<PeriodCostLine, "document_kind" | "link_path"> {
  const originId = encodeURIComponent(String(row.origin_id ?? ""));
  switch (row.origin_kind) {
    case "deposit":
      return {
        document_kind: "Bank deposit fee",
        link_path: `/accounting/banks/deposits?deposit_id=${originId}`,
      };
    case "movement":
      return {
        document_kind: "Bank movement expense",
        link_path: `/accounting/banks/movements?movement_id=${originId}`,
      };
    case "merchant_settlement":
      return {
        document_kind: "Merchant settlement fee",
        link_path: `/accounting/banks/settlements?settlement_id=${originId}`,
      };
    default:
      return {
        document_kind: "Bank expense",
...
export async function fetchBankExpenseCostLines(
  pg: RawPg,
  from: string,
  to: string
  ...
  const schema = await pg.raw(
    `SELECT to_regclass('public.bank_journal_entry') IS NOT NULL
    AND to_regclass('public.bank_journal_line') IS NOT NULL AS ready`,
    []
  );
  if (schema.rows[0]?.ready !== true) return [];
  const rows = await pg.raw(
    `SELECT e.id,e.transaction_id,e.reference,e.description,e.kind,
  ...
  return rows.rows.map((row) => ({
    ...
    document_number: String(row.reference),
    ...bankExpenseDocument(row),
    document_date:
      row.document_date instanceof Date
        ? row.document_date.toISOString()
        : String(row.document_date),
    counterparty: row.counterparty == null ? null : String(row.counterparty),
    account_list_id: String(row.account_list_id),
    account_full_name: String(row.account_name),
    account_type: String(row.account_type),
    bucket: "cost",
    amount_cents: Number(row.amount_cents),
    description: String(row.description),
    document_status: row.kind === "reversal" ? "reversal" : "posted",
    qb_synced: false,
    qb_ref: null,
  }));
}
