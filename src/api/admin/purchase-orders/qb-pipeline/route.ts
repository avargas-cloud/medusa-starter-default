...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  ...
  const { rows } = await client.query(
    `SELECT
       ...
       pipe.created_at,
       pipe.updated_at,
       -- vendor name from snapshot or id
  ...
  const filtered = searchLower
    ? rows.filter((r) =>
        [r.po_number, r.draft_number, r.vendor_name, r.qb_list_id, r.last_error, r.qb_txn_number]
          .filter(Boolean)
          .join(" ")
          .toLowerCase()
  ...
  const counts: Record<string, number> = { waiting: 0, processing: 0, synced: 0, error: 0 };
  for (const r of rows) {
