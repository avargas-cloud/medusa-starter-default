...
export const GET = async (req: MedusaRequest, res: MedusaResponse) => {
    ...
    try {
        ...
        let items: { weight: number; length: number; width: number; height: number; quantity: number }[] = []
        let shipToName = ""
        let shipToAddress = ""
        let shipToCity = ""
        let shipToState = ""
        ...
        if (draftOrderId) {
            ...
            if (addrRes.rows[0]) {
                ...
                shipToName = a.company || `${a.first_name || ""} ${a.last_name || ""}`.trim()
        ...
        const singlePkg = packages.reduce((acc, p) => ({
        ...
        for (const code of serviceCodes) {
            try {
                const priceCents = await getUPSRate(code, {
                    ...
                    shipperAddress: process.env.UPS_ORIGIN_ADDRESS || "2760 W 84th St Unit 4",
        ...
        return res.status(500).json({ error: "Failed to get UPS rates", details: err.message })
    } finally {
        await client.end()
    }
}
