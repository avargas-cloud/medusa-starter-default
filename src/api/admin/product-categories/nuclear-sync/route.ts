...
export const POST = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  try {
    ...
    console.log(
      `[NUCLEAR-SYNC] 📦 Found ${allCategories.length} total categories`
    );
    ...
    for (const category of allCategories) {
      ...
      const uniqueAttrIds = [
        ...new Set(attributeValues.map((av: any) => av.attribute_key_id)),
      ];
      ...
      if (phase1Success % 10 === 0) {
        console.log(
          `[NUCLEAR-SYNC]    Progress: ${phase1Success} categories configured...`
        );
      }
    }

    console.log(
      `[NUCLEAR-SYNC] ✅ PHASE 1 Complete: ${phase1Success} configured, ${phase1Skipped} skipped (no products)`
    );
    ...
    console.log(
      `[NUCLEAR-SYNC] 📦 Generating counts for ${categoriesWithFilters.length} categories...`
    );
    ...
    for (const category of categoriesWithFilters) {
      try {
        ...
        if (phase2Success % 10 === 0) {
          console.log(
            `[NUCLEAR-SYNC]    Progress: ${phase2Success} filters generated...`
          );
        }
      } catch (error: any) {
        console.error(
          `[NUCLEAR-SYNC]    ❌ Error for ${category.name}:`,
