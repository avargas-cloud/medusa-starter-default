...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  try {
    ...
    const attributeKeyIds = attributesResult.rows.map(
    ...
    const added = attributeKeyIds.filter(
      (id: string) => !existingAvailableIds.has(id)
    ).length;
    const removed = Array.from(existingAvailableIds).filter(
      (id) => !currentAttrIds.has(id)
    ).length;
