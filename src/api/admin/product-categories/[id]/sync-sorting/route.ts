...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  try {
    ...
    const cleanedProductOrder = currentProductOrder.filter((id: string) =>
      actualProductIds.has(id)
    );

    // 6. Calculate what was removed
    const removedSubcategories =
      currentSubcategoryOrder.length - cleanedSubcategoryOrder.length;
    const removedProducts =
      currentProductOrder.length - cleanedProductOrder.length;
