...
export async function generateFiltersForCategory(
    ...
    console.log(`[FILTER-GEN] Category: ${categoryId}, Active filters: ${filterIds.length}`)
    ...
    for (let index = 0; index < filterIds.length; index++) {
        ...
        const linksForAttribute = await knex("product_product_productattributes_attribute_value")
            .select("attribute_value_id")
            .whereIn("product_id", productIds)
            .distinct()

        const valueIdsInUse = linksForAttribute.map((l: any) => l.attribute_value_id)
