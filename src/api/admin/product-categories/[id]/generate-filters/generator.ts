...
export async function generateFiltersForCategory(
  ...
  console.log(
    `[FILTER-GEN] Category: ${categoryId}, Active filters: ${filterIds.length}`
  );
  ...
  console.log(
    `[FILTER-GEN] Found ${attributeKeys?.length || 0} attribute definitions`
  );

  if (!attributeKeys || attributeKeys.length === 0) {
    console.log(`[FILTER-GEN] No attributes found for IDs:`, filterIds);
    return {
      filters: [],
      metadata: {
        total_filters: 0,
  ...
  if (includeDescendants) {
    ...
    console.log(
      `[FILTER-GEN] Including ${descendantIds.length} descendant categories for product counting`
    );
  } else {
    console.log(
      `[FILTER-GEN] NOT including descendants (include_descendants_tree = false)`
    );
  }
  ...
  console.log(
    `[FILTER-GEN] Found ${totalProducts} published products in category ${categoryId} (${includeDescendants ? `including ${descendantIds.length} descendants` : "descendants NOT included"})`
  );

  if (totalProducts === 0) {
    console.log(
      `[FILTER-GEN] No products in category, returning empty filters`
    );
    return {
      filters: [],
      metadata: {
  ...
  for (let index = 0; index < filterIds.length; index++) {
    ...
    if (!attribute) {
      console.log(
        `[FILTER-GEN] Skipping ${filterId} - not found in attribute keys`
      );
      continue;
    }
    ...
    console.log(
      `[FILTER-GEN] Generating filter for: ${attribute.handle} (${filterType || "null"})`
    );

    // ✅ NEW APPROACH: Only fetch values that are CURRENTLY linked to products in this category
    // This prevents accumulation of old/historical values when attributes change
    const linksForAttribute = await knex(
    ...
    const valueIdsInUse = linksForAttribute.map(
      (l: any) => l.attribute_value_id
    );

    // Now fetch ONLY the values that are actually in use
    const attributeValues = await remoteQuery({
      ...
      variables: {
        filters: {
          ...
          id: valueIdsInUse, // ✅ ONLY values currently linked to products
        },
      },
    });

    console.log(
      `[FILTER-GEN]   → Found ${attributeValues?.length || 0} total values for ${attribute.handle}`
    );
    ...
    switch (filterType) {
      ...
      case "range":
        ...
        if (
          rangeFilter.range &&
          rangeFilter.range.available_values.length > 0
        ) {
  ...
  filters.forEach((f) => {
    if (f.options) {
      ...
      console.log(
        `  - ${f.name}: ${f.toggle.count_enabled}/${f.toggle.count_total}`
      );
    }
  });
...
async function generateCheckboxFilter(
  ...
  try {
    ...
    console.log(
      `[FILTER-GEN]       → Querying links for ${productIds.length} products x ${valueIds.length} values`
    );
    ...
    console.log(
      `[FILTER-GEN]       → Found ${links?.length || 0} product-attribute links`
    );
    ...
    console.log(
      `[FILTER-GEN]       → Generated ${options.length} checkbox options with counts`
    );
    ...
    console.error(
      `[FILTER-GEN]     ✗ Error in generateCheckboxFilter:`,
...
async function generateToggleFilter(
  ...
  const productsWithAttribute = new Set(
    links.map((link: any) => link.product_id)
  );
