...
export async function registerMedusaPayment(
    ...
    try {
        ...
        if (!collectionId) {
            ...
            logger.info(`[registerMedusaPayment] Created PaymentCollection ${collectionId} for order ${order_id}`)
        } else {
            ...
            try {
                ...
                logger.info(`[registerMedusaPayment] Normalized PaymentCollection ${collectionId} amount → ${medusaInvoiceTotal}`)
            } catch (updateErr: any) {
                logger.warn(`[registerMedusaPayment] Could not update collection amount (non-fatal): ${updateErr.message}`)
            }
        }
        ...
        logger.info(`[registerMedusaPayment] Captured $${(amount / 100).toFixed(2)} on payment ${payment.id} for order ${order_id}`)
        return payment.id

    } catch (err: any) {
        try { scope.resolve('logger').warn(`[registerMedusaPayment] Failed (non-fatal) for order ${order_id}: ${err.message}`) } catch {}
        return null
    }
}
