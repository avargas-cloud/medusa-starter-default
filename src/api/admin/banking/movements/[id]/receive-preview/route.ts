...
import { movementReceivePreviewSchema } from "../../../../../../lib/banking/movement-types";
...
export async function POST(req: AuthenticatedMedusaRequest, res: MedusaResponse) {
  try { const access = await reviewCommandRequest(req, "post");
    return res.json({ ...await previewMovement(access.id, access.actorId, access.key, bankBody(movementReceivePreviewSchema, req.body), true),
