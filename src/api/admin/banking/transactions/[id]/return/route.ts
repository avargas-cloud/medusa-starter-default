...
const bodySchema = reviewVersions.extend({ reason: z.string().trim().min(1).max(1000).optional() }).strict();
export async function POST(req: AuthenticatedMedusaRequest, res: MedusaResponse) {
  try {
    const { id, actorId, key } = await reviewCommandRequest(req);
    return res.json(await changeTransactionReviewState(id, actorId, key, "return", bankBody(bodySchema, req.body)));
