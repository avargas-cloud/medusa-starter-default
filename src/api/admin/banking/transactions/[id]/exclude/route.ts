...
const bodySchema = reviewVersions.extend({ reason: z.string().trim().min(1).max(1000) }).strict();
export async function POST(req: AuthenticatedMedusaRequest, res: MedusaResponse) {
  try {
    const { id, actorId, key } = await reviewCommandRequest(req);
    return res.json(await changeTransactionReviewState(id, actorId, key, "exclude", bankBody(bodySchema, req.body)));
