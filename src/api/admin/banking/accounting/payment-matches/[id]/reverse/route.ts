...
export async function POST(req: AuthenticatedMedusaRequest, res: MedusaResponse) {
  try { const { canPost, actorId } = await reviewAccess(req, "post");
    return res.json({ ...await reverseReceiptAccounting("payment_match", bankBody(bankId, req.params.id), actorId, reviewKey(req), 
      bankBody(receiptReverseSchema, req.body)), can_post: canPost });
