...
export async function POST(req: AuthenticatedMedusaRequest, res: MedusaResponse) {
  try { const { actorId, canPost } = await reviewAccess(req, "post");
    return res.json({ ...await saveOpening(actorId, reviewKey(req), bankBody(openingSaveSchema, req.body)), can_post: canPost });
  } catch (error) { return bankFailure(res, error); }
}

export async function GET(req: AuthenticatedMedusaRequest, res: MedusaResponse) {
  try { const { canPost } = await reviewAccess(req); return res.json({ ...await listOpenings(), can_post: canPost }); }
  catch(error) { return bankFailure(res,error); }
}
