...
import { readBankingControl, setBankingControl } from "../../../../lib/banking/control";
import { bankBody, bankFailure } from "../_lib/http";

const bodySchema = z.object({ enabled: z.boolean(), reason: z.string().trim().min(1).max(500).nullable().optional() });
...
export async function POST(req: AuthenticatedMedusaRequest, res: MedusaResponse) {
  try {
    ...
    return res.json({ control: await setBankingControl(actorId, body.enabled, body.reason ?? null) });
  } catch (error) { return bankFailure(res, error); }
}
