...
export async function GET(
  ...
  try {
    ...
    const context = await readStatement(bankBody(bankId, req.params.id));
    if (context.statement.status !== "closed")
      throw new BankingError("BANKING_STATEMENT_CLOSED_REQUIRED", 409);
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="statement-${context.statement.id}.json"`
    );
    return res.json({
      statement: context.statement,
      closed_snapshot: context.statement.closed_snapshot,
