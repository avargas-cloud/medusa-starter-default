...
export const GET = async (req: MedusaRequest, res: MedusaResponse) => {
    ...
    try {
        ...
        if (orderId) {
            ...
            const items: { variant_id: string; unit_price: number; quantity: number }[] = []
            ...
            cartTotalCents = items.reduce((sum, item) =>
                sum + Math.round(Number(item.unit_price) * item.quantity * 100), 0)
            ...
            cartTotalCents = items.reduce((sum: number, item: any) =>
                sum + Math.round(item.unit_price * item.quantity * 100), 0)
