...
export default async function financeRefundCreatedHandler({
  ...
  if (!order || !order.customer_id) {
    console.warn(`[financeRefundCreatedHandler] Order ${orderId} has no customer linked. Skipping AR ledger for refund.`)
    return
  }
  ...
  if (existingRecords && existingRecords.length > 0) {
    console.log(`[financeRefundCreatedHandler] Refund ${refundId} already exists in finance ledger. Skipping.`)
    return
  }
  ...
  try {
    ...
    console.log(`[financeRefundCreatedHandler] Successfully mirrored refund ${refundId} to customer ${order.customer_id} ledger.`)
  } catch (err: any) {
    console.error(`[financeRefundCreatedHandler] Error replicating refund to finance ledger:`, err.message)
  }
}
