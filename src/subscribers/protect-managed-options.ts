...
export default async function protectManagedOptionsHandler({
  ...
  console.log(
    "   Note: This fires AFTER deletion. Cannot block the operation."
  );
  ...
  try {
    ...
    const wasManaged = attributeKeys.some(
      (key: any) => key.label === option.title
    );

    if (wasManaged) {
      console.error(
        `   ⚠️ WARNING: Managed option "${option.title}" was manually deleted!`
      );
      console.error(
        `   This may cause data inconsistency. Please investigate.`
      );
    } else {
