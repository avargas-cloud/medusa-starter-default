...
export default async function orderNotifications({
  ...
  if (payload.no_notification) {
    logger.info(
      `📧 [OrderNotifications] Skipping — no_notification flag is set`
    );
    return;
  }
  ...
  try {
    ...
    logger.error(
      `📧 [OrderNotifications] Failed to send email for ${eventName}: ${err.message}`
    );
...
async function handleOrderPlaced(payload: any, orderService: any, logger: any) {
  ...
  if (order.metadata?.pos_created === true) {
    logger.info(
      `📧 [OrderNotifications] Skipping order.placed email for POS Order #${order.display_id}`
    );
    return;
  }
  ...
  if (!customerEmail) {
    logger.warn(
      `📧 [OrderNotifications] Order ${orderId} has no email, skipping confirmation`
    );
    return;
  }

  logger.info(
    `📧 Sending order confirmation email to ${customerEmail} for order #${order.display_id}`
  );
  ...
  logger.info(
    `✅ [OrderNotifications] Order confirmation sent to ${customerEmail}`
  );
}
...
async function handleFulfillmentCreated(
  ...
  if (order.metadata?.pos_created === true) {
    logger.info(
      `📧 [OrderNotifications] Skipping fulfillment email for POS Order #${order.display_id}`
    );
    return;
  }
  ...
  const isPickup =
    fulfillment.provider_id?.includes("store-pickup") ||
  ...
  if (isPickup && fulfillment.location_id) {
    try {
      ...
      const location = await stockLocationModule.retrieveStockLocation(
      ...
      logger.info(
        `📍 Pickup location: ${location.name} - ${location.address?.address_1}`
      );
    } catch (locErr: any) {
  ...
  const customerEmail = order.email;
  logger.info(
    `📧 Sending ${isPickup ? "ready for pickup" : "fulfillment created"} email to ${customerEmail}`
  );
  ...
  logger.info(
    `✅ [OrderNotifications] Fulfillment email sent to ${customerEmail}`
  );
}
...
async function handleShipmentCreated(
  ...
  if (!orderId) {
    logger.warn(
      "📧 [OrderNotifications] Could not find order for shipment, skipping"
    );
    return;
  }
  ...
  if (order.metadata?.pos_created === true) {
    logger.info(
      `📧 [OrderNotifications] Skipping shipment email for POS Order #${order.display_id}`
    );
    return;
  }

  const trackingNumbers =
    fulfillment.labels?.map((l: any) => l.tracking_number).filter(Boolean) ||
    [];
  const trackingUrls =
    fulfillment.labels?.map((l: any) => l.tracking_url).filter(Boolean) || [];
...
async function handleDeliveryCreated(
  ...
  if (!orderId) {
    logger.warn(
      "📧 [OrderNotifications] Could not find order for delivery, skipping"
    );
    return;
  }
  ...
  if (order.metadata?.pos_created === true) {
    logger.info(
      `📧 [OrderNotifications] Skipping delivery email for POS Order #${order.display_id}`
    );
    return;
  }
...
function buildReadyForPickupEmail(
  order: any,
  fulfillment: any,
  locationAddress?: any
): string {
  ...
  const currency = order.currency_code || "USD";
  const locName =
    locationAddress?.name || fulfillment.location?.name || COMPANY_NAME;
