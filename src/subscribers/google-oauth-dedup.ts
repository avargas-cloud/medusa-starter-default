...
export default async function googleOAuthDedupSubscriber({
    ...
    try {
        ...
        const newMeta = (newCustomer as any).metadata as Record<string, unknown> | null
        if (newMeta?.qb_list_id) {
            logger.info(`[Email Dedup] Skipping ${email} — new customer has qb_list_id, may be a legitimate QB duplicate`)
            return
        }
        ...
        try {
            const existing = await sql<Array<{ id: string; email: string; has_account: boolean; created_at: Date }>>`
                SELECT id, email, has_account, created_at
                FROM customer
            ...
            logger.info(`[Email Dedup] Duplicate detected for ${email}: new=${newCustomerId}, master=${masterCustomer.id}`)
            ...
            `
            if (updated.length > 0) {
                logger.info(`[Email Dedup] Re-linked ${updated.length} auth_identity record(s) to master customer ${masterCustomer.id}`)
            }
            ...
            if (ordersUpdated.count > 0) {
                logger.info(`[Email Dedup] Re-assigned ${ordersUpdated.count} orders to master customer`)
            }

            // Soft-delete the duplicate new customer
            await customerModule.deleteCustomers([newCustomerId])
            logger.info(`[Email Dedup] ✅ Soft-deleted duplicate customer ${newCustomerId} — user will log in as ${masterCustomer.id}`)
        } finally {
            await sql.end()
        }
        ...
        logger.error(`[Email Dedup] Error during dedup for customer ${newCustomerId}: ${err.message}`)
    }
}
