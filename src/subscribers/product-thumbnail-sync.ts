...
export default async function productThumbnailSyncSubscriber({
    ...
    try {
        ...
        if (!product.thumbnail && product.images && product.images.length > 0) {
            ...
            await productService.updateProducts(productId, { thumbnail: firstImageUrl })
        ...
        try {
            await syncProductToMeiliSearchWorkflow(container).run({ input: { productId } })
            logger.info(`[ProductThumbnailSync] ✅ Meilisearch synced for "${product.title}"`)
        } catch (meiliErr: any) {
            logger.warn(
        ...
        logger.error(`[ProductThumbnailSync] ❌ Error for product ${productId}: ${err.message}`)
    }
}
