import { ITaxProvider, TaxCalculationContext, ItemTaxCalculationLine, ShippingTaxCalculationLine, ItemTaxLineDTO, ShippingTaxLineDTO } from "@medusajs/framework/types"
...
export default class PosTaxProvider implements ITaxProvider {
    ...
    async getTaxLines(
        ...
        console.log("[PosTaxProvider] Context:", JSON.stringify(context.address?.metadata ?? {}, null, 2))
        
        // 1. Is it Tax Exempt?
        // We look for a custom flag in the context or customer metadata
        const grps = context.customer?.customer_groups as any[] || []
        const isExemptGrp = grps.some(g => g === "tax-exempt" || g.name === "tax-exempt" || (typeof g === 'object' && g.name?.toLowerCase().includes("exempt")))
        const isExempt = isExemptGrp || context.address?.metadata?.tax_mode === "exempt"
        
        console.log("[PosTaxProvider] isExempt evaluated to:", isExempt)

        if (isExempt) {
            // Return 0% lines for everything using the official 0% Tax Rate ID
            let taxLines: (ItemTaxLineDTO | ShippingTaxLineDTO)[] = itemLines.map((l) => ({
        ...
        let taxLines: (ItemTaxLineDTO | ShippingTaxLineDTO)[] = itemLines.flatMap((l) => {
            return {
