...
const PACKING_EFFICIENCY = 0.7; // 70% — items don't pack perfectly
 ...
 */
function fitSingleBox(
  items: CartItem[]
): { box: BoxSize; totalWeight: number } | null {
  const totalVolume = items.reduce(
    (sum, item) => sum + itemVolume(item) * item.quantity,
    0
  );
  ...
  const maxItemDim = items.reduce(
    (max, item) => Math.max(max, item.length, item.width, item.height),
    0
  );
...
function fitTwoBoxes(items: CartItem[]): PackageSpec[] {
  ...
  for (const ratio of splitRatios) {
    ...
    const box1 = STANDARD_BOXES.find(
      (b) => b.volume >= vol1 && b.maxWeight >= w1
    );
    const box2 = STANDARD_BOXES.find(
      (b) => b.volume >= vol2 && b.maxWeight >= w2
    );
...
export function packItems(
  ...
  if (longItems.length > 0) {
    ...
    for (const unit of units) {
      ...
      const wouldExceedCross =
        chunkCrossSection + unit.crossSection > LONG_BOX_MAX_CROSS_SECT;
      ...
      if (chunkCount > 0 && (wouldExceedWeight || wouldExceedCross)) {
        ...
        console.log(
          `📦 Long bundle pkg${++pkgNum} (×${chunkCount}): ${maxLength}×${maxWidth}×${maxHeight}in, ${chunkWeight.toFixed(2)}lbs, cross=${chunkCrossSection.toFixed(2)}in²`
        );
        chunkWeight = 0;
        chunkCrossSection = 0;
        chunkCount = 0;
  ...
  if (regularItems.length > 0) {
    ...
    if (single) {
      console.log(
        `📦 Box packing: single ${single.box.name} (${single.box.length}×${single.box.width}×${single.box.height}in, ${single.totalWeight.toFixed(2)}lbs)`
      );
      packages.push({
      ...
      console.log(
        `📦 Box packing: 2 packages — ${twoBoxes.map((p) => `${p.length}×${p.width}×${p.height}in ${p.weight.toFixed(2)}lbs`).join(" + ")}`
      );
