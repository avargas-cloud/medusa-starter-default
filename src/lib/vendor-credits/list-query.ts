...
export interface ListVendorCreditsFilters {
  vendorId?: string;
  status?: string;
  /** Free-text search over number / vendor name / reason / memo (ILIKE). */
  q?: string;
  limit: number;
  offset: number;
}
...
export function buildListVendorCreditsQuery(filters: ListVendorCreditsFilters): BuiltQuery {
  ...
  if (filters.q) {
    ...
    clauses.push(
      `(vc.number ILIKE $${idx} OR vc.vendor_name_snapshot ILIKE $${idx} OR vc.reason ILIKE $${idx} OR vc.memo ILIKE $${idx})`
    );
  }
      ...
      LEFT JOIN LATERAL (
        SELECT json_agg(
                 json_build_object(
                   ...
                   'bill_number', vb.number,
                   'amount_cents', ca.amount_cents
                 ) ORDER BY ca.applied_at DESC
               ) AS applied_to
          FROM vendor_credit_application ca
          JOIN vendor_bill vb ON vb.id = ca.vendor_bill_id
