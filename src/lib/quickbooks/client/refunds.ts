...
export const qbRefunds = {
    ...
    async syncRefund(payload: RefundSyncPayload) {
        if (DRY_RUN) {
            console.log(`[QB-DRY-RUN] Would sync refund ${payload.refundId} for $${payload.amount}`)
            return { success: true, checkTxnId: 'DRY-RUN-CHECK', receivePaymentTxnId: 'DRY-RUN-RECEIVE-PAYMENT' }
        }
        ...
        if ((checkResult as any).status === 'failed' || !(checkResult as any).result?.checkTxnId && !(checkResult as any).result?.CheckRet?.TxnID) {
             // In QBXML, CheckRet has TxnID. The operation queue maps it to txnId. Let's extract properly:
            const checkTxnIdRaw = (checkResult as any).result?.txnId || (checkResult as any).result?.CheckRet?.TxnID || (checkResult as any).txnId;
            if (!checkTxnIdRaw) {
                throw new Error(`Check sync failed or did not return TxnID: ${(checkResult as any).error}`);
            }
        }
        ...
        const checkTxnId = (checkResult as any).txnId || (checkResult as any).result?.CheckRet?.TxnID;
        ...
        if (!rpData.operation_id) {
            throw new Error("Bridge did not return an operation_id for ReceivePayment sync");
        }

        const rpResult = await pollOperationResult(rpData.operation_id);
        if ((rpResult as any).status === 'failed') {
            throw new Error(`ReceivePayment sync failed in QuickBooks: ${(rpResult as any).error}`);
        }

        const rpTxnId = (rpResult as any).txnId || (rpResult as any).result?.ReceivePaymentRet?.TxnID;
