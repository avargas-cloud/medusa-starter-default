 ...
 */
export async function checkCustomersCore(container: any): Promise<CheckCustomersResult> {
    ...
    try {
        ...
        if (!initRes.ok) {
            ...
            logger.error(`❌ ${error}`)
            return { success: false, stats, customersOnlyInQb: [], customersOnlyInMedusa: [], error }
        }
        ...
        logger.info(`📊 Found ${medusaWithQbId.length} Medusa customers with QB IDs`)
