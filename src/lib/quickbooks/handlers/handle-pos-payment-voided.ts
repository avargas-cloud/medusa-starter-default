 ...
 */
export async function handlePosPaymentVoided({ event, container }: SubscriberArgs<any>) {
    ...
    if (payment.metadata?.qb_source === "sales_receipt") {
        logger.info(`${LOG_PREFIX} ⏭️ Sales Receipt payment — void must be done on the SR itself, skipping standalone ReceivePayment void`)
    ...
    logger.info(`${LOG_PREFIX} 🎯 Voiding ReceivePayment TxnID=${qbTxnId} in QuickBooks...`)
    try {
        ...
        logger.info(`${LOG_PREFIX} ✅ Void queued in QB Bridge. OperationID: ${result.operationId}`)
        ...
        logger.error(`${LOG_PREFIX} ❌ Failed to void payment in QB: ${err.message}`)
        try {
            await financeService.updateCustomerPayments({
                id: payment_id,
