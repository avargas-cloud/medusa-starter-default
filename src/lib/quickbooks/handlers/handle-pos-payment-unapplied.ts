...
export async function handlePosPaymentUnapplied({ event, container }: SubscriberArgs<any>) {
    ...
    if (!invoice || !invoice.metadata) {
        logger.warn(`${LOG_PREFIX} pos_invoice ${invoice_id} not found or missing metadata. Cannot find invoiceTxnId.`)
        return
    }
    ...
    if (!invoiceTxnId) {
        logger.error(`${LOG_PREFIX} ❌ Invoice TxnID not found for pos_invoice ${invoice_id}. Cannot unapply payment ${paymentTxnId}.`)
        return
    }

    // 3. Fetch EditSequence by polling QBXML via the Bridge
    logger.info(`${LOG_PREFIX} 🔍 Fetching Payment ${paymentTxnId} from QB to get EditSequence...`)
    
    let editSequence: string | undefined
    try {
        const paymentData = await bridgeFetch("GET", `/api/payments/${paymentTxnId}`)
        const operationId = paymentData?.operationId
        if (!operationId) {
            throw new Error("No operationId returned for payment query")
        }
        
        logger.info(`${LOG_PREFIX} ⏳ Waiting for QB response (Operation ID: ${operationId})...`)
        const rawResult = await pollRawOperationResult(operationId, (msg: string) => logger.info(msg))

        const resObj = rawResult?.QBXML?.QBXMLMsgsRs?.ReceivePaymentQueryRs?.ReceivePaymentRet
        ...
        if (!editSequence) {
            logger.error(`${LOG_PREFIX} ❌ Could not find EditSequence in QB response for Payment ${paymentTxnId}`)
            return
        }
    } catch (err: any) {
        logger.error(`${LOG_PREFIX} ❌ Failed to fetch EditSequence: ${err.message}`)
        return
    }

    // 4. Fire the Bridge Request to Unapply Payment from Invoice!
    logger.info(`${LOG_PREFIX} 🎯 Unapplying Payment (TxnID: ${paymentTxnId}) from Invoice (TxnID: ${invoiceTxnId}) in QB...`)
    ...
    if (!applyResult.success) {
        logger.error(`${LOG_PREFIX} ❌ Failed to unapply payment in QB: ${applyResult.error}`)
    } else {
        const opId = applyResult.data?.operationId
        if (opId && opId !== "DRY_RUN") {
            logger.info(`${LOG_PREFIX} ✅ Unapplication request queued. OperationID: ${opId}`)
