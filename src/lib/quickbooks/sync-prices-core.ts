...
export async function syncPricesCore(
  ...
  try {
    log(
      `💰 Starting QuickBooks PRICE Sync (ONLY)...${dryRun ? " [DRY RUN — no changes will be written]" : ""}`
    );
    log(
    ...
    try {
      ...
      if (wholesalePriceListId) {
        ...
        if (wholesaleGroupId) {
          ...
          warn(
            `⚠️  Wholesale customer group not found — prices will still be stored in price list but without customer group rule`
          );
        }
      } else {
        warn(
          `⚠️  Wholesale Price List "Wholesale Pricing" not found — wholesale prices will NOT be updated`
        );
      }
    } catch (e: any) {
      ...
      if (statusJson.success && statusJson.operation) {
        if (statusJson.operation.status === "completed") {
          ...
          if (queryRs) {
            ...
            qbData = Array.isArray(inventoryItems)
              ? inventoryItems
              : [inventoryItems];
          }
    ...
    for (const variant of qbVariants) {
      ...
      if (
        Math.abs(currentAmount - newPrice) < 0.01 &&
      ...
      if (dryRun) {
        const wsPreview = wholesalePriceListId
          ? ` (wholesale would be: $${smartRound(newPrice * 0.9).toFixed(2)})`
          : "";
        log(
          `   [DRY RUN] Would update ${variant.sku || variant.id}: $${currentAmount.toFixed(2)} → $${newPrice.toFixed(2)}${wsPreview}`
        );
        stats.updatedPrice++;
      ...
      if (wholesalePriceListId && variant.price_set) {
        ...
        if (existingWholesale.length > 0) {
          ...
          if (existingWholesale.length > 1) {
            const duplicateIds = existingWholesale
              .slice(1)
              .map((p: any) => p.id);
    ...
    if (!dryRun) {
      log(
        `⚡ Applying Retail: ${retailUpdates.length} updates, ${retailCreates.length} creates, ${retailDeletes.length} deletes in mass batches...`
      );
      const BULK_SIZE = 500;
      ...
      if (retailUpdates.length > 0) {
        for (let i = 0; i < retailUpdates.length; i += BULK_SIZE) {
          ...
          try {
            ...
            logger.error(`   ❌ Retail update chunk failed: ${err.message}`);
          }
        }
      ...
      if (retailCreates.length > 0) {
        for (let i = 0; i < retailCreates.length; i += BULK_SIZE) {
          ...
          try {
            ...
            logger.error(`   ❌ Retail create chunk failed: ${err.message}`);
          }
        }
      ...
      if (retailDeletes.length > 0) {
        for (let i = 0; i < retailDeletes.length; i += BULK_SIZE) {
          ...
          try {
            ...
            logger.error(`   ❌ Retail delete chunk failed: ${err.message}`);
          }
        }
      ...
      if (wholesalePriceListId) {
        log(
          `⚡ Applying Wholesale: ${wholesaleUpdates.length} updates, ${wholesaleCreates.length} creates, ${wholesaleDeletes.length} deletes in mass batches...`
        );
        ...
        if (wholesaleUpdates.length > 0) {
          for (let i = 0; i < wholesaleUpdates.length; i += BULK_SIZE) {
            ...
            try {
              ...
              logger.error(
                `   ❌ Wholesale update chunk failed: ${err.message}`
              );
            }
          }
        ...
        if (wholesaleCreates.length > 0) {
          for (let i = 0; i < wholesaleCreates.length; i += BULK_SIZE) {
            ...
            try {
              ...
              logger.error(
                `   ❌ Wholesale create chunk failed: ${err.message}`
              );
            }
          }
        ...
        if (wholesaleDeletes.length > 0) {
          for (let i = 0; i < wholesaleDeletes.length; i += BULK_SIZE) {
            ...
            try {
              ...
              logger.error(
                `   ❌ Wholesale delete chunk failed: ${err.message}`
              );
            }
          }
    ...
    log(
      `Updated (Retail):      ${stats.updatedPrice}${dryRun ? " (would update)" : ""}`
    );
    log(
      `Updated (Wholesale):   ${stats.updatedWholesale}${dryRun ? " (would update)" : ""} — auto-calculated at 10% off`
    );
    ...
    if (!dryRun && stats.updatedPrice > 0) {
      ...
      try {
        ...
        warn(
          `⚠️ Meilisearch re-index failed (non-blocking): ${meiliErr.message}`
        );
      }
    }
