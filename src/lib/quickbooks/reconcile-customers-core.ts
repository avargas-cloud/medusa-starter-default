...
export async function reconcileCustomersCore(
    ...
    try {
        ...
        for (const qb of qbCustomers) {
            ...
            if (qb.CompanyName) {
                const normalCompany = qb.CompanyName.toLowerCase().replace(/\s+/g, ' ').trim()
        ...
        log(`📊 Found ${medusaCustomers.length} customers in Medusa database to review.`)
        ...
        for (const customer of medusaCustomers) {
            const medusaFullName = `${customer.first_name || ''} ${customer.last_name || ''}`.toLowerCase().replace(/\s+/g, ' ').trim()
            ...
            else if (actualEmail && !isPlaceholderEmail && qbByEmail.has(actualEmail)) {
            ...
            else if (customer.company_name && qbByCompany.has(customer.company_name.toLowerCase().replace(/\s+/g, ' ').trim())) {
                match = qbByCompany.get(customer.company_name.toLowerCase().replace(/\s+/g, ' ').trim())
                matchType = "company"
            }

            // Analyze Match Result
            const nameDisplay = `${customer.first_name || ''} ${customer.last_name || ''}`.trim() || customer.company_name || 'Unknown'
        ...
        log(`⚠️ Wrong IDs:     ${stats.wrong_id} ${isDryRun ? '(would fix)' : '(fixed)'}`)
        log(`⚠️ Missing IDs:   ${stats.missing_id} ${isDryRun ? '(would assign)' : '(assigned)'}`)
        ...
        if (!isDryRun && updates.length > 0) {
            ...
            if (issues.length > 0) {
                ...
                const examples = issues.filter(i => i.issue !== "unmatched").slice(0, 10)
                for (const ex of examples) {
                    log(`   - ${ex.name} (${ex.email}) [via ${ex.match_method}] -> ID: ${ex.new_qb_id}`)
                }
            }
        }
