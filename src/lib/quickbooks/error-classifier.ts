...
// 3175/3176 ("…already in use. …failed to acquire the lock." / "…could not be
// locked.") are LOCKs, not duplicates — but their message contains "already in
// use", which RX_DUPLICATE also matches. LOCK is evaluated before DUPLICATE in
// classifyQbError, so adding 3175 / 3176 / "could not be locked" / "failed to
// acquire the lock" here routes them to the transient lock class (retry with
// backoff) instead of being mis-classified as a terminal duplicate.
const RX_LOCK =
  /modified by another user|list has been modified|could not be locked|failed to acquire the lock|\b3170\b|\b3175\b|\b3176\b/i;
