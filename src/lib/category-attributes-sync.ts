 ...
 */
export function scheduleCategoryUpdate(
  categoryId: string,
  _scope: any,
  headers?: any
): void {
  console.log(
    `🔄 [SCHEDULER] Scheduling update for ${categoryId}, has headers: ${!!headers}, cookie: ${!!headers?.cookie}`
  );
  ...
  if (!pendingCategories.has(categoryId)) {
    ...
    setTimeout(async () => {
      ...
      console.log(
        `🚀 [SCHEDULER] Executing update for ${categoryId}, captured cookie: ${!!capturedHeaders.cookie}`
      );
      ...
      try {
        ...
        if (!response.ok) {
          console.error(
            `❌ [CATEGORY-ATTRS] Failed to sync category ${categoryId}: ${response.status}`
          );
        } else {
          console.log(
            `✅ [CATEGORY-ATTRS] Successfully queued sync for ${categoryId}`
          );
        }
      } catch (error: any) {
        console.error(
          `❌ [CATEGORY-ATTRS] Error syncing category ${categoryId}:`,
          error.message
        );
      }
    }, 2000);
    ...
    console.log(
      `⏭️  [SCHEDULER] Category ${categoryId} already pending, skipping duplicate`
    );
  }
}
