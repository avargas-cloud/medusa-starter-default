...
import {
  ...
  type ReceiptLine,
  type ReceiptOrigin,
  type ReceiptPostInput,
  type ReceiptPreview,
  type ReceiptReverseInput,
} from "./receipts-types";
import {
  appendReviewEvent,
  reviewCapacity,
...
async function buildReceiptPreview(
  ...
  expectedHash: string,
  feeAttested?: true
): Promise<{ evidence: ReceiptEvidence; preview: ReceiptPreview }> {
  const evidence = await receiptEvidence(client, kind, id);
  if (evidence.blockers.length)
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion -- guarded by the `if (evidence.blockers.length)` above, so index 0 always exists
    throw new BankingError(evidence.blockers[0]!, 409);
  if (evidence.source_hash !== expectedHash)
  ...
  const preview: ReceiptPreview = {
    ...
    day: evidence.source.day,
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion -- amount_cents is only null when a blocker was pushed (BANKING_RECEIPT_AMOUNT_INVALID / BANKING_RECEIPT_SOURCE_MISSING), and the `if (evidence.blockers.length)` above already threw in that case
    ...
    preview_hash: reviewHash({
      source_hash: evidence.source_hash,
      lines: evidence.lines,
      fee_attested: feeAttested ?? false,
      history: history.map((e) => e.id),
    }),
  };
  return { evidence, preview };
}
export async function previewReceiptAccounting(
  kind: ReceiptOrigin,
  id: string,
  input: { expected_source_hash: string; fee_attested?: true }
): Promise<ReceiptPreview> {
  const body = receiptPreviewSchema.parse(input);
  return receiptRead(
    async (client) =>
      (
        await buildReceiptPreview(
          client,
          kind,
          id,
...
export async function postReceiptAccounting(
  ...
  actorId: string,
  key: string,
  input: ReceiptPostInput
): Promise<ReceiptContext> {
  const body = receiptPostSchema.parse(input);
  return runReviewCommand(
    { actorId, key, operation: `receipt_${kind}_post`, entityId: id, body },
    async (client) => {
      const { evidence, preview } = await buildReceiptPreview(
        client,
        kind,
        id,
        body.expected_source_hash,
      ...
      await client.query(
        `INSERT INTO bank_journal_entry(id,receipt_id,deposit_id,transaction_id,kind,day,currency,amount_cents,
      source_hash,source_snapshot,reference,description,actor_id) VALUES($1,$2,$3,$4,$5,$6,'USD',$7,$8,$9::jsonb,$10,$11,$12)`,
        [
          ...
          JSON.stringify({
            ...evidence.snapshot,
            fee_attested: body.fee_attested ?? false,
          }),
      ...
      for (const allocation of evidence.allocations) {
        ...
        await client.query(
          `INSERT INTO bank_receipt_consumption(id,entry_id,receipt_id,payment_id,opening_item_id,amount_cents,origin_kind,origin_id)
        VALUES($1,$2,$3,$4,$5,$6,$7,$8)`,
          [
            ...
            allocation.opening_item_id ?? null,
...
export async function reverseReceiptAccounting(
  ...
  actorId: string,
  key: string,
  input: ReceiptReverseInput
): Promise<ReceiptContext> {
  const body = receiptReverseSchema.parse(input);
  return runReviewCommand(
    { actorId, key, operation: `receipt_${kind}_reverse`, entityId: id, body },
    async (client) => {
      ...
      const active = history.find(
        (e) => e.id === body.posting_id && e.kind === kind && !e.reversed_by
      );
      if (!active)
      ...
      const original = (
        await client.query<Original>(
          "SELECT * FROM bank_journal_entry WHERE id=$1",
          [active.id]
        )
      ).rows[0]!;
      if (kind === "receipt") {
        const consumed = await client.query(
          `SELECT id FROM bank_receipt_consumption c WHERE receipt_id=$1
        AND NOT EXISTS(SELECT 1 FROM bank_journal_entry r WHERE r.reverses_entry_id=c.entry_id) LIMIT 1`,
          [original.receipt_id]
        );
        if (consumed.rowCount)
          throw new BankingError("BANKING_RECEIPT_CONSUMED", 409);
      }
