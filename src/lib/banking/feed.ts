...
export function feedTransaction(value: unknown): FeedTransaction {
  ...
  if (typeof row.pending !== "boolean")
    throw new BankingError("BANKING_INVALID_TRANSACTION", 502);
  return {
    transaction_id: string(row.transaction_id),
    account_id: string(row.account_id),
    pending_transaction_id: nullableString(row.pending_transaction_id),
    amount: decimal(row.amount),
    currency: nullableString(row.iso_currency_code),
    unofficial_currency: nullableString(row.unofficial_currency_code),
    date: date(row.date),
    authorized_date:
      row.authorized_date == null ? null : date(row.authorized_date),
    name: string(row.name),
    merchant_name: nullableString(row.merchant_name),
    pending: row.pending,
    source: row,
  };
}
    ...
    try {
      for (let page = 0; page < 100; page++) {
        const response = await request("/transactions/sync", {
          ...
          ...(batch.cursor ? { cursor: batch.cursor } : {}),
          count: 500,
        ...
        batch.removed.push(
          ...response.removed.map((value: unknown) => {
            const row = object(value);
        ...
        if (
          batch.added.length + batch.modified.length + batch.removed.length >
          bankingLimits().batch
        ) {
        ...
        batch.historicalComplete =
          response.transactions_update_status === "HISTORICAL_UPDATE_COMPLETE";
        batch.initialComplete =
          batch.historicalComplete ||
          response.transactions_update_status === "INITIAL_UPDATE_COMPLETE";
        if (!response.has_more) return batch;
        if (!batch.cursor || seenCursors.has(batch.cursor))
          throw new BankingError("BANKING_CURSOR_NOT_ADVANCING", 502);
