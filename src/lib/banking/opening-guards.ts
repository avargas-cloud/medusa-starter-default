...
export async function assertNoOpeningClear(
  client: PoolClient,
  id: string
): Promise<void> {
...
export async function openingClearProjection<T extends { id: string }>(
...
> {
  const claims = (
    await client.query<{
      ...
      `SELECT claim.id,claim.transaction_id,claim.item_id,item.reference
