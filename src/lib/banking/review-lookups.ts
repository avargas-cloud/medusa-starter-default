...
export async function validateCategory(client: PoolClient, listId: string): Promise<Category> {
  const result = await client.query<Category>(`SELECT qb_list_id AS id,full_name AS name,account_type
    ...
    AND account_type <> 'NonPosting' FOR SHARE`, [listId]);
  if (!result.rows[0]) throw new BankingError("BANKING_CATEGORY_INVALID", 409);
  return result.rows[0];
}

export async function validateCounterparty(client: PoolClient, type: "vendor" | "customer" | null,
  id: string | null): Promise<Counterparty | null> {
  if (!type && !id) return null;
  if (!type || !id) throw new BankingError("BANKING_COUNTERPARTY_INVALID");
  const sql = type === "vendor"
  ...
  if (!result.rows[0]) throw new BankingError("BANKING_COUNTERPARTY_INVALID", 409);
  return result.rows[0];
}
...
export async function lookupAccounts(q: string) {
  ...
  SELECT (SELECT COUNT(*)::text FROM matching) AS count,
    COALESCE((SELECT jsonb_agg(page ORDER BY usage_count DESC,last_used DESC NULLS LAST,name,id) FROM page),'[]'::jsonb) AS accounts`, [q]);
  return { accounts: result.rows[0]!.accounts, count: Number(result.rows[0]!.count) };
}
...
export async function lookupParties(q: string) {
  ...
  SELECT (SELECT COUNT(*)::text FROM matching) AS count,
    COALESCE((SELECT jsonb_agg(page ORDER BY usage_count DESC,last_used DESC NULLS LAST,name,type,id) FROM page),'[]'::jsonb) AS parties`, [q]);
  return { parties: result.rows[0]!.parties, count: Number(result.rows[0]!.count) };
}
