...
export function limitCode(
  kind: "CONNECTION" | "ACCOUNT" | "TRANSACTION" | "RUN" | "WEBHOOK" | "BATCH"
): string {
  return bankingConfig().environment === "production"
    ? `BANKING_${kind}_LIMIT`
    : `BANKING_SANDBOX_${kind}_LIMIT`;
}
