...
export async function openingPeriod(
  client: PoolClient,
  day: string
): Promise<void> {
...
export async function validateOpening(
  ...
  const context = await openingContext(client, id),
    b = context.opening;
  ...
  if (
    live.setup.cut_date !== b.cut_date ||
    live.account.id !== b.account_list_id ||
    live.account.account_type !== b.account_snapshot.account_type ||
    live.account.currency !== "USD"
  ) {
    context.blockers.push("BANKING_OPENING_MAPPING_STALE");
  }
  const duplicate = await client.query(
    `SELECT id FROM bank_opening_balance WHERE status='adopted'
    AND kind=$1 AND account_list_id=$2 AND cut_date=$3 AND id<>$4 LIMIT 1`,
    [b.kind, b.account_list_id, b.cut_date, id]
  );
  if (duplicate.rowCount)
    context.blockers.push("BANKING_OPENING_ALREADY_ADOPTED");
  for (const item of context.items) {
    if (item.payment_id) {
      const payment = await openingPaymentSnapshot(
        client,
        item.payment_id,
        b.cut_date
      );
      ...
      if (
        payment.snapshot.payment_fingerprint !==
    ...
    const claims = await client.query(
      `SELECT 1 FROM bank_opening_item other JOIN bank_opening_balance parent ON parent.id=other.opening_id
    ...
    if (claims.rowCount)
      context.blockers.push("BANKING_OPENING_SOURCE_ALREADY_CLAIMED");
  }
  const overlap = await client.query(
    `SELECT 1 FROM bank_journal_line line JOIN bank_journal_entry entry ON entry.id=line.entry_id
    WHERE line.account_list_id=$1 AND entry.day<$2 AND entry.kind<>'reversal' LIMIT 1`,
    [b.account_list_id, b.cut_date]
  );
  if (overlap.rowCount)
    context.blockers.push("BANKING_OPENING_PRECUT_JOURNAL_OVERLAP");
...
export function openingPreview(context: OpeningContext): OpeningPreview {
  ...
  return {
    ...
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion -- validateOpening throws BANKING_OPENING_BALANCE_UNKNOWN before this is reached when either balance is null
    difference_cents: context.difference_cents!,
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion -- non-null difference_cents (see above) implies book_balance_cents is non-null (openingDifference)
    book_balance_cents: b.book_balance_cents!,
    statement_balance_cents: b.statement_balance_cents,
