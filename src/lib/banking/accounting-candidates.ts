...
type CandidateRow = { kind: DuplicateCandidate["kind"]; id: string; reference: string; amount_cents: string; day: string; state: string };

/** Suggestions never establish identity. Exact source IDs are explicit links; all other hints require a reason. */
export async function expenseCandidates(client: PoolClient, context: SourceContext, reference = ""): Promise<DuplicateCandidate[]> {
  ...
  const rows = (await client.query<CandidateRow>(`WITH bills AS (
  ...
  [source.day, source.amount_cents ?? 0, context.counterparty_type === "vendor" ? context.counterparty_id : null, reference])).rows;
  if (rows.length > 200) throw new BankingError("BANKING_EXPENSE_TOO_MANY_CANDIDATES", 409);
  return rows.map(row => ({ key: `${row.kind}:${row.id}`, kind: row.kind, id: row.id,
    reference: row.reference, amount_cents: Number(row.amount_cents), day: row.day,
    link_path: row.kind === "vendor_bill" ? `/vendor-bills/${row.id}` : row.kind === "wire" ? "/china-finance" : "/reports/profit-loss",
    definite: reference === `${row.kind}:${row.id}` || (row.kind !== "payroll" && reference === row.id),
    fingerprint: reviewHash(row) }));
}

export function validateExpenseResolutions(draft: ExpenseDraft, candidates: DuplicateCandidate[]): void {
  if (draft.nature !== "new_direct_expense" || !draft.attested) throw new BankingError("BANKING_EXPENSE_ATTESTATION_REQUIRED", 409);
  if (new Set(draft.dismissals.map(d => d.key)).size !== draft.dismissals.length) throw new BankingError("BANKING_EXPENSE_DUPLICATE_RESOLUTION", 409);
  const keys = new Set(candidates.map(c => c.key));
  if (draft.dismissals.some(d => !keys.has(d.key))) throw new BankingError("BANKING_EXPENSE_CANDIDATES_CHANGED", 409);
  for (const candidate of candidates) {
    if (candidate.definite) throw new BankingError("BANKING_EXPENSE_ALREADY_RECOGNIZED", 409);
    if (!draft.dismissals.some(d => d.key === candidate.key && d.reason.trim().length >= 8)) {
