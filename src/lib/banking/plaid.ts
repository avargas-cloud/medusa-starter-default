...
export async function plaidRequest(
  ...
  if (!clientId || !secret)
    throw new BankingError("BANKING_PLAID_NOT_CONFIGURED", 503);
  ...
  try {
    ...
    throw new BankingError("BANKING_PROVIDER_UNREACHABLE", 502);
  }
  ...
  if (!response.ok) {
    const code =
      ...
      /^[A-Z0-9_]{1,80}$/.test(result.error_code)
        ? result.error_code
        : "BANKING_PROVIDER_ERROR";
...
export function decimal(value: unknown): string {
  if (typeof value !== "number" && typeof value !== "string")
    throw new BankingError("BANKING_INVALID_AMOUNT", 502);
  const result = String(value);
  if (!/^-?\d+(\.\d+)?$/.test(result) || result.length > 40)
    throw new BankingError("BANKING_INVALID_AMOUNT", 502);
  return result;
}
