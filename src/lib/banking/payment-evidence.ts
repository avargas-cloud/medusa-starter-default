...
export const PAYMENT_APPLICATION_STATUSES = ["available", "partially_applied", "applied"] as const;
export function paymentEconomicStatus(status: string): string {
  return (PAYMENT_APPLICATION_STATUSES as readonly string[]).includes(status) ? "available" : status;
}
...
export const PAYMENT_FINGERPRINT_SQL = LEGACY_PAYMENT_FINGERPRINT_SQL.replace(
  "'status',mp.status", `'fingerprint_version',2,'status',CASE WHEN mp.status IN (${PAYMENT_APPLICATION_STATUSES.map(value => `'${value}'`).join(",")}) THEN 'available' ELSE mp.status END`
);
/** A stored snapshot without a version keeps its original comparison; never rewrite closed evidence. */
export function paymentFingerprintSql(snapshotSql: "dl.payment_snapshot" | "r.match_snapshot") {
  return `(CASE WHEN ${snapshotSql}->>'fingerprint_version'='2' THEN ${PAYMENT_FINGERPRINT_SQL} ELSE ${LEGACY_PAYMENT_FINGERPRINT_SQL} END)`;
}
export function matchesPaymentFingerprint(expected: string, payment: { source_hash: string; legacy_source_hash?: string }): boolean {
  return expected === payment.source_hash || expected === payment.legacy_source_hash;
}
...
export const DEPOSIT_PAYMENT_ELIGIBLE_SQL = PAYMENT_ELIGIBLE_SQL.replace("('ach','zelle','check')", "('cash','ach','zelle','check')") + " AND mp.amount::numeric=trunc(mp.amount::numeric)";
...
export function paymentReservedCentsSql(exclude: {
  deposit?: "$2::text" | "NULL::text"; transaction?: "$3::text" | "t.id" | "NULL::text";
} = {}) {
...
export function depositReservedSql(excludeSql: "$2::text" | "NULL::text") {
