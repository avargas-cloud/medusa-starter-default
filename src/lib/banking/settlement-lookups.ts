...
export type SettlementLookupKind = "card_payment" | "receipt" | "refund" | "reserve_release";
export function listSettlementSources(kind: SettlementLookupKind,q: string) {
  return receiptRead(async client => {
    const search=`%${q.replace(/[\\%_]/g,"\\$&")}%`;
    let rows: Array<Record<string,unknown>>;
    if (kind === "refund") {
      rows=(await client.query(`SELECT p.id,COALESCE(p.reference,p.display_id::text,p.id) AS reference,
        ...
        ORDER BY p.batch_day DESC NULLS LAST,p.id LIMIT 101`,[search])).rows;
    } else if (kind === "reserve_release") {
      rows=(await client.query(`SELECT e.id||':'||l.role AS id,e.reference||' / '||l.role AS reference,e.day,
        ...
        ORDER BY e.day DESC,e.id,l.role LIMIT 101`,[search])).rows;
    } else {
      rows=(await client.query(`SELECT p.id,COALESCE(p.reference,p.display_id::text,p.id) AS reference,p.batch_day AS day,
        ...
        WHERE p.deleted_at IS NULL AND p.source='pos' AND p.type='payment' AND upper(p.currency)='USD'
          ...
          AND (p.id ILIKE $1 OR p.reference ILIKE $1 OR p.display_id::text ILIKE $1)
        ORDER BY p.batch_day DESC NULLS LAST,p.id LIMIT 101`,[search,kind === "receipt"])).rows;
    }
    return {sources:rows.slice(0,100).map(row => ({...row,kind,amount_cents:Number(row.amount_cents),
      available_cents:row.available_cents == null ? null : Number(row.available_cents)})),more:rows.length>100};
  });
}
