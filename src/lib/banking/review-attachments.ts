...
import type { Review, ReviewVersions } from "./review-types";
...
type AttachmentBody = ReviewVersions & {
  name: string;
  mime_type: string;
  content_base64: string;
};

export function validateReviewAttachment(
  body: Pick<AttachmentBody, "name" | "mime_type" | "content_base64">
): Buffer {
  if (
    !body.name.trim() ||
  ...
  if (
    !body.content_base64 ||
    body.content_base64.length > 6990508 ||
    body.content_base64.length % 4 !== 0 ||
    !/^[A-Za-z0-9+/]*={0,2}$/.test(body.content_base64)
  )
    throw new BankingError("BANKING_ATTACHMENT_INVALID");
  const bytes = Buffer.from(body.content_base64, "base64");
  if (
    bytes.toString("base64") !== body.content_base64 ||
    bytes.length === 0 ||
...
export async function addReviewAttachment(
  id: string,
  actorId: string,
  key: string,
  body: AttachmentBody
): Promise<{ attachment: ReviewAttachment | undefined; review: Review }> {
  if (body.mime_type !== "application/pdf")
    throw new BankingError("BANKING_ATTACHMENT_TYPE_INVALID");
  ...
  return runReviewCommand(
    ...
    async (client) => {
      ...
      const replaced = await client.query<ReviewAttachment>(
        `DELETE FROM bank_review_attachment
      WHERE transaction_id=$1 AND deleted_at IS NULL AND detached_at IS NULL
      RETURNING ${ATTACHMENT_COLUMNS}`,
        [id]
      );
      const result = await client.query<ReviewAttachment>(
        `INSERT INTO bank_review_attachment
      ...
      const review = await persistReview(
        ...
        context,
        {
          ...
          confirmed_at: null,
          source_version:
            context.review?.source_version ?? context.tx.source_version,
        },
        actorId,
        replaced.rowCount ? "attachment_replaced" : "attachment_added",
...
export async function detachReviewAttachment(
  attachmentId: string,
  actorId: string,
  key: string,
  body: ReviewVersions
  ...
  return runReviewCommand(
    ...
    async (client) => {
      ...
      FROM bank_review_attachment WHERE id=$1 AND deleted_at IS NULL FOR UPDATE`,
        [attachmentId]
      );
      const attachment = result.rows[0];
      if (!attachment)
        throw new BankingError("BANKING_ATTACHMENT_NOT_FOUND", 404);
      ...
      if (context.review?.status === "excluded")
        throw new BankingError("BANKING_RESTORE_REQUIRED", 409);
      if (attachment.detached_at)
        throw new BankingError("BANKING_ATTACHMENT_ALREADY_DETACHED", 409);
      // Operator decision 2026-09-09: detach hard-deletes the row's bytes, no history kept.
      await client.query("DELETE FROM bank_review_attachment WHERE id=$1", [
        attachmentId,
      ]);
      return {
...
export async function downloadReviewAttachment(id: string): Promise<{
  name: string;
  mime_type: string;
  bytes: Buffer;
}> {
  ...
  const result = await getDbPool().query<
    ReviewAttachment & { content_base64: string }
  >(
    `SELECT
    ...
    WHERE att.id=$1 AND att.deleted_at IS NULL AND c.environment=${bankingEnvSql()}`,
    [id]
  );
  const attachment = result.rows[0];
