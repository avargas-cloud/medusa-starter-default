...
export function settlementTotals(lines: SettlementLine[]): SettlementTotals {
  const totals: SettlementTotals = {
    ...
    net_cents: 0,
    surcharge_audit_cents: 0,
  };
  const fields = {
    receipt: "receipts_cents",
    refund: "refunds_cents",
    chargeback: "chargebacks_cents",
    fee: "fees_cents",
    reserve_hold: "reserve_held_cents",
    reserve_release: "reserve_released_cents",
  } as const;
  for (const line of lines) {
    if (
      !Number.isSafeInteger(line.amount_cents) ||
      line.amount_cents <= 0 ||
      !Number.isSafeInteger(line.surcharge_cents) ||
      line.surcharge_cents < 0
    )
    ...
    totals.net_cents +=
      line.amount_cents *
      (["receipt", "reserve_release"].includes(line.kind) ? 1 : -1);
  }
  if (
    Object.values(totals).some(
      (n) => !Number.isSafeInteger(n) || Math.abs(n) > 999999999999
    )
  )
    throw new BankingError("BANKING_SETTLEMENT_AMOUNT_INVALID");
  return totals;
...
export function settlementStructuralBlockers(
  lines: SettlementLine[]
): string[] {
  const blockers: string[] = [];
  if (!lines.length || lines.length > 100)
    blockers.push("BANKING_SETTLEMENT_LINES_INVALID");
  if (
    new Set(lines.map((l) => `${l.kind}:${l.source_id.trim().toLowerCase()}`))
      .size !== lines.length
  )
    blockers.push("BANKING_SOURCE_DUPLICATED");
  for (const line of lines) {
    if (line.kind !== "receipt" && line.surcharge_cents !== 0)
      blockers.push("BANKING_SETTLEMENT_SURCHARGE_INVALID");
    if (
      ["fee", "reserve_hold"].includes(line.kind)
    ...
    else if (line.documented_capacity_cents < line.amount_cents)
...
export function buildSettlementLines(
  ...
  if (bank.account_type !== "Bank" || bank.currency !== "USD")
  ...
  for (const [index, line] of lines.entries()) {
    ...
    const allowed =
      line.kind === "fee"
        ...
        : ["refund", "chargeback"].includes(line.kind)
          ? ["OtherCurrentLiability", "AccountsPayable"]
          : ["OtherCurrentAsset"];
    if (
      !account ||
    ...
    result.push(
      movementLine(
        `${line.kind === "fee" ? "expense" : "counterpart"}_${index}`,
        account,
        line.amount_cents,
        !["receipt", "reserve_release"].includes(line.kind)
      )
    );
  }
  return result;
