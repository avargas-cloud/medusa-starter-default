...
export const statementCents = z
  ...
  .min(-999999999999)
  .max(999999999999);
const amount = statementCents.refine(
  (value) => value !== 0,
...
export const statementLineSchema = z
  ...
  .strict();
export const statementSaveSchema = z
  .object({
    ...
    opening_balance_cents: statementCents,
    closing_balance_cents: statementCents,
    declared_line_count: z.number().int().min(0).max(1000),
    declared_credits_cents: statementCents.nonnegative(),
    declared_debits_cents: statementCents.nonnegative(),
    completeness_attested: z.boolean(),
    lines: z.array(statementLineSchema).max(1000),
  })
  .strict();
export const statementRevisionSchema = z
  .object({ expected_revision: z.number().int().positive() })
  .strict();
export const statementMatchSchema = statementRevisionSchema.extend({
  allocations: z
    .array(
      z
        .object({
...
export const statementUnmatchSchema = statementRevisionSchema.extend({
  match_ids: z.array(movementId).min(1).max(100),
  reason: z.string().trim().min(8).max(1000),
});
export const statementCloseSchema = statementRevisionSchema.extend({
...
export type StatementDocument = Omit<
  StatementInput,
  "id" | "expected_revision" | "lines"
> & {
  id: string;
  revision: number;
  status: "draft" | "closed";
  ...
  closed_snapshot: StatementSnapshot | null;
  history: unknown[];
};
...
export type StatementMatch = {
  ...
  book_kind: StatementBookItem["kind"];
  book_id: string;
  amount_cents: number;
  book_hash: string;
  line_hash: string;
};
export type StatementOutstanding = {
  kind: StatementBookItem["kind"];
  id: string;
  amount_cents: number;
  source_hash: string;
};
export type StatementSnapshot = {
  ...
  outstanding: StatementOutstanding[];
  lines: Array<{ id: string; hash: string; amount_cents: number }>;
...
export type StatementContext = {
  ...
  lines: StatementLine[];
  book_items: StatementBookItem[];
  matches: StatementMatch[];
  blockers: string[];
  difference_cents: number;
  book_balance_cents: number;
  deposits_in_transit_cents: number;
  outstanding_disbursements_cents: number;
  source_hash: string;
