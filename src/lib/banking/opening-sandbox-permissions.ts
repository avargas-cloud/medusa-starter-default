...
export async function runOpeningPermissions({
  client,
  test,
  bankOpeningId,
  ufOpeningId,
  ...
  const baseline = (
    await client.query(
      `SELECT id,reference,books_evidence_id,revision FROM bank_opening_balance
    WHERE id=ANY($1::text[]) ORDER BY id`,
      [[bankOpeningId, ufOpeningId]]
    )
  ).rows;
  assert(
    baseline.length === 2 &&
      baseline.every((row) => String(row.reference).startsWith(openingPrefix)),
    "Only owned opening fixtures"
  );
  assert(transactionId.startsWith(rootPrefix));
  assert(
    (
      await client.query(
        "SELECT 1 FROM bank_opening_item WHERE id=$1 AND opening_id=$2",
        [itemId, bankOpeningId]
      )
    ).rowCount
  );
  const ledgerCounts = async (): Promise<
    { entries: string; lines: string; consumption: string } | undefined
  > =>
    (
      await client.query<{
        entries: string;
        lines: string;
        consumption: string;
      }>(`SELECT (SELECT count(*)::text FROM bank_journal_entry) AS entries,
    (SELECT count(*)::text FROM bank_journal_line) AS lines,(SELECT count(*)::text FROM bank_receipt_consumption) AS consumption`)
    ).rows[0];
  const ledgerBefore = await ledgerCounts();
  const staff = actor + "_opening_staff",
    permissionId = rootPrefix + "opening_permission";
  ...
  const permission = async (
    canPost: boolean,
    reviewAndClose: boolean
  ): Promise<void> =>
    transaction(client, async () => {
      await withReviewLock(client);
      const changed = await client.query(
        `UPDATE bank_review_permission SET can_post=$2,can_review=$3,can_close=$3
      WHERE id=$1 AND user_id=$4`,
        [permissionId, canPost, reviewAndClose, staff]
      );
      assert(changed.rowCount === 1, "Only the owned permission was changed");
    });
  const call = async (
    route: typeof detailRoute,
    id: string,
    body: TestValue = {}
  ): Promise<{
    status: number;
      ...
      scope: {
        resolve: (name: string) => {
          if (name === "user")
            return {
              retrieveUser: async () => ({
                email: "openings-v10-staff@example.invalid",
              }),
            };
          if (name === POS_USER_MODULE)
            return {
              listPosUsers: async () => [
                { can_view_accounting: accountingVisible },
              ],
            };
  ...
  const cases = [
    ...
    [
      ...
      {
        expected_revision: 2,
        reason: "Must be rejected before any state mutation",
      },
    ],
    [
      "clear",
      ...
      itemId,
      {
        ...
        expected_item_hash: "a".repeat(64),
      },
    ],
    [
      "unclear",
      ...
      itemId,
      {
        clear_id: rootPrefix + "permission_clear",
  ...
  for (const [label, route, id, body] of cases) {
    ...
    test.check(
      denied.status === 403 && denied.result.code === "BANKING_ACCESS_DENIED",
  ...
  const evidenceId = String(
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion -- el assert de arriba (baseline.length===2, ids ANY($1) sobre exactamente [bankOpeningId, ufOpeningId]) ya garantiza que bankOpeningId está entre las filas devueltas
    baseline.find((row) => row.id === bankOpeningId)!.books_evidence_id
  );
  for (const [label, route, id] of [
    ["detail", detailRoute, bankOpeningId],
    ["private evidence", evidenceRoute, evidenceId],
  ] as const) {
    const denied = await call(route, id);
    test.check(
      denied.status === 403 && denied.result.code === "BANKING_ACCESS_DENIED",
      `Opening ${label} denies staff with no banking/accounting access`
    );
  }
  ...
  test.check(
    allowed.status === 200 &&
      (allowed.result.opening as TestValue)?.id === bankOpeningId,
  ...
  test.check(
    evidence.status === 200 &&
      ...
      evidence.bytes.subarray(0, 5).toString() === "%PDF-",
    "Delegated can_post alone reads the actual private PDF bytes"
  );
  if (draftOpeningId) {
    const draft = (
      await client.query(
        "SELECT revision,status FROM bank_opening_balance WHERE id=$1",
        [draftOpeningId]
      )
    ).rows[0];
    assert(draft?.status === "draft");
    const preview = await call(previewRoute, draftOpeningId, {
      expected_revision: draft.revision,
    });
    // This is a positive authorization test only when it reaches a successful real preview.
    test.check(
      preview.status === 200 &&
        /^[a-f0-9]{64}$/.test(String(preview.result.preview_hash)),
  ...
  assert.deepEqual(
    await ledgerCounts(),
    ledgerBefore,
    "Permission probes and previews create zero journal/consumption rows"
  );
  test.check(
    true,
    "Opening permission verification leaves the ledger unchanged"
  );
}
