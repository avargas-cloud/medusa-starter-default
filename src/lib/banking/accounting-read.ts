...
export async function accountingContext(
  client: PoolClient,
  id: string
): Promise<{
  ...
  const draft =
    ...
    attested,dismissals,source_hash FROM bank_direct_expense WHERE transaction_id=$1 AND deleted_at IS NULL`,
        [id]
      )
    ).rows[0] ?? null;
  let candidates: DuplicateCandidate[] = [];
  try {
    candidates = await expenseCandidates(client, context, draft?.reference);
  } catch (error) {
    ...
    if (
      ...
      error.code !== "BANKING_EXPENSE_TOO_MANY_CANDIDATES"
    )
      throw error;
  ...
  const history = (
    await client.query<JournalEntry>(
      `SELECT e.id,e.day,e.kind,e.reference,e.description,
    ...
    FROM bank_journal_entry e WHERE e.transaction_id=$1 ORDER BY e.created_at,e.id`,
      [id, context.source_hash]
    )
  ).rows;
  if (
    context.blockers.includes("BANKING_EXPENSE_RULE_STALE") ||
    context.blockers.includes("BANKING_EXPENSE_CLOSED_EVIDENCE_STALE")
  ) {
...
export async function readAccountingTransaction(
  id: string
...
): Promise<{ transactions: AccountingListRow[]; count: number }> {
  requireBankingEnabled();
  const client = await getDbPool().connect();
  try {
    return await transaction(client, async () => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-non-null-assertion -- SELECT COUNT(*)+jsonb_agg sin GROUP BY siempre devuelve exactamente una fila
      const result = (
        await client.query<{ count: number; page: ListRow[] }>(
    ...
    SELECT (SELECT COUNT(*)::integer FROM matching) AS count,
      ...
      for (const row of result.page) {
        ...
        rows.push({
          ...
          day: source.day,
          name: source.name,
          amount_cents: signedBankCents(row.amount),
          currency: source.currency,
          review_status: source.review_status,
          accounting_status: row.posting_hash
            ? row.reversed
              ? "reversed"
              : "posted"
            : detail.blockers.length
              ...
              : row.draft_hash
                ...
                : "pending",
          stale: Boolean(
            (row.posting_hash ?? row.draft_hash) &&
            ((row.posting_hash ?? row.draft_hash) !== detail.source_hash ||
              detail.blockers.includes("BANKING_EXPENSE_RULE_STALE") ||
              detail.blockers.includes("BANKING_EXPENSE_CLOSED_EVIDENCE_STALE"))
          ),
        });
      }
...
function signedBankCents(amount: string): number | null {
  ...
  // eslint-disable-next-line @typescript-eslint/no-non-null-assertion -- String.split siempre devuelve al menos un elemento, así que `whole` nunca es undefined
