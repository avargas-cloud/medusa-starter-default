...
export async function applyCompletionGuardCorrection(
  client: PoolClient,
  manifest: Array<{ name: string; sha256: string }>
): Promise<void> {
  const file = `${completionDirectory}/migrations-applied.json`,
    prior = JSON.parse(readFileSync(file, "utf8")) as {
      manifest: Array<{ name: string; sha256: string }>;
    };
  ...
  assert(
    reviewedVersions.some(
      (v) =>
        JSON.stringify(v) ===
        JSON.stringify(prior.manifest.map((m) => m.sha256))
    ),
    "Only explicitly measured sandbox SQL versions can receive the reviewed correction"
  );
  await client.query("BEGIN ISOLATION LEVEL REPEATABLE READ READ ONLY");
  const before = await completionPreflight(client);
  await client.query("ROLLBACK");
  const functions = (s: string): RegExpMatchArray[] =>
    Array.from(s.matchAll(/CREATE FUNCTION ([a-z_]+)\([\s\S]*?\$\$;/g));
  const sources = [
    completionMovementSql,
    completionMerchantSql,
    settlementSchemaSql,
    statementGuardSql,
    statementMatchSql,
  ];
  const selected = [
    ...functions(completionClaimSql).filter(
      (m) => m[1] === "bank_completion_validate_claim"
    ),
    ...functions(completionJournalSql).filter(
      (m) => m[1] === "bank_completion_journal_balance"
    ),
    ...functions(completionSchemaSql).filter(
      (m) => m[1] === "bank_completion_document_guard"
    ),
    ...sources.flatMap(functions),
  ];
  assert(selected.length >= 10 && selected.length <= 20);
  const triggers = sources.flatMap((s) =>
    Array.from(s.matchAll(/CREATE (?:CONSTRAINT )?TRIGGER ([a-z_]+)[\s\S]*?;/g))
  );
  const added: string[] = [];
  ...
  try {
    ...
    assert.deepEqual(
      await completionTableFingerprints(client, before.schema.columns),
      before.data
    );
    for (const match of selected)
      try {
        ...
        throw new Error(
          `${match[1]}: ${detail.message}; position=${detail.position}; ${detail.where ?? ""}`
        );
      }
    for (const match of triggers)
      if (
        !(
          await client.query("SELECT 1 FROM pg_trigger WHERE tgname=$1", [
            match[1],
          ])
        ).rowCount
      ) {
        await client.query(match[0]);
        // eslint-disable-next-line @typescript-eslint/no-non-null-assertion -- capture group 1 is a required (non-optional) group in the trigger regex, always present on a match
        added.push(match[1]!);
      }
    assert(
      [0, 6].includes(added.length),
      "Only reviewed movement/merchant guard triggers may be added"
    );
    assert.deepEqual(
      await completionTableFingerprints(client, before.schema.columns),
      before.data
    ...
    assert.deepEqual(
      await completionTableFingerprints(
        client,
        before.migration_tracking.columns
      ),
      before.migration_tracking.fingerprints
    );
  ...
  writeFileSync(
    `${completionDirectory}/guard-correction-${Date.now()}.json`,
    JSON.stringify(
      {
        before,
        prior_manifest: prior.manifest,
        manifest,
        replaced_functions: selected.map((m) => m[1]),
  ...
  writeFileSync(
    file,
    JSON.stringify(
      { ...prior, manifest, correction_at: new Date().toISOString() },
      null,
      2
    ),
    { mode: 0o600 }
  );
  // eslint-disable-next-line no-console -- deliberate CLI/manual-review output for this one-off sandbox migration script
  console.log(
    "PASS reviewed SQL guard correction: existing rows and migration tracking unchanged"
  );
}
