...
export const bankId = (prefix: string): string =>
...
export async function connectionRow(
  ...
  const result = await client.query<ConnectionRow>(
    `SELECT id,provider_item_id,access_token_encrypted,cursor,status,
    ...
    refresh_requested_at FROM bank_connection WHERE id=$1 AND environment=${bankingEnvSql()} AND deleted_at IS NULL`,
    [id]
  );
  if (!result.rows[0])
    throw new BankingError("BANKING_CONNECTION_NOT_FOUND", 404);
  return result.rows[0];
}

/** Session lock also covers network calls. Transaction writes remain short and atomic. */
export async function withBankLock<T>(
  id: string,
  ...
  try {
    const result = await client.query<{ locked: boolean }>(
      "SELECT pg_try_advisory_lock(hashtextextended($1::text, 7241)) AS locked",
      [id]
    );
    ...
    if (locked) {
      try {
        await client.query(
          "SELECT pg_advisory_unlock(hashtextextended($1::text, 7241))",
          [id]
        );
      } catch {
        broken = true;
