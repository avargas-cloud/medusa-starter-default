...
export const dailyConfirmSchema = z
  .object({
    date: reviewDate,
  ...
  .strict();
export const dailyReopenSchema = z
  .object({
    date: reviewDate,
...
export async function confirmDailyReview(
  actorId: string,
  key: string | undefined,
  body: z.infer<typeof dailyConfirmSchema>
): Promise<{ date: string; status: "closed"; revision: number }> {
  return runReviewCommand(
    { actorId, operation: "close_day", entityId: body.date, key, body },
    async (client) => {
      const day = await loadDayClose(client, body.date);
      if ((day?.revision ?? 0) !== body.expected_revision)
        throw new BankingError("BANKING_REVIEW_CONFLICT", 409);
      if (day?.status === "closed")
        throw new BankingError("BANKING_DAY_CLOSED", 409);
      const live = await loadDailyInputs(client, body.date);
      if (live.input_hash !== body.input_hash)
        throw new BankingError("BANKING_DAY_CHANGED", 409);
      if (live.blockers.length)
        throw new BankingError("BANKING_DAY_NOT_READY", 409);
      for (const block of live.snapshot.accounts.filter(
        ...
        for (const row of block.transactions) {
          ...
          await validateCounterparty(
            client,
            review.counterparty_type,
            review.counterparty_id
          );
          if (review.mode === "match") {
            const match = await validateMatchedPayment(
              client,
              row.id,
              review.matched_payment_id ?? ""
            );
            if (match.source_hash !== review.match_snapshot?.source_hash)
              throw new BankingError("BANKING_MATCH_CHANGED", 409);
          } else if (review.mode === "deposit") {
            const deposit = await validateMatchedDeposit(
              client,
              row.id,
              review.matched_deposit_id ?? ""
            );
            assertDepositSourceHash(
              review.deposit_snapshot?.source_hash as string | undefined,
              deposit.source_hash
            );
          } else {
            if (!review.category_list_id)
              throw new BankingError("BANKING_CATEGORY_REQUIRED", 409);
      ...
      const snapshot = {
        ...live.snapshot,
        accounts: live.snapshot.accounts.map((block) => ({
          ...block,
          transactions: block.transactions.map((row) => ({
            ...row,
            day_closed: true,
      ...
      await appendReviewEvent(client, {
        ...
        action: "day_closed",
        actor_id: actorId,
        details: { revision, input_hash: live.input_hash },
      });
...
export async function reopenDailyReview(
  actorId: string,
  key: string | undefined,
  body: z.infer<typeof dailyReopenSchema>
): Promise<{ date: string; status: "open"; revision: number }> {
  return runReviewCommand(
    { actorId, operation: "reopen_day", entityId: body.date, key, body },
    async (client) => {
      const day = await loadDayClose(client, body.date);
      if (!day || day.revision !== body.expected_revision)
        throw new BankingError("BANKING_REVIEW_CONFLICT", 409);
      if (day.status !== "closed" || !day.snapshot)
        throw new BankingError("BANKING_DAY_NOT_CLOSED", 409);
      const history = [
        ...day.history,
        {
      ...
      await client.query(
        `UPDATE bank_day_close SET status='open',revision=$2,history=$3::jsonb,
      ...
      await appendReviewEvent(client, {
        ...
        details: {
          revision,
          previous_revision: day.revision,
          reason: body.reason,
        },
      });
      await applyRulesForAccounts(
        client,
        day.snapshot.accounts.map((block) => block.account.id),
        actorId
      );
