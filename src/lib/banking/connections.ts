...
import { BankingError, bankingErrorCode, encryptBankToken, decryptBankToken, sandboxTokenKey } from "./security";
...
export async function createLinkToken(actorId: string, connectionId?: string) {
  ...
  if (process.env.BANKING_SANDBOX_WEBHOOK_URL) {
    const url = new URL(process.env.BANKING_SANDBOX_WEBHOOK_URL);
...
export async function connectBank(publicToken: string, actorId: string) {
  const key = sandboxTokenKey();
  if (!publicToken.startsWith("public-sandbox-") || publicToken.length > 500) throw new BankingError("BANKING_SANDBOX_TOKEN_REQUIRED");
  ...
  return withBankLock("banking-connect", async (client) => {
    const found = await client.query<{ id: string; access_token_encrypted: string | null }>(
      "SELECT id,access_token_encrypted FROM bank_connection WHERE linked_public_token_hash=$1 AND environment='sandbox'", [hash]);
    ...
    if (!connectionId) {
      ...
      if (Number(count.rows[0]?.count) >= 3) throw new BankingError("BANKING_SANDBOX_CONNECTION_LIMIT", 409);
      ...
      await client.query(`INSERT INTO bank_connection
        ...
        VALUES($1,'plaid','sandbox',$2,$3,$4)`, [connectionId, `pending:${hash}`, actorId, hash]);
      try {
        ...
        if (!accessToken.startsWith("access-sandbox-")) throw new BankingError("BANKING_SANDBOX_TOKEN_REQUIRED");
      ...
      try {
        ...
        if (institutionId) {
          ...
          const duplicate = await lockedClient.query(`SELECT id FROM bank_connection WHERE institution_id=$1
            AND environment='sandbox' AND status<>'disconnected' AND id<>$2 AND deleted_at IS NULL`, [institutionId, id]);
