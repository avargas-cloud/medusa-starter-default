...
export async function createLinkToken(
  ...
  const request: Record<string, unknown> = {
    ...
    client_name: "EcoPowerTech Store POS",
    country_codes: ["US"],
    language: "en",
  };
  ...
  const webhookUrl =
    key.environment === "production"
      ? process.env.BANKING_WEBHOOK_URL
      : process.env.BANKING_SANDBOX_WEBHOOK_URL;
  if (webhookUrl) {
    const url = new URL(webhookUrl);
    if (url.protocol !== "https:" || url.username || url.password)
      throw new BankingError("BANKING_INVALID_WEBHOOK_URL");
  ...
  if (connectionId) {
    return withBankLock(connectionId, async (client) => {
      ...
      request.access_token = decryptBankToken(
...
export async function connectBank(
  ...
  accounts: Awaited<ReturnType<typeof selectedAccountRows>>;
}> {
  const key = bankingTokenKey();
  ...
  if (!publicToken.startsWith(`public-${prefix}-`) || publicToken.length > 500)
    throw new BankingError("BANKING_SANDBOX_TOKEN_REQUIRED");
  ...
  return withBankLock("banking-connect", async (client) => {
    const found = await client.query<{
      ...
      `SELECT id,access_token_encrypted FROM bank_connection WHERE linked_public_token_hash=$1 AND environment=${bankingEnvSql()}`,
      [hash]
    );
    let connectionId = found.rows[0]?.id;
    if (connectionId && !found.rows[0]?.access_token_encrypted)
      throw new BankingError("BANKING_EXCHANGE_OUTCOME_UNKNOWN", 409);
    if (!connectionId) {
      const count = await client.query<{
        count: string;
      }>(`SELECT count(*) FROM bank_connection
        WHERE environment=${bankingEnvSql()} AND status<>'disconnected' AND deleted_at IS NULL`);
      if (Number(count.rows[0]?.count) >= bankingLimits().connections)
        throw new BankingError(limitCode("CONNECTION"), 409);
      ...
      await client.query(
        ...
        VALUES($1,'plaid',${bankingEnvSql()},$2,$3,$4)`,
        [connectionId, `pending:${hash}`, actorId, hash]
      );
      try {
        ...
        if (!accessToken.startsWith(`access-${prefix}-`))
          throw new BankingError("BANKING_SANDBOX_TOKEN_REQUIRED");
        await client.query(
          `UPDATE bank_connection SET provider_item_id=$2,access_token_encrypted=$3,updated_at=now()
          WHERE id=$1`,
          [
            connectionId,
            string(result.item_id),
            encryptBankToken(accessToken, connectionId, key),
          ]
        );
      } catch {
        await client.query(
          "UPDATE bank_connection SET status='error',last_error_code='BANKING_EXCHANGE_OUTCOME_UNKNOWN',updated_at=now() WHERE id=$1",
          [connectionId]
        );
        throw new BankingError("BANKING_EXCHANGE_OUTCOME_UNKNOWN", 409);
      }
    ...
    return withBankLock(id, async (lockedClient) => {
      ...
      if (!connection.access_token_encrypted)
        throw new BankingError("BANKING_CONNECTION_DISCONNECTED", 409);
      try {
        const response = await plaidRequest("/accounts/get", {
          access_token: decryptBankToken(
        ...
        if (institutionId) {
          const result = await plaidRequest("/institutions/get_by_id", {
            institution_id: institutionId,
            country_codes: ["US"],
          });
          name = string(object(result.institution).name);
          const duplicate = await lockedClient.query(
            `SELECT id FROM bank_connection WHERE institution_id=$1
            AND environment=${bankingEnvSql()} AND status<>'disconnected' AND id<>$2 AND deleted_at IS NULL`,
            [institutionId, id]
          );
          if (duplicate.rowCount)
            throw new BankingError("BANKING_RECONNECT_EXISTING_BANK", 409);
        }
        await transaction(lockedClient, async () => {
          await saveAccounts(lockedClient, id, response.accounts);
          await lockedClient.query(
            `UPDATE bank_connection SET institution_id=$2,institution_name=$3,
        ...
        return {
          connection_id: id,
          accounts: await selectedAccountRows(lockedClient, id),
        };
      } catch (error) {
        await lockedClient.query(
          "UPDATE bank_connection SET last_error_code=$2,updated_at=now() WHERE id=$1",
          [id, bankingErrorCode(error)]
        );
        throw error;
      }
    });
