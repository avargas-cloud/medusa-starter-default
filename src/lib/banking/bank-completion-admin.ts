...
type ConstraintRow = { table_name: string; name: string; definition: string };
...
type ConflictsRow = { users: number; providers: number; staff: number };
export const completionAdminIds = {
  user: "user_e2e_bank_completion_admin",
  auth_identity: "authid_e2e_bank_completion_admin",
  provider_identity: "provid_e2e_bank_completion_admin",
} as const;
...
export async function auditCompletionAdmin(client: PoolClient): Promise<{
  columns: ColumnRow[];
  triggers: TriggerRow[];
  constraints: ConstraintRow[];
  indexes: IndexRow[];
  foreignKeys: ForeignKeyRow[];
  counts: Record<string, { count: string; hash: string }>;
  conflicts: ConflictsRow | undefined;
}> {
  ...
  for (const table of tables) {
    counts[table] =
      ...
      md5(COALESCE(string_agg(md5(to_jsonb(t)::text),'' ORDER BY to_jsonb(t)::text),'')) AS hash FROM public."${table}" t`)
      ).rows[0]!;
  }
  const conflicts = (
    await client.query<ConflictsRow>(
      `SELECT
    ...
    (SELECT count(*)::int FROM pos_user WHERE lower(email)=$1) AS staff`,
      ["sandbox@test.com"]
    )
  ).rows[0];
  return {
...
async function foreignAdminHashes(
  client: PoolClient
): Promise<Record<string, { count: string; hash: string }>> {
  const result: Record<string, { count: string; hash: string }> = {};
  for (const table of tables) {
    const id =
      table === "pos_user"
        ? null
        : completionAdminIds[table as keyof typeof completionAdminIds];
    result[table] =
      // eslint-disable-next-line @typescript-eslint/no-non-null-assertion -- count(*) sin GROUP BY siempre devuelve exactamente una fila
      (
        await client.query<{ count: string; hash: string }>(
          `SELECT count(*)::text AS count,
      md5(COALESCE(string_agg(md5(to_jsonb(t)::text),'' ORDER BY to_jsonb(t)::text),'')) AS hash
      FROM public."${table}" t WHERE $1::text IS NULL OR t.id<>$1::text`,
          [id]
        )
      ).rows[0]!;
  }
...
export async function createCompletionAdmin(
  client: PoolClient,
  password: string
): Promise<{
  created: boolean;
  ids: typeof completionAdminIds;
  existing_rows_preserved: boolean;
  pos_user_writes: number;
}> {
  requireBankingSandbox();
  assert(
    typeof password === "string" && password.length >= 8,
    "Explicit sandbox test credential required"
  );
  const dump = resolve(
    __dirname,
    "../../../../sandbox-artifacts/snapshots/pre-bank-completion-v11-v13.dump"
  );
  const tocPath = dump.replace(/dump$/, "toc"),
    metadataPath = dump.replace(/dump$/, "json");
  assert(
    existsSync(dump) && existsSync(tocPath) && existsSync(metadataPath),
    "Verified pre-bootstrap snapshot required"
  );
  ...
  for (const table of tables)
    assert(
      toc.includes(` ${table} `),
      `Snapshot lacks auth prerequisite ${table}`
    );
  const passwordHash = await new CompletionEmailPass(
    { logger },
    {}
  ...
  try {
    ...
    await client.query(
      "SELECT pg_advisory_xact_lock(hashtextextended('banking-completion-test-admin',7241))"
    );
    await client.query(
      'LOCK TABLE public."user",auth_identity,provider_identity IN SHARE ROW EXCLUSIVE MODE'
    );
    await client.query("LOCK TABLE pos_user IN SHARE MODE");
    const audit = await auditCompletionAdmin(client);
    assert.deepEqual(
      audit.conflicts,
      { users: 0, providers: 0, staff: 0 },
      "Existing identity is never updated, relinked or reused"
    );
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion -- audit.counts se llena recorriendo `tables`, que incluye literalmente "user"
    assert.equal(audit.counts.user!.count, "13");
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion -- audit.counts se llena recorriendo `tables`, que incluye literalmente "auth_identity"
    assert.equal(audit.counts.auth_identity!.count, "140");
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion -- audit.counts se llena recorriendo `tables`, que incluye literalmente "provider_identity"
    assert.equal(audit.counts.provider_identity!.count, "50");
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion -- audit.counts se llena recorriendo `tables`, que incluye literalmente "pos_user"
    assert.equal(audit.counts.pos_user!.count, "9");
    assert.deepEqual(
      audit.triggers.map((row) => [
        row.table_name,
        row.trigger_name,
        row.enabled,
      ]),
      [
        ["pos_user", "pos_user_email_lowercase", "O"],
        ["user", "user_email_lowercase", "O"],
      ]
    );
    assert(
      audit.triggers.every(
        (row) =>
          createHash("sha256")
            .update(String(row.function_definition))
            .digest("hex") ===
          "476069d64a831662466dde2e2df309eb1157cc99cf6deabf05523dea19d18469"
      ),
      "Audited lowercase-only trigger function changed"
    );
    assert.equal(
      audit.foreignKeys.length,
      1,
      "Authentication FK closure changed"
    );
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion -- el assert.equal de arriba (length===1) garantiza el índice 0
    ...
    for (const [table, id] of Object.entries(completionAdminIds)) {
      assert.equal(
        (
          await client.query(`SELECT 1 FROM public."${table}" WHERE id=$1`, [
            id,
          ])
        ).rowCount,
        0,
        "Owned admin ID collision"
      );
    }
    const before = await foreignAdminHashes(client);
    await client.query(
      `INSERT INTO public."user"(id,email,first_name,last_name,metadata)
      VALUES($1,$2,'Sandbox','Banking Verification','{"fixture":"e2e_bank_completion_admin"}'::jsonb)`,
      [completionAdminIds.user, "sandbox@test.com"]
    );
    await client.query(
      "INSERT INTO auth_identity(id,app_metadata) VALUES($1,jsonb_build_object('user_id',$2::text))",
      [completionAdminIds.auth_identity, completionAdminIds.user]
    );
    await client.query(
      `INSERT INTO provider_identity(id,entity_id,provider,auth_identity_id,provider_metadata)
      VALUES($1,$2,'emailpass',$3,jsonb_build_object('password',$4::text))`,
      [
        completionAdminIds.provider_identity,
        "sandbox@test.com",
        completionAdminIds.auth_identity,
        passwordHash,
      ]
    );
    assert.deepEqual(
      await foreignAdminHashes(client),
      before,
      "All existing auth/user/POS staff rows remain byte-identical"
    );
    const after = await auditCompletionAdmin(client);
    for (const table of ["user", "auth_identity", "provider_identity"]) {
      assert.equal(
        // eslint-disable-next-line @typescript-eslint/no-non-null-assertion -- `table` viene del array literal ["user","auth_identity","provider_identity"], subset de `tables` que llenó counts
        Number(after.counts[table]!.count),
        // eslint-disable-next-line @typescript-eslint/no-non-null-assertion -- `table` viene del array literal ["user","auth_identity","provider_identity"], subset de `tables` que llenó counts
        Number(audit.counts[table]!.count) + 1
      );
    }
    ...
    const linkage = (
      await client.query(
        `SELECT u.id FROM public."user" u JOIN auth_identity a ON a.app_metadata->>'user_id'=u.id
  ...
  return {
    created: true,
    ids: completionAdminIds,
    existing_rows_preserved: true,
    pos_user_writes: 0,
  };
}

export async function completionInfrastructureHealth(): Promise<
...
> {
  ...
  for (const [name, url] of [
    ["meili", "http://localhost:7799/health"],
    ["backend", "http://localhost:9099/health"],
    ["pos", "http://localhost:3099/login"],
  ] as const) {
    try {
      ...
      checks[name] =
        response.status === 200 ? "healthy" : `http_${response.status}`;
    } catch {
