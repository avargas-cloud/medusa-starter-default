...
): Promise<{ account: AccountSetupRow | undefined }> {
  return runReviewCommand(
    { actorId, operation: "account_setup", entityId: accountId, key, body },
    async (client) => {
      if (body.review_start_date > reviewToday())
        throw new BankingError("BANKING_FUTURE_START_DATE", 400);
      const row = (
        await client.query<{ setup_revision: number; currency: string | null }>(
          `SELECT a.setup_revision,a.currency FROM bank_account a JOIN bank_connection c ON c.id=a.connection_id
       WHERE a.id=$1 AND a.deleted_at IS NULL AND c.environment=${bankingEnvSql()} AND c.deleted_at IS NULL FOR UPDATE OF a`,
          [accountId]
        )
      ).rows[0];
      if (!row) throw new BankingError("BANKING_ACCOUNT_NOT_FOUND", 404);
      if (row.setup_revision !== body.expected_revision)
        throw new BankingError("BANKING_REVIEW_CONFLICT", 409);
      if (!row.currency)
        throw new BankingError("BANKING_CURRENCY_REQUIRED", 409);
      ...
      const result = await client.query<AccountSetupRow>(
        `UPDATE bank_account SET review_start_date=$2::text,
      ...
      RETURNING id,review_start_date,opening_bank_balance,opening_balance_date,
        ...
        [
          accountId,
          body.review_start_date,
          body.opening_bank_balance,
          body.opening_reference,
          body.opening_book_balance ?? null,
        ]
      );
