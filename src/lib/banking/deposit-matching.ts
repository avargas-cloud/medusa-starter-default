...
export function assertDepositSourceHash(
...
export async function validateMatchedDeposit(
  client: PoolClient,
  ...
  if (deposit.status !== "ready")
    throw new BankingError("BANKING_DEPOSIT_NOT_READY", 409);
  if (deposit.stale)
    throw new BankingError("BANKING_DEPOSIT_SOURCE_STALE", 409);
  const valid = await client.query<{ id: string }>(
    `SELECT d.id ${DEPOSIT_MATCH_FROM_SQL}
    WHERE t.id=$1 AND d.id=$2 AND ${DEPOSIT_MATCH_VALID_SQL} FOR SHARE OF d`,
  ...
  for (const line of [...deposit.lines].sort((a, b) =>
    ...
    await validateDepositFunding(
      ...
      depositId,
      deposit.currency,
      // eslint-disable-next-line @typescript-eslint/no-non-null-assertion -- depositAccount ya lanza BANKING_ACCOUNT_SETUP_REQUIRED si review_start_date es null
...
export async function depositSuggestions(
  ids: string[]
): Promise<DepositSuggestion[]> {
  if (!ids.length) return [];
  const result = await getDbPool().query<DepositSuggestion>(
    `WITH ranked AS (SELECT t.id AS transaction_id,
    ...
    SELECT transaction_id,deposit_count,jsonb_build_object('id',id,'reference',reference,'date',date,
      'net_amount',net_amount,'source_hash',source_hash) AS best_deposit FROM ranked WHERE rank=1`,
    [ids]
  );
  return result.rows;
}
