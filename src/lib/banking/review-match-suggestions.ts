...
const idsSchema = z.string().max(12900).transform(value => value.split(","))
  .pipe(z.array(z.string().regex(/^[A-Za-z0-9_-]{1,128}$/)).min(1).max(100))
  .refine(ids => new Set(ids).size === ids.length, "Duplicate transaction IDs");
export const matchSuggestionsQuery = z.object({ ids: idsSchema.optional(), date: reviewDate.optional() })
  .strict().refine(value => Boolean(value.ids) !== Boolean(value.date), "Provide IDs or date");
export type SuggestionBest = { id: string; display_id: number | null; customer_id: string;
  customer_name: string; date: string; reference: string | null; source_hash: string };
export type MatchSuggestion = { transaction_id: string; candidate_count: number; best: SuggestionBest | null;
  reason: string; ambiguous: boolean };
type SuggestionRow = { transaction_id: string; candidate_count: string; best: SuggestionBest | null;
  reference_match: boolean | null; date_distance: number | null };

export function summarizeMatchSuggestion(row: SuggestionRow): MatchSuggestion {
  const count = Number(row.candidate_count);
  if (!Number.isSafeInteger(count) || count < 0) throw new BankingError("BANKING_COUNT_INVALID", 500);
  const reason = count === 0 ? "No individual receipt matches this amount and currency."
    : `${count > 1 ? "Multiple receipts match" : "Same amount and currency"}${row.reference_match ? "; reference matches" : ""}`
      + (row.date_distance === 0 ? "; same date." : `; ${row.date_distance} day(s) apart.`);
  return { transaction_id: row.transaction_id, candidate_count: count, best: row.best,
    reason, ambiguous: count > 1 };
}

/** Read-only overlay. Never called by source ingestion, daily hashes or snapshot writers. */
export async function matchSuggestions(input: { ids?: string[]; date?: string }) {
  if (Boolean(input.ids) === Boolean(input.date) || (input.ids && (!input.ids.length || input.ids.length > 100
  ...
  const deposits = new Map((await depositSuggestions(result.rows.map(row => row.transaction_id))).map(row => [row.transaction_id, row]));
  return { suggestions: result.rows.map(row => ({ ...summarizeMatchSuggestion(row),
