...
const migrationClasses = [
  Migration20260908193000,
  Migration20260908223000,
  Migration20260909005000,
  Migration20260909022000,
  Migration20260909040000,
  Migration20260909060000,
];
...
export async function completionBootstrapManifest(): Promise<
  Array<{ name: string; path: string; sha256: string; sql: string[] }>
> {
  const archived = JSON.parse(
    readFileSync(resolve(completionDirectory, "pre-edit-hashes.json"), "utf8")
  ) as Record<string, string>;
  const migrations = [];
  for (const [index, MigrationClass] of migrationClasses.entries()) {
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion -- completionLegacyMigrations has exactly 6 entries, one per migrationClasses entry being iterated
    ...
    assert.equal(
      hash,
      archived[path],
      `Original WIP migration changed: ${name}`
    );
    assert.equal(
      hash,
      expectedHashes[index],
      `Audited migration content changed: ${name}`
    );
    const instance = Object.create(MigrationClass.prototype) as InstanceType<
      typeof MigrationClass
    >;
    const sql: string[] = [];
    instance.addSql = (statement) => {
...
async function assertEmptyBanking(client: PoolClient): Promise<void> {
  const objects = (
    await client.query(`SELECT 'relation' AS kind,c.relname AS name FROM pg_class c
    ...
    WHERE n.nspname='public' AND starts_with(t.typname,'bank_')`)
  ).rows;
  assert.equal(
    objects.length,
    0,
    "Bootstrap requires an entirely absent Banking schema; partial/installed schema is never replayed"
  );
  assert.equal(
    (
      await client.query(
    ...
    0,
    "Bootstrap migration tracking already exists; inspect instead of replaying"
  );
}
  ...
  return new Promise((done, reject) => {
    ...
    child.on("close", (code) =>
      code === 0
        ...
        : reject(new Error("COMPLETION_SNAPSHOT_TOC_FAILED"))
    );
    child.stdin.end(bytes);
  });
}

async function verifiedBootstrapSnapshot(
  client: PoolClient,
  baseline: Awaited<ReturnType<typeof completionPreflight>>
): Promise<{ path: string; sha256: string; toc_sha256: string }> {
  const containerSystem = execFileSync(
    "sg",
    [
      "docker",
      "-c",
    ...
    { env: cleanProcessEnv, encoding: "utf8", timeout: 15000 }
  ).trim();
  const hostSystem = String(
    (
      await client.query(
  ...
  assert.equal(
    containerSystem,
    hostSystem,
    "Docker snapshot must target the exact inspected PostgreSQL cluster"
  );
  ...
  if (existsSync(snapshot)) {
    assert(
      ...
      "Existing snapshot lacks verification metadata; inspect before retry"
    );
    bytes = readFileSync(snapshot);
    const prior = JSON.parse(readFileSync(metadataPath, "utf8")) as {
      sha256: string;
      system_identifier: string;
      data: unknown;
      protected_data: unknown;
      migration_tracking: unknown;
    };
    ...
    assert.deepEqual(
      prior.data,
      baseline.data,
      "Snapshot source rows changed; never reuse a stale snapshot"
    );
  ...
  assert.deepEqual(
    ...
    baseline.protected_data,
    "Source rows changed while snapshot was taken"
  );
  ...
  if (!existsSync(metadataPath))
    writeFileSync(
      ...
      JSON.stringify(
        {
          sha256: sha256(bytes),
          system_identifier: hostSystem,
          data: baseline.data,
          protected_data: baseline.protected_data,
          migration_tracking: baseline.migration_tracking,
          created_at: new Date().toISOString(),
          purpose: "Empty Banking schema bootstrap before V11-V13",
        },
        null,
        2
      ),
...
export async function bootstrapCompletionPrerequisites(
  client: PoolClient
): Promise<{
  scope: string;
  executed_at: string;
  archive: { path: string; sha256: string; toc_sha256: string };
  migrations: Array<{
    name: string;
    path: string;
    sha256: string;
  ...
  const existingNames = (
    await client.query(
      "SELECT id,name,executed_at FROM mikro_orm_migrations ORDER BY id"
    )
  ).rows;
  assert.equal(
    existingNames.length,
    296,
    "Approved bootstrap baseline is exactly 296 existing migration records"
  );
  ...
  try {
    ...
    assert.deepEqual(
      ...
      "Migration tracking changed after the verified snapshot"
    );
    const sourceTables = [
      ...new Set([
        ...Object.keys(baseline.data),
    ...
    await client.query(
      `LOCK TABLE ${sourceTables.map((table) => `public."${table}"`).join(",")} IN SHARE MODE`
    );
    assert.deepEqual(
      await completionTableFingerprints(client, baseline.schema.columns),
      baseline.data
    ...
    const tables = (
      ...
      WHERE schemaname='public' AND starts_with(tablename,'bank_') ORDER BY tablename`)
    ).rows.map((row) => row.tablename);
    assert.deepEqual(
      tables,
      expectedTables,
      "Bootstrap creates exactly 23 approved Banking tables"
    );
    const bankingData = await completionTableFingerprints(
      client,
      await completionColumns(client, tables)
    );
    assert(
      Object.values(bankingData).every((table) => table.count === "0"),
      "Bootstrap must never seed banking rows"
    );
    const triggers = (
      await client.query(
    ...
    const functions = (
      await client.query(`SELECT p.proname,pg_get_functiondef(p.oid) AS definition FROM pg_proc p
      JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='public' AND starts_with(p.proname,'bank_') ORDER BY p.proname`)
    ).rows;
    assert.equal(functions.length, 12);
    assert(
      functions
        .find((row) => row.proname === "bank_receipt_check_consumption")
        ?.definition.includes("NEW.opening_item_id IS NOT NULL"),
    ...
    assert.deepEqual(
      records.filter(
        (row) => !completionLegacyMigrations.includes(row.name as string)
      ),
      existingNames
    );
    assert.deepEqual(
      records
        .filter((row) =>
          completionLegacyMigrations.includes(row.name as string)
        )
        .map((row) => row.name)
        .sort(),
  ...
  const record = {
    scope: "empty-schema-bootstrap-delta",
    executed_at: new Date().toISOString(),
    archive,
    migrations: migrations.map(({ name, path, sha256: hash, sql }) => ({
      name,
      path,
      sha256: hash,
      statement_count: sql.length,
    })),
    table_count: 23,
    function_count: 12,
    trigger_count: 20,
  ...
  writeFileSync(
    resolve(completionDirectory, `bootstrap-${Date.now()}.json`),
