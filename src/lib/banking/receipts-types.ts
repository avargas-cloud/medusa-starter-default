...
export type ReceiptSetup = {
  id: string;
  revision: number;
  cut_date: string;
  currency: "USD";
  ar_account: AccountingAccount;
  clearing_account: AccountingAccount;
  attested: true;
  frozen: boolean;
};
export type ReceiptSetupContext = {
...
export const receiptSetupSchema = z
  .object({
    ...
    cut_date: reviewDate,
    ar_account_list_id: z.string().min(1).max(128),
    clearing_account_list_id: z.string().min(1).max(128),
    local_usd_attested: z.literal(true),
  })
  .strict();
export const receiptPreviewSchema = z
  .object({
    expected_source_hash: z.string().regex(/^[a-f0-9]{64}$/),
    fee_attested: z.literal(true).optional(),
  })
  .strict();
export const receiptPostSchema = receiptPreviewSchema.extend({
  preview_hash: z.string().regex(/^[a-f0-9]{64}$/),
});
export const receiptReverseSchema = z
  .object({
    posting_id: z.string().min(1).max(128),
    day: reviewDate,
