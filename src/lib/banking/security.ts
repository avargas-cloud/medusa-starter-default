...
export type BankingKeyRing = {
  environment: BankingEnvironment;
  active: BankingKey;
  byId(id: string): BankingKey | undefined;
};
...
function productionConfigError(): string | null {
  ...
  if (!actual || !["postgres:", "postgresql:"].includes(actual.protocol))
    return "BANKING_DATABASE_NOT_CONFIGURED";
  if (
    actual.host !== match[1] ||
    actual.port !== match[2] ||
    actual.pathname !== match[3]
  )
    return "BANKING_DATABASE_TARGET_MISMATCH";
  if (isSandboxTarget(actual)) return "BANKING_DATABASE_TARGET_MISMATCH";
  if (!process.env.PLAID_CLIENT_ID || !process.env.PLAID_PRODUCTION_SECRET)
    return "BANKING_PLAID_NOT_CONFIGURED";
  const webhook = httpsUrlError(
    process.env.BANKING_WEBHOOK_URL,
    "BANKING_INVALID_WEBHOOK_URL"
  );
  if (webhook) return webhook;
  const redirect = httpsUrlError(
    process.env.BANKING_OAUTH_REDIRECT_URI,
    "BANKING_INVALID_REDIRECT_URI"
  );
...
function productionKeyRing(): BankingKeyRing | null {
  ...
  let parsed: unknown;
  try {
    parsed = JSON.parse(process.env.BANKING_TOKEN_KEYS_JSON || "");
  } catch {
    return null;
  }
...
export function bankingConfig(): {
  ...
  const requested =
    ...
    process.env.BANKING_ENV === "production";
  if (!requested)
    return {
      enabled: false,
      environment: "sandbox",
      unavailable_reason: "BANKING_SANDBOX_ONLY",
      config_error: null,
    };
  ...
  if (error)
    return {
      ...
      unavailable_reason: "BANKING_CONFIG_ERROR",
      config_error: error,
    };
  return {
    enabled: true,
...
export function requireBankingSandbox(): void {
  if (bankingConfig().environment !== "sandbox" || !bankingConfig().enabled)
    throw new BankingError("BANKING_SANDBOX_ONLY", 503);
...
export function requireBankingEnabled(): BankingEnvironment {
  ...
  if (!config.enabled)
    throw new BankingError(
      config.unavailable_reason ?? "BANKING_SANDBOX_ONLY",
      503
    );
...
export function manualRefreshAllowed(): boolean {
  return (
    bankingConfig().environment !== "production" ||
    process.env.BANKING_MANUAL_REFRESH === "true"
  );
}
...
export function bankingEnvSql(): "'sandbox'" | "'production'" {
  return bankingConfig().environment === "production"
    ? "'production'"
    : "'sandbox'";
}
...
export function bankingTokenKey(): BankingKeyRing {
  if (requireBankingEnabled() === "sandbox") {
    ...
    return {
      environment: "sandbox",
      active,
      byId: (id) => (id === active.id ? active : undefined),
    };
  }
...
function aadFor(
  ...
  return Buffer.from(
    environment === "sandbox"
      ? `banking:sandbox:${connectionId}:v1`
      : `banking:production:${connectionId}:${keyId}`
  );
}
...
export function encryptBankToken(
  token: string,
  connectionId: string,
  material: BankingKeyRing | Buffer
): string {
  const ring = ringOf(material);
  if (ring.active.key.length !== 32 || !token || !connectionId)
    throw new BankingError("BANKING_ENCRYPTION_INVALID");
...
export function decryptBankToken(
  ...
  try {
    ...
    if (envelope.version === 1) {
      if (ring.environment !== "sandbox" || envelope.key_id !== "sandbox-v1")
        throw new Error();
    } else if (envelope.version === 2) {
      if (
        ring.environment !== "production" ||
        !PRODUCTION_KEY_ID.test(envelope.key_id)
      )
        throw new Error();
