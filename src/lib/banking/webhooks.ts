...
export async function verifyBankWebhook(
  raw: Buffer,
  signature: string,
  getKey: (kid: string) => Promise<JWK> = async (kid) =>
    object(
      (await plaidRequest("/webhook_verification_key/get", { key_id: kid })).key
    )
  ...
  try {
    ...
    if (
      !timingSafeEqual(digest, Buffer.from(payload.request_body_sha256, "hex"))
    )
      throw new Error();
...
export async function receiveBankWebhook(
  raw: Buffer,
  signature: string
): Promise<{ received: boolean }> {
  requireBankingEnabled();
  const payload = await verifyBankWebhook(raw, signature);
  if (payload.environment !== bankingConfig().environment)
    throw new BankingError("BANKING_WEBHOOK_ENVIRONMENT_MISMATCH", 400);
  ...
  try {
    await transaction(client, async () => {
      ...
      const duplicate = await client.query(
        "SELECT id FROM bank_webhook_event WHERE event_digest=$1",
        [digest]
      );
      ...
      if (cap !== null) {
        const count = await client.query<{ count: string }>(
          "SELECT count(*) FROM bank_webhook_event"
        );
        if (Number(count.rows[0]?.count) >= cap)
          throw new BankingError(limitCode("WEBHOOK"), 409);
      }
      await client.query(
        `INSERT INTO bank_webhook_event(id,provider,environment,connection_id,event_digest,event_type,payload,received_at)
...
export async function drainBankWebhooks(): Promise<void> {
  requireBankingEnabled();
  for (let i = 0; i < 50; i++) {
    const client = await getDbPool().connect();
    ...
    try {
      const processed = await transaction(client, async () => {
        ...
        const connection = await client.query<{ id: string }>(
          `SELECT id FROM bank_connection
          WHERE provider_item_id=$1 AND environment=${bankingEnvSql()} AND deleted_at IS NULL`,
          [itemId]
        );
        const connectionId = connection.rows[0]?.id;
        if (!connectionId)
          throw new BankingError("BANKING_WEBHOOK_ITEM_UNKNOWN", 409);
        ...
        const lock = await client.query<{ locked: boolean }>(
          "SELECT pg_try_advisory_xact_lock(hashtextextended($1::text, 7241)) AS locked",
          [connectionId]
        );
        ...
        const current = await client.query<{ status: string }>(
          "SELECT status FROM bank_connection WHERE id=$1 AND deleted_at IS NULL",
          [connectionId]
        );
        if (!current.rows[0])
          throw new BankingError("BANKING_WEBHOOK_ITEM_UNKNOWN", 409);
        const error =
          event.payload.error && typeof event.payload.error === "object"
            ? object(event.payload.error).error_code
            : null;
        ...
        if (current.rows[0].status !== "disconnected") {
          await client.query(
            `UPDATE bank_connection SET sync_requested_at=now(),
            ...
            [connectionId, pending, loginRequired || revoked, repaired]
          );
        }
        await client.query(
          `UPDATE bank_webhook_event SET connection_id=$2,status='processed',attempts=attempts+1,
          processed_at=now(),last_error_code=NULL,updated_at=now() WHERE id=$1`,
      ...
      if (eventId)
        await client.query(
          `UPDATE bank_webhook_event SET status='failed',attempts=attempts+1,
        last_error_code=$2,updated_at=now() WHERE id=$1`,
          [eventId, bankingErrorCode(error)]
        );
      else throw error;
    } finally {
      client.release();
