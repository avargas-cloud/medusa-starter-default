...
export async function saveAccounts(
  ...
  if (!Array.isArray(input))
    throw new BankingError("BANKING_INVALID_ACCOUNTS", 502);
  ...
  for (const value of input) {
    ...
    if (providerIds.includes(providerId))
      throw new BankingError("BANKING_DUPLICATE_ACCOUNT", 502);
    ...
    if (persistentId) {
      const existing = await client.query<{
        ...
        `SELECT a.id,a.connection_id FROM bank_account a JOIN bank_connection c ON c.id=a.connection_id
         WHERE a.persistent_account_id=$1 AND c.environment=${bankingEnvSql()} AND c.status <> 'disconnected'`,
        [persistentId]
      );
      if (
        existing.rows.some((account) => account.connection_id !== connectionId)
      ) {
        throw new BankingError("BANKING_DUPLICATE_CONNECTION", 409);
      }
      if (existing.rows.length > 1)
        throw new BankingError("BANKING_AMBIGUOUS_ACCOUNT", 409);
      if (existing.rows[0])
        await client.query(
          "UPDATE bank_account SET provider_account_id=$2,updated_at=now() WHERE id=$1",
    ...
    const balances = {
      ...
      unofficial_currency_code: nullableString(
    ...
    await client.query(
      ...
      [
        ...
        persistentId,
        string(row.name),
        nullableString(row.official_name),
        nullableString(row.mask),
        string(row.type),
  ...
  await client.query(
    `UPDATE bank_account SET is_active=false,updated_at=now()
    WHERE connection_id=$1 AND NOT(provider_account_id=ANY($2::text[]))`,
    [connectionId, providerIds]
  );
  const count = await client.query<{ count: string }>(
    "SELECT count(*) FROM bank_account WHERE is_active AND deleted_at IS NULL"
  );
  if (Number(count.rows[0]?.count) > bankingLimits().accounts)
    throw new BankingError(limitCode("ACCOUNT"), 409);
}

export type SelectedAccountRow = {
  id: string;
  connection_id: string;
  name: string;
  mask: string | null;
...
export async function selectedAccountRows(
  client: PoolClient,
  connectionId: string
): Promise<SelectedAccountRow[]> {
  return (
    await client.query<SelectedAccountRow>(
      `SELECT id,connection_id,name,mask,type,subtype,currency,is_selected AS selected,qb_list_id,
    balances->>'current' AS current_balance,balances->>'available' AS available_balance
    FROM bank_account WHERE connection_id=$1 AND is_active=true AND deleted_at IS NULL ORDER BY name,id`,
      [connectionId]
    )
  ).rows;
}
