...
import {
  receiptSetupSchema,
  type ReceiptSetup,
  type ReceiptSetupContext,
  type ReceiptSetupInput,
} from "./receipts-types";
import {
  reviewCapacity,
  runReviewCommand,
...
export async function receiptRead<T>(
...
export function receiptMapping(
  ...
  return {
    ...account,
    qb_currency_ref: account.currency,
    currency: known ?? (local ? "USD" : null),
  };
}
...
export async function receiptSetup(
  client: PoolClient
): Promise<ReceiptSetup | null> {
  return (
    (
...
export async function receiptSetupContext(
  client: PoolClient
): Promise<ReceiptSetupContext> {
  const accounts = await receiptAccounts(client);
  return {
    setup: await receiptSetup(client),
    ar_accounts: accounts
      .filter((a) => a.account_type === "AccountsReceivable")
      .filter((a) => receiptMapping(a, true).currency === "USD"),
    clearing_accounts: accounts
      .filter((a) => a.account_type === "OtherCurrentAsset")
      .filter((a) => receiptMapping(a, true).currency === "USD"),
    coverage: "partial",
  };
}
export const readReceiptSetup = (): Promise<ReceiptSetupContext> =>
  receiptRead(receiptSetupContext);
export async function saveReceiptSetup(
  actorId: string,
  key: string,
  input: ReceiptSetupInput
): Promise<ReceiptSetupContext> {
  const body = receiptSetupSchema.parse(input);
  return runReviewCommand(
    { actorId, key, operation: "receipt_setup", entityId: "local-usd", body },
    async (client) => {
      const previous = await receiptSetup(client);
      if (previous?.frozen)
        throw new BankingError("BANKING_RECEIPT_SETUP_FROZEN", 409);
      if ((previous?.revision ?? 0) !== body.expected_revision)
        throw new BankingError("BANKING_RECEIPT_SETUP_STALE", 409);
      if (body.cut_date > reviewToday())
        throw new BankingError("BANKING_RECEIPT_CUT_INVALID", 409);
      const accounts = await receiptAccounts(client, [
        body.ar_account_list_id,
        body.clearing_account_list_id,
      ]);
      const ar = accounts.find((a) => a.id === body.ar_account_list_id);
      const clearing = accounts.find(
        (a) => a.id === body.clearing_account_list_id
      );
      if (
        !ar ||
        ...
        ar.id === clearing.id ||
        receiptMapping(ar, true).currency !== "USD" ||
        receiptMapping(clearing, true).currency !== "USD"
      ) {
      ...
      await client.query(
        `INSERT INTO bank_accounting_setup
      ...
      actor_id=EXCLUDED.actor_id,updated_at=now()`,
        [
          ...
          ar.id,
          clearing.id,
          JSON.stringify(receiptMapping(ar, true)),
          JSON.stringify(receiptMapping(clearing, true)),
          actorId,
        ]
      );
