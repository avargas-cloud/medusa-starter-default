...
export const receiptEvidence = (
  ...
  kind === "receipt"
    ? paymentReceiptSource(client, id)
    : kind === "deposit"
      ? depositReceiptSource(client, id)
      : matchReceiptSource(client, id);
...
export async function receiptContext(
  ...
  if (kind === "receipt") {
    const capacity = (
      ...
      COALESCE((SELECT ${paymentReservedCentsSql()} FROM customer_payment mp WHERE mp.id=$1),0) AS reserved`,
        [id]
      )
    ).rows[0];
    ...
    available =
      ...
      !evidence.blockers.length
        ? Math.max(0, posting.amount_cents - Number(capacity?.reserved ?? 0))
        : 0;
  }
  if (posting?.stale && !posting.reversed_by)
    evidence.blockers.push("BANKING_RECEIPT_SOURCE_DRIFT");
  return {
    source: evidence.source,
    source_hash: evidence.source_hash,
    eligible: evidence.blockers.length === 0,
...
export const readReceiptAccounting = (
  kind: ReceiptOrigin,
  id: string
): Promise<ReceiptContext> =>
  receiptRead((client) => receiptContext(client, kind, id));
...
export const listReceiptAccounting = (
  kind: ReceiptOrigin,
  filters: ReceiptFilters
): Promise<{ items: ReceiptContext[]; count: number }> =>
  receiptRead(async (client) => {
    // Posted identities drive exception coverage even after sources are deleted or refunded.
    const query =
      kind === "receipt"
        ? `SELECT COALESCE(a.payment_id,mp.id) AS id,COALESCE(posted.day,mp.batch_day,'') AS day
    ...
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion -- COUNT(*) aggregate without GROUP BY always returns exactly one row
    const result = (
      await client.query<{ count: number; items: Array<{ id: string }> }>(
        `WITH sources AS (${query}),
    ...
    for (const row of result.items)
      items.push(await receiptContext(client, kind, row.id));
