...
export const ruleSchema = z
  .object({
    ...
    account_id: bankIdSchema,
    active: z.boolean(),
    priority: z.number().int().min(0).max(10000),
    match_field: z.enum(["merchant", "description"]),
  ...
  .strict();
export const ruleSaveSchema = ruleSchema.extend({
...
async function prepare(
  ...
  Awaited<ReturnType<typeof planRuleChanges>> & {
    candidate: ReviewRule;
    existing: ReviewRule | null | undefined;
  }
> {
  const existing = input.id
    ? (
        await client.query<ReviewRule>(
          "SELECT * FROM bank_review_rule WHERE id=$1 AND deleted_at IS NULL",
          [input.id]
        )
      ).rows[0]
    : null;
  if (input.id && !existing)
    throw new BankingError("BANKING_RULE_NOT_FOUND", 404);
  ...
  const account = (
    await client.query<{
     ...
     WHERE a.id=$1 AND a.deleted_at IS NULL AND c.deleted_at IS NULL AND c.environment=${bankingEnvSql()}`,
      [input.account_id]
    )
  ).rows[0];
  if (!account) throw new BankingError("BANKING_ACCOUNT_NOT_FOUND", 404);
  if (!account.review_start_date)
    throw new BankingError("BANKING_SETUP_REQUIRED", 409);
  if (account.currency?.toUpperCase() !== input.currency)
    throw new BankingError("BANKING_CURRENCY_MISMATCH", 409);
  await validateCategory(client, input.category_list_id);
  const party = await validateCounterparty(
    client,
    input.counterparty_type ?? null,
    input.counterparty_id ?? null
  );
  // Stable draft identity makes equal-priority ordering identical in preview and save.
  const id =
    input.id ?? `brule_${stableReviewHash({ actorId, input }).slice(0, 32)}`;
  if (
    !existing &&
    (await client.query("SELECT id FROM bank_review_rule WHERE id=$1", [id]))
      .rowCount
  ) {
    throw new BankingError("BANKING_RULE_ALREADY_EXISTS", 409);
  }
  const candidate: ReviewRule = {
    id,
...
export async function listReviewRules(): Promise<{
  ...
  const rules = (
    await getDbPool()
      .query(`SELECT r.*,q.full_name AS category_name,q.account_type,
    ...
    WHERE r.deleted_at IS NULL ORDER BY r.priority,r.id`)
  ).rows;
  return { rules, count: rules.length };
}

export async function previewReviewRule(
  actorId: string,
  input: RuleInput
): Promise<
  Pick<
  ...
  try {
    return await transaction(client, async () => {
      ...
      const {
        ...
        skipped_manual,
        skipped_closed,
      } = await prepare(client, actorId, input);
      return {
        preview_hash,
        affected_count,
        days,
...
export async function saveReviewRule(
  ...
  body: z.infer<typeof ruleSaveSchema>
): Promise<{ rule: ReviewRule; applied_count: number }> {
  const { preview_hash, ...input } = body;
  return runReviewCommand(
    { actorId, operation: "save_rule", entityId: input.id ?? "new", key, body },
    async (client) => {
      const plan = await prepare(client, actorId, input);
      if (plan.preview_hash !== preview_hash)
        throw new BankingError("BANKING_RULE_PREVIEW_STALE", 409);
      if (
        !plan.existing &&
        Number(
          (await client.query("SELECT count(*) AS n FROM bank_review_rule"))
            .rows[0].n
        ) >= 100
      ) {
