...
export async function loadReviewContext(
  ...
  const result = await client.query<ReviewTransaction>(
    ...
    WHERE t.id=$1 AND t.deleted_at IS NULL AND a.deleted_at IS NULL
      AND c.deleted_at IS NULL AND c.environment=${bankingEnvSql()} FOR UPDATE OF t`,
    [id]
  );
  const tx = result.rows[0];
  if (!tx) throw new BankingError("BANKING_TRANSACTION_NOT_FOUND", 404);
  const previous = await client.query<Review>(
    `SELECT ${REVIEW_COLUMNS}
    FROM bank_transaction_review WHERE transaction_id=$1 AND deleted_at IS NULL FOR UPDATE`,
    [id]
  );
  const review = previous.rows[0] ?? null;
  if (
    (review?.revision ?? 0) !== versions.expected_revision ||
    tx.source_version !== versions.expected_source_version
  ) {
...
export async function persistReview(
  ...
  const nextPayment =
    changes.matched_payment_id === undefined
      ? before?.matched_payment_id
      : changes.matched_payment_id;
  if (
    nextMode !== before?.mode ||
    nextPayment !== before?.matched_payment_id ||
    changes.status === "excluded"
  ) {
    ...
    if (posted.rowCount)
      throw new BankingError("BANKING_MATCH_ACCOUNTING_REVERSAL_REQUIRED", 409);
  }
  if (!before) await reviewCapacity(client, "bank_transaction_review", 2000);
  const next: Review = {
    id: before?.id ?? bankId("btr"),
    transaction_id: context.tx.id,
    revision: 0,
    source_version: context.tx.source_version,
    status: "draft",
    mode: "categorize",
  ...
  const values = [
    ...
    next.status,
    next.mode,
    next.category_list_id,
    next.counterparty_type,
    next.counterparty_id,
    next.counterparty_name,
    next.comment,
    next.matched_payment_id,
    JSON.stringify(next.match_snapshot),
    JSON.stringify(next.category_snapshot),
    next.origin,
    next.rule_id,
    next.rule_version,
    ...
    next.confirmed_by,
    next.confirmed_at,
    next.exclusion_reason,
    next.matched_deposit_id,
    JSON.stringify(next.deposit_snapshot),
  ];
  const result = await client.query<Review>(
    `INSERT INTO bank_transaction_review (${REVIEW_COLUMNS})
    ...
    exclusion_reason=EXCLUDED.exclusion_reason,updated_at=now() RETURNING ${REVIEW_COLUMNS}`,
    values
  );
  await appendReviewEvent(client, {
    ...
    action,
    actor_id: actorId,
    details: { ...details, before, after: result.rows[0] },
  });
  // eslint-disable-next-line @typescript-eslint/no-non-null-assertion -- upsert por PK (transaction_id) del INSERT...ON CONFLICT DO UPDATE de arriba siempre afecta y devuelve exactamente 1 fila
...
export type SaveReviewBody = ReviewVersions & {
  ...
  expected_deposit_source_hash?: string | null;
...
export async function saveTransactionReview(
  id: string,
  actorId: string,
  key: string,
  body: SaveReviewBody
  ...
  return runReviewCommand(
    { actorId, key, operation: "review_save", entityId: id, body },
    async (client) => {
      const context = await loadReviewContext(client, id, body);
      if (context.review?.status === "excluded")
        throw new BankingError("BANKING_RESTORE_REQUIRED", 409);
      const category =
        body.mode === "categorize" && body.category_list_id
          ? await validateCategory(client, body.category_list_id)
          : null;
      const match =
        body.mode === "match" && body.matched_payment_id
          ? await validateMatchedPayment(client, id, body.matched_payment_id)
          : null;
      if (match)
        assertMatchSourceHash(
          body.expected_match_source_hash,
          match.source_hash,
          match.legacy_source_hash
        );
      const deposit =
        body.mode === "deposit" && body.matched_deposit_id
          ? await validateMatchedDeposit(client, id, body.matched_deposit_id)
          : null;
      if (deposit)
        assertDepositSourceHash(
          body.expected_deposit_source_hash,
          deposit.source_hash
        );
      const party =
        body.mode === "deposit"
          ...
          : match
            ...
            : await validateCounterparty(
                client,
                body.counterparty_type ?? null,
                body.counterparty_id ?? null
              );
      const before = context.review;
      const changed =
        (before?.mode ?? "categorize") !== body.mode ||
      ...
      const review = await persistReview(
        ...
        context,
        {
          status: "draft",
          mode: body.mode,
          category_list_id: category?.id ?? null,
          category_snapshot: category ? { ...category } : null,
          counterparty_type: party?.type ?? null,
          counterparty_id: party?.id ?? null,
          counterparty_name: party?.name ?? null,
          comment: body.comment,
          matched_deposit_id: deposit?.id ?? null,
          deposit_snapshot: deposit ? { ...deposit } : null,
          matched_payment_id: match?.id ?? null,
          match_snapshot: match ? { ...match } : null,
          manual_override: Boolean(before?.manual_override || changed),
          origin: changed ? "manual" : (before?.origin ?? "manual"),
          rule_id: changed ? null : (before?.rule_id ?? null),
          rule_version: changed ? null : (before?.rule_version ?? null),
          confirmed_by: null,
          confirmed_at: null,
          exclusion_reason: null,
        },
...
export async function confirmTransactionReview(
  ...
  return runReviewCommand(
    { actorId, key, operation: "review_confirm", entityId: id, body },
    async (client) => {
      const context = await loadReviewContext(client, id, body);
      const before = context.review;
      ...
      if (
        ...
        !context.tx.opening_reference
      ) {
        throw new BankingError("BANKING_ACCOUNT_SETUP_REQUIRED", 409);
      }
      if (before.source_version !== context.tx.source_version)
        throw new BankingError("BANKING_REVIEW_STALE", 409);
      const party = await validateCounterparty(
        client,
        before.counterparty_type,
        before.counterparty_id
      );
      ...
      if (before.mode === "match") {
        if (!before.matched_payment_id)
          throw new BankingError("BANKING_MATCH_REQUIRED");
        const match = await validateMatchedPayment(
          client,
          id,
          before.matched_payment_id
        );
        assertMatchSourceHash(
          before.match_snapshot?.source_hash as string | undefined,
          match.source_hash,
          match.legacy_source_hash
        );
        changes.match_snapshot = { ...match };
        changes.counterparty_type = "customer";
        changes.counterparty_id = match.customer_id;
        changes.counterparty_name = match.customer_name;
      } else if (before.mode === "deposit") {
        if (!before.matched_deposit_id)
          throw new BankingError("BANKING_DEPOSIT_MATCH_INVALID", 409);
        const deposit = await validateMatchedDeposit(
          client,
          id,
          before.matched_deposit_id
        );
        assertDepositSourceHash(
          before.deposit_snapshot?.source_hash as string | undefined,
          deposit.source_hash
        );
        changes.deposit_snapshot = { ...deposit };
      } else {
        if (!before.category_list_id)
          throw new BankingError("BANKING_CATEGORY_REQUIRED");
        changes.category_snapshot = {
          ...(await validateCategory(client, before.category_list_id)),
        };
      }
      return {
        review: await persistReview(
          client,
          context,
          changes,
          actorId,
...
export async function changeTransactionReviewState(
  id: string,
  actorId: string,
  key: string,
  action: "return" | "exclude",
  body: ReviewVersions & { reason?: string }
): Promise<{ review: Review }> {
  return runReviewCommand(
    { actorId, key, operation: `review_${action}`, entityId: id, body },
    async (client) => {
      const context = await loadReviewContext(client, id, body);
      if (action === "exclude" && !body.reason?.trim())
        throw new BankingError("BANKING_EXCLUSION_REASON_REQUIRED");
      const review = await persistReview(
        client,
        context,
        {
          status: action === "exclude" ? "excluded" : "draft",
          confirmed_by: null,
          confirmed_at: null,
          matched_payment_id: null,
          match_snapshot: null,
          ...
          exclusion_reason:
            action === "exclude"
              ? // eslint-disable-next-line @typescript-eslint/no-non-null-assertion -- el guard de arriba (`!body.reason?.trim()` para action==="exclude") ya garantiza reason no vacío acá
                body.reason!.trim()
              : null,
        },
        actorId,
