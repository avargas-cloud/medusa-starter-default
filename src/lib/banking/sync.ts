...
export async function syncBank(connectionId: string, trigger: "initial" | "manual" | "scheduled" | "webhook" = "scheduled") {
  ...
  return withBankLock(connectionId, async (client) => {
    ...
    if (row.status === "awaiting_selection") return { status: "awaiting_selection" };
    const accessToken = decryptBankToken(row.access_token_encrypted, connectionId, key);
    ...
    await transaction(client, async () => {
      ...
      await client.query(`INSERT INTO bank_sync_run(id,connection_id,trigger,status,started_at,cursor_before)
        VALUES($1,$2,$3,'running',now(),$4)`, [runId, connectionId, trigger, row.cursor]);
    });
    try {
      ...
      const itemResponse = await plaidRequest("/item/get", { access_token: accessToken });
      const item = object(itemResponse.item);
      const providerStatus = itemResponse.status == null ? null : object(itemResponse.status);
      const transactionsStatus = providerStatus?.transactions == null ? null : object(providerStatus.transactions);
      const lastProviderUpdate = nullableString(transactionsStatus?.last_successful_update);
      const accountResponse = await plaidRequest("/accounts/get", { access_token: accessToken });
      ...
      const counts = await transaction(client, async () => {
        ...
        await client.query(`UPDATE bank_connection SET status='active',consent_expiration_time=$2,
        ...
        [connectionId, nullableString(item.consent_expiration_time), row.sync_requested_at, lastProviderUpdate]);
        await client.query(`UPDATE bank_sync_run SET status='succeeded',finished_at=now(),cursor_after=$2,
      ...
      return { status: batch.initialComplete || row.initial_sync_complete ? "synced" : "importing", ...counts };
    } catch (error) {
      const code = bankingErrorCode(error);
      await client.query(`UPDATE bank_sync_run SET status='failed',finished_at=now(),error_code=$2,updated_at=now() WHERE id=$1`, [runId, code]);
      await client.query(`UPDATE bank_connection SET status=$2,last_error_code=$3,updated_at=now() WHERE id=$1`,
        [connectionId, code === "ITEM_LOGIN_REQUIRED" ? "reauth_required" : "error", code]);
      throw error;
    }
  });
}
...
export async function syncPendingBanks() {
  ...
  for (const candidate of candidates.rows) {
    try { await syncBank(candidate.id); }
    catch (error) { console.warn(`[banking] ${bankingErrorCode(error)}`); }
  }
}
