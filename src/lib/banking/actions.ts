...
export async function selectBankAccounts(
  connectionId: string,
  accountIds: string[]
): Promise<Awaited<ReturnType<typeof syncBank>>> {
  if (
    !accountIds.length ||
  ...
  await withBankLock(connectionId, async (client) => {
    ...
    await transaction(client, async () => {
      ...
      if (valid.rowCount !== accountIds.length)
        throw new BankingError("BANKING_INVALID_SELECTION");
      await client.query(
        "UPDATE bank_account SET is_selected=(id=ANY($2::text[])),updated_at=now() WHERE connection_id=$1",
        [connectionId, accountIds]
      );
      await client.query(
        "UPDATE bank_connection SET status='active',sync_requested_at=now(),updated_at=now() WHERE id=$1",
        [connectionId]
      );
    });
  });
...
export async function refreshBank(
  ...
  await withBankLock(connectionId, async (client) => {
    ...
    await client.query(
      "UPDATE bank_connection SET refresh_requested_at=$2,sync_requested_at=now(),updated_at=now() WHERE id=$1",
      [connectionId, requestedAt]
    );
  });
...
export async function reconnectBank(connectionId: string): Promise<{
  ...
  return withBankLock(connectionId, async (client) => {
    ...
    return {
      ...
      accounts: await selectedAccountRows(client, connectionId),
    };
  });
}
...
export async function disconnectBank(
  ...
  return withBankLock(connectionId, async (client) => {
    ...
    await transaction(client, async () => {
      await withReviewLock(client);
      await client.query(
        `UPDATE bank_connection SET status='disconnected',access_token_encrypted=NULL,
        sync_requested_at=NULL,last_error_code=NULL,updated_at=now() WHERE id=$1`,
        [connectionId]
      );
      await client.query(
        "UPDATE bank_account SET is_active=false,updated_at=now() WHERE connection_id=$1",
        [connectionId]
      );
    });
...
export async function mapBankAccount(
...
): Promise<{ account_id: string; qb_list_id: string | null }> {
  bankingTokenKey();
  const result = await getDbPool().query<{ connection_id: string }>(
    `SELECT a.connection_id FROM bank_account a JOIN bank_connection c ON c.id=a.connection_id
     WHERE a.id=$1 AND c.environment=${bankingEnvSql()} AND a.deleted_at IS NULL`,
    [accountId]
  );
  const row = result.rows[0];
  if (!row) throw new BankingError("BANKING_ACCOUNT_NOT_FOUND", 404);
  return withBankLock(row.connection_id, async (client) =>
    transaction(client, async () => {
      ...
      const current = await client.query<{ qb_list_id: string | null }>(
        "SELECT qb_list_id FROM bank_account WHERE id=$1 AND deleted_at IS NULL FOR UPDATE",
        [accountId]
      );
      if (!current.rows[0])
      ...
      if (current.rows[0].qb_list_id !== qbListId) {
        ...
        const closed = await client.query(
          `SELECT id FROM bank_day_close
        WHERE status='closed' AND deleted_at IS NULL
          AND EXISTS(SELECT 1 FROM jsonb_array_elements(COALESCE(snapshot->'accounts','[]'::jsonb)) b
            WHERE b->'account'->>'id'=$1) LIMIT 1`,
          [accountId]
        );
        if (closed.rowCount)
          throw new BankingError("BANKING_REOPEN_REQUIRED", 409);
      }
      if (qbListId) {
        const valid = await client.query(
        ...
        if (!valid.rowCount)
          throw new BankingError("BANKING_INVALID_QB_ACCOUNT");
        const duplicate = await client.query(
          "SELECT id FROM bank_account WHERE qb_list_id=$1 AND id<>$2 AND is_active AND deleted_at IS NULL",
          [qbListId, accountId]
        );
        if (duplicate.rowCount)
          throw new BankingError("BANKING_QB_ACCOUNT_ALREADY_MAPPED", 409);
      }
      await client.query(
        "UPDATE bank_account SET qb_list_id=$2,updated_at=now() WHERE id=$1",
        [accountId, qbListId]
      );
