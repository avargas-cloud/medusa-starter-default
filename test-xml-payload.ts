...
import { buildQbItems, buildShippingQbItem, buildQbOrderDiscountLines } from './src/lib/quickbooks/order-flow-core'
...
async function run() {
    ...
    // Fetch order 1171
    const { rows: orderRows } = await c.query('SELECT * FROM "order" WHERE display_id = 1171')
    const order = orderRows[0]
    
    // Fetch items
    const { rows: items } = await c.query("SELECT * FROM order_item WHERE order_id = $1", [order.id])
    
    // Fetch variants metadata
    for(const item of items) {
       const { rows: variants } = await c.query("SELECT metadata, sku FROM product_variant WHERE id = $1", [item.variant_id])
       item.variant = variants[0] || {}
    }
    order.items = items
    
    // Fetch shipping
    const { rows: shipping } = await c.query("SELECT * FROM order_shipping_method WHERE order_id = $1", [order.id])
    ...
    if (orderDiscountTotal > 0) {
        ...
        buildQbOrderDiscountLines(orderDiscountTotal, discountPercent).forEach((l: any) => prebuiltItems.push(l))
    }
