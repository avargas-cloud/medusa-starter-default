...
export default async function fixWebPayments({ container }: { container: MedusaContainer }) {
    ...
    console.log(`Found ${applications.length} bogus PaymentApplications. Pruning...`)
    ...
    for (const app of applications) {
        ...
        if (app.payment.source === 'web' && app.payment.status === 'applied') {
            ...
            console.log(` -> Reverted Web Payment ${app.payment.id} to 'available'. Deleted app ${app.id}.`)
            totalFixed++;
        }
    }

    console.log(`Web Payment Fix Complete. Reverted ${totalFixed} CustomerPayments to 'available'.`)
}
